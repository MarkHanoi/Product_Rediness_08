...
const REFRESH = process.env.BEAM_SNAPSHOT_REFRESH === '1';
...
describe(`beam snapshot parity (${BEAM_FIXTURES.length} fixtures)`, () => {
  for (const f of BEAM_FIXTURES) {
    writeFileSync(
      resolve(CONFIG_DIR, `${f.id}.json`),
      JSON.stringify({ id: f.id, description: f.description, beam: f.beam, worldY: f.worldY }, null, 2) + '\n',
    );
  }

  for (const f of BEAM_FIXTURES) {
    it(`${f.id} matches snapshot`, () => {
      const desc = produceBeam(f.beam, NO_JOINS, f.worldY);
