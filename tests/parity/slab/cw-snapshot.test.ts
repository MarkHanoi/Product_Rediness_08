...
const REFRESH = process.env.SLAB_SNAPSHOT_REFRESH === '1';
...
describe(`slab snapshot parity (${SLAB_FIXTURES.length} fixtures)`, () => {
  for (const f of SLAB_FIXTURES) {
    writeFileSync(
      resolve(CONFIG_DIR, `${f.id}.json`),
      JSON.stringify({ id: f.id, description: f.description, slab: f.slab, worldY: f.worldY }, null, 2) + '\n',
    );
  }

  for (const f of SLAB_FIXTURES) {
    it(`${f.id} matches snapshot`, () => {
      const desc = produceSlab(f.slab, NO_JOINS, f.worldY);
