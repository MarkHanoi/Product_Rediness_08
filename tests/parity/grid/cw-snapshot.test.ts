...
const REFRESH = process.env.GRID_SNAPSHOT_REFRESH === '1';
...
describe(`grid snapshot parity (${GRID_FIXTURES.length} fixtures)`, () => {
  for (const f of GRID_FIXTURES) {
    writeFileSync(
      resolve(CONFIG_DIR, `${f.id}.json`),
      JSON.stringify({ id: f.id, description: f.description, grid: f.grid, worldY: f.worldY }, null, 2) + '\n',
    );
  }

  for (const f of GRID_FIXTURES) {
    it(`${f.id} matches snapshot`, () => {
      const desc = produceGrid(f.grid, NO_JOINS, f.worldY);
