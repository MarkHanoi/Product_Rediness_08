// Column producer parity-snapshot fixture suite (W-1C-2 disk-based upgrade).
//
// Mirrors `tests/parity/stair/cw-snapshot.test.ts`.
// Refresh by deleting the snapshot file or running with the env var
// `COLUMN_SNAPSHOT_REFRESH=1`.
...
const REFRESH = process.env.COLUMN_SNAPSHOT_REFRESH === '1';
...
describe(`column snapshot parity (${COLUMN_FIXTURES.length} fixtures)`, () => {
  for (const f of COLUMN_FIXTURES) {
    writeFileSync(
      resolve(CONFIG_DIR, `${f.id}.json`),
      JSON.stringify({ id: f.id, description: f.description, column: f.column, worldY: f.worldY }, null, 2) + '\n',
    );
  }

  for (const f of COLUMN_FIXTURES) {
    it(`${f.id} matches snapshot`, () => {
      const desc = produceColumn(f.column, NO_JOINS, f.worldY);
