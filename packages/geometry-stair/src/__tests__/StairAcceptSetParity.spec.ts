        ...
        stores: {
            wallStore: {
                getLevels: () => [
                    ...
                    { id: TOP_LEVEL, elevation: FLOOR_TO_FLOOR },
                ],
            },
 ...
 * silently swallowed.
 */
const GEOMETRY_REFUSAL = /(Riser height|Tread depth|tread depth)/i;

function commandGeometryRefusals(result: SolverResult2D): string[] {
    ...
    // command judges.
    const input = makeAdapter().toCreateStairInput({ ...result, isValid: true });
    if (!input) return ['ADAPTER-PRODUCED-NO-INPUT'];
    const verdict = new CreateStairCommand(input).canExecute(makeCtx());
