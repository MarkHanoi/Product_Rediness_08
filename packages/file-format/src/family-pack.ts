...
import { writeDeterministicZip, type DeterministicEntry } from './zip-deterministic.js';
 ...
 * errors throw.
 */
export async function packFamily(input: FamilyPackInput): Promise<FamilyPackResult> {
  return tracer.startActiveSpan(
    'pryzm.family.persistence.save',
    {
      attributes: {
    ...
    async (span): Promise<FamilyPackResult> => {
      const t0 = nowMs();
      try {
        ...
        const stampedManifest: FamilyManifest = {
          ...manifest,
        ...
        const events: readonly FamilyEvent[] = input.events ?? [];
        ...
        if (input.signingKey) {
          try {
            ...
            const sig = await subtle.sign(
              { name: 'Ed25519' },
              input.signingKey,
              enc.encode(manifestJson) as unknown as ArrayBuffer,
            );
            signatureBytes = new Uint8Array(sig);
          } catch (err) {
            const message = `[packFamily] Ed25519 signing failed: ${(err as Error).message}`;
            span.setStatus({ code: SpanStatusCode.ERROR, message });
        ...
        const message = `[packFamily] unexpected error: ${(err as Error).message}`;
