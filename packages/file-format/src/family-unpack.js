...
async function extractEntry(entry, entryPath) {
    try {
        return await entry.async('uint8array');
    }
    catch (err) {
        throw new CorruptZipEntryError(entryPath, err.message);
    }
}
export async function unpackFamily(input) {
    return tracer.startActiveSpan('pryzm.family.persistence.load', {
        attributes: {
        ...
        try {
            ...
            try {
                ...
                const message = `[unpackFamily] manifest.json is not valid JSON: ${err.message}`;
                span.setStatus({ code: SpanStatusCode.ERROR, message });
            ...
            try {
                ...
                const message = `[unpackFamily] document.json is not valid JSON: ${err.message}`;
                span.setStatus({ code: SpanStatusCode.ERROR, message });
            ...
            if (ifcEntry) {
                ...
                try {
                    ifcMapping = JSON.parse(dec.decode(ifcBytes));
                }
                catch {
            ...
            if (eventLogEntry) {
                ...
                for (const line of text.split('\n')) {
                    ...
                    try {
                        ...
                        const message = `[unpackFamily] event-log line ${lineIdx} not JSON: ${err.message}`;
                        span.setStatus({ code: SpanStatusCode.ERROR, message });
                        return { ok: false, reason: 'event-log-parse-error', message };
                    }
                }
            ...
            let recordedSchemaHash = null;
            if (schemaHashEntry) {
                const b = await extractEntry(schemaHashEntry, FAMILY_PATHS.schemaHash);
                recordedSchemaHash = dec.decode(b).trim();
            }
            const recomputedHash = await sha256Hex(enc.encode(canonicalise(document) + canonicalise(projectIfcMapping(document))));
            ...
            if (input.verifyingKey) {
                ...
                try {
                    ...
                    const ok = await subtle.verify({ name: 'Ed25519' }, input.verifyingKey, signature, manifestBytes);
                    if (!ok) {
                    ...
                    const message = `[unpackFamily] signature verification threw: ${err.message}`;
                    span.setStatus({ code: SpanStatusCode.ERROR, message });
                    return { ok: false, reason: 'signature-mismatch', message };
                }
            }
            ...
            const message = `[unpackFamily] unexpected error: ${err.message}`;
...
//# sourceMappingURL=family-unpack.js.map