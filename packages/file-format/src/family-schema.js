...
export function parseFormatVersion(value) {
...
export function compareFormatVersion(a, b) {
...
export function classifyFormatVersion(value) {
    const parsed = parseFormatVersion(value);
    if (!parsed)
        return 'unparseable';
    if (SUPPORTED_FORMAT_VERSIONS.includes(value))
        return 'supported';
...
export function isSupportedFormatVersion(value) {
...
export function formatVersionRefusal(value) {
...
export const BAKEABLE_SOLID_KINDS = ['extrude'];
...
export const CONNECTOR_KINDS_WITH_WRITER = ['insertion', 'opening'];
