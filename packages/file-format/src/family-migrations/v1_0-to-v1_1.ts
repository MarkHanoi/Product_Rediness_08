...
export function makeV1_0ToV1_1Migrator(): Migrator {
  return {
    id: 'pryzm-family:1.0->1.1',
    from: V1_0,
    to: V1_1,
    description:
      'v1.0 -> v1.1: drop the dead `document.defaults` channel (C111 §5.3-b) ' +
    ...
    apply(input: RawFamily): RawFamily {
      ...
      const { defaults: _deadSecondDefaultChannel, ...documentWithoutDefaults } =
        input.document as typeof input.document & { defaults?: unknown };
...
export const v1_0ToV1_1Migrator: Migrator = makeV1_0ToV1_1Migrator();
...
export function assertVersionCoherent(
  input: RawFamily,
  expected: string,
): void {
  ...
  if (m !== d) {
    throw new Error(
      `[pryzm-family:1.0->1.1] incoherent bundle: manifest.formatVersion=${m} ` +
  ...
  if (m !== expected) {
    throw new Error(
      `[pryzm-family:1.0->1.1] expected formatVersion ${expected}, got ${m}.`,
    );
  }
}
...
export const ALL_VERSION_MIGRATORS: readonly Migrator[] = [v1_0ToV1_1Migrator];
