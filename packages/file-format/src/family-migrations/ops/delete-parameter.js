            ...
            for (const profile of input.document.profiles) {
                for (const entity of profile.entities) {
                    for (const [key, value] of Object.entries(entity.data)) {
                        if (key === 'p1' || key === 'p2' || key === 'center')
                            continue;
                        if (typeof value !== 'string')
                            continue;
                        if (!new RegExp(`\\b${escapeRegex(target.name)}\\b`).test(value))
                            continue;
                        throw new Error(`cannot delete parameter ${params.parameterId}: profile ${profile.id} entity ` +
