...
// side-car entry.
export function makeRebindIfcMigrator(from, to, params) {
    return {
        id: `rebind-ifc:${params.parameterId}`,
        from,
        to,
        description: params.newPset === null
        ...
        apply(input) {
            ...
            const newInDoc = params.newPset === null || params.newProperty === null
                ? null
                : { psetName: params.newPset, propertyName: params.newProperty };
            const parameters = input.document.parameters.map((p) => p.id === params.parameterId ? { ...p, ifcMapping: newInDoc } : p);
            let ifcMapping = input.ifcMapping;
            if (ifcMapping) {
                const filtered = ifcMapping.parameters.filter((m) => m.parameterId !== params.parameterId);
                const next = newInDoc === null
                    ? filtered
                    : [
                        ...filtered,
                        {
...
//# sourceMappingURL=rebind-ifc.js.map