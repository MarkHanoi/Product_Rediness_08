 ...
 */
export function typeValuesChecksum(values) {
    const json = canonicalise(values);
    let h = 0x811c9dc5;
...
function valueFitsDataType(p, value) {
    switch (p.dataType) {
        case 'length':
        case 'angle':
        case 'number':
...
export function makeSetTypeValuesMigrator(from, to, params) {
    return {
        id: `set-type-values:${params.typeId}`,
        from,
        to,
        description: `set ${Object.keys(params.values).length} value(s) on type ${params.typeId}` +
            (params.name === undefined ? '' : ` and rename it to "${params.name}"`),
        apply(input) {
            const doc = input.document;
            const target = doc.types.find((t) => t.id === params.typeId);
            if (!target)
                throw new Error(`type ${params.typeId} not found`);
            // ── the rename, if one was asked for ──────────────────────────────────
            let nextName = target.name;
            if (params.name !== undefined) {
                const trimmed = params.name.trim();
                if (trimmed.length === 0) {
                    throw new Error(`a type name cannot be empty; type ${params.typeId} keeps the name '${target.name}'`);
                }
                ...
                const clash = doc.types.find((t) => t.id !== params.typeId && t.name.trim().toLowerCase() === trimmed.toLowerCase());
                if (clash) {
                    throw new Error(`a type named '${trimmed}' already exists in this definition (${clash.id}); ` +
                        'type names must be unique — nothing was changed');
                }
                nextName = trimmed;
            }
            // ── every key must name a parameter, and fit its declared type ─────────
            const byId = new Map(doc.parameters.map((p) => [p.id, p]));
            for (const [pid, value] of Object.entries(params.values)) {
                const p = byId.get(pid);
                if (p === undefined) {
                    throw new Error(`type '${nextName}' was given a value for ${pid}, which this definition declares no ` +
                        'parameter for; a type value keyed to nothing is a value no resolver will ever read');
                }
                if (!valueFitsDataType(p, value)) {
                    throw new Error(`parameter '${p.name}' is declared as ${p.dataType} and the value given for it is a ` +
            ...
            const types = doc.types.map((t) => t.id === params.typeId
                ? { ...t, name: nextName, values, checksum: typeValuesChecksum(values) }
                : t);
            return {
                manifest: { ...input.manifest },
                document: { ...doc, formatVersion: to, types },
...
//# sourceMappingURL=type-values.js.map