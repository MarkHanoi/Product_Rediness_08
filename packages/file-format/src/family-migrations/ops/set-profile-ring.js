...
export function makeSetProfileRingMigrator(from, to, params) {
    return {
        ...
        apply(input) {
            ...
            if (params.points.length < MIN_RING_POINTS) {
                throw new Error(`profile '${profile.name}' was given ${params.points.length} point(s); a profile needs at ` +
            ...
            for (const p of params.points) {
                if (!ULID_RE.test(p.id)) {
                    throw new Error(`profile point id ${JSON.stringify(p.id)} is not a 26-character Crockford-base32 ULID, ` +
                ...
                if (incoming.has(p.id)) {
                    throw new Error(`point ${p.id} is listed twice; a profile point has one position, and two would make ` +
                        'the last writer the winner silently');
                }
                incoming.add(p.id);
                if (!Number.isFinite(p.x) || !Number.isFinite(p.z)) {
                    throw new Error(`point ${p.id} was given a non-finite coordinate (x=${String(p.x)}, z=${String(p.z)}); ` +
            ...
            for (const e of profile.entities) {
                if (e.kind !== 'point') {
                    throw new Error(`profile '${profile.name}' entity ${e.id} is a '${e.kind}'; writing a flattened ring over ` +
            ...
            if (idSetChanged && profile.constraints.length > 0) {
                ...
                throw new Error(`profile '${profile.name}' carries ${profile.constraints.length} constraint(s) and this ` +
            ...
            if (added.length > 0) {
                ...
                for (const id of added) {
                    if (constraintIds.has(id)) {
                        throw new Error(`minted point id ${id} is already a constraint id on profile '${profile.name}'; ` +
            ...
            const entities = params.points.map((p) => {
                ...
                if (!existing) {
                    ...
                    return { id: p.id, kind: 'point', data: { x: p.x, z: p.z } };
                }
                if (typeof existing.data['x'] !== 'number' || typeof existing.data['z'] !== 'number') {
                    throw new Error(`profile '${profile.name}' point ${existing.id} has an expression-valued coordinate; ` +
...
//# sourceMappingURL=set-profile-ring.js.map