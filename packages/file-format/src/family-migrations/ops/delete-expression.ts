...
export function makeDeleteExpressionMigrator(
  from: string,
  to: string,
  params: DeleteExpressionParams,
): Migrator {
  return {
    ...
    apply(input: RawFamily): RawFamily {
      ...
      if (target.expression === null || target.expression.trim().length === 0) {
        throw new Error(
          `parameter ${params.parameterId} carries no expression; there is nothing to delete, and ` +
      ...
      const parameters = input.document.parameters.map((p) => {
        ...
        const restored =
          params.replacementDefault !== undefined
