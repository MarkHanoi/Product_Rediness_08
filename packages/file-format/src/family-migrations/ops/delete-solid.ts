...
export function makeDeleteSolidMigrator(
  from: string,
  to: string,
  params: DeleteSolidParams,
): Migrator {
  return {
    ...
    apply(input: RawFamily): RawFamily {
      ...
      if (!target) throw new Error(`solid ${params.solidId} not found`);

      const consumer = doc.solids.find(
        (s) =>
          s.kind === 'boolean' &&
      ...
      if (consumer) {
        throw new Error(
          `cannot delete solid ${params.solidId}: boolean solid ${consumer.id} consumes it as an ` +
            'operand; delete that feature first',
        );
      }

      const solids = doc.solids.filter((s) => s.id !== params.solidId);
      const featureEdges = doc.featureEdges.filter(
        (e) => e.from !== params.solidId && e.to !== params.solidId,
      );
      ...
      const stillReferenced =
        ...
        solids.some(
          (s) =>
            ('profileId' in s && s.profileId === profileId) ||
      ...
      const profiles =
        params.keepProfile === true || stillReferenced
          ? doc.profiles
