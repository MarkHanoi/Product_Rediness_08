...
export function makeUpdateProfileMigrator(
  from: string,
  to: string,
  ...
  return {
    ...
    apply(input: RawFamily): RawFamily {
      ...
      for (const p of params.points) {
        if (seen.has(p.id)) {
          throw new Error(
            `point ${p.id} is listed twice; a profile point has one position, and two would ` +
      ...
      if (params.points.length !== profile.entities.length) {
        throw new Error(
          `profile '${profile.name}' carries ${profile.entities.length} entit(ies) and the update ` +
      ...
      for (const e of profile.entities) {
        if (!seen.has(e.id)) {
          throw new Error(
            `profile '${profile.name}' carries entity ${e.id}, which the update does not name; ` +
      ...
      const byId = new Map(params.points.map((p) => [p.id, p] as const));
      const entities = profile.entities.map((e) => {
        const next = byId.get(e.id)!;
        if (e.kind !== 'point') {
          throw new Error(
            `profile '${profile.name}' entity ${e.id} is a '${e.kind}'; writing a flattened ring ` +
        ...
        if (!Number.isFinite(next.x) || !Number.isFinite(next.z)) {
          throw new Error(
            `point ${e.id} was given a non-finite coordinate (x=${String(next.x)}, ` +
