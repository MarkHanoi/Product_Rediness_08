...
import type { FamilyDocument, Profile, SolidFeature } from '../../family-schema.js';
 ...
 */
export function makeSetExtrudeWorkPlaneMigrator(
  from: string,
  to: string,
  params: SetExtrudeWorkPlaneParams,
): Migrator {
  return {
    ...
    apply(input: RawFamily): RawFamily {
      ...
      if (!solid) {
        throw new Error(
          `solid ${params.solidId} is not carried by this definition; it cannot be put on a work plane`,
        );
      }
      if (solid.kind !== 'extrude') {
        throw new Error(
          `solid ${params.solidId} is kind '${solid.kind}', and only an 'extrude' has a sweep axis a ` +
      ...
      if (!plane) {
        throw new Error(
          `reference plane ${params.planeId} is not carried by this definition; a shape cannot be built ` +
      ...
      if (![x, y, z].every((c) => Number.isFinite(c))) {
        throw new Error(
          `reference plane ${plane.name} (${plane.id}) has a non-finite normal (${x}, ${y}, ${z}); it ` +
      ...
      if (
        ...
        throw new Error(
          `reference plane ${plane.name} (${plane.id}) has origin (${plane.origin.x}, ${plane.origin.y}, ` +
      ...
      if (!profile) {
        throw new Error(
          `solid ${params.solidId} references profile ${solid.profileId}, which this definition does ` +
      ...
      const nextSolid: SolidFeature = { ...solid, direction };
      const nextProfile: Profile = { ...profile, planeId: plane.id };
