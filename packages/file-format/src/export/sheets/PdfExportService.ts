...
import { titleBlockValueCapMm, titleBlockLabelCapMm, capMmToPdfPt } from './PaperTextStandard';
...
class PdfExportServiceImpl {
    ...
    private _drawTitleBlock(
        ...
        for (const field of placement.fields) {
            ...
            // §TITLE-BLOCK-TEXT-HAS-ONE-UNIT (L-10689) — sized in CAP HEIGHT
            // MILLIMETRES, floored at the SAME `MIN_PAPER_TEXT_HEIGHT_MM` that
            // L-10680 established for the drawing text. The fixed `4` here was
            // 1.010 mm of cap height: the founder's unreadable room labels, on
            // the same page, in the block that names the drawing.
            pdf.setFontSize(capMmToPdfPt(titleBlockLabelCapMm()));
            pdf.setTextColor('#888888');
            pdf.text(field.label ?? '', fx + 0.8, fy - field.height + 2.5);

            // Value
            const valueCapMm = titleBlockValueCapMm(field.fontSize);
            pdf.setFontSize(capMmToPdfPt(valueCapMm));
            pdf.setTextColor('#111111');
            if (field.bold) pdf.setFont('helvetica', 'bold');
            // ⚠ AND IT NEVER TRUNCATED. The editor clips its zones
            // (`overflow:hidden` + ellipsis) and the PDF did not, so a long
            // project name ran off the title block and off the page. Raising the
            // lettering to a legible size would have made that worse, so it is
            // fixed in the same breath rather than left as the next report.
            pdf.text(this._ellipsise(pdf, value, field.width - 1.6), fx + 0.8, fy - field.height * 0.3);
    ...
    /**
     * §TITLE-BLOCK-TEXT-HAS-ONE-UNIT (L-10689) — clip a value to its zone.
     *
     * Measured against the FONT ACTUALLY SET (`pdf.getTextWidth` reads the
     * current size and style), never an estimate, so the ellipsis appears where
     * the glyphs actually stop. A non-positive width means the caller gave a
     * zone with no room, and the honest answer is to draw nothing rather than
     * one floating character.
     */
    private _ellipsise(pdf: jsPDF, text: string, maxWidthMm: number): string {
        if (!text || maxWidthMm <= 0) return '';
