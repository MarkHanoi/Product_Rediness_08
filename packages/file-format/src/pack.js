...
import { EVENT_BATCH_SIZE, PATHS, } from './types.js';
...
export async function pack(input) {
    return tracer.startActiveSpan('pryzm.file-format.pack', {
        attributes: {
        ...
        try {
            ...
            for (const c of manifest.chunks)
                referencedHashes.add(c.hash);
            for (const lvl of manifest.levels) {
                if (lvl.latestChunkHash)
                    referencedHashes.add(lvl.latestChunkHash);
            }
            ...
            if (input.signingKey) {
                try {
                    ...
                    const sig = await subtle.sign({ name: 'Ed25519' }, input.signingKey, manifestBytes);
                    zip.file(PATHS.signature, new Uint8Array(sig), { compression: 'STORE' });
                }
                catch (err) {
                    const message = `[pack] Ed25519 signing failed: ${err.message}`;
                    span.setStatus({ code: SpanStatusCode.ERROR, message });
            ...
            const message = `[pack] unexpected error: ${err.message}`;
...
function getSubtle() {
    ...
    if (!subtle) {
        throw new Error('[pack] globalThis.crypto.subtle is not available — Node 20+ or a modern browser is required for Ed25519 signing.');
    }
    return subtle;
}
