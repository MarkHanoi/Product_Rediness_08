...
export const MIGRATIONS = [
    {
        ...
        toVersion: 1,
        migrate: async () => {
            throw new MigrationStubError('PRYZM 1 → v1 migration: not yet implemented in Phase 1.  Use the PRYZM 1 importer plugin (Phase 3D).');
        },
    },
];
/**
...
export class FutureVersionError extends Error {
    ...
    constructor(fileVersion, supportedVersion) {
        super(`Cannot open: project is schema v${fileVersion}, this PRYZM build supports up to v${supportedVersion}.  Update PRYZM.`);
 ...
 * the current version.
 */
export async function migrate(rawManifest, zip) {
    ...
    let currentManifest = rawManifest;
    let currentZip = zip;
    ...
    for (const step of MIGRATIONS) {
        if (step.fromVersion < currentVersion)
            continue;
        if (step.fromVersion > currentVersion) {
            throw new Error(`[file-format] Missing migration step from v${currentVersion} to v${step.fromVersion} — registry has a gap.  This is a build error, not a file error.`);
        }
        ...
        if (currentVersion !== step.toVersion) {
            throw new Error(`[file-format] Migration step ${step.fromVersion}→${step.toVersion} returned schemaVersion=${currentVersion}.  This is a step-implementation bug.`);
        }
        if (currentVersion >= PRYZM_FORMAT_SCHEMA_VERSION)
            break;
    }
    if (currentVersion !== PRYZM_FORMAT_SCHEMA_VERSION) {
        throw new Error(`[file-format] Migration ended at v${currentVersion}, expected v${PRYZM_FORMAT_SCHEMA_VERSION}.  Registry is incomplete.`);
    }
    return {
...
function readSchemaVersion(rawManifest) {
    if (typeof rawManifest !== 'object' ||
        rawManifest === null ||
        !('schemaVersion' in rawManifest)) {
        throw new Error('[file-format] manifest.json is missing the required `schemaVersion` field.');
    }
    const v = rawManifest.schemaVersion;
    if (typeof v !== 'number' || !Number.isInteger(v) || v < 0) {
        throw new Error(`[file-format] manifest.json has invalid schemaVersion=${String(v)} (must be a non-negative integer).`);
    }
    return v;
}
