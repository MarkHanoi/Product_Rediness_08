...
import { migrate, FutureVersionError, MigrationStubError, } from './migrations/index.js';
 ...
 * throw.
 */
export async function unpack(input) {
    return tracer.startActiveSpan('pryzm.file-format.unpack', {
        attributes: {
        ...
        try {
            ...
            try {
                ...
                const message = `[unpack] not a valid ZIP: ${err.message}`;
            ...
            try {
                ...
                const message = `[unpack] manifest.json is not valid JSON: ${err.message}`;
                span.setStatus({ code: SpanStatusCode.ERROR, message });
            ...
            try {
                ...
                const message = `[unpack] migration failed: ${err.message}`;
                span.setStatus({ code: SpanStatusCode.ERROR, message });
            ...
            if (input.verifyingKey) {
                ...
                if (migrated.migratedFromVersion !== null) {
                    const message = '[unpack] signature verification is not supported on migrated files in v1.';
                    span.setStatus({ code: SpanStatusCode.ERROR, message });
                ...
                try {
                    ...
                    const ok = await subtle.verify({ name: 'Ed25519' }, input.verifyingKey, sigBytes, manifestBytes);
                    if (!ok) {
                    ...
                    const message = `[unpack] Ed25519 verification failed: ${err.message}`;
                    span.setStatus({ code: SpanStatusCode.ERROR, message });
                    return { ok: false, reason: 'signature-mismatch', message };
                }
            }
            ...
            for (const entry of eventsBatchEntries) {
                ...
                try {
                    ...
                    const message = `[unpack] event batch ${entry.name} failed to decode: ${err.message}`;
                    span.setStatus({ code: SpanStatusCode.ERROR, message });
            ...
            span.setAttributes({
                ...
                'pryzm.file-format.unpack.migratedFromVersion': migrated.migratedFromVersion ?? -1,
            ...
            const message = `[unpack] unexpected error: ${err.message}`;
...
function collectSortedBatchEntries(zip) {
    const out = [];
    const folder = zip.folder(PATHS.eventsDir.replace(/\/$/, ''));
    folder?.forEach((relativePath, file) => {
        if (file.dir)
            return;
        if (!/^\d{6}\.evt\.bin$/.test(relativePath))
            return;
        out.push({
...
function getSubtle() {
    ...
    if (!subtle) {
        throw new Error('[unpack] globalThis.crypto.subtle is not available — Node 20+ or a modern browser is required for Ed25519 verification.');
    }
    return subtle;
}
