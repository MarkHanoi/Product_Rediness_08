/**
 * CurvedLeafGeometry — §FEAT-CURVED-WINDOW-LEAF (L-957)
 *
 * THE LEAF OF A WINDOW HOSTED IN A CURVED WALL, BUILT ON THE WALL'S OWN ARC.
 *
 ...
 * ruling. Hence: heads, sills and transoms traverse the arc and must bend;
 * jambs, mullions and sash stiles are rulings and stay straight — they only need
 * RE-SEATING onto the arc (position + local heading), which is what
 * {@link arcSeat} returns. If a compound (non-vertical-axis) curve is ever added
 * to `WallData`, this decomposition stops being true and this comment is where a
 * reader will find out why.
 *
 * ── What a "swept box" is here ───────────────────────────────────────────────
 * Every member of the leaf is authored by `WindowBuilder.buildVisuals` as an
 * axis-aligned box in GROUP-LOCAL space: +X along the wall, +Y up, +Z out through
 * the exterior face. {@link sweptBoxGeometry} takes exactly those box arguments
 * and returns the same solid with its X extent wrapped onto the host arc — its
 * two faces are concentric arcs and its ends are RADIAL, matching the radial
 * jambs `CurvedWallOpeningBuilder` already cuts the void with. The depth extent
 * is measured along the local NORMAL, so pane thickness is radial too.
 *
 * ⚠ A STRAIGHT HOST NEVER REACHES THIS MODULE. `WindowBuilder` calls
 * {@link leafArc}, gets `null`, and emits the ordinary `BoxGeometry` it always
 * has. That is not an optimisation, it is the safety property slice 0 pins: the
 * straight leaf is byte-identical because it is literally the same code path.
 */
...
/**
 * The host wall's arc, expressed in the window group's LOCAL frame.
 *
 * `WindowBuilder` places the group at `hostedElementFrame().{x,z}` with
 * `rotation.y = rotationY`, which makes group-local +X the centreline TANGENT at
 * the window's centre and group-local +Z the outward NORMAL there. So the local
...
export interface LeafArc {
     ...
     *
     * This is the whole reason pane and reveal cannot disagree at their shared
...
/**
 * The `LeafArc` for a window at `offset`/`width` on `wall`, or `null` when the
 * host is not a curved one and the ordinary flat leaf must be built.
 *
...
export function curvedLeafRefusal(wall: unknown): string | null {
    ...
    if (rakeShearPerMetre(w.rakeAngleDeg) !== 0) {
        return 'A window cannot follow the curve of a wall that is also raked: '
 ...
 *
 * Arguments mirror `WindowBuilder`'s `addBox(w, h, d, x, y, z)` exactly, so a
 * call site converts by swapping the function — there is no second coordinate
 ...
 *
 * A jamb, mullion or sash stile is a vertical ruling of the sweep and stays
 ...
 * or a mullion halfway along a wide curved window would stand proud of the glass
