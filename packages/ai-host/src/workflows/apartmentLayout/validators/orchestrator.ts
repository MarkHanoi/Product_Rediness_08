...
//
// Runs the 11 shipped validator slices in ONE pass on a single
...
//   • Per-validator order is fixed (G-1 → G-2 → ... → A-5) so the report
//     surfaces violations in a stable, test-pinnable sequence.
...
/**
 * Run the 11 shipped validators on a single apartment layout.
 *
...
export function validateApartmentLayout(
    ...
    // ── Dimensional (G-class) — 6 validators, fixed order ───────────────────
    // Each validator reads only the fields it cares about from
    // ApartmentLayoutRoom; the superset shape feeds all six unchanged.
    ...
    // ── Topology (A-class) — 5 validators, fixed order ──────────────────────
    // Topology validators take rooms as `{ id, type }`. Our richer rooms
    // satisfy that shape structurally (id + type are present), so we pass
    // input.rooms through directly — no projection / no allocation.
