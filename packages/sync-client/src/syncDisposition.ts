...
export const SYNC_DISPOSITIONS: Readonly<Record<string, SyncDisposition>> = {
  ...
  // ── §FEAT-CONSTRUCTION-BOUNDARY-LINE (L-7962, C106) — the AUTHORED setting-out line ──
  //
  ...
  // NOTHING (C106 §6 — deleting it does not delete what was built along it), so there
  ...
  // column attached to it (C106 §3). Two collaborators dragging the same line are not
  ...
  // Declared in the SAME commit as the verbs. YjsDocAdapter._applyDeclaredProperties
  // warns for EVERY dispatched type, so an undeclared verb is a production warning
  // whether or not a gate is watching (the L-937 shape).
  'siteworks.setWidth':     { kind: 'element-property', subject: 'id', conflict: 'disclose' },
  'siteworks.setThickness': { kind: 'element-property', subject: 'id', conflict: 'disclose' },
  'siteworks.setRole':      { kind: 'element-property', subject: 'id', conflict: 'disclose' },
  'siteworks.batch.create': { kind: 'not-synced', reason: 'MULTI-SUBJECT NEEDS-MULTI-SUBJECT-KIND: the payload is `{surfaces: SiteworksCreateSpec[]}` applied in ONE produceCommand, and there is no singular create to re-dispatch (C116 6a), so one `subject` key cannot name the set. Same backlog as spaceEnvelope.batch.create and beam.batch.create. CONSEQUENCE, STATED PLAINLY: until the multi-subject kind exists a collaborator never receives the surface AT ALL - not a stale value, no value - while the three siteworks.set* verbs above ARE declared, so an edit would merge onto an element the peer never got. That asymmetry is why this row exists rather than silence.' },
  'siteworks.delete':       { kind: 'not-synced', reason: 'LIFECYCLE + MULTI-SUBJECT NEEDS-TOMBSTONE-KIND: the payload is `{ids: string[]}`, so it needs BOTH the tombstone kind annotation.delete is waiting on AND the multi-subject kind, and declaring it against either alone would replicate half a gesture. C116 8 records that this family triggers NO cascade - no room redetection, no wall rebuild, no slab region trace - so unlike spaceEnvelope.delete there is no second write for a tombstone route to replicate; the only thing owed is the tombstone itself.' },

  // ⛔ NOT SYNCED — and each for a reason already ruled on elsewhere in this table.
  'spaceEnvelope.batch.create': { kind: 'not-synced', reason: 'MULTI-SUBJECT §NEEDS-MULTI-SUBJECT-KIND: payload is `{envelopes: CreateSpaceEnvelopeSpec[]}` applied in ONE produceCommand; no singular create is re-dispatched, so one `subject` key cannot name the set. See beam.batch.create. ⭐ THE MOST CONSEQUENTIAL MEMBER OF THAT BACKLOG (L-13012): this is the verb that CREATES the authored massing the whole STR §26.4 story is built on, so until the multi-subject kind exists a collaborator never receives the envelope at all — not a stale value, no value. The three `spaceEnvelope.set*` verbs above ARE declared, so an edit would merge onto an element the peer never got. ⛔ AND THE PAYLOAD IS NOT ONLY A CREATE SET — §ENVELOPE-DRAW R8 (L-13047) added `supersedes`, a REMOVAL set of spaceEnvelopeIds the same batch retires so authoring twice REPLACES instead of accumulating (the founder\'s L-13038 ruling). So this one verb carries a multi-subject create AND a multi-subject delete in one atomic gesture, and it therefore needs BOTH the multi-subject kind and the tombstone kind `spaceEnvelope.delete` is waiting on — declaring it against either alone would replicate half a gesture. That is a WORSE failure than not replicating it: a peer that receives the creates without the removals ends up with the duplicated envelopes the supersession exists to prevent, on a storey the author sees as clean.' },
  // ─── ADR-0383 S4 · THE THREE MASSING-GROUP VERBS ────────────────────────────────────
  // ⛔ ALL THREE ADDRESS A SET BY GROUP MEMBERSHIP, WHICH NO EXISTING KIND CAN NAME. The
  // subject is not an element id at all — it is *"every envelope whose `group.id` is X"*, a
  // membership QUERY resolved against the local store at execute time. That is a third thing
  // alongside the multi-subject and tombstone kinds already on the backlog, and it is worse
  // than either: two peers can resolve the SAME payload to DIFFERENT record sets, because
  // membership depends on state each of them holds separately.
  //
  // ⚠ CONSEQUENCE, STATED PLAINLY (C114 §14i clause 3): a master plan is a SINGLE-USER
  // artefact today. `spaceEnvelope.batch.create` already has no disposition (L-13012), so a
  // collaborator never receives the envelopes AT ALL; these three verbs make that larger
  // rather than introducing it. Recorded here instead of left silent, because the three
  // `spaceEnvelope.set*` verbs ARE declared synced, and that asymmetry is what a reader needs.
  'spaceEnvelope.group.setStoreys': { kind: 'not-synced', reason: 'MEMBERSHIP-SET + MULTI-SUBJECT + LIFECYCLE §NEEDS-MULTI-SUBJECT-KIND / §NEEDS-TOMBSTONE-KIND: the payload is `{groupId, targetStoreys, added[]}` and the subject is a GROUP, not an element — one `subject` key cannot name a membership set, and two peers may resolve the same groupId to different records. It is also BOTH directions in one verb (ADR-0383 §4 / C114 §6e clause 4), so a shrink is a multi-subject DELETE that additionally clears `withinId` on the rooms inside the removed storeys (C114 §8) — exactly the second write `spaceEnvelope.delete` is already waiting on a tombstone kind for. Declaring it against any one of the three kinds would replicate part of a gesture, which is worse than not replicating it: a peer that receives a grow without the ring copy, or a shrink without the withinId clear, holds a building the author never made.' },
  'spaceEnvelope.group.rename':     { kind: 'not-synced', reason: 'MEMBERSHIP-SET §NEEDS-MULTI-SUBJECT-KIND: the payload is `{groupId, label}` and it rewrites `group.label` on EVERY member in one produceCommand (deliberately — `label` is denormalised across members and this verb is its ONLY writer, per SpaceEnvelopeGroupSchema). The subject is therefore a membership set resolved locally, not a `spaceEnvelopeId`. ⚠ A per-record `element-property` route is NOT the fix and must not be reached for: it would replicate N independent property writes, so a peer joining mid-flight could apply some and not others and land in exactly the label DISAGREEMENT this verb exists to repair — with the reader correctly reporting a drift neither user caused.' },
  'spaceEnvelope.group.dissolve':   { kind: 'not-synced', reason: 'MEMBERSHIP-SET §NEEDS-MULTI-SUBJECT-KIND: the payload is `{groupId}` and it clears `group` on every member. ⭐ It does NOT delete (ADR-0383 §4), so unlike `spaceEnvelope.delete` no tombstone is owed — the only thing missing is a kind that can name the membership set. ⛔ And the set must be resolved on the ORIGIN and travel as ids: resolving `groupId` again on the peer would clear the group on whatever THAT peer thinks the members are, which after any divergence is a different building.' },
  ...
  // hardest. A boundary line OWNS NOTHING (C106 §6): deleting it removes ONE record from
  ...
  'boundaryLine.delete': { kind: 'not-synced', reason: 'LIFECYCLE §NEEDS-TOMBSTONE-KIND: payload is `{boundaryLineId}` only. NO cascade — a boundary line owns nothing (C106 §6), so unlike pool/lift/balcony this needs only the tombstone kind itself. See annotation.delete.' },
  ...
  'curtain-wall.changeLevel': { kind: 'not-synced', reason: 'DOCUMENT MOVE §NEEDS-DOC-MOVE-KIND: `{curtainWallId, levelId}`. The assembly moves as one and its panels are a COLLECTION on the record (see curtain-wall.addPanel), so this is a document move over a multi-member element. See wall.changeLevel.' },
