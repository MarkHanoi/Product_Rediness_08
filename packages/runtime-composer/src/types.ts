...
export interface RuntimeEvents {
  ...
  /*
   * ═══════════════════════════════════════════════════════════════════════════
   * §CEB-DELETE-COLUMN-CENSUS (P3-C2, 2026-08-31) — THIS IS THE ONLY `.deleted`
   * CHANNEL IN THE CONTRACT, AND ADDING MORE HERE ALONE WOULD BE A REGRESSION.
   * ═══════════════════════════════════════════════════════════════════════════
   * The AXIS-C measurement: 40 delete verbs, 2 mirrored (this one and
   * `slab.removeHole`, which is an ELEMENT_UPDATE_VERBS row rather than a
   * removal). 26 of the 32 declared element channels are `.created`. That reads
   * like 38 missing rows. It is not, and the reason is worth keeping.
   *
   * ⓵ WHAT IS ACTUALLY DISPATCHED. Of the 40, twenty-nine have ZERO literal
   *    dispatch site outside their own handler (audit 2026-08-31, 5120 production
   *    files). `wall.delete` and `curtain-wall.delete` have one each and BOTH are
   *    `apps/bench` benchmarks. A channel for a verb nothing dispatches is a
   *    channel into nothing — the same defect as P3-C3, arrived at from the other
   *    side.
   *
   * ⓶ WHAT THE PRODUCT ACTUALLY DELETES WITH. `element.delete` (4 live sites:
   *    initUI, canvasSubjectDelete, PropertyPanel, SelectionManager) and
   *    `element.deleteBatch` (ContextualEditBar). `DeleteElementHandler`
   *    (`plugins/view/src/handlers/DeleteElement.ts`) declares
   *    `affectedStores: []`, returns `{forward: [], inverse: []}` and delegates to
   *    `window.commandManager.execute(resolveDeleteCommand(...))`, which removes
   *    from the LEGACY geometry stores itself and self-discovers across ~16 of
   *    them. THERE ARE NO PATCHES FOR A BRIDGE TO READ, and the removal has
   *    already happened by the time this bridge would emit.
   *
   * ⛔ SO A PER-FAMILY `.deleted` CHANNEL PLUS A SUBSCRIBER THAT REMOVES FROM THE
   * LEGACY STORE WOULD BE A SECOND PRODUCER ON A RING `DeleteElementCommand`
   * ALREADY OWNS. And the emit half ALONE — which is all this file can do — mints
   * nine more zero-subscriber channels, i.e. the P3-C3 defect that the same brief
   * asks to remove. Both halves must land in ONE change, in a lane that owns this
   * file AND the app-side mirror, and it must REPLACE the legacy discovery scan
   * rather than race it.
   *
   * WHY `boundaryLine.deleted` IS THE EXCEPTION AND NOT THE TEMPLATE: a boundary
   * line is a BUS-ONLY family. `DeleteElementCommand`'s store scan does not reach
   * it, so this channel is the family's only removal path and races nothing. That
   * property — not its shape — is what qualified it, and it is the question to ask
   * of any family proposed for a channel next.
   */
  ...
  'slab.created': {
    ...
    /** §FT1: slab element id.
     *  ⚠ §FIX-SLAB-BATCH-CEB-SHAPE — this used to read "present for single
     *  slab.create only". It was already false: `slab.batch.create` fans out one
     *  event per member and has carried `id` since TASK-01. The stale half-truth
     *  is worth correcting because it is the sentence under which the batch arm's
     *  `if (!s.id) continue;` looked deliberate rather than a silent drop of every
     *  handler-minted id. Both arms now take the id from the COMMIT when the
     *  payload omits one. */
    readonly id?: string;
    /** §FT1: IFC GUID — as generated by SlabPlanToolHandler (crypto.randomUUID()).
     *  Payload-only: it is not on the L0 `Slab` schema, so there is no committed
     *  value to prefer and the §FT1 mirror mints one when this is absent. */
    readonly ifcGuid?: string;
    /** §FT1: plan-view polygon — {x, y}[] where y = worldZ (plan-tool coordinate
     *  convention). Matches SlabData.polygon shape.
     *  ⛔ THE SECOND COORDINATE IS worldZ, NEVER the world `y`. Both bridge arms
     *  reach it through `planPolygonFromCommittedBoundary`, which is the ONE rule
     *  correct under both live conventions; the batch arm relayed the world Vec3
     *  raw until §FIX-SLAB-BATCH-CEB-SHAPE, which put the LEVEL ELEVATION in this
     *  field and collapsed the ring to zero area. */
  ...
  /*
   * ═══════════════════════════════════════════════════════════════════════════
   * §CEB-DEAD-CHANNEL-CENSUS (P3-C3) — the `*.layer-updated` TRIO: emitted into
   * ZERO subscribers, from verbs with ZERO live dispatch sites.
   * Disposition: KEEP-WITH-REASON. NOT a wiring gap.
   * ═══════════════════════════════════════════════════════════════════════════
   * The P3 AXIS-C brief names `slab/ceiling/floor.updateLayers` as "the bridge's
   * ONLY update arm besides the two tables", which made them look like the most
   * valuable three rows in this file to wire up. They are the opposite.
   *
   * MEASURED:
   *   grep -rn "on('slab.layer-updated'"   apps packages plugins  ->  0  (all 3)
   *   grep -n  "slab.updateLayers" apps/editor/src/engine/initBusHandlers.ts
   *     -> 2 hits, BOTH INSIDE COMMENTS (:78, :1725). The audit's "1 dispatch
   *        site" for this verb is a comment, not a dispatch.
   *   `ceiling.updateLayers` / `floor.updateLayers` -> 0 dispatch sites at all.
   *
   * ⛔ AND THE COMMENTS SAY WHY, IN THE PRODUCT'S OWN WORDS. `initBusHandlers.ts`
   * :1719-1730 (§FIX-SLAB-TYPE-SWAP, mirroring §FIX-FLOOR-TYPE-SWAP L-106):
   * `element.changeType` DELIBERATELY STOPPED dispatching `slab.updateLayers`
   * because that verb "targeted the plugin Immer slab store, which is DETACHED /
   * empty for SlabTool-created slabs", and now runs `UpdateSlabLayersCommand` on
   * the LEGACY SlabStore — the store the fragment builder, the plan projector, the
   * IFC exporter and persistence read — plus a ring-buffer PatchPair for undo
   * parity. The layer/type swap IS live; it does not travel this way.
   *
   * SO WIRING A ROW INTO `ELEMENT_UPDATE_VERBS` FOR THESE THREE WOULD MINT A
   * RIVAL of that path, on the same store, for the same gesture — the failure
   * `ELEMENT_UPDATE_VERBS`'s own header forbids by name for
   * `slab.movePolygon` ("adding a rival path here would put two producers on one
   * ring"). The three channels stay declared, unwired, and named here so the next
   * reader does not have to re-derive the refutation.
   */
  ...
  /*
   * ═══════════════════════════════════════════════════════════════════════════
   * §CEB-DEAD-CHANNEL-CENSUS (P3-C3, 2026-08-31) — door / window / stair are
   * DECLARED, NEVER EMITTED and NEVER SUBSCRIBED. Disposition: KEEP-WITH-REASON.
   * ═══════════════════════════════════════════════════════════════════════════
   * Measured, not assumed — for each of the three channels below,
   *   grep -rn "emit('<ch>'" apps packages plugins  ->  0
   *   grep -rn "on('<ch>'"   apps packages plugins  ->  0
   * and there is no `case 'door.create'` / `'window.create'` / `'stair.create'`
   * anywhere in `CommandEventBridge`.
   *
   * ⛔ THAT IS NOT A MISSING LISTENER. It is superseded architecture, and the
   * supersession is legible at each dispatch site:
   *   · DOOR   — `plugins/door/src/tool.ts:84` dispatches `wall.createOpening`,
   *     and its own comment at :98 records that it USED to follow with
   *     `door.create` and no longer does. Doors reach the model as hosted
   *     openings (C15), relayed by `wall.opening.created`, which HAS a live
   *     subscriber. Emitting `door.created` beside it would put two producers on
   *     one ring.
   *   · WINDOW — the same route, and stronger: `window.create` has ZERO literal
   *     dispatch sites in production (audit 2026-08-31, 5120 files).
   *   · STAIR  — `stair.create` has FOUR live dispatch sites and is one of the 22
   *     verbs with an EXECUTED read-back proof, so the family already lands
   *     without this channel; and the channel carries no geometry (commandId,
   *     commandType, levelId, elementCount) so no mirror could rebuild from it
   *     even if one existed.
   *
   * WHY KEPT RATHER THAN DELETED: removing a key from `RuntimeEvents` is a
   * NON-ADDITIVE change to a published event contract (C47), and these three cost
   * nothing at runtime — no emitter reaches them. The honest state is a named
   * zero with its reason attached (§CONTEXT-DATA-HONESTY / C70 L-INV-1), not a
   * silent deletion and not a silent survival. ⛔ Do not "fix" these by adding an
   * emitter: a `door.created` with no consumer is the P3-C3 defect, and a
   * `door.created` WITH one is a rival of the opening path.
   */
