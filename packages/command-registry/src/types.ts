export enum CommandType {
    ...
    /**
     * L-7201 — floor-to-floor HEIGHT edit. Distinct from UPDATE_LEVEL because it
     * is a rigid translation of every level ABOVE, not a property write on one
    ...
    // §FIX-HOSTED-TYPE-CHANGE (L-620) — the properties-panel Door/Window Type swap.
    UPDATE_DOOR_SYSTEM_TYPE = 'UPDATE_DOOR_SYSTEM_TYPE',
    UPDATE_WINDOW_SYSTEM_TYPE = 'UPDATE_WINDOW_SYSTEM_TYPE',
    // §FEAT-WINDOW-TYPE-BATCH (ADR-0315) — retype MANY windows (or 'all') in ONE undo step.
    ...
    // §FEAT-WALL-TYPE-BATCH (RAC prep) — retype MANY walls (or 'all') in ONE undo step.
    UPDATE_WALLS_SYSTEM_TYPE_BATCH = 'UPDATE_WALLS_SYSTEM_TYPE_BATCH',
    // §FEAT-WALL-COLOR-BATCH (ADR-0314) — recolour MANY walls (or 'all') in ONE undo step.
    UPDATE_WALLS_COLOR_BATCH = 'UPDATE_WALLS_COLOR_BATCH',
    // §FEAT-WALL-RAKE-BATCH (ADR-0315) — rake MANY walls (or 'all') in ONE undo step.
    ...
    // §FEAT-FLOOR-SURFACE-FINISH (L-1881) — the floor twin of the two above.
    SET_FLOOR_FINISH = 'SET_FLOOR_FINISH',
    SET_FLOOR_FINISH_BATCH = 'SET_FLOOR_FINISH_BATCH',
    ...
    // §PER-CATEGORY-VIEW-VISIBILITY (L-1894) — the per-CATEGORY twin of the row above.
    ...
    // §ROOMTYPE142 — content-based name+occupancy autofill for many rooms, one undo.
    BULK_AUTO_CLASSIFY_ROOMS          = 'BULK_AUTO_CLASSIFY_ROOMS',
