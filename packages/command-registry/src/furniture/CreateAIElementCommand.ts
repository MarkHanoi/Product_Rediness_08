...
// §FIX-INTERIOR-FFL-SEATING — the floor datum is the FINISHED floor, resolved once at
// the chokepoint (C11 §5.4), never re-derived here as `level.elevation`.
...
export class CreateAIElementCommand implements Command {
    ...
    execute(context: CommandContext): CommandResult {
        ...
        // §FIX-INTERIOR-FFL-SEATING — an AI element (kitchen run, joinery, appliance)
        // stands on the FINISHED floor, not the structural slab top. Resolve the
        // seating datum at THIS item's plan position: one level carries several
        // finishes of different thickness plus unfinished regions, so the answer is
        // position-dependent. Falls back to the slab top where nothing covers it.
