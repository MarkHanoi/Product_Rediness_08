...
export class UpdateDoorLeafColorCommand implements Command {
    readonly affectedStores = ["door", "wall"] as const;
    id: string = crypto.randomUUID();
    type = CommandType.UPDATE_DOOR_LEAF_COLOR;
    ...
    execute(context: CommandContext): CommandResult {
        ...
        this.oldValue = richDoor?.leafColor ?? '#f2f0ed';

        wallStore.updateDoor(this.doorId, { leafColor: this.newValue });
        if (doorStore.has(this.doorId)) {
            doorStore.update(this.doorId, { leafColor: this.newValue });
        }
    ...
    undo(context: CommandContext): CommandResult {
        ...
        context.stores.wallStore.updateDoor(this.doorId, { leafColor: this.oldValue });
        if (doorStore.has(this.doorId)) {
            doorStore.update(this.doorId, { leafColor: this.oldValue! });
        }
        return { success: true, affectedElementIds: [this.doorId] };
    }
