...
export class UpdateDoorFireRatingCommand implements Command {
    readonly affectedStores = ["door", "wall"] as const;
    id: string = crypto.randomUUID();
    type = CommandType.UPDATE_DOOR_FIRE_RATING;
    ...
    execute(context: CommandContext): CommandResult {
        ...
        this.oldValue = door.fireRating || '';
        wallStore.updateDoor(this.doorId, { fireRating: this.newValue });
        if (doorStore.has(this.doorId)) {
            doorStore.update(this.doorId, { fireRating: this.newValue });
        }
    ...
    undo(context: CommandContext): CommandResult {
        ...
        context.stores.wallStore.updateDoor(this.doorId, { fireRating: this.oldValue });
        if (doorStore.has(this.doorId)) {
            doorStore.update(this.doorId, { fireRating: this.oldValue! });
        }
        return { success: true, affectedElementIds: [this.doorId] };
    }
