 ...
 *
 * Protocol: `../types`. `pointInPolygonXZ` comes from `@pryzm/geometry-kernel`
 * DIRECTLY (§C73-PIP-CANONICAL definition site) — the plugin-side copy reached
 * it via `@pryzm/plugin-sdk`, but command-registry is L2 and the SDK is L5:
 * keeping that specifier would MINT a new upward import (scout §3.1b).
...
// §C73-PIP-CANONICAL — delegates to THE kernel ray cast at its definition site
...
export class CreateElevationMarkCommand implements Command {
    ...
    execute(ctx: CommandContext): CommandResult {
        return _tracer().startActiveSpan('pryzm.annotation.elevation_mark.create', (span) => {
            try {
                const viewStore   = resolveViewDefinitionStore(ctx);
                ...
                if (!view) { span.end(); return { success: false, affectedElementIds: [], error: 'Failed to create elevation ViewDefinition.' }; }
