...
/**
 * §WALL-PROVENANCE (L-13117) — ONE claim that this wall is an edge of another element.
 *
 * ⛔ **THIS IS A LICENSED COPY, NOT A SECOND DECLARATION.** The MASTER is
 * `ElementDerivationSchema` in `packages/schemas/src/provenance/ElementDerivation.ts`, which
 * carries the full rationale and is the thing `Wall.parse` validates against. It is mirrored
 * here as a plain interface rather than imported because `@pryzm/schemas` is not a declared
 * dependency of this package (measured 2026-09-07: `packages/geometry-wall/package.json` lists
 * nine `@pryzm/*` deps and `schemas` is not among them; the two existing imports of it in this
 * package are phantom, resolving only by hoist).
 *
 * ⚠ C84 EI-8a: a licensed copy MUST be pinned to its master by an executed test comparing every
 * field — a comment is not a synchronisation mechanism, and this repo has watched that fail
 * twice. `apps/editor/__tests__/L13117WallProvenanceSurvivesReload.test.ts` is that pin (it reads
 * BOTH declarations out of source and compares the field SETS).
 * Add a field to the master and this mirror in the SAME commit, or that test fails.
 */
export interface WallDerivation {
    /** Id of the element this wall was derived FROM — a space-envelope id today. */
    sourceElementId: string;
    /** Family of the source, so a reader knows which store resolves it. `'space-envelope'` today. */
    sourceKind: string;
    /**
     * Index into the source's stored OPEN footprint ring; edge *i* runs
     * `footprint[i] → footprint[(i + 1) % n]`. `-1` means the edge could NOT be recovered —
     * never index 0, because attaching a real wall to the wrong face moves the wrong wall.
     */
    edgeIndex: number;
    /** Which producer stamped this row. Free text — the generators are not enumerated anywhere. */
    generator: string;
    /** `'primary'` = the source this wall was generated from; `'also'` = a further boundary it is. */
    claim: 'primary' | 'also';
    /** `'shell'` / `'partition'` for a wall. Absent when the producer does not distinguish roles. */
    role?: string;
    /** The source ring was reconciled with the wall domain before indexing — see the master. */
    ringWelded?: boolean;
}
...
export interface WallData extends CoreElement {
    ...
    //
    // The element-lineage half of provenance. `provenance` (C75 `ValueOrigin`) says what
    // KIND of thing produced a value; it carries NO ELEMENT ID, and that measured gap is
    // why the envelope→wall relationship lived only in a semantic-graph side table that
    // exactly ONE producer writes. A wall generated by any other route therefore reported
    // "No walls are linked to this envelope yet" on a face drag and moved nothing.
    //
    // ⛔ THREE VALUES, NOT TWO (§CONTEXT-DATA-HONESTY — failure and empty are different):
    //   undefined ⇒ NOT RECORDED. A wall saved before this field existed, or made by a
    //               producer that does not stamp. A consumer may conclude NOTHING from it —
    //               in particular it is NOT "this wall belongs to no envelope".
    //   []        ⇒ KNOWN-EMPTY. A stamping producer looked and found no source element.
    //   [row, …]  ⇒ the claims, in the producer's own order.
    // Never defaulted to `[]` anywhere on the read or write path. Absent stays absent, and
    // a pre-L-13117 wall re-serialises byte-identically (C47 §1.2 additive-optional).
    //
    // An ARRAY because one wall can bound two sources (a shell edge AND a shared partition
    // edge), and because C80's "two sources placing one wall differently ⇒ CONTESTED" is
    // only statable if both rival claims can be present.
