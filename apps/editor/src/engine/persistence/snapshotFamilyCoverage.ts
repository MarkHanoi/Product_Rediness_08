...
export const SNAPSHOT_FAMILY_COVERAGE: readonly SnapshotFamilyRow[] = Object.freeze([
    ...
    // ── C116 / ADR-0384 — SITEWORKS: roads, parking, pedestrian areas ─────────
    { storeKey: 'siteworks', status: 'persisted', snapshotKey: 'siteworks',
      reason: 'C116 §9 · ADR-0384 — AUTHORED paved surfaces (roads, parking areas, pedestrian areas) as ONE kind wearing three roles. ⭐ THE ROW LANDS IN THE SAME COMMIT AS THE CHANNEL AND EVERY LEG IS EXECUTED BY A NAMED ARM, which is the R11 / L-11530 lesson applied FORWARDS — the `component` and `spaceEnvelope` discipline, not the `balcony` one: (1) `StoresSlot.siteworks` is declared in `packages/runtime-composer/src/types.ts` and ADOPTED in `composeRuntime.ts` from the `siteworks` PluginRegistry descriptor, never constructed; (2) `ProjectSerializer.serialize()` reads it through the shared lazy `readPluginStore(\'siteworks\')` and writes `siteworks`, omit-when-absent per C47 so an untouched project stays byte-identical and no SNAPSHOT_SCHEMA_VERSION bump is needed; (3) `restoreCompoundFamilies()` reads that slice back through `resolveComposedStoreFromWindow(\'siteworks\')` in the loader\'s COMMON TAIL — never inside one branch, which is L-11528; (4) `apps/editor/__tests__/siteworksReachableThroughComposedRuntime.test.ts` boots the REAL composition root, dispatches `siteworks.batch.create`, reads the record back OUT of `rt.stores.siteworks` (C16 CA-21 — never the handler\'s return, never a spy), then serialises, clears, restores and reads it back again, and a scramble control breaks each leg in turn to prove the arms bind. ⛔ THE LOSS THIS PREVENTS IS TOTAL, NOT PARTIAL: C116 §2 declines this family a plugin DTO twin, a `core-app-model` store and a `packages/stores` entry BY CONSTRUCTION, so there is no legacy mirror to leave anything on screen — the centreline, the width, the thickness, the role and the material vanish together, exactly the asymmetry that let `balcony` go unnoticed for four days while its slab and railings survived. ⛔ AND WHAT THIS ROW DOES NOT CLAIM (C84 §6.2c): nothing about IFC export, which C116 §1 EXCLUDES until `IfcCourse` vs `IfcPavement` is resolved, and nothing about the DERIVED ring — the swept footprint of a linear surface is deliberately NOT a field (ADR-0384 D2), so what is persisted is the centreline and the width the user authored, and the ring is recomputed by `sweepCentrelineToRing` on read.' },
