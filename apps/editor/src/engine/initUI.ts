...
export async function initUI(p: UIParams): Promise<void> {
    ...
    // every 3D entry and DIFFS against the first one, which is the one measurement
    // that tells "present from first paint" apart from "arrived via a view switch".
    // Read-only, one traverse per 3D activation — the same order as the three overlay
    // gates initScene already runs on that event.
    installLineworkProbe();
    ...
    async function processIfcFile(file: File, importMode: 'reference' | 'native' = 'reference', addLevels = false): Promise<void> {
        ...
        file.arrayBuffer().then(async (buffer: ArrayBuffer) => {
            ...
            // survives into project B — which is exactly what that arm exists to catch, and
            // could not, for as long as it keyed on `isIfcGroup`. Stamping the owner here is
            // what makes "this project's import" distinguishable from "the previous project's".
            try {
                const rt = window.runtime;
                (renderedModel.group.userData as Record<string, unknown>).projectId =
                    rt ? resolveActiveProjectId(rt) : null;
            } catch { /* attribution must never break an import */ }
