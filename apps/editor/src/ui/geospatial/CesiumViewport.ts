...
// §MASSING-FOLLOWS-THE-ARC (L-11172) — curved shell walls reach the globe as their chord run
// (the repo's ONE sampler + ONE density, see formaWallCurve.ts), and the perimeter tracer
// lives beside the other pure massing decisions so the ring the user sees is provable.
import { expandFormaWallsToChords, type FormaWallCurve } from "./formaWallCurve";
import { reconstructPerimeterRingFromWalls } from "./formaPerimeterRing";
 ...
 * constants are removed: `scene.globe.clippingPolygons` can only clip the GLOBE SURFACE, but the visible
 * "outside terrain" in Forma is a stack of FLAT ENTITY POLYGONS (land-use / sea / roads / rails, drawn at
 * absolute height with `clampToGround:false`, §CTX-ABS-SEAT) that a globe clip provably cannot touch — so
 * it could never produce the clean floating disc, and the tan skirt wall read as the founder's "red ring".
 * The feature reverts to LOGGED-NOT-BUILT (V1-audit L-645): a correct build needs the SPEC + ratifying
 * contract + a clip mechanism chosen against the C12 quantized-mesh terrain reality FIRST, not a third
 * render-time guess.
 */
/**
...
export class CesiumViewport {
  ...
  public renderFormaMassing(input: {
    ...
    walls: ReadonlyArray<{
      ...
      /**
       * §MASSING-FOLLOWS-THE-ARC (L-11172) — present ⇒ the wall is the quadratic Bézier
       * `a → control → b` (`Wall.curve` projected to the plan, plus its pre-trim frame).
       * Every consumer that turns this wall into plan geometry — the storey ring, the
       * per-wall-box fallback, the footprint bbox — expands it through the repo's ONE
       * sampler/density (`formaWallCurve.ts`). Absent ⇒ straight, byte-identical to before.
       */
  ...
  /**
   * §CTX-EARTH-SLAB (L-645) — RETIRED. This method now ONLY tears down any leftover globe clip; it never
   * re-applies one. Kept (and called on every context load + on clearContextBuildings) so a browser
   * running a STALE cached bundle — which may still hold `scene.globe.clippingPolygons` from the shipped
   * slab — is HEALED back to full terrain on the next load. Idempotent, Forma-safe, never throws.
   *
   * ── ROOT CAUSE OF THE TWO FAILED FIXES (why the globe-clip slab is the wrong primitive here) ──
   *  • SYMPTOM "outside terrain still fully visible": `scene.globe.clippingPolygons` clips ONLY the GLOBE
   *    SURFACE. But in Forma the visible "outside terrain / city / sea" is NOT the globe — it is a stack
   *    of FLAT ENTITY POLYGONS (land-use ~8 km, sea ~11 km, parks, roads, rails) drawn at ABSOLUTE height
   *    with `clampToGround:false` (§CTX-ABS-SEAT, L-635, because Forma runs `depthTestAgainstTerrain=false`),
   *    plus the extruded building tiers. A globe clip provably cannot touch entities/primitives, so every
   *    wide flat layer stayed exactly where it was — the clip changed almost nothing on screen. (The
   *    WebGL-2 `isSupported` gate from fix #2 was real but not the cause: the founder's context IS WebGL 2,
   *    the clip DID apply, and it STILL did not clip, because there was nothing on the globe to clip.)
   *  • SYMPTOM "bright red ring": no code anywhere draws red (verified: no Color.RED / #FF0000 / edgeColor).
   *    The ONLY geometry at the disc boundary was the cut-section SKIRT — a 64-segment vertical `wall`
   *    entity, material `FORMA_PALETTE.ground` (#D6C7A6, a warm tan), ~60 m tall — which, seen edge-on at
   *    the rim under the warm Forma key light, read as the "red/terracotta ring". It was our skirt, not a
   *    Cesium clip edge (ClippingPolygonCollection draws no edge).
   *
   * A correct "clean floating disc" needs (a) a terrain clip that also removes/masks the FLAT entity
   * ground layers to the disc, and (b) outside-terrain suppression — chosen against the C12 quantized-mesh
   * terrain reality (clippingPlanes vs a baked bounded mesh), behind the SPEC + ratifying contract the
   * V1-audit L-645 requires. That is deliberately NOT attempted as a third render-time guess here; the
  ...
  private groupWallsIntoStoreyBands(
    ...
    heightM: number;
    levelId?: string;
    walls: Array<{ a: { x: number; z: number }; b: { x: number; z: number }; height: number; thickness: number; curve?: FormaWallCurve | null }>;
  }> {
    const STOREY_BAND_TOL_M = 0.5;
    type Band = {
      baseElevation: number;
    ...
    for (const w of walls) {
      ...
      // §MASSING-FOLLOWS-THE-ARC (L-11172) — carry the curve into the band; a straight wall's
      // band object keeps exactly the four keys it had.
      band.walls.push(w.curve
        ? { a: w.a, b: w.b, height: h, thickness: w.thickness, curve: w.curve }
        : { a: w.a, b: w.b, height: h, thickness: w.thickness });
  ...
  private extrudeWallsAsBoxes(
    walls: ReadonlyArray<{ a: { x: number; z: number }; b: { x: number; z: number }; height: number; thickness: number; curve?: FormaWallCurve | null }>,
    bandBottom: number,
    bandElevBase: number,
    bandIndex: number,
    viewer: Cesium.Viewer,
    ...
    try {
      // §MASSING-FOLLOWS-THE-ARC (L-11172) — a curved wall falls back to a fan of chord boxes
      // along its arc, not one box across its chord (straight-only input is the same array).
      for (const w of expandFormaWallsToChords(walls)) {
  ...
  private reconstructPerimeterRing(
    walls: ReadonlyArray<{ a: { x: number; z: number }; b: { x: number; z: number }; curve?: FormaWallCurve | null }>,
  ): Array<{ x: number; z: number }> | null {
    // §MASSING-FOLLOWS-THE-ARC (L-11172) — the tracer moved VERBATIM to formaPerimeterRing.ts
    // so the ring the storey prism extrudes (and the façade study samples) is provable under
    // the unit config; curved shell walls are expanded into their chord run there, so the
    // ring follows the Bézier. This delegate keeps every call site untouched.
    return reconstructPerimeterRingFromWalls(walls);
  }
  ...
  private footprintBBoxXZ(
    ...
    // §MASSING-FOLLOWS-THE-ARC (L-11172) — an arc that bows past its chord must reach the extent.
    for (const w of expandFormaWallsToChords(walls)) { acc(w.a.x, w.a.z); acc(w.b.x, w.b.z); }
