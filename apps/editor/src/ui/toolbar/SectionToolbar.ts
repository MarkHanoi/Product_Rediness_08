...
export class SectionToolbar {
    ...
    triggerCommand(commandType: string, payload: Record<string, unknown> = {}): void {
        ...
        if (refuseUnbacked(commandType, SECTION_TOOLBAR_ID)) return;
    ...
    private _makeButton(def: SectionToolbarButton): HTMLButtonElement {
        ...
        applyCommandBacking(btn, def.commandType, SECTION_TOOLBAR_ID);
        btn.addEventListener('click', () => {
            ...
            // §L-MOUNT Phase 3 — refuse, never dispatch into nothing. Guards the
            // PROGRAMMATIC door too (triggerCommand), which `disabled` cannot.
            if (refuseUnbacked(def.commandType, SECTION_TOOLBAR_ID)) return;
