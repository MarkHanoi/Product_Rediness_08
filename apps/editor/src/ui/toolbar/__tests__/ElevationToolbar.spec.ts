...
describe('ElevationToolbar — wave-6-c-d6 binding contract', () => {
    ...
    it('triggerCommand() dispatches elevation-interior when BACKED, refuses when not', () => {
        const rt = makeRuntime();
        ...
        if (isBacked('elevation-interior')) {
    ...
    it('button click without runtime — warns when BACKED, is inert when refused', () => {
        ...
        if (isBacked('elevation-interior')) {
            ...
            expect(btn.disabled).toBe(true);
            expect(btn.title).toContain('elevation-interior');
        }
        warn.mockRestore();
    });
