...
export class RoomToolbar {
    ...
    triggerCommand(commandType: string, payload: Record<string, unknown> = {}): void {
        ...
        if (refuseUnbacked(commandType, ROOM_TOOLBAR_ID)) return;
    ...
    private _makeButton(def: RoomToolbarButton): HTMLButtonElement {
        ...
        applyCommandBacking(btn, def.commandType, ROOM_TOOLBAR_ID);
        btn.addEventListener('click', () => {
            ...
            // §L-MOUNT Phase 3 — refuse, never dispatch into nothing. Guards the
            // PROGRAMMATIC door too (triggerCommand), which `disabled` cannot.
            if (refuseUnbacked(def.commandType, ROOM_TOOLBAR_ID)) return;
