...
export function buildElementsCard(bag: UBPBag): HTMLElement {
    const categories: Array<{ label: string; icon: string }> = [
        ...
        // §LIFT94 (L-11342) — THE FOUNDER: *"lift is not in the model elements within
        // the project browser"*. Measured 2026-08-25: `grep -rln lift
        // apps/editor/src/ui/ViewBrowser/` returned NOTHING. The lift was not hidden,
        // filtered or mis-counted — it was never declared, so the card could not have
        // shown it under any circumstances.
        //
        // ⛔ AND IT IS NOT `window.liftStore`. That global holds the LOD-200 MASSING
        // lift (C104 §1). The C104 COMPOUND the plan tool creates is
        // `runtime.stores.lift`, so this row follows the `Roofs` precedent — the one
        // category already read off the bag instead of a window global — rather than
        // the fifteen `window.*Store` rows above it. See `_rawCategoryElements`.
        { label: 'Lifts',             icon: `<svg width="13" height="13" viewBox="0 0 13 13" fill="none"><rect x="2.5" y="1.5" width="8" height="10" rx="0.5" stroke="#888" stroke-width="1.1"/><line x1="6.5" y1="1.5" x2="6.5" y2="11.5" stroke="#888" stroke-width="1"/><path d="M4 5.5L5 4l1 1.5" stroke="#888" stroke-width="1" stroke-linecap="round" stroke-linejoin="round"/><path d="M7 7.5L8 9l1-1.5" stroke="#888" stroke-width="1" stroke-linecap="round" stroke-linejoin="round"/></svg>` },
