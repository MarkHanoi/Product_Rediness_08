 ...
 *   [CeilingTypeSelectorWidget] Created new type: Custom Ceiling Type st-1787522180714
 *   [CeilingTypeSelectorWidget] Created new type: zfh st-1787522228499
 *
 ...
 *     `UpdateCeilingLayersCommand` on the LEGACY store is intact and exercised.
 ...
 *
 * So no `UPDATE_CEILING_TYPE` appeared in his console for the exact reason that he
 ...
 * `alert('... Re-select the ceiling to see it in the list.')` — a workaround shipped
 ...
 * committing it to this ceiling stays an explicit Apply. That is the wall widget's
 ...
 * declares only wall / door / window. There is no ceiling adapter to dispatch to.
...
export function buildCeilingTypeSelectorWidget(
    ...
    function _adoptCreatedType(t: any): void {
        ...
        // deliberately does not retype this ceiling; the user is told the remaining
        // gesture instead of being left to wonder whether anything happened.
        status.style.display = 'block';
        status.textContent = `Type "${t.name}" created and selected — press Apply to use it on this ceiling.`;
    }
...
function _handleDuplicate(
    ...
    else console.warn('[CeilingTypeSelectorWidget] Type created but not found in catalogue:', newId);
}
