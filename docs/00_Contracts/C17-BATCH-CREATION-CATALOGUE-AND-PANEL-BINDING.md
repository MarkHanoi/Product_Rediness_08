...
| AI panel › "Generate apartment layout (AI)" | **"Generate apartment layout"** (#51 capstone) | `apartment.generate-layout` → `apartment.layout-execute` | (AI workflow — NOT a batch command) | project | 6 | ✅ **A1–A7 + auto-pipeline** (floor-finish + D-CE → D-FLE → D-LE) per C09 §3.4 / §3.4.1 / §3.4.2. Two-phase generative L7.5: reached from the **AI panel** leaf (not `dispatchBatchEntry` — it is a generative workflow, not a deterministic batch), runs in-process on the AiPlane (C09 §2.4), commits via the command bus in one `runBatch`. Post-build the auto-pipeline chain emits floor finishes, ceilings, furniture + lighting end-to-end. §11 modal carries §MODAL-DYNAMIC + §ROOM-AREAS + §WINDOW-SYMBOLS + §A11Y. |
