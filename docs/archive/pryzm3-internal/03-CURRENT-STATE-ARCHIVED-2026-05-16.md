...
|---|---|---|---|---|
| Q1 | S01–S06 | Foundation | Monorepo skeleton, vite multi-app, `packages/domain/` baseline, ADR-0201 (Pascal adoption), ADR-0202 (CRDT bridge), ADR-0203 (object storage), ADR-0204 (wire format), ADR-0205 (worker pool) | ✅ done |
...
| Q2 | S31–S36 | Plan view | `packages/drawing-engine/`, plan-view rendering pipeline, viewports, view-templates, ADR-0216 (drawing-engine architecture), ADR-0224 (constraint solver) | ✅ done |
| Q3 | S37–S42 | Sheets + schedules | `apps/sheets/`, schedule formula library (ADR-0227), schedule export formats (ADR-0240), ADR-0241 (portfolio aggregate placement) | ✅ done |
| Q4 | S43–S47 | Sync + awareness beta | CRDT sync end-to-end (SPEC-03), presence cursors, soft-locks (ADR-0219), beta multi-user demo | ✅ done |
| —  | S48 | Phase 2 close-out | Migration tooling for PRYZM 1 → 2 (ADR-0244), enterprise security baseline (ADR-0221, SPEC-35) | ✅ done |
...
| Q2 | S55–S60 | IFC component editor | `apps/component-editor/`, family editor (SPEC-FAMILY-EDITOR), IFC Tier-1 round-trip live (`ifc-export-tier1`, `ifc-import-tier2` workflows), `.pryzm-family` format (SPEC-26 + ADR-0217) | ✅ done — `family-editor-quality-gates` workflow green |
| Q3 | S61–S66 | Plugin SDK foundation | `packages/plugin-sdk/` skeleton, plugin sandbox (ADR-0209), 5 reference plugins (BCF, IFC export/import, IFC inspector, Rhino import) — all 5 with green workflows today | ⚠ partial — SDK is skeleton-only; 0 LOC of stable public API |
...
Files changed: `docs/archive/pryzm3-internal/reference/adrs/ADR-0245-mixed-auth-supabase-replit-pg.md` (NEW), `docs/02-decisions/contracts/C08-COLLABORATION-AND-SECURITY.md §1.1` (amended), `docs/archive/pryzm3-internal/03-CURRENT-STATE.md §10` (this entry), `docs/archive/pryzm3-internal/00-PROCESS-TRACKER.md §5b` (ADR-0245 row added).
...
(4) Ratifications confirmed against on-disk reality (per `chunks/26 §26.0` + `§26.11`): ADR-0241, ADR-0242, ADR-0243, ADR-0244 all on disk (44 ratified ADRs total). Cast-count 769 → 764 confirmed. Top-level `src/` directory count is 35, not 36 (00-INDEX off-by-one to be fixed by Z.20).
...
> "subordinate to: 1. The 12 specs in `docs/00_NEW_ARCHITECTURE/specs/` (SPEC-01..SPEC-12). 2. The 22 strategic ADRs in `docs/00_NEW_ARCHITECTURE/adrs/` (ADR-0201..ADR-0224)..."
...
- `docs/archive/pryzm3-internal/reference/adrs/` — 44 strategic ADR files (ADR-0201..ADR-0224 plus 20 supplements)
- `docs/02-decisions/adrs/` — 56 code-level ADR files (NNNN-slug.md)

Total = **140 architectural decision documents at WRONG paths in every phase doc citation.** When a Wave 9-20 engineer greps for SPEC-04 or strategic ADR-0218, they hit broken links. **Wave 8 D1 must run a citation-rot codemod BEFORE Wave 9 starts** to rewrite all 51 occurrences.
...
**The 12 SPECs and 22+ strategic ADRs that the founder named explicitly DO EXIST** at `docs/archive/pryzm3-internal/reference/specs/SPEC-01..SPEC-12*` and `docs/archive/pryzm3-internal/reference/adrs/ADR-0201..ADR-0224*`. The founder's prior reminder is correct. My Round-4 framing of "DOES NOT EXIST" was a **path-grep mistake** — the path didn't resolve, but the CONTENT was moved, not deleted.
...
1. CDE module with ISO 19650 status codes (SPEC-32 + ADR-0231)
2. Stakeholder Review Wedge — free unlimited reviewer seats (SPEC-33 + ADR-0236)
3. Hybrid Data Sovereignty — local/cloud-region/hybrid/self-host (SPEC-34 + ADR-0237)
4. Browser Security & Enterprise Hardening — BYOK + CSP/COOP/COEP + SOC2 (SPEC-35 + ADR-0238)
5. COBie 2.4 Export with NIBS validator pass (SPEC-36 + ADR-0234)
6. Federated Clash Detection — server-side BVH on N projects (SPEC-37 + ADR-0232)
7. MEP Systems — HVAC + Electrical + Plumbing + Sprinkler + Gas (SPEC-38 + ADR-0233)
8. EIR / BEP / TIDP / MIDP document chain with CDE gate (SPEC-39)
9. buildingSMART IFC4 Certification (SPEC-40 + ADR-0235)
