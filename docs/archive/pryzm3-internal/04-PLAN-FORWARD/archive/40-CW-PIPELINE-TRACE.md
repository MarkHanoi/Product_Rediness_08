...
**Created:** 2026-05-05  
**Last updated:** 2026-05-06 Session 6 — BN-05/BN-06/BN-07 remediation applied (see §Session 6 Bottleneck Fixes below); prior: 294-wall / 21-slab live session; BN-04 §PERF-ADAPTIVE-DRAIN-V2 implemented — dual-mode drain thresholds fix drain budget cascade regression; budget-reset-per-batch fix added)  
...
**Per-frame budget (§PERF-ADAPTIVE-DRAIN-V2 — current values after BN-04 fix):**

| Parameter | Pre-fix (session 1) | Post-fix-3 (sessions 1–4) | Post-BN-04-fix (session 5) |
|-----------|---------------------|---------------------------|---------------------------|
| `MAX_BUILDS_PER_FRAME` (initial) | 5 | **20** | **20** (unchanged) |
| Adaptive cap — interactive | 12 | **30** | **30** (unchanged) |
| Adaptive cap — batch mode | 12 | **30** | **50** (new) |
| Adaptive floor | 2 | **5** | **5** (unchanged) |
| Decrement threshold — interactive | >14ms | **>14ms** | **>14ms** (unchanged) |
| Decrement threshold — batch mode | >14ms | **>14ms** | **>45ms** (new) |
| Increment threshold — interactive | <8ms | **<8ms** | **<8ms** (unchanged) |
| Increment threshold — batch mode | <8ms | **<8ms** | **<25ms** (new) |
| Budget reset per batch | ❌ only constructor | ❌ only constructor | ✅ `resumeAndFlush()` resets to 20 |

**Why two separate threshold sets (BN-04, 2026-05-06):**  
The 14ms decrement was calibrated for *interactive* mode (stay within one 60fps frame). During
a batch, OBC+PASCAL renders are **suppressed** — the only constraint is the 50ms LONGTASK
threshold. At 14ms the budget cascaded 20→7 in 16 frames for a 294-wall session:

```
Frame  1: built=20  frameMs=30.6ms  →  nextBudget=19  (30.6 > 14ms → decrement)
...
Frames 17–27: oscillates 7–8  (27 total drain frames, 432ms rAF overhead alone)
```

With batch thresholds (<25ms → increment, >45ms → decrement, cap 50):  
Budget stabilises at ~20–25 walls/frame → ~14 drain frames vs 27 → saves ~13 × 16ms rAF overhead.
...
### 294-wall / 21-slab — Live Measured Session (2026-05-06, §PERF-ADAPTIVE-DRAIN-V2 target)

```
T=+0ms       §TRACE-CW#1 START  slabCount=21 isRedo=false shadersPrewarmed=true (PREWARM-SKIP)
             → estimatedWalls=294; PREWARM-SKIP (PSO cache warm from prior run, no scale jump)

T=+14.3ms    §DIAG PHASE-1-SLAB-LOOP-DONE  collected=294  slabLoopMs=3.4ms
T=+15.9ms    §DIAG PHASE-2-ADDMANY-DONE    count=294       addManyMs=1.6ms
             [CurtainWallStore] §BATCH-CW-PAUSE-ADDMANY: phase1CloneMs=0.4ms
               addManyPausedMs=0.3ms  busEmitMs=0.1ms  totalPhase2Ms=0.4ms
             [CurtainWallBuilder] §BATCH-CW-PAUSE-ADDMANY: 294 walls → _pausedBuildsMap in 0.10ms
T=+16.1ms    §DIAG PHASE-3-REG-QUEUED   levelGroups=21  regQueueMs=0.1ms  totalProcessSlabsMs=5.2ms
T=+16.2ms    §TRACE-CW#1 RUNBATCH-RETURNED  runBatchMs=7.7ms
T=+16.3ms    §TRACE-CW#1 COMPLETE  walls=294  (rAF drain + registration + event drain pending)

  ↳ SYNC PHASE: 16.3ms for 294 walls (21 slabs) — excellent (BN-01 + BN-02 fixes holding)

≈ rAF frame boundary ≈

[LONGTASK 52ms — first drain frames (rAF drain in progress)]

  rAF drain frame  1: built=20  remaining=274  frameMs=30.6ms  nextBudget=19
  rAF drain frame  2: built=19  remaining=255  frameMs=25.7ms  nextBudget=18
  rAF drain frame  3: built=18  remaining=237  frameMs=23.3ms  nextBudget=17
  rAF drain frame  4: built=17  remaining=220  frameMs=19.7ms  nextBudget=16
  rAF drain frame  5: built=16  remaining=204  frameMs=16.5ms  nextBudget=15
  rAF drain frame  6: built=15  remaining=189  frameMs=28.6ms  nextBudget=14
  rAF drain frame  7: built=14  remaining=175  frameMs=38.7ms  nextBudget=13
  rAF drain frame  8: built=13  remaining=162  frameMs=29.5ms  nextBudget=12
  rAF drain frame  9: built=12  remaining=150  frameMs=24.3ms  nextBudget=11
  rAF drain frame 10: built=11  remaining=139  frameMs=13.9ms  nextBudget=11
  rAF drain frame 11: built=11  remaining=128  frameMs= 9.0ms  nextBudget=11
  rAF drain frame 12: built=11  remaining=117  frameMs=18.2ms  nextBudget=10
  rAF drain frame 13: built=10  remaining=107  frameMs=22.1ms  nextBudget= 9
  rAF drain frame 14: built= 9  remaining= 98  frameMs=13.6ms  nextBudget= 9
  rAF drain frame 15: built= 9  remaining= 89  frameMs=20.3ms  nextBudget= 8
  rAF drain frame 16: built= 8  remaining= 81  frameMs=14.9ms  nextBudget= 7  ← FLOOR
  rAF drain frame 17: built= 7  remaining= 74  frameMs= 8.8ms  nextBudget= 7
  rAF drain frame 18: built= 7  remaining= 67  frameMs=14.0ms  nextBudget= 7
  rAF drain frame 19: built= 7  remaining= 60  frameMs= 7.8ms  nextBudget= 8  ← brief increment
  rAF drain frame 20: built= 8  remaining= 52  frameMs=18.9ms  nextBudget= 7  ← back to floor
  rAF drain frame 21: built= 7  remaining= 45  frameMs= 6.6ms  nextBudget= 8
  rAF drain frame 22: built= 8  remaining= 37  frameMs=17.1ms  nextBudget= 7
  rAF drain frame 23: built= 7  remaining= 30  frameMs= 4.9ms  nextBudget= 8
  rAF drain frame 24: built= 8  remaining= 22  frameMs=13.3ms  nextBudget= 8
  rAF drain frame 25: built= 8  remaining= 14  frameMs=17.0ms  nextBudget= 7
  rAF drain frame 26: built= 7  remaining=  7  frameMs= 6.8ms  nextBudget= 8
  rAF drain frame 27: built= 7  remaining=  0  frameMs=12.6ms  nextBudget= 8

  ↳ BN-04 OBSERVED: budget cascades 20→7 in 16 frames (14ms decrement threshold too tight
    for batch-mode suppressed rendering). Budget oscillates 7–8 for remaining 11 frames.
    27 drain frames × ~16ms rAF = 432ms pure scheduling overhead.
    Per-wall cost: 0.7–2.8ms (mean ~1.6ms). Fix: §PERF-ADAPTIVE-DRAIN-V2 raises batch-mode
    thresholds to 25ms/45ms — budget stabilises at ~20–25 → ~14 frames expected post-fix.

T=+823.1ms   §TRACE BUILD-QUEUE-DRAINED  regQueue=21 / totalExpected=294
T=+826.7ms   §TRACE REGISTRATION-DRAIN-DONE  (21 registerMany, ~3.6ms)
T=+826.7ms   §TRACE SHADOW-REACTIVATION-START → setTimeout(drainSlice, 30000) queued
T=+826.7ms   §TRACE FINAL-SWEEP-START  skipRedetect=true

[LONGTASK 123ms — yielded event drain delivering 294 events]

T=+864.3ms   §TRACE ON-COMPLETE-START  (294 events, 2 chunks, 37.6ms delivery)
T=+864.4ms   §PERF-VIEW-BATCH-SUPPRESS  suppression lifted; markLevelsDirty DEFERRED to post-render
T=+864.5ms   §TRACE ON-BATCH-END-DEFERRED → overlay dismiss scheduled for post-render
T=+865.0ms   §TRACE ON-POST-BATCH-DISPATCHED
T=+865.0ms   §PERF-SKIP-PBR PBR-UPGRADE-SKIPPED
T=+865.0ms   REDETECT_ROOMS SKIPPED for 21 levels (skipRedetectRooms=true)

[GPU Monitor T+2s:  geometries:45  drawCalls:16  tris:232  — stable throughout 60s observation]
[NO EdgeProjector LONGTASKs observed in 60s post-batch window — plan view projection fast]

  ↳ TOTAL TIME TO OVERLAY DISMISS: ~865ms  (27 drain frames, 807ms drain + 58ms post)
  ↳ First render LONGTASK: 52ms + 123ms (only!) — prewarm working correctly (BN-02 fix confirmed)
  ↳ Target: ≤5s — ACHIEVED at 294 walls. Scale concern: 1000-wall batches (50 levels)
    projected ~4.5s drain at budget=7; with BN-04 fix projected ~2.5s drain at budget=20+.
```

---

## Current Bottlenecks (live state — 2026-05-06; 66-wall, 96-wall, 189-wall, 273-wall, 294-wall measured)
...
| rAF geometry drain (CW) | **807ms** / 27 frames (294 walls; budget cascades 20→7) | `CurtainWallBuilder._drainBuildQueue` | ✅ BN-04 fixed (§PERF-ADAPTIVE-DRAIN-V2: batch thresholds 25ms/45ms, cap 50, budget reset per batch) |
...
## Session 6 Bottleneck Fixes (2026-05-06)

Source: `attached_assets/Pasted--42-CW-Batch-Pipeline-Session-6-Analysis-Remediation-Cr_1778031532844.txt`  
Build status: **`pnpm run build` clean — 0 TypeScript errors**.

### BN-05 — Prewarm Failure → 14,175ms LONGTASK (3-part fix)

**Root cause:** `_prewarmCurtainWallShaders()` returned `void` and was not guarded against
pipeline-not-ready states. When it was called while rpm was in `error`/`initializing`/`binding`
phase, `rpm.render(0)` silently became a no-op, yet `_shadersPrewarmed` was unconditionally set
to `true`. The next CW execute() skipped prewarm entirely, leaving all PSOs cold → 14,175ms
LONGTASK on the first post-batch frame.  
WebGL context loss and WebGPU device loss also invalidated PSOs without resetting the flag.

| Sub | File | Fix |
|-----|------|-----|
| BN-05a | `CreateCurtainWallsOnAllSlabsCommand.ts` | Changed `_prewarmCurtainWallShaders()` return type to `boolean`. Added pipeline phase guard at entry (`error`/`initializing`/`binding` → `return false`). Added `scene/camera/renderer` null guard → `return false`. Caller sets `_shadersPrewarmed = true` ONLY when method returns `true`. |
| BN-05b | `CreateCurtainWallsOnAllSlabsCommand.ts` | Added timing validity check: if 3 render passes complete in < 30ms, renders were no-ops → `return false`, log `§BN-05b §PREWARM-FAILED`. Timing window `__prewarmRenderStart` captured before probes are added. |
| BN-05c | `initScene.ts` | `webglcontextlost` handler now calls `window.__resetCwPrewarm()` immediately after `evt.preventDefault()`, before the rAF loop is stopped. |
| BN-05c | `createRenderer.ts` | WebGPU device-lost recovery block now calls `window.__resetCwPrewarm()` before the 2-second recovery delay. |

### BN-06 — Slab PBR Upgrade 15,037-mesh Traversal Collision

**Root cause:** `CreateSlabsOnAllFloorsCommand.runBatch()` did not set `skipPbrUpgrade: true`.
`BatchCoordinator`'s post-batch PBR upgrade pass scheduled 126 `requestIdleCallback` chunks
to traverse the entire 15,037-mesh scene. These chunks fired concurrently with the subsequent
CW batch's PSO first-render window, fragmenting the main thread and compounding cold-PSO compile
time (causing BN-07 as a direct side-effect).

| File | Fix |
|------|-----|
| `CreateSlabsOnAllFloorsCommand.ts` | Added `skipPbrUpgrade: true` to `batchCoordinator.runBatch()` options. `skipRedetectRooms: false` left intact — slabs define room boundaries. Safe because `SlabFragmentBuilder` explicitly sets `metalness`/`roughness` on all three material paths (verified). |

### BN-07 — 10s Batch-Starvation on DEFERRED-RESUME-FLUSH (observability fix)

**Root cause:** Derived entirely from BN-06 (PBR rIC storm starved the second batch's
`scheduleOnce` rAF callback for ~10 s). BN-06 fix eliminates the starvation. BN-07 fix adds
an observability warning so any future regression is surfaced immediately.

| File | Fix |
|------|-----|
| `BatchCoordinator.ts` | Captures `_resumeQueuedAt = performance.now()` before `scheduleOnce()` call. Inside the callback, computes `_resumeDelay`. If delay > 2000ms, logs `§BN-07 §WARN DEFERRED-RESUME-FLUSH delayed Xms` with diagnostic hint pointing to `skipPbrUpgrade`. |
...
- `42-CW-BATCH-PIPELINE-SESSION-6.txt` — source analysis for BN-05/BN-06/BN-07 (Session 6)
