...
- `phases/audits/PRYZM2-WIREUP-PLAN-S72/24-pryzm1-src-coverage-audit.md` — the per-folder cross-audit (adds 17 folder-level gaps + 31 sub-phases + ADR-0241/042/043).
- `phases/audits/PRYZM2-WIREUP-PLAN-S72/25-architecture-docs-cross-alignment.md` — the per-document cross-audit (closes CONFLICT §6.10 via ADR-0244 + adds 7 doc-PRs).
...
(The SOC2 audit-log retention details — `§29 #7` — close at the document level via ADR-0221 + ADR-0228 Part G; concrete schema lights up at S57. That is closed for plan purposes; the implementation milestone is tracked separately.)
...
|---|---|---|---|
| 1 | "One Renderer is two" — `packages/renderer/` vs `packages/render-runtime/` | ADR-0222 Part A | S25 |
| 2 | Library rAF (Cesium / OBC) breaks the "1 frame owner" claim | ADR-0223 | S25 (warn) → S30 (error) |
| 3 | `src/render/` + `src/rendering/` legacy overlap | ADR-0222 Part A → SPEC-27 §4.3 deletion at S55 | S55 |
| 4 | Geometry-kernel boundary unscoped re context envelopes | SPEC-13 + SPEC-21 Step 4 | S25 |
| 5 | Renderer topology + backend runtime | ADR-0222 Part B–F | S25 |
...
| 8 | Service-role-key in `server.js` | SPEC-08 §6 + ADR-0228 Part F + CI gate `pnpm spec:audit-secrets` | S26 |
...
| 18 | Free-tier exclusion from PDF-to-BIM | SPEC-28 §2 + ADR-0229 Part B | S65 |
| 19 | PDF-to-BIM cost ceiling | SPEC-28 §3 + ADR-0229 Part C | S65 |
| 20 | Self-host BYO-key safety cap | SPEC-28 §11 | S70 |
| 21 | PDF-to-BIM scope (the moat) | ADR-0229 + SPEC-31 (S50) | S49–S72 |
...
| 27 | Hidden-line classifier; WebGL2 first, WebGPU compute later | SPEC-30 §3.2 + ADR-0225 Part E | S35 (WebGL2); post-GA (compute) |
...
| 39 | `src/lifecycle/` placement undecided | ADR-0230 | S25–S70 |
| 40 | `src/engine/EngineBootstrap.ts` deletion | SPEC-27 §4.3 | S61 |
| 41 | OBC removed from editor → plugin | SPEC-12 §5 + ADR-0223 + SPEC-27 §4.3 | S55 |
...
| #29 item | Closes via |
|---|---|
| #1 Robustness budget per kernel op | ADR-0220 (already merged S23) |
| #2 First-party catalog inventory | ADR-0217 (already merged S20) |
| #3 OTel pipeline | ADR-0214 (already merged S07) |
| #4 View-state authority | ADR-0216 (already merged S17) |
| #5 Telemetry pipeline | ADR-0214 |
| #6 Soft-locks | ADR-0219 (already merged S22) |
| #7 SOC2 evidence | ADR-0221 + ADR-0228 Part G; schema S57 |
| #8 Stripe entitlements | ADR-0213 (already merged S07) |
| #9 Bake debounce | ADR-0215 (already merged S08) |
| #10 Renderer topology | ADR-0222 Part A |
| #11 Single-frame-owner & library rAF | ADR-0222 Part F + ADR-0223 |
| #12 BullMQ-Redis on Replit | ADR-0222 Part C + SPEC-15 §3.3 |
| #13 Yjs server placement | ADR-0222 Part D + SPEC-15 §2.2 |
| #14 Lifecycle placement | ADR-0230 |
| #15 Node version | ADR-0222 Part B |
| #16 three.js pin | ADR-0225 |
| #17 WebGPU adoption gate | ADR-0225 Part C |
| #18 React-19 in deps | ADR-0226 |
| #19 Schedule DSL | ADR-0227 |
| #20 Light parametric expressions vs schedule formulas | ADR-0227 Part E |
| #21 Authority unification | ADR-0228 |
| #22 RLS service-role-key | ADR-0228 Part F + S26 removal |
| #23 Workspace permission taxonomy | ADR-0228 Part B |
| #24 Plan / role / RLS coordination | ADR-0228 Parts A–E |
| #25 PDF-to-BIM scope | ADR-0229 |
| #26 Library quarantine policy | ADR-0223 |
| #27 Capacity cut list extension (T1.7 + T1.8) | ADR-0218 (extended 2026-04-27) |
...
Every gap has a sprint home; no gap is unscheduled. **S31 is the heaviest sprint of the 36-month plan**; its slip risk is mitigated by ADR-0218 Tier-1 cuts T1.7 (S55) + T1.8 (S41), either of which buys back a sprint of calendar.
...
1. **v2 backlog items** — items in ADR-0218 Tier-1/Tier-2/Tier-3 that may be cut at phase gates land in `docs/operations/v2-candidates.md` if cut; this plan does not pretend they are closed if they are deferred.
...
| 93 | `src/portfolio/PortfolioSemanticGraph.ts` unwired | **ADR-0241** + G.17 | S84 |
| 94 | `src/physics/` + `src/render/` debug overlay unwired | **ADR-0242** + G.18 + G.28 | S84 |
...
**Decisions still pending (3 ADRs to author before their sub-phase ships):** ADR-0241 (portfolio aggregate placement), ADR-0242 (`src/physics/` runtime vs dev-only), ADR-0243 (`src/utils/*` inline vs `packages/utils`). All three default to no-op-safe positions if not ratified by their sprint window.

### §3.6.2 Document-level gaps (Chunk 25 — 1 item closed, 7 doc-PRs queued)

| # | Gap | Closes via | Sprint |
|---|---|---|---|
| 103 | Customer migration (PRYZM 1 → PRYZM 2) — `CONFLICT-ANALYSIS §6.10` had no owner ADR | **ADR-0244 — Customer migration** + Wireup **G.32 — PRYZM 1 lights-out** | S22 latest (ADR) / S84 D9 (G.32 wireup) |
