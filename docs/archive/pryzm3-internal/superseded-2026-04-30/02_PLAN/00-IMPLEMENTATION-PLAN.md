...
| 7 | **Doc-link sweep**: `sed`-replace broken references to `00_NEW_ARCHITECTURE/` → `docs/03_PRYZM3/01_ARCHITECTURE/` and `wireup-S72/chunks/` → `docs/03_PRYZM3/wireup-S72/chunks/` across `02_PLAN/`, `03_STATUS/`, and `00_VISION/`. Owner: `02_PLAN/07-GAP-CLOSURE.md` per its existing scope. | `rg "00_NEW_ARCHITECTURE/" docs/` returns 0 hits | §6.6 of this plan | 0.5 day |
