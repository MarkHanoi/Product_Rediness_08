...
| **OI-057** | **Post-batch wall-join: correct, but timing-implicit + untested; plugin store keeps pre-miter baselines.** Architect report: "before, batch walls had wrong joins; adding a door/window on one wall fixes them." **Diagnosis (2026-05-25, static trace — full text in [C16 §12](../02-decisions/contracts/C16-COMMAND-AUTHORING-PROTOCOL.md)):** joins **are** resolved post-batch. `CreateWallsOnAllSlabsCommand` wraps in `batchCoordinator.runBatch`; `_setupBatch` pauses the wall builder; the deferred `resume()` schedules `WallRebuildCoordinator._flush()`, which runs `WallJoinResolver.resolveLevel(store.getAll().filter(levelId))` over the **complete** wall set per level. Single-slab `CreateWallsFromSlabCommand` (no runBatch) coalesces to one `_flush` with the same effect. Door/window add "fixes" joins because `wallStore.update` re-triggers the *same* level-wide `resolveLevel`. **Two residual gaps (no code change made — per architect "check that or add it to the backlog"):** (a) the order invariant "`resume()→_flush()` runs before `_executeFinalSweep()`'s `discardAndSuppress()`" is **implicit** (holds only because the build queue can't drain before `_flush`) with **no test**; (b) the event-sourced **plugin `WallsState`** (from `wall.batch.create`) keeps **pre-miter baselines** — the trim is written only to the legacy `wallStore.baseLine`; a rebuild purely from the plugin store would show untrimmed joins. **Action:** add the §8 B-5 invariant test (one `resolveLevel`/level post-batch, all walls present) BEFORE touching batch timing; promote to SPEC task if plugin-store baseline divergence surfaces on reload. New invariant **B-5** codified in C16 §8. | P2 | 🔍 **Open — diagnosed 2026-05-25 (architecturally sound; fragility backlogged, no risky change made)** |
| **OI-058** | **Scene Registry to replace `scene.traverse` visibility/selection (pascalorg insight).** From the pascalorg/editor research ([PASCALORG-EDITOR-RESEARCH.md](reference/PASCALORG-EDITOR-RESEARCH.md) §3.1): Pascal keeps `Map<id→Object3D>` + `byType`/`byLevel` sets and **never traverses the scene graph** for lookup/visibility/selection. PRYZM's `ProjectVisibilitySection` (hide/isolate/re-apply/reset) + selection do **full `scene.traverse` with `userData` matching** — O(n) per op, and the traverse is exactly why instanced aggregates (no per-element `userData.id`) needed the §INSTANCED-ISOLATE-FIX special-casing. **Fix:** introduce a SceneRegistry (`id→Object3D` + `byType` + **`byLevel`**), composing the pieces PRYZM already has (`elementRegistry`, `ViewDependencyTracker._elementLevelMap`, `InstancedElementRenderer._elements`); route visibility/selection through it → O(k) not O(n), and the instanced-aggregate case becomes natural. Improves the perf NFTs + C16 CA-DOCTRINE-L. Read the repo `wiki/architecture/` first for the registry API. | P2 | 🔍 **Open — researched 2026-05-25 (pascalorg-aligned; highest-value architecture key)** |
...
| ID | Item | Priority | Status |
|---|---|---|---|
| **OI-054** | **Undo is broken for plan-view element creation (`store.applyPatch is not a function`).** Workflow: analyse → review → check → document → fix. Live log (2026-05-24, drew 9 walls in plan view, then Ctrl+Z): `[applyRingBufferSide] failed — skipping store update: TypeError: store.applyPatch is not a function` at `PatchSnapshot.ts:262` (called from `BimService.ts:159` and `initUI.ts:2884` `_withPausedObserversForUndo`). The handler then logs `[Undo] ring-buffer undo applied — stores: Array(1)` **as if it succeeded** (it didn't — the patch was swallowed, the wall stays), and a subsequent Ctrl+Z hits `CommandManagerImpl: UNDO: history empty — nothing to undo`. **Root cause (CONFIRMED 2026-05-24 — static trace):** a **dual-store / store-unification (TASK-08) gap**. (1) `applyRingBufferSide` ([`packages/command-bus/src/PatchSnapshot.ts:262`](../../packages/command-bus/src/PatchSnapshot.ts#L262)) calls `store.applyPatch(patches)`. (2) Both undo store-maps — `_buildRingBufferStoreMap()` ([`apps/editor/src/engine/initUI.ts:2785`](../../apps/editor/src/engine/initUI.ts#L2785)) and `BimService._buildStoreMap()` ([`apps/editor/src/engine/BimService.ts:197`](../../apps/editor/src/engine/BimService.ts#L197)) — map `wall → window.wallStore`, which is the **legacy** `geometry-wall` `WallStore` ([`WallStore.ts:81`](../../packages/geometry-wall/src/WallStore.ts#L81)): it has `add/update/remove/getById/getAll` but **no `applyPatch`** (and it is what drives the mesh via `WallFragmentBuilder`). The ring-buffer inverse patches, however, were produced against the **plugin** `WallStore extends Store<WallData>` (which *does* have `applyPatch`). → `store.applyPatch` is `undefined` → `TypeError`. (3) `applyRingBufferSide` **swallows** the throw (C03 §4.1 "MUST NOT throw") → callers log `[Undo] ring-buffer undo applied` though nothing happened. (4) The plan-view create did NOT dual-write to `commandManager.history` (it is empty), so the legacy fallback can't reverse it either. **Unevenness:** ring-buffer undo works only for element types whose `window.<x>Store` is already a plugin `Store<T>` with `applyPatch` (e.g. `plan-view/LevelStore`, `view/store`); it is **broken for every type still on a legacy Map store** (wall confirmed; likely slab/room/curtain-wall/door/window/furniture which also map to legacy `window.*` stores). **Fix direction (NOT a safe blind edit — needs live testing):** the correct end-state is **TASK-08 store unification** (make `window.wallStore` the plugin `Store<WallData>` whose `applyPatch` propagates to the mesh), OR route a create-inverse through the existing delete bridge (remove from legacy store + mesh + VDT + bimManager). Interim safety: `applyRingBufferSide` MUST report apply success so callers stop logging false "undo applied" and can fall back — but the cursor is already consumed by `undoPatch()`, so the honest fix must **pre-check `applyPatch` availability before consuming the ring-buffer cursor**. Ties to the deferred **U-B1–U-B5** cluster (DAILY-USE-FIX-LOG) + OI-043 (P6) + TASK-08. **This is an architectural change to the undo/store layer that risks the working create/render path; it should be designed + applied WITH live undo testing, not blind.** **Secondary issues surfaced in the SAME trace** (track separately): (1) ✅ **FIXED 2026-05-24 (§G3-STALE-FIX, Round 54)** — `[VDT] §G3-STALE-EVENT` fired on every plan-view wall create because the `§P2.1` bridge called `wallStore.add()` (sync → StoreEventBus → VDT._onStoreChange) BEFORE `viewDependencyTracker.registerElement`. Reordered: register VDT + bimManager before the add() mirror. (2) `REDETECT_ROOMS` executes **2× per single wall create** (redetect storm; task #54 Part 2). (3) **EdgeProjector cache 0% hit rate** — every create re-projects ALL existing walls (`§PERF-CACHE-MISS` for every element, version bumps for all, gen +2 per wall) → O(N) reprojection per create instead of incremental. (4) **Auto-save after every single wall** (snapshot + thumbnail + server sync per wall) — should debounce/coalesce during rapid drawing. (5) GPU monitor `Geometry count grew 2200%` → verify `WallFragmentBuilder.removeWallFragments()` / `CurtainWallBuilder._disposeChildren()` actually dispose on undo. (6) `Destroyed texture ShadowDepthTexture used in a submit` — recurrence of deferred #47 (WebGPU shadow). **Progress 2026-05-24:** the full undo architecture (3 store layers, dual Path A/B, the canonical `runtime.undoStack` apply path, invariants U-1…U-7, bugs B1/B2/B3) is now documented in **C03 §4** (rewritten). **B3 FIXED** — `applyRingBufferSide` returns `{applied,failed}` + logs the missing-`applyPatch` store loudly; `initUI`/`BimService` stop the false "undo applied" log and fall back to `commandManager.undo()` on total failure (verified: command-bus typecheck + 26/26 tests; editor typecheck clean). 4 hand-rolled apply sites identified (initUI, BimService, NavigationAreaLayout, DockingLayout). **Still open:** B1 (route all 4 through `runtime.undoStack`, U-5) + B2 (TASK-08 store unification so the inverse patch reverts the mesh, U-7) — both need live undo testing. The architecturally-sound fix is **ADR-0251** (single source of truth + mesh derived via dirty-diff subscription, pascalorg-aligned; incremental per-type, live-gated). **Per-type rollout shipped 2026-05-24 (ADR-0251):** `elementUndoStoreAdapter` (duck-typed: add/remove|delete/getById|get/update) + `adaptElementStoreMap` wired into all 4 hand-rolled undo/redo sites for **wall, slab, room, curtain-wall, furniture, column, beam, stair, handrail, roof, floor, ceiling, plumbing** — undo+redo revert both data and mesh; unit-gated 7/7 (`apps/editor/__tests__/elementUndoStoreAdapter.test.ts`). **door/window** (hosted, two-part wall-opening undo) + **level** (Path-A) left RAW → B3 fallback. **Live-verify pending** (architect). **⚠️ NOT CONFIRMED (live trace 2026-05-24):** a live Ctrl+Z after drawing plan walls undoes via `commandManager` (CREATE_ANNOTATION → history empty) and NEVER reaches the adapter — no `[elementUndoStoreAdapter]`/`[Undo]` log. So undo is firing through a `commandManager` path, not the ring buffer. Root cause is either U-1 (ring buffer empty — creates not pushed) or U-5 (≥5 fragmented entry points: initUI keydown, BimService.undo ← ContextualEditBar, NavigationAreaLayout, DockingLayout, SaveUndoRedoHUD→runtime.undoStack). Diagnostics added (`[Undo-DIAG/*]`). Contract C03 §4.5/§4.7 corrected to truth (runtime.undoStack uses snapshots → non-functional; fix = consolidate onto one path fed an adapter-wrapped mesh-driving provider). **✅ ROOT CAUSE FOUND + FIXED 2026-05-24 (unified path):** the live 3D trace was the tell — the undo **button** (`SaveUndoRedoHUD`, `runtime===null`) called `commandManager.undo()` **only** and never consulted the ring buffer; plan elements are bus-only (ring buffer only) → "history empty", while 3D walls dual-dispatch into `commandManager` so the button undid them. It was a **trigger divergence**, not a broken applicator (the ring-buffer adapter path was correct but only the keyboard reached it). **Fix:** ONE unified path `apps/editor/src/engine/undo/performUndoRedo.ts` (`performUndo`/`performRedo`) called by EVERY trigger (HUD buttons, initUI keydown, BimService ← ContextualEditBar, Nav/Docking GIS-reset) — ring-buffer-first (adapter, drives mesh) + coverage pre-check (uncovered door/window/level → no cursor step → `commandManager.undo()` fallback) + **shadow-drop** of the dual-dispatch twin via new `CommandManager.dropEntriesForTargets()` (U-8 — no phantom double-undo) + adapter now also cleans `bimManager`+`elementRegistry` (no leak). The 4 hand-rolled maps deleted. C03 §4.5/§4.6 (U-5,U-8)/§4.7 rewritten to the shipped design; ADR-0251 "Interim shipped" added. Gated: editor typecheck 0 errors; 12/12 unit tests (`performUndoRedo.test.ts` 5/5 + `elementUndoStoreAdapter.test.ts` 7/7). **✅ LIVE-CONFIRMED (architect) for wall undo → pushed `2bd910b`.** **All-elements audit (Round 57):** the unified path is element-agnostic, so 15+ types were already covered; the ONE real gap was curtain-wall (`affectedStores=['curtainwall']` one-word key missing from `buildUndoStoreMap` — same bug as walls) → fixed (added `curtainwall`+`curtainPanel`) + added a coverage regression gate (6th test) so future key drift fails CI; full per-element table documented in **C03 §4.8**. grid/plumbing/view/sheet are commandManager bridges (cm-fallback by design). **+ REDO-SHAPE-FIX (Round 58, C03 §4.7 B4):** redo re-add now restores the EXACT legacy object captured at undo (`_undoRestoreSnapshots`), not the L1 forward value — fixes curtain-wall "redo did nothing" (the §P3.1-CW bridge renames bayWidth→gridXSpacing etc., so a raw L1 re-add yielded 0 panels). General for all elements. | **P1** | 🟢 **FIXED + audited for ALL elements; UNDO+REDO live-confirmed for wall + curtain-wall** (unified `performUndoRedo` + adapter snapshot/restore; editor typecheck clean + 14/14 tests; C03 §4.5–§4.8). Follow-ups (non-regressing, separate from undo correctness): (a) hosted door/window two-part undo; (b) cross-stack redo ordering / ADR-0251 single store; (c) ✅ **FIXED (Round 59)** — curtain-PANEL §G3-STALE-EVENT storm: VDT now attributes `<parent>::child` ids to the parent (§CW-PANEL-PARENT) + §P3.1-CW bridge & adapter register the CW in VDT BEFORE add; GPU geometry "leak" assessed = NOT a real undo leak (disposal correctly wired via `CurtainWallBuilder.remove`→`_disposeChildren`; the count is intentionally-shared cached panel geometry). |

### UI / Tools

| ID | Item | Priority | Status |
|---|---|---|---|
| **OI-055** | **"Add level" / "Add level" button in the Project Browser is a no-op.** Workflow: analyse → review → check → document → fix. Repro: Project Browser → PROJECT → `Ground (0m) ACTIVE` → click **"Add level"** → nothing happens (no new level row appears). **Root cause (2026-05-27, static trace):** `ProjectTreeSection.ts:159` (the Project Browser unified-browser handler) dispatched `bus.executeCommand(cmd.type, cmd)` — both wrong: (a) `cmd.type` resolves to the legacy enum value `'CREATE_LEVEL'` for which NO bus handler is registered (CommandBus throws `no handler registered for: CREATE_LEVEL` — swallowed by the unhandled-rejection path); (b) the 2nd arg is the full `AddLevelCommand` class instance, but the bus handler `level.add` ([`initBusHandlers.ts:257`](../../apps/editor/src/engine/initBusHandlers.ts#L257)) expects the **raw payload** `{ levelId, name, elevation, height }`. **Fix:** dispatch via the canonical pattern `bus.executeCommand('level.add', { levelId, name, elevation, height })` — matches the 3 other call sites (`PlanViewToolOverlay`, `StairLevelRequiredPanel`, `GridsLevelsRailPanel`). **Sibling defect fixed in the same change:** `FloorPropertySection.ts:182` "Save Layers" button had the identical pattern (`cmd.type = 'UPDATE_FLOOR'`, no handler; handler `floor.update` consumes raw payload) — same fix applied. **NOT touched (separate scope):** `AuditGridZone.ts:130` dispatches `AutoRemediateCommand` via `cmd.type = 'AUTO_REMEDIATE'` for which NO bus handler exists at all — needs handler creation in `initBusHandlers.ts` first; logged as a sibling task. | **P1** | ✅ **FIXED 2026-05-27** — editor typecheck clean; tied to C02 §3.3 |
...
The following files have been archived to `docs/archive/pryzm3-internal/archive/` and are no longer authoritative:
