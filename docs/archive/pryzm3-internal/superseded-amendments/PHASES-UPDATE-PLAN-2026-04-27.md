...
> - `adrs/ADR-0201..ADR-0224` (the 22 strategic ADRs in `00_NEW_ARCHITECTURE/adrs/`).
...
|---|---|---|---|
| `ADR-0201` (PHASE-1A) | Typed-ID brand strategy | ADR-0201 (strategic) = Pascal-B adoption | ✗ different topic |
| `ADR-0202` (PHASE-1A, PHASE-2) | Command handler signature / "ADR-0202 spike" | ADR-0202 (strategic) = CRDT bridge | ✗ partial overlap |
| `ADR-0203` (PHASE-1A) | Scheduler API | ADR-0203 (strategic) = R2 / MinIO storage | ✗ |
| `ADR-0204` (PHASE-1A) | MessagePack codec | ADR-0204 (strategic) = msgpackr wire format | ✓ **same topic, finer scope** |
| `ADR-0205` (PHASE-1A) | `PrimitiveCommitter` interface | ADR-0205 (strategic) = worker-pool policy | ✗ |
| `ADR-0206` (PHASE-1A, PHASE-1) | Idle-continuation N-frame budget | ADR-0206 (strategic) = WebGPU/WebGL2 render mode | ✗ |
| `ADR-0207` (PHASE-1A) | WebGPU/WebGL2 dual-mode | ADR-0207 (strategic) = OTel/Tempo telemetry | ✗ — and the topic actually maps to **strategic ADR-0206** |
| `ADR-0208` (PHASE-1B) | Wall-tool sub-mode triage | ADR-0208 (strategic) = IFC scope | ✗ |
| `ADR-0210` (PHASE-1, PHASE-1D) | Bake worker coalescing window | ADR-0210 (strategic) = 250 ms bake debounce | ✓ **same decision** |
| `ADR-0216` (PHASE-1D) | Coalescing window (mentioned mid-text) | ADR-0216 (strategic) = drawing engine architecture | ✗ |
| `ADR-0217` (PHASE-1D) | `.pryzm` ZIP format v1 | ADR-0217 (strategic) = type catalog scope | ✗ |
| `ADR-0223` (PHASE-2B) | Plan view renderer architecture | (no strategic counterpart) | n/a — topic now subsumed by SPEC-04 + strategic ADR-0216 |
| `ADR-0224` (PHASE-2B) | Section view cut algorithm | ADR-0224 (strategic) = constraint solver | ✗ — see ADR-0224 strategic, "Naming note" |
| `ADR-0225` (PHASE-2B) | Multi-view sync strategy | (no strategic counterpart) | n/a |
...
2. **Sprint-scoped / code-level ADR series** — `docs/02-decisions/adrs/NNNN-<slug>.md` (4-digit zero-padded number). Each is local to a sprint and a code change. These are the ADRs that the phase docs were originally calling `ADR-0201..ADR-0225` inline.
...
> Where a phase-doc ADR is "the same decision" as a strategic ADR (currently only `ADR-0204 codec` and `ADR-0210 coalescing`), the phase doc cites the **strategic** ADR as the source-of-truth and the sprint-scoped ADR is **deleted from the planning narrative** (it never needed to exist as a separate decision).
...
|---|---|---|
| `ADR-0201` (typed-ID brand strategy) | `code-level ADR docs/02-decisions/adrs/0001-typed-id-brand.md` | Sprint-scoped; not a cross-cutting decision. |
| `ADR-0202` (command handler signature) | `code-level ADR docs/02-decisions/adrs/0002-command-handler-signature.md` | Sprint-scoped. **Distinct from** strategic ADR-0202 (CRDT bridge). |
| `ADR-0202 spike` (the CRDT pre-S01 spike) | `[strategic ADR-0202] CRDT spike per SPEC-03 §3` | This **is** strategic ADR-0202 — the CRDT-event-log bridge — and stays as `[strategic ADR-0202]`. |
| `ADR-0203` (scheduler API) | `code-level ADR docs/02-decisions/adrs/0003-scheduler-priority-vs-tickpriority.md` | Sprint-scoped. |
| `ADR-0204` (MessagePack codec) | `[strategic ADR-0204]` | Same decision; the phase-doc spike output goes into the existing strategic ADR-0204. **The sprint-scoped ADR-0204 is deleted.** |
| `ADR-0205` (`PrimitiveCommitter` interface) | `code-level ADR docs/02-decisions/adrs/0005-primitive-committer-interface.md` | Sprint-scoped. |
| `ADR-0206` (idle-continuation budget) | `code-level ADR docs/02-decisions/adrs/0006-idle-continuation-budget.md` | Sprint-scoped. |
| `ADR-0207` (WebGPU/WebGL2 dual-mode) | `[strategic ADR-0206]` | Same decision; **delete the sprint-scoped ADR-0207**. The dual-mode strategy and parity gate are owned by strategic ADR-0206. |
| `ADR-0208` (wall-tool sub-mode triage) | `code-level ADR docs/02-decisions/adrs/0008-wall-tool-submodes.md` | Sprint-scoped. |
| `ADR-0210` (bake worker coalescing window) | `[strategic ADR-0210]` | Same decision (250 ms). **Delete the sprint-scoped ADR-0210.** |
| `ADR-0216` (PHASE-1D mid-text "coalescing window") | `[strategic ADR-0210]` | This was a misnumbering inside PHASE-1D; the coalescing decision is strategic ADR-0210. |
| `ADR-0217` (`.pryzm` ZIP format v1) | `code-level ADR docs/02-decisions/adrs/0017-pryzm-zip-format-v1.md` | Sprint-scoped. (Note: strategic ADR-0217 = type catalog scope, unrelated.) |
| `ADR-0223` (plan view renderer architecture) | `code-level ADR docs/02-decisions/adrs/0023-plan-view-canvas2d-renderer.md` | Sprint-scoped. **Cite strategic ADR-0216 (drawing engine architecture) as parent.** Plan-view renderer is one back-end of the SPEC-04 vector primitive model. |
| `ADR-0224` (section view cut algorithm) | `code-level ADR docs/02-decisions/adrs/0024-section-cut-algorithm.md` | Sprint-scoped. **Note** strategic ADR-0224 = constraint solver; strategic ADR-0224's "Naming note" already documents this collision. |
| `ADR-0225` (multi-view sync strategy) | `code-level ADR docs/02-decisions/adrs/0025-multi-view-sync.md` | Sprint-scoped. |
...
|---|---|---|
| **ADR-0201** (Pascal-B adoption) | PHASE-1A §1 framing | Confirms "patterns yes, fork no". |
| **ADR-0203** (R2 + MinIO behind a driver) | PHASE-1D S21 (bake worker), S22 (sync server) | All R2 calls go through the storage driver, not the SDK. |
| **ADR-0205** (worker-pool policy) | PHASE-1D S21 (bake worker), PHASE-1A S03 (browser pool) | Browser pool capped at 4; server BullMQ + worker_threads. |
| **ADR-0207** (OTel + Tempo + Honeycomb) | PHASE-1A S01 (OTel SDK lands) | P8 CI gate at warning S04, error S08. |
| **ADR-0208** (IFC scope) | PHASE-1A S02 (IFC plan reconfirmed); PHASE-3 S55–S58 | IFC4 read+write Pset round-trip; IFC4.3 advanced post-GA. |
| **ADR-0209** (Web Worker plugin sandbox) | PHASE-1A S01 (sandbox 5-day spike); PHASE-3 S62 (SDK 1.0) | Sandbox model decided pre-S01. |
| **ADR-0211** (project / view / element-class permissions) | PHASE-2 S43 (role/permission matrix live) | No per-instance ACL in v1. |
| **ADR-0212** (docker-compose self-host) | PHASE-3 S67 (self-host packaging) | docker-compose for v1; no Helm. |
| **ADR-0213** (persistence operational) | PHASE-1D S19 (chunked binary), S20 (`.pryzm` v1) | Chunk format + cache invalidation rules. |
| **ADR-0214** (AI L7.5 operational) | PHASE-2 S47 (AI decomposition); PHASE-3 S50–S54 | AI scope and approval queue. |
| **ADR-0215** (visibility-intent placement) | PHASE-2 S46 (waves 1–5); PHASE-3 S49 (waves 6–11) | Where Visibility-Intent rules live. |
| **ADR-0216** (drawing engine architecture) | PHASE-2A S30 (edge projection / poche); PHASE-2B S31 (plan view rebuild) | Vector primitives → 3 back-ends. **[BLOCKING]** for S29. |
| **ADR-0217** (type catalog scope) | PHASE-1C S11 (first per-family type lands) | SPEC-05 supersedes legacy Contract 17. **[BLOCKING]** for S11. |
| **ADR-0218** (capacity cut list) | PHASE-1 §6 (M12 gate); PHASE-2 §6 (M24 gate); PHASE-3 §6 (M36 gate) | The standing menu of cuts at slip thresholds. |
| **ADR-0219** (soft-lock semantics) | PHASE-2 S45 (soft locks) | TTL, mid-edit lock loss, AI-batch semantics. **[BLOCKING]** for S48. |
| **ADR-0220** (kernel robustness) | PHASE-1B S07 (wall producer) | Robustness budget + property-test PR-merge gate. **[BLOCKING]** for S07. |
| **ADR-0221** (enterprise security & residency) | PHASE-2 S43 (RLS); PHASE-3 S55 (SAML/OIDC), S58 (SCIM), S64 (audit streaming), S70 (multi-region) | C3 sales gate. **[BLOCKING]** before S40. |
| **ADR-0224** (constraint solver) | PHASE-2A (no solver, light expressions only); PHASE-3 S49–S54 (full solver in Component Editor) | planegcs MIT; Phase 3A only. **[BLOCKING]** for S49. |
...
> 2. The 22 strategic ADRs in `docs/00_NEW_ARCHITECTURE/adrs/` (ADR-0201..ADR-0224 of the strategic series).
...
|---|---|---|
| `ADR-0202 spike` | `[strategic ADR-0202] CRDT spike (per SPEC-03 §3)` | Disambiguates the CRDT spike from the sprint-scoped command-handler ADR. |
| `ADR-0207 (WebGPU/WebGL2 dual-mode)` | `[strategic ADR-0206] (WebGPU/WebGL2 dual-mode)` | Strategic ADR-0206 = render mode. |
| `ADR-0207 fallback path` | `[strategic ADR-0206] fallback path` | Same. |
| `ADR-0207 mandates two CI matrices` | `[strategic ADR-0206] mandates two CI matrices (per SPEC-04 visual-diff parity gate)` | Strategic ADR-0206 + SPEC-04 jointly. |
| `ADR-0210 coalescing window` | `[strategic ADR-0210] (250 ms bake debounce)` | Strategic ADR-0210 is the coalescing decision; force 250 ms (not 500 ms). |
| `coalescing window = 500 ms` | `coalescing window = 250 ms ([strategic ADR-0210])` | Several phase docs still cite 500 ms. Hard fix. |
| `ADR-0216 draft (coalescing window)` | `[strategic ADR-0210] (250 ms bake debounce)` | PHASE-1D mid-text misnumbering. |
| `ADR-0217 — `.pryzm` v1 format` | `code-level ADR docs/02-decisions/adrs/0017-pryzm-zip-format-v1.md (`.pryzm` v1 format)` | Sprint-scoped. |
| `ADR-0209 (plugin sandbox)` | `[strategic ADR-0209] (Web Worker plugin sandbox)` | Same decision. |
...
[NEW]
```markdown
> **Velocity-slip cut list.** Every M-gate in this phase is governed by `[strategic ADR-0218]` — the standing capacity cut list. The phase-specific risks below are *additional* to the cuts already enumerated in ADR-0218 §Tier-1, §Tier-2, §Tier-3. If actual velocity at the gate is amber/red, cuts are applied in order from ADR-0218 before phase-specific mitigations.
```
...
- [ACTION] `INSERT` at the top of the deferred table:
  ...
  | Helm / single-binary self-host | Post-GA — `[strategic ADR-0212]` |
  ```

**Line 462 ("TRAA jitter under idle-continuation"):**

- [ACTION] `REPLACE` `chosen in ADR-0206 spike` with `chosen in code-level ADR docs/02-decisions/adrs/0006-idle-continuation-budget.md`. (Distinct from strategic ADR-0206 which is render mode.)

**Line 557 ("ADR-0210 coalescing window (250 ms) implemented"):**

- [ACTION] `REPLACE` with `[strategic ADR-0210] (250 ms bake debounce) implemented per SPEC-02 §5`. ✓ Already at 250 ms — no value change.

**Line 560 ("ADR-0210 pricing audit"):**

- [ACTION] `REPLACE` `ADR-0210 pricing audit` with `[strategic ADR-0210] §pricing-audit (per SPEC-02 §5.3)`.

**Line 629 (R1-03 row):**

- [ACTION] `REPLACE` `Coalescing window in ADR-0210` with `Coalescing window in [strategic ADR-0210]`.

**Line 631 (R1-05 row):**

- [ACTION] `REPLACE` `idle-continuation budget chosen in ADR-0206` with `idle-continuation budget chosen in code-level ADR docs/02-decisions/adrs/0006-idle-continuation-budget.md`.
...
- [ACTION] `REPLACE` the entire ADR-drafts table with:
  ...
  | MessagePack codec | S04 D2 | F (drafted by A) | **`[strategic ADR-0204]` ratifies** — phase-doc ADR-0204 deleted; bench numbers attached to the strategic ADR's "Phase rollout S04". |
  ...
  | WebGPU/WebGL2 dual-mode | S06 D1 | F (drafted by B) | **`[strategic ADR-0206]` ratifies** — phase-doc ADR-0207 deleted; CI matrix wiring documented in the strategic ADR's "Phase rollout S04/S08". |
  | Plugin sandbox model | **S01 D1–D5 spike** | F (drafted by A+B) | **`[strategic ADR-0209]` ratifies** — 5-day spike output linked from ADR-0209. |
  ```
...
- After the existing S01 task list, `INSERT`:
  ```markdown
  - **S01-T0 — Plugin sandbox spike (D1–D5, F + A + B paired)**: 5-day measurement of postMessage RPC cost on the target plugin shapes (per `[strategic ADR-0209]` Phase rollout S01). Output: 1-page report linked from `00_NEW_ARCHITECTURE/adrs/ADR-0209-plugin-sandbox.md`.
  - **S01-T1 — OTel SDK wrapper lands (D2–D8, Agent A)**: `packages/otel/` per `[strategic ADR-0207]`. First spans emitted from `packages/wire/`. Honeycomb dev account wired in S02.
  ```
...
- [ACTION] `INSERT_AFTER` the existing `IdleContinuation` description:
  ```markdown
  - **Browser worker pool capped at 4** per `[strategic ADR-0205]`. The frame scheduler must refuse to spawn a 5th. Document the cap in `packages/frame-scheduler/README.md`.
  ```

**S06 (lines 574+):**

- [ACTION] `REPLACE` "ADR-0207 fallback path" with "`[strategic ADR-0206]` fallback path".
- [ACTION] `INSERT_AFTER` S06-T1:
  ```markdown
  - **S06-T2 — Visual-diff CI gate at warning level (D7, Agent B)**: per `[strategic ADR-0206]` Phase rollout S08. The 24-scene corpus is **not** required at S06; warning-level on a 4-scene smoke set is.
  ```

**Line 626 (the WebGPU-Linux risk row):**

- [ACTION] `REPLACE` `ADR-0207 mandates two CI matrices` with `[strategic ADR-0206] mandates two CI matrices (per SPEC-04 visual-diff parity gate)`.

**Line 726 (K1A-3 kill switch):**

- [ACTION] `REPLACE` `ADR-0207 is amended to make WebGL2 the default` with `[strategic ADR-0206] is *not* amended (the dual-path is canonical); instead, the WebGPU CI matrix is moved to allowed-flake while the underlying issue is investigated. Per ADR-0206 §Phase-rollout, the dual-path stays through GA.`
...
- [ACTION] `REPLACE` `(ADR-0208 detail)` with `(code-level ADR docs/02-decisions/adrs/0008-wall-tool-submodes.md)`.
- [ACTION] `INSERT_AFTER` the §1.3 introduction:
  ```markdown
  > **Type catalog dependency.** Wall types (`Wall.standard`, `Wall.shear`, `Wall.elemented`, `Wall.partitioning`) are declared in `packages/types-builtin/wall/` per SPEC-05 §7.1. Walls in S07–S12 must reference a type id; instance-only walls are forbidden. Per `[strategic ADR-0217]` Phase rollout S11, the type-completeness lint becomes PR-blocking from S11 — but Phase 1B walls already comply by writing types into `packages/types-builtin/`.
  ```

**Line 53 (the `errors.ts` row):**

- [ACTION] `REPLACE` `mandatory in PRYZM 2 per ADR-0202` with `mandatory in PRYZM 2 per code-level ADR docs/02-decisions/adrs/0002-command-handler-signature.md`.

**Line 89 (the `WallTool.ts` row):**

- [ACTION] `REPLACE` `per ADR-0208's wall-tool sub-mode triage` with `per code-level ADR docs/02-decisions/adrs/0008-wall-tool-submodes.md`.

**S07 (the wall producer sprint):**

- [ACTION] `INSERT` a new task:
  ```markdown
  - **S07-T0 — Robustness property-test suite scaffold (D2, Agent A)** — `packages/geometry-kernel/__tests__/robustness/` lands per `[strategic ADR-0220]` Phase rollout S07. The `wall-join.spec.ts` property test (two walls at angle θ ∈ [1°, 179°] with thickness t ∈ [50 mm, 600 mm]; assert miter joint manifold + area within 1% of analytic) MUST pass before S07 close. From S08, the suite is a PR-merge gate.
  ```
...
- [ACTION] `INSERT` to the exit criteria:
  ```markdown
  - [ ] `manifold-3d` pinned to exact SHA in `package.json` per `[strategic ADR-0220]`.
  - [ ] `kernel.error` OTel span emitted on every `Result.err` per `[strategic ADR-0220]` §OpenTelemetry.
  ```
...
- [ACTION] `INSERT` to the goal description:
  ```markdown
  > **Type-catalog gate (`[strategic ADR-0217]`)**: by S11 close, `packages/types-builtin/{door,window,roof}/` MUST contain at least the v1 starter types per SPEC-05 §7.3 (8 doors, 8 windows, 4 roofs). The type-completeness lint (`tools/lint-type-completeness.ts`) is PR-blocking from this sprint.
  ```
...
- [ACTION] `INSERT_AFTER` §1:
  ```markdown
  ### §1.x Type catalog hardening (`[strategic ADR-0217]`)
...
[ACTION] `INSERT` per sprint:
  ...
  - [ ] Property-test for the family in `packages/geometry-kernel/__tests__/robustness/<family>.spec.ts` green (per `[strategic ADR-0220]`).
  - [ ] OTel coverage lint passes per P8 (`[strategic ADR-0207]`).
  ```

**S15 (Renderer hardening — TRAA, SSGI):**

- [ACTION] `REPLACE` any reference to `ADR-0206 spike` with `code-level ADR docs/02-decisions/adrs/0006-idle-continuation-budget.md`.
...
- [ACTION] `REPLACE` every `ADR-0217` in this sprint with `code-level ADR docs/02-decisions/adrs/0017-pryzm-zip-format-v1.md` (ZIP-format ADR; sprint-scoped). Cite SPEC-02 §6 (file-format addendum) as the strategic anchor.

- Specific text fixes inside S20:
  - Line ~381 ("**ADR-0217** defines the `.pryzm` v1 format"): `REPLACE` `**ADR-0217**` with `**Code-level ADR `0017-pryzm-zip-format-v1.md`**`.
  - Line ~387 ("ADR-0217 ZIP Layout"): same.
  - Line ~535–546 (`ADR-0217 draft` paragraphs): same renumbering.
  - Line ~565 ("ADR-0217 merged"): `REPLACE` with "Code-level `0017-pryzm-zip-format-v1.md` merged".

**S21 (bake worker server-side v0):**

- [ACTION] `REPLACE` every `ADR-0216` (bake worker coalescing) reference with `[strategic ADR-0210]` (250 ms bake debounce). Specific lines:
  - Line ~545 ("ADR-0216 draft (coalescing window)"): `REPLACE` with `[strategic ADR-0210] (250 ms bake debounce per SPEC-02 §5)`.
  - Line ~549 ("ADR-0216 finalised draft — coalescing window = 250 ms"): `REPLACE` with `[strategic ADR-0210] (250 ms bake debounce — already canonical at the strategic ADR; the sprint output is the implementation log)`.
  - Line ~554 ("ADR-0216 walkthrough"): `REPLACE` with `[strategic ADR-0210] walkthrough`.
  - Line ~582 ("the 250 ms coalescing window (ADR-0216)"): `REPLACE` with `the 250 ms coalescing window ([strategic ADR-0210])`.
  - Line ~795 ("B presents ADR-0216 draft"): `REPLACE` with `B presents the implementation report against [strategic ADR-0210]`.
  - Line ~826 ("ADR-0216 merged"): `REPLACE` with `[strategic ADR-0210] implementation log linked from the ADR appendix; no separate phase-doc ADR is created`.
...
- [ACTION] `INSERT` to the S21 exit criteria:
  ```markdown
  - [ ] `[strategic ADR-0205]` worker_threads pool sizing verified.
  - [ ] `[strategic ADR-0203]` storage driver isolation lint green.
  - [ ] Per-event bake cost telemetry stream live (`bake.event.cost`); used to validate ADR-0218 cut-list pricing assumptions.
  ```
...
- [ACTION] `INSERT_AFTER` the existing list:
  ```markdown
  - **Drawing engine architecture lands (`[strategic ADR-0216]`)** — vector primitives in `packages/drawing-primitives/` with Canvas2D backend (S29), SVG and PDF backends following (S31, S33). This is the single source of truth for plan/section/sheet output. **[BLOCKING]** for Phase 2B.
  - **Soft-lock semantics (`[strategic ADR-0219]`)** — per-element TTL with role-aware lifetimes; mid-edit lock loss UX defined. **[BLOCKING]** for S48.
  - **Permission matrix (`[strategic ADR-0211]`)** — role/view/element-class enforcement at L2 + L3 + edge + UI. **[BLOCKING]** for S43.
  ```
...
- [ACTION] `INSERT` to the table:
  ```markdown
  | Constraint solver (full 2D) | Phase 3A — `[strategic ADR-0224]` |
  | SAML/OIDC SSO | Phase 3B — S55 (`[strategic ADR-0221]`) |
  | SCIM 2.0 | Phase 3B — S58 |
  | Audit-log streaming to SIEM | Phase 3D — S64 |
  | Multi-region pinning (EU-W + US-E) | Phase 3D — S70 |
  ```

**§1.5 / line 70 ("ADR-0202 spike pre-Sprint S01"):**

- [ACTION] `REPLACE` `ADR-0202 spike pre-Sprint S01 was this risk's first mitigation` with `[strategic ADR-0202] (CRDT + event log bridge) was the framing decision; the Yjs spike pre-S01 was its first mitigation`.

**Line 407 ("ADR-0202 spike artifacts"):**

- [ACTION] `REPLACE` with `[strategic ADR-0202] spike artifacts (CRDT + event log bridge per SPEC-03 §3)`.

**Line 532 (R2-02 mitigation):**

- [ACTION] `REPLACE` `ADR-0202 spike pre-S01` with `[strategic ADR-0202] CRDT spike pre-S01 (per SPEC-03 §3 + §6)`.
...
- [ACTION] `REPLACE` the entire table with:
  ...
  |---|---|---|
  | Plan-view renderer architecture (Canvas2D, dirty flags, no THREE) | S31 D1 | code-level `0023-plan-view-canvas2d-renderer.md` — **subordinate to `[strategic ADR-0216]`**. |
  | Section-view cut algorithm | S35 D1 | code-level `0024-section-cut-algorithm.md`. **Note**: `[strategic ADR-0224]` is the *constraint solver*; the section-cut decision is the sprint-scoped 0024 file in `docs/02-decisions/adrs/`. |
  | Multi-view sync strategy | S36 D1 | code-level `0025-multi-view-sync.md`. |
  ```

**S31 (line 328) "F finalises ADR-0223":**

- [ACTION] `REPLACE` with `F finalises code-level `0023-plan-view-canvas2d-renderer.md` — "Plan view renderer: Canvas2D back-end of the SPEC-04 vector primitive model per [strategic ADR-0216] (drawing engine architecture)…"`. The existing description body (Canvas2D, dirty-flag, world XZ → canvas xy) is preserved.
...
**S35 (line 832) "ADR-0224 defines the section cut algorithm":**

- [ACTION] `REPLACE` `ADR-0224 defines the section cut algorithm` with `Code-level ADR `0024-section-cut-algorithm.md` defines the section cut algorithm`.
- [ACTION] `INSERT_AFTER` the section description:
  ```markdown
  > **Naming clarification.** `[strategic ADR-0224]` is the *constraint solver* (`docs/00_NEW_ARCHITECTURE/adrs/ADR-0224-constraint-solver.md`). The section-cut decision documented in this sprint is the sprint-scoped `docs/02-decisions/adrs/0024-section-cut-algorithm.md`. Both files exist; the strategic ADR-0224's "Naming note" already documents the historical collision.
  ```

**S36 (line 1022) "ADR-0225 multi-view sync":**
...
- [ACTION] `REPLACE` `ADR-0224 defines fallback` with `Code-level `0024-section-cut-algorithm.md` defines fallback`.
...
- [ACTION] `INSERT` to the exit criteria of S41:
  ```markdown
  - [ ] Schedule formula library covers the SPEC-06 §3 fixed-formula set. **No DSL evaluator** in v1 per `[strategic ADR-0218]` T1.3.
  ```
...
- [ACTION] `REPLACE` the bullet "Self-host packaging — `docker-compose up`…" with:
  ```markdown
  - **Self-host packaging (`[strategic ADR-0212]`)** — `docker-compose up -d` deploys editor + sync-server + bake-worker + ai-worker + Postgres + Redis + MinIO + Caddy + observability stack on a fresh Linux VM in < 10 minutes. Helm chart and single-binary are post-GA.
  ```

- [ACTION] `INSERT_AFTER` the "Self-host packaging" bullet:
  ```markdown
  - **Enterprise security & residency (`[strategic ADR-0221]`)** — SAML 2.0 + OIDC SSO (S55), SCIM 2.0 provisioning (S58), TOTP MFA (mandatory for Enterprise admin), audit-log streaming to webhook/SIEM (S64), per-tenant region pinning across EU-W and US-E (S70). WebAuthn/passkeys deferred to v2 (Tier-1 cut). AP-Southeast deferred to Phase 4 (Tier-1 cut). SCIM is Tier-2 cut candidate.
  - **Constraint solver (`[strategic ADR-0224]`)** — `planegcs` integrated for the Component Editor at S49–S54. SolveSpace WASM reserved as backup; license incompatible with default SaaS.
  - **Type catalog freeze (`[strategic ADR-0217]`)** — at S70 the M36 ship-with-product catalog (12 walls, 8 floors/roofs, 8 doors, 8 windows, 40 materials, plus loadable-family starter set) is frozen.
  - **Marketplace v1 fallback (`[strategic ADR-0218]` T1.1)** — if launch partners aren't signed by S60, marketplace ships first-party-only at GA.
  ```
...
- [ACTION] `INSERT_AFTER` the diagram:
  ...
  > - **3A** anchors `[strategic ADR-0214]` (AI L7.5 ops) and `[strategic ADR-0224]` (constraint solver). The AI sub-phase is **Tier-3 cuttable** to "critic-only at GA" per `[strategic ADR-0218]` T3.1.
  > - **3B** anchors `[strategic ADR-0208]` (IFC scope), SPEC-11 (full IFC pipeline), and the loadable-family authoring per SPEC-05 §8.
  > - **3C** anchors `[strategic ADR-0209]` (sandbox) and SPEC-09 (plugin SDK 1.0).
  > - **3D** anchors `[strategic ADR-0212]` (self-host) + `[strategic ADR-0221]` (enterprise security).
  ```
...
- [ACTION] `INSERT_AFTER` the goal description:
  ```markdown
  > **Constraint-solver kickoff (`[strategic ADR-0224]` Phase rollout S49)**: in parallel with VI waves, `packages/constraint-solver/` lands with `planegcs` integrated. First 5 constraint types (distance, angle, parallel, perpendicular, equal-length) working by S49 close. **No production usage** until S52; this sprint is foundation.
  ```
...
- [ACTION] `INSERT_AFTER` the goal description:
  ```markdown
  > **AI scope per `[strategic ADR-0214]`**: this sprint is generator-pattern. If `[strategic ADR-0218]` T3.1 fires (40–60% velocity slip), generator features ship as preview-only at GA and the critic-pattern is the only L7.5 capability marketed.
  ```
...
- [ACTION] `INSERT_AFTER` the goal description:
  ```markdown
  > **IFC scope per `[strategic ADR-0208]` + SPEC-11**: v1 ships IFC4 read+write Pset round-trip for the 14-family entity table in ADR-0208. IFC4.3 advanced (alignment, road, rail) is post-GA. MEP and structural-analytical are Phase 3+ marketplace plugins. The S55–S58 entity coverage MUST match ADR-0208's table; if the M30 velocity check trips T3.3 (40–60% slip), drop stairs/railings/curtain-wall/furniture from IFC export — keep the core 6 (walls, slabs, columns, beams, doors, windows, spaces).
  ```

  Also `INSERT` to S55 exit criteria:
  ```markdown
  - [ ] Type round-trip preserved via `IfcRelDefinesByType` per SPEC-05 §5.
  - [ ] Round-trip CI corpus passes for the entity table in `[strategic ADR-0208]`.
  ```
...
- [ACTION] `INSERT`:
  ```markdown
  > **Constraint solver dependency (`[strategic ADR-0224]`)**: the Component Editor consumes `packages/constraint-solver/` for sketch-level constraints. Family parameters expose solver variables as user-editable inputs. Solver runs reactively on every constraint addition/edit, debounced via the same 250 ms policy as bake (`[strategic ADR-0210]`).
  > **Type-catalog dependency (`[strategic ADR-0217]`)**: family/type/instance schemas authored in this editor land in `packages/types-schema/loadable/` and ship in the M36 catalog freeze at S70.
  > **Tier-2 cut warning (`[strategic ADR-0218]` T2.2)**: if M30 velocity is 20–40% slipping, the Component Editor (D10) is the largest Tier-2 cut. Its deferral defers the full constraint solver with it; the light expression evaluator (Phase 2A) remains.
  ```

**S62 (Plugin SDK 1.0):**

- [ACTION] `REPLACE` "ADR-0209 (plugin sandbox) re-validated" with `[strategic ADR-0209] (Web Worker plugin sandbox) re-validated against final implementation; Tier-3 cut T3.4 (substitute internal pen-test) is rejected unless explicit sign-off from F.`
...
- [ACTION] `INSERT` to the exit criteria:
  ...
  - [ ] Resource limits enforced per `[strategic ADR-0209]` §resource-limits (256 MiB heap, 4 ms per-frame CPU, 50 req/min/plugin/actor network).
  ```
...
- [ACTION] `INSERT_AFTER` the goal description:
  ```markdown
  > **First-party-only fallback (`[strategic ADR-0218]` T1.1)**: if launch partners (≥ 5 third-party plugin developers committed to GA) are not signed by S60, marketplace ships first-party-only at GA. The full revenue-share infra still lands; only the third-party listing surface is held back.
  ```
...
- [ACTION] `REPLACE` the entire goal/why-now block with:
  ```markdown
  **Goal**: per `[strategic ADR-0212]` and SPEC-12, ship a single `docker-compose.yml` that brings up the full stack (editor + sync-server + bake-worker + ai-worker + Postgres 16 + Redis 7 + MinIO + Caddy + Grafana/Tempo/Prom/Loki) on one Linux VM with `docker compose up -d` in < 10 minutes. Air-gap install via tarball (Tier-2 cut candidate).

  **Why now**: D7 (open self-host) is a binding GA requirement (`08-VISION.md` D7 + Ask 04 confirmation). C3 (large enterprise) cannot adopt without this. `[strategic ADR-0221]` (Enterprise security) requirements (RLS, MFA, audit log) must ship in the same image set.
  ```

- [ACTION] `INSERT` to the exit criteria:
  ```markdown
  - [ ] Air-gap install tested on a network-isolated VM (Tier-2 cut candidate per `[strategic ADR-0218]` T2.5; must pass unless cut applied).
  - [ ] Caddy TLS termination + reverse proxy live in compose stack per `[strategic ADR-0212]`.
  - [ ] License-key check is offline (signed JWT bundled in install) per `[strategic ADR-0212]` §air-gap.
  ```
...
- [ACTION] `INSERT`:
  ```markdown
  > **Pen test scope per `[strategic ADR-0221]` + SPEC-08 §9**: external pen test (third-party). Tier-3 cut T3.4 (internal pen test substitute) is **rejected unless explicit sign-off from F**.
  > **CI gate per `[strategic ADR-0221]`**: service-role-key isolation lint (only `apps/sync-server/`, `apps/bake-worker/`, `apps/ai-worker/`, edge functions allowed) MUST be green at this sprint and forever after.
  ```
...
- [ACTION] `INSERT`:
  ```markdown
  > **Multi-region delivery per `[strategic ADR-0221]`**: EU-West (Frankfurt) and US-East (Virginia) regions live by S70 close. AP-Southeast deferred to Phase 4 (Tier-1 cut per `[strategic ADR-0218]` T1.2).
  > **Browser parity per `[strategic ADR-0206]`**: visual-diff CI gate at error level across the 24-scene corpus AND across Chrome/Firefox/Safari/Edge.
  ```
...
- [ACTION] `REPLACE` the GA gate exit criteria block with the union of:
  - Existing block.
  - `[strategic ADR-0218]` final cut accounting written to `docs/03-execution/status/cut-list-log.md`.
  - `[strategic ADR-0220]` property-test suite green across all element families on Node 20 + Chrome + Safari + Firefox.
  - `[strategic ADR-0221]` SOC 2 Type 1 audit kicked off (M30) and on track for Type 1 by GA.
  - `[strategic ADR-0207]` Tempo prod live in EU-W + US-E with on-call dashboard.
  - `[strategic ADR-0208]` IFC round-trip CI corpus 100% green for the 14-family table.
  - `[strategic ADR-0217]` ship-with-product catalog frozen and shipping.
  - `[strategic ADR-0224]` snapshot suite green across Node 20 + Chrome + Safari + Firefox.

**§9 ("What Phase 3 explicitly did NOT do — post-GA roadmap seeds"):**

- [ACTION] `REPLACE` line 618 (`IFC 4.3 advanced features (per ADR-0208)`) with:
  ```markdown
  - **IFC4.3 advanced features** (alignment, road, rail) — per `[strategic ADR-0208]`.
  - **Per-instance ACL overrides** — per `[strategic ADR-0211]`.
  - **WebAuthn / passkeys** — per `[strategic ADR-0221]`.
  - **AP-Southeast region** — per `[strategic ADR-0221]` + `[strategic ADR-0218]` T1.2.
  - **Helm chart, single-binary, BYOC** — per `[strategic ADR-0212]`.
  - **MEP / structural-analytical / civil-infra IFC** — per `[strategic ADR-0208]`.
  - **Schedule formula DSL** — per `[strategic ADR-0218]` T1.3.
  - **3D constraints, NURBS constraints** — per `[strategic ADR-0224]`.
  - **FedRAMP, ISO 27001, HIPAA BAA** — per `[strategic ADR-0221]` Compliance posture.
  ```
...
| `packages/drawing-primitives/` skeleton + tests for hatch alignment + dash-phase preservation | PHASE-2A | S25 (per `[strategic ADR-0216]` Phase rollout) | B |
...
| `[strategic ADR-0218]` final cut accounting written to `docs/03-execution/status/cut-list-log.md` | PHASE-3 | S72 | F |

---

## §5 Cuts that may be exercised at each phase gate

For agent reference during velocity check-ins. From `[strategic ADR-0218]`. **No agent applies cuts unilaterally** — cuts are F's call at the gate.
...
| All phase docs have an `[strategic ADR-0218]` reference in their risk register | `for f in docs/00_NEW_ARCHITECTURE/phases/PHASE-*; do rg -L 'strategic ADR-0218' "$f" \|\| true; done` | No paths printed. |
| No `coalescing window = 500 ms` remains | `rg -n '500 ms' docs/00_NEW_ARCHITECTURE/phases/ \| rg -i 'coalesc'` | Empty. |
| `[strategic ADR-0216]` cited at S29 | `rg -n 'strategic ADR-0216' docs/00_NEW_ARCHITECTURE/phases/PHASE-2A-*` | At least one match in/near S29-S30. |
| `[strategic ADR-0217]` cited at S11 | `rg -n 'strategic ADR-0217' docs/00_NEW_ARCHITECTURE/phases/PHASE-1B-*` | At least one match in/near S11. |
| `[strategic ADR-0219]` cited at S43–S48 | `rg -n 'strategic ADR-0219' docs/00_NEW_ARCHITECTURE/phases/PHASE-2-*` | At least one match. |
| `[strategic ADR-0220]` cited at S07–S08 | `rg -n 'strategic ADR-0220' docs/00_NEW_ARCHITECTURE/phases/PHASE-1B-*` | At least one match. |
| `[strategic ADR-0221]` cited in PHASE-3 sprints S55, S58, S64, S70 | `rg -n 'strategic ADR-0221' docs/00_NEW_ARCHITECTURE/phases/PHASE-3-*` | ≥ 4 matches. |
| `[strategic ADR-0224]` cited in PHASE-2A (light expr) and PHASE-3 S49–S58 | `rg -n 'strategic ADR-0224' docs/00_NEW_ARCHITECTURE/phases/` | ≥ 3 matches. |
...
- `Strategic ADR parent` (where applicable): e.g. `0023` cites `[strategic ADR-0216]`; `0017` is independent of any strategic ADR; `0008` cites `[strategic ADR-0220]` (kernel robustness) as its *constraint context*.
...
- **Sprint exit criteria that already match the SPECs/ADRs** (e.g. PHASE-1's `[strategic ADR-0210]` 250 ms reference at line 557 already aligned).
...
- **The `08-VISION` differentiator commitments** (D1–D10). Cuts in `[strategic ADR-0218]` are scope reductions within commitments, not commitment reversals.
- **The 36-month total budget.** `[strategic ADR-0218]` T3.5 (M36 → M40 slip) is a last-resort lever, not a default.
