...
| **BIM 1/2/3** Live Parametric (14 deliverables, ~17 wk) | [APARTMENT-BIM2-BIM3-DATA-MANAGEMENT-AND-LIVE-PARAMETRIC-SYSTEM](APARTMENT-BIM2-BIM3-DATA-MANAGEMENT-AND-LIVE-PARAMETRIC-SYSTEM.md) | 🟨 1/14 — D-α-0 (`ApartmentParameterSchema` + `RoomParameterSchema` in `@pryzm/schemas/apartment`) ✅ 2026-05-30. Partial via modal-dynamic (pre-execute path). |
...
|---|---|---|
| **T1.W** Window-emission engine | NEW pure sub-engine | ✅ ALL FIVE T1.W parts shipped 2026-05-30 — engine (T1.W-A) + buildLayoutCommands wiring (T1.W-B) + shell-wall-host resolver (T1.W-C ai-host) + executor wiring (T1.W-C editor) + modal-preview render (T1.W-D). Generated apartments now have real windows on the shell with per-room finishes. |
