...
| **D2.2** | `validateRoomFit` | DIMENSIONAL §9.2 | 3 d | ✅ shipped 2026-05-30 — `dimensions/validateRoomFit.ts` implements G5 furniture-fit lower-bound: per-room, sums (sizeW + 2·clearSide) × (sizeD + clearFoot) × count for every required FurnitureSpec; HARD-rejects rect area < lower bound (placement impossible); SOFT penalty when within 1.2× tight band. Exports `requiredFurnitureAreaM2(type)` for downstream consumers (UI hints, debug). Rooms without furniture program (corridor / utility) always pass. Pure + L2-clean, no THREE/DOM. +7 pin tests. Not yet wired into enumerate.ts as a gate — wire-in slice queued. |
