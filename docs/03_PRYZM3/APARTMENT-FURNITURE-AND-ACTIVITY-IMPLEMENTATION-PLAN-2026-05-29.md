...
| Z.−0a | **BIM 2/3 D-α** | L0 data substrate — schemas / stores / commands / patch-merge / propagation-engine P1 (recomputeImpact) | **4 / 6** | 4 wk | 🟨 |
...
| Z.6 | **Tier 5** | Cognition L3 — Semantic Topology (EdgeType enum + bubbleGraph populate + wallsAndDoors reads EdgeType + edgeRealisation axis) | **4 / 4** | 2.5 wk | ✅ |
...
| Z.8 | **Tier 7** | Cognition L4 — Compositional Geometry (alignmentField · wetStackAlignment · openingCadence · proportionalElegance — all SCORING form; L4-δ-1b CONSTRUCTIVE subdivider variant shipped 2026-05-31) | **4 / 4** scoring + 1 / 3 constructive | 3 wk | 🟨 |
...
| Z.19 | **Tier 18** | Housekeeping — F8.1 orphan audit ✅ · F8.2 door/window defaults resolver ✅ (overlaps T1.D) · F8.3 materialIntent labels ✅ | **3 / 3** | 3 d | ✅ |
...
- **Test pass count** (load-bearing snapshots): ai-host **917/917** · stores 204/204 · schemas 164/164 · editor (layoutCardModel 12/12 + layoutThumbnail 23/23) — pre-existing AiHost.* SCC failures unchanged
- **Latest commits**: `f9aed02` L4-δ-1b · `D-α-3 P1` · `6cffd11` F8.1 (3-track parallel multi-agent run) — branch synced
...
| ID | Subphase | What | Status |
|---|---|---|---|
| **F8.1** | `FurnitureType` audit | After F1–F7, sweep `FurnitureType` for orphans (types no archetype references). Either retire or add to a "manual-only" tier. | ✅ **shipped 2026-05-31** (multi-agent track C) — `packages/ai-host/__tests__/furnishCatalogueAudit.test.ts` (+10 pin tests across 6 describes) audits the four-way relationship: geometry-furniture FurnitureType (110+) ↆ D-FLE FurnitureKind (~50) ↆ footprints ↆ archetypes. Fails LOUDLY on: FurnitureKind without footprint, footprint without FurnitureKind, archetype referencing unknown FurnitureKind, archetype-only-kind without footprint. INFORMATIONAL orphan count: **85** types are in geometry-furniture but not in FurnitureKind (manual-placement-only — drag-and-drop catalogue, GLB import, parametric variants, 25 outdoor trees, 8 plants, named-style chairs/sofas/beds). Asserted < 100 so future expansions surface as a test signal. |
...
| **F8.3** | Material intent labels | Pin each `FurnitureType` with a `materialIntent` field (`timber-warm` / `metal-cool` / `fabric-soft` / etc.) so future material-intelligence layers (cognition-stack §3.A/D) have a substrate. | ✅ **shipped 2026-05-31** — `packages/geometry-furniture/src/FurnitureMaterialIntent.ts`. 11-category enum: timber-warm / timber-light / metal-cool / metal-warm / fabric-soft / ceramic-clean / glass-translucent / plastic-utility / mixed-kitchen / plant-natural / mixed-unknown. Exhaustive `Record<FurnitureType, FurnitureMaterialIntent>` map covering 110+ types (the full FurnitureType union including parametric beds, kitchens, wardrobes, plant_01-08, arbol_t_01-25 trees, ai_element + glb_import). `deriveMaterialIntent(type)` throws on unknown type (fail-explicit contract). TypeScript Record check enforces exhaustiveness at compile time; +17 pin tests cover spot-checks (sofas = fabric, wet fixtures = ceramic, Nordic = light timber, kitchens = mixed, plants/trees = plant-natural, AI/GLB = mixed-unknown, throws on unknown). Substrate for future material-resolver pass that maps `(intent, typology)` → concrete `materialId` per Nordic / Mediterranean / Haussmann etc. |
...
| **L3-γ-3** | `wallsAndDoors.ts` reads EdgeType for geometric treatment | COGNITION §8 | 1 wk | ✅ **shipped 2026-05-31** — `buildWallsAndDoors` now builds an `edgeKindByPair` lookup from `graph.edges[i].kind` (populated by L3-γ-2's `classifyEdge`) and sizes each emitted door by its semantic role: **SOCIAL_FLOW 1.10 m** (passage between social spaces), **CEREMONIAL_THRESHOLD 1.00 m** (arrival ritual), **BUFFER 0.90 m** (standard residential), **SERVICE_ACCESS 0.90 m** (privacy via T1.D's wt-upvc-casement finish, not width), **INTIMATE_ACCESS 0.80 m** (master↔ensuite narrower privacy reading). Edges without `kind` (AI-path back-compat) and caller-supplied `opts.doorWidthM` overrides both fall back to the global default — no regressions for existing test fixtures. Short walls clamp the preferred width to fit instead of dropping the door. +8 pin tests in `tglWallsAndDoors.test.ts`. ai-host **883/883**. |
...
| ID | Deliverable | Doc source | Est | Status |
|---|---|---|---|---|
| **L4-δ-1** | `AlignmentField` pre-subdivide axis-line snap | COGNITION §8 | 1.5 wk | ✅ **both forms shipped**: SCORING (2026-05-30) — `ObjectiveVector.alignmentField` axis with 50 mm bucket tolerance + 7 pin tests; CONSTRUCTIVE L4-δ-1b (2026-05-31, multi-agent track A) — post-pass on `subdivide()` clusters every room-rect coord (X + Z independently) by 50 mm and snaps each edge to the cluster mean. New `SubdivideOptions.alignmentSnap` flag defaults ON; `snapAxisLines()` exported for reuse. Defensive per-axis guard prevents rect-inverting snaps. +9 pin tests across cluster behaviour, tolerance, no-op-on-aligned, transitive cluster, opt-out flag. Matches the scoring axis's same sort-sweep so snapped layouts max the scoring axis by construction. |
...
| **F3.5** | WC (wc_washbasin + wc_mirror + accessories) | FURNITURE §4.4 | 0.5 d | ✅ closed 2026-05-31 — `wc` added to `FurnishableOccupancy` union; new `'wc'` archetype wires toilet_radiator (required, longest wall, excludeDoorSwing) + wc_washbasin (required, wall-opposite-door, 'wc-basin' group) + wc_mirror (optional, beside basin, same group). minAreaM2 1.2 (UK cloakroom-WC envelope). `programRules.wc.furnitureSpec` ALSO closes the queue note in code — wc_washbasin promoted from fixture-only to a real renderable in the spec (sizeW 450 × sizeD 300, clearFoot 550, clearSide 50, placementRule 'opposite_door'); wc_mirror added as optional (placementRule 'beside_group'). Required-furniture list extended `+wc_washbasin`; optional `+wc_mirror`. +5 pin tests. ai-host **875/875**. |
