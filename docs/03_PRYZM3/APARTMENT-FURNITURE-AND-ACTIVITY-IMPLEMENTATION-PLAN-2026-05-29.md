...
| **T1.D** | Per-room default door + window system types | MATRIX §D + queue `ai-creation-default-element-types` | 1 week | 🟨 **2026-05-30** — pure resolver shipped: `resolvers/defaultElementTypes.ts` (`defaultDoorSystemTypeId(roomA, roomB)` + `defaultWindowSystemTypeId(roomType)` + `*Reason` companions). Rules: wet rooms (bathroom/ensuite/wc/utility) → `dt-white-primed`; kitchen → `dt-glazed-timber` (sight to living); else → editor default solid-timber. Windows: bathroom/ensuite/wc/utility → `wt-upvc-casement`; kitchen → `wt-upvc-tilt-turn`; living/dining/bedroom/master/study → timber-casement; corridor/hall → single-pane. Catalogue-id alignment pinned by tests against the DoorSystemTypeStore + WindowSystemTypeStore built-in id sets. +19 pin tests. Wiring layer: executePlan's `stampDoorSysType` + the future T1.W window engine pick up the per-pair/per-room calls — pending. |
