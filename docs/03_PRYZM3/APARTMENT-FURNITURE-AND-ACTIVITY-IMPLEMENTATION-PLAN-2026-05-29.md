...
| **F3.4** | Bathroom + ensuite | Add `vanity_unit` (required if room ≥ 5 m², replaces toilet_radiator's integrated basin), `bathroom_mirror` (required, anchor: 'on-vanity'), `mirror_light` (required), `towel_rail` (required), `bath` (optional, area-gated). | 🟨 partial — `vanity_unit` + `bathroom_mirror` + `towel_rail` (F1.5) shipped into bathroom archetype. Missing: mirror_light (F1.5' in geometry-lighting), bath (F1.6' in geometry-plumbing). |
...
| **F1.5** | Bathroom vanity primitives (`vanity_unit`, `bathroom_mirror`, `mirror_light`, `towel_rail`) | geometry-furniture + geometry-lighting | 20 d | 🟨 furniture-side shipped 2026-05-30 — FurnitureType union + FurnitureCategoryMap + NEW BathroomVanityBuilder.ts (VanityUnitBuilder + BathroomMirrorBuilder + TowelRailBuilder) + FurnitureFactory arms; ai-host FurnitureKind + footprints; bathroom archetype wires the S4 system (vanity+mirror as paired 'vanity' group anchored opposite door); programRules.bathroom mirrors. **`mirror_light` queued separately in `geometry-lighting`** — it's a LightingFixtureType, not a FurnitureType; shipping the furniture trio with the wall-light queued does NOT violate §0.2 because the trio each closes its own ladder (no half-typed element). +5 pin tests. |
...
| **F3.4** | Bathroom/ensuite (vanity_unit + bathroom_mirror + mirror_light + towel_rail + bath) | FURNITURE §4.4 | 0.5 d | 🟨 partial — vanity_unit + bathroom_mirror + towel_rail (F1.5) landed; mirror_light + bath block on F1.5'/F1.6' |
