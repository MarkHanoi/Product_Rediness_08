...
|---|---|---|---|---|
| **T1.W** | Window emission engine (NEW pure sub-engine) | single-apartment-fix-pass-spec #5 + STATUS §3 Tier 1A | 2 weeks | 🟨 **T1.W-A (P1) shipped 2026-05-30** — pure window-placement engine in `windowEmission/{types,emitWindows}.ts`: `WindowableRoomType` union + `WINDOW_SPECS` (per-room width/height/sill/min-wall-length spec table — UK residential typicals + privacy-height sills for wet rooms + viewable-height sills for living/dining + 1000 mm sill for kitchen worktop clearance), `emitWindowsForRoom(roomType, externalWalls, name?)` picks the LONGEST viable wall and places one centred window; falls back to the smaller `minWidthMm` variant when no wall hosts the preferred. Pure + deterministic — ties broken by lowest wallIndex. `emitAllWindows()` convenience flattens across rooms. +18 pin tests. **T1.W-B wiring** (bubble-graph BOUNDS → per-room externalWalls, window.createOpening dispatch with T1.D's `defaultWindowSystemTypeId` finish) and **T1.W-C modal preview** still queued. |
