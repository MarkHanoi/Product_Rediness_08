...
| **SL-3** | **Façade orientation** | ✅ **EXISTS (2026-05-25)** — `FacadeOrientationService` (`spatial-index`) | per wall: exterior classification (referenced by ≤1 room's `boundingWallIds`) + outward normal (away from the bounding room centroid) + compass orientation (N/E/S/W) from `trueNorth` (base North = −Z). Pure math in `FacadeOrientationMath` (14 unit tests); `getFacades` / `exteriorWalls` / `facadesByOrientation(levelId, 'S')`. window-wired | "windows on south façade", "shading on west", exterior-wall prompts |
...
| **2b** ✅ (#34) | **First consuming command LANDED** — `CreateFloorsByRoomTypeCommand` (`command-registry/floors`): per room read `occupancyType` → finish category (timber: living/bedroom/dining; tile-stone: kitchen/bathroom/wc/utility) → `CreateFloorCommand` in the room boundary (`RoomVertex`/`FloorVertex` both `{x,z}`), `hostRoomId` linked, dedup on existing host floor, one undo unit via `runBatch`. Wired into C17 catalogue (Architecture › Floor › "Floor finish by room type") + AI panel; status-gating switched live-vs-phased. **#28/#29 ceilings-by-room also LANDED** (`CreateCeilingsByRoomCommand`) — both now share the rule-of-three helper `command-registry/rooms/perRoomBoundary.ts` (`buildPerRoomBoundaryElements`), which also fixed a latent redo-append bug in #34. **Next:** #31/#33 (per-room slab/screed) + #41 (per-room lighting, point-based) reuse the helper. | floors + ceilings land per room |
| **3a** ✅ | **SL-3 façade orientation service LANDED** — `FacadeOrientationService` (`spatial-index`), pure math in `FacadeOrientationMath` (no barrel-load side effects; 14 unit tests), window-wired. | `facadesByOrientation(levelId,'S')` returns the south exterior walls for a known model |
| **3b** ✅ | **Consume SL-3 / SL-2** — **windows-on-façade LANDED** (`CreateWindowsOnWallsCommand`, #11–13; build-layer resolves façade walls via injected `getFacadeWallIds` → SL-3) **+ doors-between-adjacent-rooms LANDED** (`CreateDoorsBetweenAdjacentRoomsCommand`, #7/#9/#10; SL-2 shared-wall via `boundingWallIds`; optional type filter). Both hosted via `CreateWallOpeningCommand` (C15), CREATE + AI panels. **Remaining:** #35 balconies; #2 exterior-walls-with-precut-openings. | windows on requested façade; doors only between truly adjacent rooms |
...
7. *Bathroom doors on walls shared between a bathroom and a corridor/bedroom…* — per-room-adjacency · SL-1 ✅ + SL-2 ✅ · **P3 ✅ LANDED** (`CreateDoorsBetweenAdjacentRoomsCommand` — general "door on every shared interior wall"; optional `between` type filter covers the bathroom↔corridor/bedroom rule; hosted via `CreateWallOpeningCommand` C15). #9/#10 are type-filtered variants of the same command.
...
12. *Full-height windows on all south-facing exterior walls…* — per-facade · SL-3 ✅ · **P3 ✅ LANDED** (`CreateWindowsOnWallsCommand`; build resolves S exterior walls via SL-3, command distributes windows; hosted via `CreateWallOpeningCommand` C15). #11/#13 (room-filtered, frosted) are refinements of the same path.
...
28. *Suspended ceiling grid in all office areas…* — per-room · SL-1 ✅ · **P2 ✅ LANDED** (`CreateCeilingsByRoomCommand` — suspended grid in office/meeting types).
29. *Plasterboard ceiling in apartment rooms (excl. bathrooms → moisture-resistant)…* — per-room · SL-1 ✅ · **P2 ✅ LANDED** (`CreateCeilingsByRoomCommand` — plasterboard default; per-room moisture-resistant variant is a refinement).
...
**#51 (capstone) — `apartment.generate-layout`.** The "50 + 1" prompt: given an apartment shell (perimeter walls + entrance door + windows), the AI generates N **ranked interior layout options** (internal walls + doors), validates them against hard space-planning rules, scores them, and commits the chosen option as **one undoable batch** (rooms auto-detected after). Two-phase generative L7.5 workflow (SPEC-47 pattern) — *generate → preview/approve → execute*. Composes the small pieces + SL-1/SL-2/SL-3; the only NEW code is the prompt template + the pure validator + scorer. **Fully specified in `SPEC-APARTMENT-LAYOUT-GENERATOR.md`** (A1–A7 phased plan). Buildable now; the per-element catalogue commands (#34/#28/#41/#11/#7) furnish the generated shell. Depends on Phase 3 ✅ (SL-3 shell analysis + C15 hosted openings).
