...
| **3b** 🟡 | **Consume SL-3 / SL-2** — **windows-on-façade LANDED** (`CreateWindowsOnWallsCommand`, #11–13; CREATE › Architecture › Window + AI, hosted via `CreateWallOpeningCommand` C15; build-layer resolves façade walls via the injected `getFacadeWallIds` → SL-3). **Remaining:** #35 balconies; doors-between-adjacent-rooms (#7/#9/#10) via SL-2. | windows land only on the requested façade; doors only between truly adjacent rooms |
