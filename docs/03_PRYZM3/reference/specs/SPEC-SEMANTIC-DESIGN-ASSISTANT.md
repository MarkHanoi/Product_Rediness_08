...
| **3b** | **Consume SL-3 / SL-2** — windows on exterior/oriented façades (#11–13, #35 balconies) via a hosted-window batch command (C15); doors between adjacent rooms (#7/#9/#10) via SL-2. | windows land only on the requested façade; doors only between truly adjacent rooms |
