...
| **A4-register** | bind the impl into `WorkflowRegistry`/AiPlane + cost meter (SPEC-28) + `MockAnthropicRelay` (SPEC-47 §7) + the real `shellReader` (A3 wrapper over stores + SL-3) + AIStore `pendingLayoutOptions` + the `runtime.events` emitter | a real (mock-backed) run surfaces options through the AI plane |
