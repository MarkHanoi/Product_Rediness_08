...
| L-33 | founder | **Second parametric kitchen creation fails** — after placing one L-Shape kitchen run, trying to create a SECOND one had an issue (couldn't place / errored). | Modeling/Creation/Kitchen | BROKEN→FIXED | ADR-0112 `§FIX-KITCHEN-SECOND-PLACE` — two coupled causes: (1) `KitchenCabinetTool._placeKitchen` sent `rotation` as a `{x,y,z}` **object**; `furniture.create` `canExecute` validates `Number.isFinite(rotation)` so the bus THREW `rotation must be finite`, silently rejecting EVERY 3D kitchen commit (the `.catch` swallowed it) → now a **scalar** yaw. (2) The tool `deactivate()`d after one commit → now **re-arms** (reset yaw + rebuild ghost) for continuous placement; `newKitchenRunId()` is monotonic so back-to-back ids differ. C11 |
| L-34 | founder | **Kitchen placement PREVIEW is a bounding RECTANGLE, not the real L-shape** — the ghost (plan dashed rect + 3D) must PRECISELY match the real parametric kitchen footprint (L / U / galley / single-wall) in BOTH plan and 3D, not a coarse box. | Modeling/Creation/Kitchen | BROKEN→FIXED | ADR-0112 `§FEAT-KITCHEN-ACCURATE-PREVIEW` — 3D ghost is now built by the SAME `KitchenCabinetEngine.create(cfg)` the commit uses (re-skinned with the §41 preview material) → exact configured massing, live per slider; plan ghost draws the real `KitchenPlanSymbolBuilder.buildConfigLinework(cfg)` (true L/U/galley/single/island outline + glyphs) instead of the bounding-box footprint. Preview ≡ placed by construction. C06/C11 |
| L-35 | founder | **Kitchen 2D PLAN SYMBOL must be professional/elegant/modern** — currently a poor rectangle; needs true cabinet runs + sink/hob/fridge glyphs + cabinet-door-swing arcs + work-triangle, per typical-kitchen-layout drafting convention (single-wall/galley/L/U/island — ref image). Accurate to the actual configured run. | Views/Drawing/Kitchen | BROKEN→FIXED | ADR-0112 `§FEAT-KITCHEN-PLAN-SYMBOL` — `KitchenPlanSymbolBuilder` already drew per-arm run outlines + unit dividers + sink/hob/fridge/washer glyphs + door-swing arcs + countertop line + dashed upper cabinets (`_tall`); now adds the dashed **work-triangle** (sink↔hob↔fridge) and inks single-ink PRYZM purple `#6600FF` (`PLAN_SYMBOL_INK`) per brand. Scoped deviation from ADR-0110's black drawing ink (noted). C06/C18 |
| L-23-RV | founder | **Kitchen Space-rotate re-verify** — founder reports kitchen doesn't rotate on SPACE; `§FIX-PARAMETRIC-SPACE-ROTATE` shipped wave 4c but founder screenshot shows OLD "Press R" copy (pre-fix bundle). Verify live end-to-end for the L-shape kitchen; fix if genuinely broken. | Editing/Rotation/Kitchen | VERIFY→VERIFIED | ADR-0107 verified sound (ADR-0112) — `KitchenCabinetTool` reuses the shared `PrePlacementRotation`: `attach()`/`reset()` on activate, SPACE +90° cumulative drives the live ghost via `onChange`→`_applyPreviewRotation`, and `_placeKitchen` commits `rotationY()`. The "Press R" copy was pre-fix; the panel hint already reads "Press Space to rotate 90°". Regression test asserts SPACE advances the (re-armed) second-placement commit yaw. |
