...
| L-56 | audit | **Plan door/window hosted on a SLAB id → orphaned opening** — prod logs show `[DoorPlanToolHandler] Door placement — wallId=slab_01KWJ3BQ… type=single` then `[DoorBuilder] Wall not found for door <id> (wallId=slab_…)` and `§P2.3: opening mirrored to legacy store slab_…`. The door never anchors/renders because its host is a slab, not a wall (violates C15 — doors/windows host ONLY on walls). | Creation/Hosting | BROKEN→FIXED | `§FIX-DOOR-SLAB-HOST`. Root cause: `DoorPlanToolHandler`/`WindowPlanToolHandler` resolved the host via `PlanViewCanvas.hitTest(sx,sy,16)`, which returns the id of the nearest PROJECTED element of **any** type (walls, slabs, IFC edges) — then passed that id straight through as `wallId`. A slab edge under the cursor (slabs sit under every wall) could win the nearest-line contest → the opening was dispatched against a `slab_…` id. The commit-side guard only rejected CURVED walls (`if (targetWall && curve)`), so an unknown/slab id (where `getById` returns `undefined`) fell through. Fix: after `hitTest`, accept the hit **only** when it resolves to a wall in the `wallStore` (`rawHitId && wallStore.getById(rawHitId)`); otherwise fall back to the geometric nearest-WALL search (which iterates walls only), and if no wall is in reach, reject the click rather than host on a slab. Added a `if (!targetWall) return` commit guard so an opening is never dispatched against a non-wall id. Both plan overlays (`SvpPlanToolOverlay`/`PlanViewToolOverlay`) delegate to these handlers, so all plan-placement paths are covered. Wall subsystem untouched. Test: `apps/editor/src/engine/views/plantools/__tests__/DoorHostResolution.slab.spec.ts` (5 cases — slab hit + no wall → no dispatch; slab hit + wall under cursor → hosts on the WALL, never a `slab_` id; valid wall hit; window parity). C11/C15, no ADR. |
