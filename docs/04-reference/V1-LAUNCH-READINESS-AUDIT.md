...
| L-32 | founder | **Slab-by-Region PREVIEW mirrored through project origin** — creating a slab via Region: the slab commits in the CORRECT place, but the transient PREVIEW renders far away, mirrored across the project origin (see image). Only slabs. | Modeling/Creation/Slab | BROKEN→FIXED | `§FIX-SLAB-REGION-PREVIEW-MIRROR` — `SlabTool.showRegionPreview()` built a `THREE.Shape` straight from the world-XZ region ring (`Vector2.x=worldX, .y=worldZ`) then laid it flat with `rotation.x = -π/2`, which maps the shape's local +Y → world **−Z**, negating Z and mirroring the ghost through the origin (the commit path `createSlabFromPolygon` uses the ring as-is → lands correctly, hence preview-only). Fix pre-negates Z when building the shape (`Vector2(x, −y)`) so the −π/2 rotation cancels the sign — the exact convention the working polyline fill preview already uses (`shape.lineTo(x, −z)`); commit unaffected (still gets the un-negated `candidatePolygon`). Rect/polyline previews use symmetric/pre-negated geometry so were never mirrored. Test: `SlabRegionPreviewMapping.test.ts` (off-origin region → preview centroid = region centroid, not its Z-negation). C06/C11, no ADR (contained coordinate-space fix) |
