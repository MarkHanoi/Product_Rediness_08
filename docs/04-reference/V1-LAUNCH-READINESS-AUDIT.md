...
| L-36 | founder | **Shower plumbing fixture faces the WRONG direction** — the wall-mounted shower (riser + rain head + mixer) is oriented incorrectly when placed/hosted (head/arm not facing correctly relative to its wall). | Plumbing/Placement | BROKEN | routed → plumbing-shower agent; C11/C15 |
| L-37 | founder | **New composite shower TYPE(s): shower + glass + gutter** — today only a bare shower head/riser exists. Want at least one richer type providing the shower PLUS a base/tray, OR at least a linear GUTTER (channel) drain (ref image 2: rain head + hand-shower + mixer + linear drain), PLUS a side GLASS panel whose direction the user can choose. A parametric walk-in shower enclosure. | Plumbing/Catalogue | BROKEN(feature) | routed → plumbing-shower agent; reuse parametric-fixture pattern (cf kitchen); C11/C18 |
| L-38 | founder | **URGENT — Site-plan overlay (PDF/image) + Project North / True North.** (1) Upload a PDF/image → drops onto the plan canvas where the user can **move / rotate / scale** it interactively, then **OK** — NO boundary tracing; goal = a properly located + **geolocated** + scaled underlay. The interactive ROTATE is first-class: aligning the image to the plan's orthogonal axis DEFINES project north. (2) Dual north: underlay = **PROJECT NORTH**; PRYZM **plan view always edits in project north** (orthogonal to the underlay); the **3D site / globe works in TRUE NORTH** (θ + geolocation). | Site/Geo/Views | BROKEN(feature)→**PARTIAL** | ADR-0114 `§FEAT-PROJECT-TRUE-NORTH` + `§FEAT-SITE-OVERLAY-PLACE`. SHIPPED this slice: pure dual-north rigid transform primitive (`projectTrueNorth.ts` — project↔true about the base point, θ=0 identity, exact round-trip, `overlayInProjectFrame` = plan axis-aligned) + underlay **geolocation record** with the TWO angles kept DISTINCT (underlay on-canvas rotation vs θ) + **"✓ Use this placement (set Project North)"** OK path (no boundary trace) mirroring θ onto `SiteLocation.trueNorth` via the P6 `dispatchSiteTrueNorth` command + back-compat persistence (`projectNorthRad`/`projectNorthSet`). 10 new tests (θ round-trip, persist/restore, plan-vs-globe) + 24 existing green. Composes ADR-0070 (gives θ a UI source) + ADR-059 (overlay defines PN) + C19 (`trueNorth`=θ, no schema change P5). **REMAINING:** plan-canvas `FloorPlanUnderlayTool` confirm→θ bridge; `CesiumViewport` applying θ to the placed massing. C18/C19/C12 + C04 |
| L-40 | founder | **3D GLOBE / 3D SITE view not reachable from the 3D view** — from the normal 3D view there's no way to switch to the geolocated globe/site view; the entry point is missing/unavailable. Must ALWAYS be reachable whenever a 3D view is active (even with no site yet → enter on a sensible default). | Site/Geo/Views | BROKEN→**FIXED** | ADR-0114 `§FEAT-SITE-VIEW-ALWAYS-ON`. Root cause: the ONLY entry was the buried GIS-rail "3D Site" button calling `window.pryzmShowFormaView`, which **dead-ended** ("GIS area not mounted yet") and needed "Activate Geospatial" first — though the Forma entry actually self-bootstraps Cesium (`mountFormaViewToggle → applyFormaView → engageFormaCesium → toggleGIS(true)`) with no site. Fix (additive): stable self-bootstrapping global `window.pryzmEnterSiteView` registered at boot in `mountGISArea`, an **always-present floating "◉ 3D Site / Globe" launcher** on `#container`, and the GIS-rail button hardened to prefer it + activate-then-retry (never dead-ends). C04 view-switch |
