...
| L-49 | founder | **Moving a wall in 3D then Undo doesn't undo the move** — select a wall in 3D, drag the gizmo to MOVE it (commits, wall visibly moves), then Ctrl+Z → the wall stays moved. Log shows undo runs on walls generally (`ring-buffer applied — stores: wall`) but this move is never reverted. Every element mutation MUST be reliably undoable (data-integrity). | Editing/Move/Undo | BROKEN→FIXED | §FIX-WALL-MOVE-UNDO-CAPTURE. ROOT: the 3D-gizmo drag-end dispatches `wall.updateBaseline`, whose bus handler declared `affectedStores: []` and returned `{forward:[],inverse:[]}` → CommandBus classified it as an EMPTY-PATCH record and SKIPPED the ring buffer; the move landed ONLY in the legacy commandManager (via the bridge). Because the unified `performUndo()` is ring-buffer-FIRST, any covered `wall` ring entry already present (the wall's own 3D create, a generated batch, …) was undone instead, and the commandManager-only move — stranded on the independent cm cursor — was never reached. FIX: `UpdateWallBaselineHandler` now declares `affectedStores:['wall']` and, when the gizmo opts in via `_recordUndo:true` (registerTransformDragHandler.ts), emits a baseLine forward/inverse PatchPair so the move is captured as ONE step on the unified ring-buffer timeline (inverse restores the exact pre-move baseline, forward re-applies; cm twin shadow-dropped after the ring undo — dual-dispatch, like a CREATE). Plan-view/property-panel callers do NOT opt in → their undo routing is untouched (L-43 sibling's domain). Test: plugins/wall/__tests__/updateBaseline-undo-capture.test.ts. C03 §4.1/§4.6. |
