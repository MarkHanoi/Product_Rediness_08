...
| L-50 | wall-audit | **TWO divergent wall-type catalogues** — `plugins/wall/src/system-type-store.ts` (handler catalogue, `wt-monolithic` body 0.1 m) vs `packages/geometry-wall/src/WallSystemTypeStore.ts` (`window.wallSystemTypeStore`, the picker's store, body 1.0 m + different built-ins + all user types). The authoritative `CreateWallHandler` (registered FIRST by `composeRuntime`→`PluginRegistry`, `PluginRegistry.ts:170`, with a fresh plugin store) validated + derived thickness from a store the user never saw; the engineLauncher §WALL-TYPE-WIRE adapter that would have fixed it was DEAD (duplicate registration skipped). Root of L-41; user/picker types invisible to the handler. C06/C11, P1. | Geometry/Walls/Types | FIXED | **§FIX-WALL-TYPE-UNIFY-CATALOGUE (ADR-0116).** Collapsed to ONE catalogue seeded at the composition root: the wall descriptor's `buildAuxiliaries` now returns `buildSharedWallCatalogue()` — a lazy adapter over the geometry-wall singleton (`window.wallSystemTypeStore`, the picker's store; typed global, P4-ok) exposing `has`/`get`/`list`/`add`. `has()` stays permissive (protects `wall.batch.create`); `get()` resolves thickness/layers from the one true store. Deleted the dead §WALL-TYPE-WIRE re-registration + its import in `engineLauncher.ts` (now plain `registerWallHandlers(_bus)`). §WALL-TYPE-THICKNESS retained (now reads the unified store). Tests: `apps/editor/__tests__/wallTypeCatalogueUnified.test.ts` (built-in + user-defined type both create at correct thickness; unknown id not rejected) + updated `bootstrap.everything.test.ts` aux assertion. |
| L-51 | wall-audit | **PLAN-view wall move/rotate commit not on the unified undo timeline** — plan move/rotate (`§FIX-PLAN-WALL-TRANSFORM`) committed via a direct `commandManager.execute(UpdateWallBaselineCommand)` — landing ONLY on the commandManager stack — while the 3D gizmo (`registerTransformDragHandler`) + plan Move/Align tools dispatch `wall.updateBaseline` on the BUS (ring buffer). The two stacks have independent cursors (ADR-051), so interleaved plan+3D moves undid OUT OF ORDER. C06/C16. | Editing/Move/Plan | FIXED | **§FIX-PLAN-WALL-MOVE-UNDO-UNIFY.** `PlanElementDragController.onEnd` wall branch now dispatches `window.runtime.bus.executeCommand('wall.updateBaseline', {wallId,newBaseLine,prevBaseLine})` — the SAME bus path as the 3D gizmo (which bridges to commandManager via initBusHandlers for the WallRebuildCoordinator; performUndoRedo reconciles the dual entry ring-first + shadow-drop). Plan + 3D moves now share the unified ring-buffer timeline. Door/window offset commits unchanged (out of wall scope). Test: `core-app-model/src/views/__tests__/planWallTransform.test.ts` rewritten — MOVE + ROTATE now assert the bus dispatch (not commandManager), carry the moved line + exact `prevBaseLine` for the inverse. |
| L-52 | wall-audit | **Plan-projection cache thrash (the L-06 storm mechanism, HIGH at scale)** — `WallFragmentBuilder.buildWall` bumped `_geometrySeq++`/`group.userData.version` UNCONDITIONALLY on every call; the `EdgeProjectorService` NME plan cache keys on that version → ~0 % hit-rate under whole-level rebuilds (a single-wall edit re-versions every wall). C04. | Perf/Walls/Plan | FIXED | **§FIX-WALL-VERSION-CONTENT-HASH.** `buildWall` now stamps a content-derived generation token via `_versionForBuild(wall, joinData, worldY)` — the token only changes when the wall's projected-geometry inputs change (own `_renderVersion` + join miter/baseline hash + `worldY`); a no-op rebuild reuses the prior token so untouched walls keep their EdgeProjector/NME cache entry (HIT). `_geometrySeq` stays the monotonic source of fresh tokens (preserves the §96-STALE-GUARD async-swap semantics); cleared per-wall in `removeWall`. Test: `packages/geometry-wall/__tests__/WallFragmentBuilderVersionHash.test.ts` (7 cases: no-op stable, own/join/worldY change bust, cross-wall isolation, legacy fallback, global uniqueness). |
| L-53 | wall-audit | **Concurrent wall baseline move is silently LWW-dropped** — the wall CRDT path (`YjsDocAdapter.applyCommand` / wall sync) does not surface a concurrent baseline move as a user-resolvable conflict (C08 §3.1/§3.3, P8). | Sync/Walls | OPEN | queued (this agent) — add baseline-geometry conflict detection + banner/dialog; test = two concurrent baseline moves → conflict, not silent drop |
| L-54 / E1 | wall-audit | **Dual junction-resolver dead compute + 540-line `_flush` guard-pile** — HIGH-RISK refactor at the top of the regression curve; simplify incrementally behind `postResolvePreserveGuard`/`wallJoinBaselineImmutable` as the net (ADR-0055A P4a/P4b). | Geometry/Walls | OPEN | scheduled follow-up — do LAST and only with budget; else leave as-is |
| L-55 | wall-audit | **Missing OTel P8 spans on wall command handlers / wall-committer** — verify the bus wrapper coverage or add spans; document the sound choice. | Observability/Walls | CLOSED (codified) | **§P8-SPAN-COVERAGE.** Verified the surface is already fully span-covered by WRAPPERS: (1) all 22 wall handlers wrap `execute` in `withHandlerSpan`; (2) the L2 bus emits `pryzm.command.execute` per execution (`CommandBus.ts:261`); (3) the committer's `onAdd/onUpdate/onRemove` are `PrimitiveCommitter` reactors that always run inside the host's `pryzm.scene.commit.batch` `withSpan` (`CommitterHost.ts:118`), whose added/updated/removed attrs carry per-batch committer telemetry. Nested per-method spans would only add noise under the parent. Codified the exception in `plugins/wall/src/committer/wall-committer.ts` header (§P8-SPAN-COVERAGE). No code change to the trace surface. |
