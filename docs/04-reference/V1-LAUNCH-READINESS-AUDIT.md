...
| L-32 | founder | **Slab-by-Region PREVIEW mirrored through project origin** — creating a slab via Region: the slab commits in the CORRECT place, but the transient PREVIEW renders far away, mirrored across the project origin (see image). Only slabs. | Modeling/Creation/Slab | BROKEN | routed → slab-preview agent; C06/C11 |
