...
| L-41 | founder | **Plan view ignores chosen wall type** — no matter which wall type is selected in plan view, the drawn wall commits as the DEFAULT (Plain Wall). 3D honours the pick; plan does not. | Modeling/Types | BROKEN→FIXED | `§FIX-PLAN-WALL-TYPE-IGNORED` (L-41). Root cause was NOT a picker-write/handler-read mismatch — that path is already sound (the pre-draw picker in `PropertyPanelPreDraw.showWallPreDraw` writes the chosen `systemTypeId` to the canonical `window.wallTool` via §WALL-TYPE-PLAN-FIX, and `WallPlanToolHandler._commitWall` reads the SAME `window.wallTool.getSystemTypeId()`; L-28's default pre-apply is idempotent and never clobbers a later pick). The real defect was downstream **thickness resolution**: the plan handler always dispatched `wall.create` with `thickness: WALL_DEFAULT_THICKNESS` (0.2 m) and relied on `CreateWallHandler`'s §WALL-TYPE-THICKNESS override to re-derive thickness from `systemTypeId`. That override runs only when a *populated* `WallSystemTypeStore` is wired into the ACTIVE handler — but `composeRuntime()` (PluginRegistry `ALL_PLUGINS`) registers the authoritative `wall.create` FIRST, seeded with a **separate plugin-side `WallSystemTypeStore`** whose built-in catalogue differs from the geometry-wall store the picker reads (`window.wallSystemTypeStore`); engineLauncher's §WALL-TYPE-WIRE adapter (which *did* point at the correct store) is then **skipped** by the "first-registration-wins" idempotent facade. Net: the picked `systemTypeId` reached the store but its thickness never resolved → the wall was STORED at 0.2 m. 3D looked correct because the 3D builder re-resolves layers/thickness from `systemTypeId` at RENDER; PLAN view reads the stored thickness → every typed wall rendered as the default "Plain Wall". **Fix:** `WallPlanToolHandler._commitWall` now resolves the thickness from the SAME catalogue the picker dropdown is populated from (`window.wallSystemTypeStore` via the existing `_getSelectedWallThickness()` helper) and sends it in the payload, so the STORED wall is correct for every view independent of which handler/catalogue the bus ended up with. `systemTypeId` is still carried; when the handler's catalogue does resolve, it re-derives the identical value (idempotent). L-28 active-by-default + default-seed untouched (no type → helper returns 0.2 m). No geometry/join/renderer change; no ADR (no contract changed). Test: `apps/editor/__tests__/PlanWallTypeThickness.test.ts` (selected type → stored thickness = type's, not default; no type → default; unknown id → safe default, id still carried). C06/C11/§WALL-TYPE-WIRE. |
