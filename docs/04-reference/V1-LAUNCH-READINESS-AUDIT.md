...
| L-43 | founder | **Wall MOVE + ROTATE don't work in PLAN view** — select a wall on the 2D plan → the transform gizmo attaches (console `[WallTransform] … gizmo aligned with direction`) but dragging to MOVE or ROTATE has NO effect; the same works in 3D. | Editing/Plan/Transform | BROKEN→FIXED | `§FIX-PLAN-WALL-TRANSFORM`. **Root cause:** the 2D plan is a **Canvas2D** surface (`PlanViewCanvas`). Selecting a wall routes through `selectionBus` → `SelectionManager` → `bim-selection-changed` → the **3D** `WallTransformController`, which attaches the THREE `TransformControls` gizmo (the `[WallTransform]` log) — but that gizmo lives in the hidden 3D scene and is **not interactive** over the Canvas2D plan, so "grabbing the gizmo" does nothing. Plan transforms are instead owned by `PlanElementDragController`, which implemented **whole-wall translate only** (it did commit `UpdateWallBaselineCommand`, so body-drag MOVE worked, but was easily missed while reaching for the dead gizmo). **ROTATE had no plan path at all** — the plan surface has no equivalent of the 3D `WallEndpointController` (endpoint-drag = rotate/stretch). **Fix:** `PlanElementDragController` now classifies a wall grab by screen-proximity to a baseline endpoint (`_pickWallEndpoint`, `ENDPOINT_GRAB_PX`): grabbing near an end **rotates/stretches** the wall about the opposite end (visual-only ghost during the drag — avoids `BaselineReversalError` on a >90° swing — committed once on release), grabbing the body **translates** (unchanged, keeps L-29 set-out dims). BOTH gestures commit the **same** `UpdateWallBaselineCommand` the 3D endpoint controller uses (P6 — one mutation path, undoable). Overlay shows live length + Δangle for the rotate. Test: `packages/core-app-model/src/views/__tests__/planWallTransform.test.ts` (body-drag MOVE dispatches the command with the translated baseline; endpoint-drag ROTATE dispatches it with one end swung — neither a no-op). **Note (spot-check):** doors/windows already move in plan via `PlanElementDragController` (offset along host); furniture/columns/etc. in plan still require the **Move tool** (`MovePlanToolHandler`) — direct body-drag in plan remains wall/door/window only by design. C06/C16/C03; no ADR (reuses the existing transform command family, no contract change). |
