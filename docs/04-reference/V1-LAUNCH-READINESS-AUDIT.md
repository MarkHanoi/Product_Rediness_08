...
| L-38 | founder | **URGENT — Site-plan overlay (PDF/image) + Project North / True North.** (1) Upload a PDF/image → it drops onto the floor plan where the user can **move / rotate / scale** it interactively, then **OK** — NO boundary tracing needed; the goal is just a properly located + **geolocated** + scaled underlay. (2) Dual north: the image/PDF defines **PROJECT NORTH** (its orthogonal axis) — PRYZM main **plan view always works on project north** (orthogonal to the underlay); the **3D site / 3D globe works on TRUE NORTH** (the underlay geolocated correctly on the map). | Site/Geo/Views | BROKEN(feature) | routed → site-overlay/project-north agent; C18/C19/C20 geospatial + C04 view-transform; relates ADR-0070 (project-north weld), geospatial-foundation; needs new ADR |
| L-39 | founder | **CRITICAL — Opening a saved project FREEZES the whole app.** Load succeeds (`Loaded: Auto-save (32 elements)`), the model paints once, then camera + buttons go dead. Console: `500× Destroyed texture [Texture "ShadowDepthTexture"] used in a submit` + `[SceneQualityTier] … tier=cinematic (shadows=high)` → `[ShadowQualityUpgrader] Level changed to "high"` (shadow map 512→high-res). WebGPU device-loss cascade. | Rendering/Shadows/Load | FIXED | `§FIX-SHADOW-LOAD-TIER-DESTROY` (ADR-0111, load-time sibling of L-25). Root cause: on project open the tier escalates to `cinematic` and `ShadowQualityUpgrader.apply()/setLevel('high')` reallocates the Pascal key light's shadow map 512→2048; with the live WebGPU renderer's `shadowMap.autoUpdate=true`, THREE performs that realloc INSIDE a `render()`/submit while the rAF loop still references the old `ShadowDepthTexture` → destroy-in-submit ×hundreds → device loss → freeze. The shipped `§SHADOW-DEVICE-LOSS-FIX` `setTimeout(0)` covered the upgrader's explicit `.dispose()` but NOT THREE's in-render realloc when `autoUpdate` is live — that load path was the gap. Fix: new ref-counted `RenderPipelineManager.setShadowReallocFrozen()` (shares the applier with the nav `setShadowPassSuppressed`) FREEZES the live shadow map (`autoUpdate=false`) around the realloc; `RenderingPipelineCoordinator` runs its shadow-level mutation through an injected guard (`setShadowReallocGuardHook`) wired by initScene to freeze → mutate → `setTimeout(0)` thaw (past the in-flight submit) + one `markDirty()`; initScene also freezes the shadow map for the whole load window (`pryzm-project-switch`→`pryzm-project-loaded`) so quality escalates once, device-idle post-load. `castShadow` never touched; no texture destroyed by the freeze; WebGPU-path only, WebGL2 unaffected. Test = `RenderPipelineManager.shadowReallocFreeze.test.ts`. |
| L-42 | founder | **Camera navigation DEAD after a project loads** — model renders but orbit/pan/zoom won't move the camera. Console: `§LOAD-WATCHDOG load still running after ~16s — phase="setup" stuck` (load took 19.6s) + a runaway `requestAnimationFrame` recursion in the ThatOpen/OBC camera-controls loop. | Rendering/Camera/Load | FIXED (via L-39) | Downstream symptom of the L-39 WebGPU device loss: once the device is lost no new frames present, so the camera-controls damping loop (the `vendor-thatopen … requestAnimationFrame` in the stack) keeps updating but nothing repaints — the model looks frozen. Keeping the device alive (`§FIX-SHADOW-LOAD-TIER-DESTROY`) restores frame presentation → camera orbits/pans/zooms again. Verified no independent control-lock during load: `§LOAD-RAF-PAUSE` pauses only the wall-rebuild scheduler and is released in `finally`; the gizmo-reattach guard already self-detaches at its source (warns once, not a per-frame peg); the redetect sweep is already chunked one-level-per-frame. The `~16s stuck "setup"` watchdog line reflects element hydration slowed by the failing render loop (the watchdog still fires across await points), not a separate synchronous main-thread hang. Camera re-arm to be reconfirmed live on prod after deploy. |
