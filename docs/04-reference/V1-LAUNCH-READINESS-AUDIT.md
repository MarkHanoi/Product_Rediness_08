...
| L-51 | wall-audit | **PLAN-view wall move/rotate commit not on the unified undo timeline** — plan move/rotate (`§FIX-PLAN-WALL-TRANSFORM`) commits via `UpdateWallBaselineCommand` but must route through the same bus+`_recordUndo` path as the 3D gizmo (`§FIX-WALL-MOVE-UNDO-CAPTURE`) so plan + 3D moves interleave-undo in order. C06/C16. | Editing/Move/Plan | OPEN | queued (this agent) — additive over `PlanElementDragController`; test = interleaved plan+3D moves undo in correct order |
