...
| SEC-XSS | Focused XSS pass over interpolating sinks (781 total; marketplace UGC highest risk) | P2 | 5 | PARTIAL (batch-6, batch-7) | `806bba43`, `577947a1` | batch-6: marketplace-web UGC download-`href` scheme sink hardened via `safeHref()`. batch-7: user-controlled **project-name** stored-XSS closed — `PlatformProjectBrowser`/`PlatformSaveController` interpolated `this.ctx.projectName` raw into `<input value="…">` (attribute breakout → markup injection); now `escHtml`-wrapped + webhook-id text node escaped for consistency; new `platformProjectNameXssEscaping.test.ts` 6/6 (behavioural + scoped source-scan regression lock). Full 781-sink sweep + repo-wide CI sink-scan STILL OPEN (see audit L-407). |
