...
|---|---|---|---|
| A-1 | **F-1** — identity/audit fields re-minted on every restore and redo (fresh `createdAt`, `version`, and **`ifcData.guid`**, the IFC join key). Produces **20 of the 24 failures** | certification §F-1 | a **contract** decision first (C13 §2 demands byte-compatible round-trips), then either patch-based redo or a documented-tolerance list. The tolerance list ships **empty on purpose** |
| A-2 | **F-2** — stairs and beams **lose their identity** across save/reload | `ImportProjectCommand.ts:680` (no `id:`), `:875` (no `beamId:`) — verified | pass the id. The identical bug was fixed in the editor loader in May 2026 under **§PERSIST-L1**; the import path never received it |
| A-3 | leg C — no CRDT transport deployed | C66: 0 tiers HELD | **founder decision** (auth on WS upgrade + ~$5–10/mo), then staging-only flag |
| A-4 | F-3/F-4 — plumbing+furniture drift **+15 mm/reload cumulative**; openings gain frame fields on reload | certification | loader merge paths |
| A-5 | undo gesture race, pinned RED-BY-DESIGN at the 125 ms cliff | P1-7, `it.fails` tests | founder decision on §UNDO-GESTURE-ID |
| A-6 | read-only capability class authored, **wiring in progress** | working tree | finish injection; do not land unwired |
...
1. **Openings are not re-clamped when their host wall shrinks.** A 0.9 m door survives on a 1.5 m
   wall; a 2.1 m door survives in a 1.0 m wall. The gate exists (`clampToWall`) with **two
   production call sites, both opening-side, zero wall-side**. (EV-03, verified)
...
**Tier 1 — model truth** (unblocks everything; all inside existing commands)
1. **A-2** pass the id in `ImportProjectCommand` (2 lines) — highest value per character in the repo
2. **`clampToWall` inside the two wall commands** (D-1)
3. **prevState on `updateDoor`/`updateWindow`** — *two arguments* that make the shipped
   openings-only fast path reachable and fix the ADR-057 drag defect
4. **type `window.semanticGraphManager`** — turns the `measuredAt` bug into a compile error
5. **A-1 contract decision** on round-trip identity — a decision, not code
...
|---|---|
| persistent, authoritative, identity-preserving model | **identity FAILS** — A-1, A-2 |
