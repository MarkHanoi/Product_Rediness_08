# Spain — Buildings / LOD / Height (context layer)

> Part of the 3D-Context-Data country study (see `../../README.md` and umbrella item **L-511**).
> Fill against the research doc `Pryzm_3D_Context_Data_Sourcing.md` — but **verify every endpoint live** before relying on it.

- **Target LOD:** LOD1 (footprint) + nDSM height
- **Source(s):** Catastro INSPIRE + PNOA-LiDAR
- **Integration effort:** MED (Barcelona pattern proven)

## Spike evidence (fill during Phase 1)
| Question | Answer | Evidence (endpoint response / sample) |
|---|---|---|
| (a) Real footprint? | TBD | |
| (b) Real height? | TBD | |
| (c) Real roof shape? | TBD | |
| CRS of the response | TBD | |
| License text (verbatim, from THIS response) | TBD | |
| Coverage gaps → fallback | TBD | |

## 5-building spot check (fill during Phase 1)
| BAG/cadastral id | dataset height | visual (satellite/StreetView) | verdict |
|---|---|---|---|

## Implementation notes (fill during Phase 2)
- Adapter: TBD · Reprojection to WGS84: TBD · Internal-mesh mapping: TBD
