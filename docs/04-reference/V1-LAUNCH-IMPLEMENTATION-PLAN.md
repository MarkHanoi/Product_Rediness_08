...
| **A1f** | **ADR-0271 P4b — WIRE the block-ring producer at L5.** The pure functions exist; nothing calls them. Needs a real source of a *manzana*'s sibling parcels, plus roads from the EXISTING single context fetch (C12 §8 forbids a new Overpass query). | **L-460** | **OPEN — the last engineering step before Barcelona is live.** ⚠ Carries a genuine open design question: Catastro INSPIRE WFS is refcat-keyed with NO bbox, so sibling parcels are not directly queryable; and closing rings from OSM road CENTRELINES yields a block systematically too large by ~half a carriageway per side, which biases the derived depth. Decide the source before building. |
| **A1g** | **ADR-0271 P5 — the Barcelona pack itself** | **L-461** | **BLOCKED — NOT ENGINEERING.** Needs a human in a browser. Partial progress 2026-07-20: the 2002 Ordinance text is now PRIMARY-sourced (BCNROC, the Ajuntament's own repository) and `13E`-vs-`13a` is resolved as a legal reading — **the pack must key on `13E` with `13a` residual, which the live MUC service alone would never have revealed.** Still open: whether that ordinance is IN FORCE (the two 2015 *Derogació* rows were never reached), the *alçada reguladora* table (now WEAKER — the generic PGM's 3,05 m was repeatedly confirmed and the 3,35 m Barcelona variant was not), and the volumetric rules (*cossos sortints* / *àtic* / *patis*) which nobody has looked at and which may make `implantación × storeys` materially incomplete. See `SOURCES.md` §C.7. |
