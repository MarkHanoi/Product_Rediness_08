...
|---|---|---|---|
| **G0 the unit tests actually gate CI** | **0.5/0.6** | shipped | **🟢 CLOSED (0.5b→0.5d).** The editor's 1,495 tests now RUN in CI **and on pushes to `main`** (which is how this project actually ships). Red surface **34 → 0**, every one triaged PRODUCT-FIRST — and three of the five groups turned out to be the **product's** fault, including **L-249, a live production bug** (live climate normals could never load). Expensive jobs stay PR-only, so the Actions bill is respected. |
