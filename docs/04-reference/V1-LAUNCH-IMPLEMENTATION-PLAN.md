...
| L-177 [HIGH ⬆⬆ 2026-07-08 founder "multiple times"] façade sun-hours STILL on transparent envelope prism, not the real house | Geospatial / façade analysis on real model (C18-23/A.24/ADR-0110) | **DEFERRED bd158aa3 (needs Cesium.Model shader drape); ESCALATED HIGH → dispatch AFTER L-183/184 frees CesiumViewport, BUNDLE with L-185** — (1) drape/vertex-colour BVH sun-hours onto the REAL placed GLB faces, (2) HIDE the analytic prism; evaluator byte-identical; live-view verification needed. §FIX-FACADE-ANALYSIS-ON-REAL-MODEL |
...
| L-185 [MED] add SEA/WATER context to Forma (3D Site) view — site on the water (Sydney/Rose Bay) but no sea shown | Geospatial / Forma water context (C18-23) | **QUEUED → Cesium/Forma-context agent (dispatch AFTER L-183/184 frees CesiumViewport)** — Forma neutral ground + flat base hides real water; OSM open ocean is coastline ways not closed polygons. Fetch natural=water/waterway polygons + build a coastline-derived sea mask → render water surface; reuse /api/overpass + §FORMA-CTX-PARKS pattern. §FEAT-FORMA-SEA-CONTEXT |

| L-186 [HIGH] 3D globe: PRYZM house LOST after globe→forma→globe (re-entry doesn't re-place real model) | Geospatial / Cesium globe re-entry re-placement | **FOLDED into running L-183/184 Cesium agent** — re-place/restore the real model on EVERY globe activation (idempotent across globe⇄forma switches); suspect the forma6 reuse-cache. §FIX-GLOBE-REENTRY-MODEL-LOST |

| L-187 [MED ✅ APPROVED 2026-07-08] extend Forma 3D-site context radius w/ distance-LOD | Geospatial / Forma context radius+LOD | **APPROVED → build in Forma-context bundle (after L-183/184/186 frees CesiumViewport)** — extend radius + near(extruded+shadows)/far(flat,no-shadow) LOD + building-count cap + progressive async fetch; bounded shadow/geometry budget. §FEAT-FORMA-CONTEXT-EXTENT-LOD |
...
| L-190 [HIGH] elevation: windows absent from geometry + set-back wall solid-not-hidden (§ELEV-LINEWEIGHT-02) | Documentation / elevation geometry inclusion + occlusion | **→ elevation agent (evidence-first; EdgeProjectorService/NativeElementMeshExporter)** — report failing stage per bug (query/geom/classify/style); include windows in elevation element query + occlusion-classify set-back walls hidden→dashed. §ELEV-LINEWEIGHT-02 |
