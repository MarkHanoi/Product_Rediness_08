...
| L-196 [HIGH] elevation set-back wall must be PARTIALLY hidden — per-segment occlusion (one edge solid where visible + dashed where occluded) | Documentation / elevation per-segment occlusion (C06 §7 / Contract 23 §9; cont. L-190) | **→ elevation agent (fenced HiddenLineRemoval.ts + tests; disjoint)** — L-190's reclassify only re-buckets a FULLY-AABB-occluded segment; split projected edges at occlusion-state transitions using the true occluder silhouette, classify each sub-segment (visible→solid, occluded→dashed). Fixture: this South Elevation wall run solid+dashed at the massing step-back. §ELEV-LINEWEIGHT-03 |
