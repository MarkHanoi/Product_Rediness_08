...
Phase mapping (see docs/04-reference/audit/3D-SITE-ANALYSIS-AUDIT.md §G):
...
Full audit: docs/04-reference/audit/COMMERCIAL-TOWER-AUDIT.md. Office mirrors resi (do NOT fork the pipeline).
...
**Severity:** P2 launch / **HIGH strategic**. **Queue:** geospatial / site-feasibility. **Contracts:** new C57 (Parcel Data Layer) + C58 (Zoning Rules Engine, fills C19 §9/§10.2) + SPEC-PARCEL-SELECTION + strategy ADR + VISION amend; reuses C19/C12/C18; mirrors L-374 provider pattern + L-373 credibility. Full scoping: docs/04-reference/geospatial/PARCEL-ZONING-FEATURE-SCOPING.md. **Status: OPEN — plan authored; execution gated behind the L-376d code lane + 2 strategic sign-offs.** **Zero dependency on the September launch work.**
...
**Severity:** P2 (architecture / consistency tech-debt; NOT launch-blocking — generation works post-L-376d). **Queue:** pipeline / tech-debt. **Contracts:** C11 §2/§5/§11, C16 §3.2/§8, ADR-0055. Full audit + matrix: docs/04-reference/audit/ELEMENT-PIPELINE-SOUNDNESS-AUDIT.md. **Status: OPEN — plan authored, not started.**
...
**Severity:** P2 / HIGH strategic-technical. **Queue:** geospatial + devops. **Full phased plan (DK-0..DK-5, effort/deps/verify per phase) + per-tool integration verdicts + the GeoJSON-canonical data flow are in docs/04-reference/jurisdictions/dk/DENMARK-GEOSPATIAL-REFERENCE-ARCHITECTURE.md §7/§8** (extends, does NOT duplicate, the L-380 P0–P5 + L-374 Phase-1/4 phases). **Status: OPEN — validated, plan authored, not started.**
...
| **L-518** | New C58 confidence tier `block-constructed` → real **"Real · constructed"** badge + honest source line, not ESTIMATED (assigned only when `alignment.depthBinding` present) | zoning / envelope provenance (C58) | ✅ **v247** — founder-confirmed the decision; governed by `jurisdictions/es/es-ct/08019-barcelona/RISK-REGISTER.md` R1 |
...
| **L-527** | CONTEXT (neighbour) building heights inaccurate — ~36% are OSM-tagless → flat 9 m default | geospatial / context height | ⏳ SOLUTION ANALYSED in `spain/SPAIN-HEIGHT-MEASUREMENT.md` (LiDAR nDSM 90th-pctile, three-field badge). **FIX = the shared nDSM module (L-511c/L-512b — same one that fixes envelope height L-525a) run over CONTEXT footprints, baked INTO the L-513a PMTiles.** One pipeline, three footprint sets. Interim: badge `assumed` heights ESTIMATED. |
| **L-526** | Verify the profunditat LEGAL basis (citation chain + 2008 Art. 327 §2 + "max depth" metric) — the RULE side of L-525 depth | zoning / legal provenance | ⏳ probe-first (L-526a–c); external research prompt in `jurisdictions/es/es-ct/08019-barcelona/L-526-LEGAL-RESEARCH-PROMPT.md` |
...
Primary-source research done (`jurisdictions/es/es-ct/08019-barcelona/L-526-LEGAL-FINDINGS.md`). The depth error is
GEOMETRY, not the rule; the citation + height are separately wrong. Actionable fix tasks:

| Task | Phase | Status |
|---|---|---|
| ~~**L-525b (THE depth fix, do first)** — `fetchBlockForParcel`/`dissolveParcelsToBlockRing` must return the **FULL illa** (~12,000 m² for a Cerdà block, not our 6,686 m² half).~~ ⛔ **REFUTED + CLOSED AS MIS-SCOPED (2026-07-21).** Live-Catastro probes: masa 02309's bbox is **113.4 × 113.8 m** with **ZERO cross-masa adjacency links** in a 444 m search — there is no sibling masa to union with — and its dissolved ring is SOLID, 6,696 m², perimeter 336 m ⇒ a genuine **~82 × 82 m block rotated ~45°** (the Eixample grid bearing), not half of anything. The "6,686 ≈ half of 12,769" arithmetic compared a rotated small block against a nominal axis-aligned one. **The real cause was L-529** (see below), not the block assembly. | zoning / block assembly (C57) | ✅ CLOSED — hypothesis refuted by probe; no work needed |
| **L-529 (THE ACTUAL depth fix)** — `insetPolygonPerEdge`'s greedy `removeSelfIntersections` **collapsed the block ring 40 verts → 2** at any inset ≥ 8 m, so `solveBlockDerivedDepth` saw ZERO interior free area, Art. 242.2 became unsatisfiable at any depth, and the solver fell to the ordinance FLOOR with `degenerate=true`. Fix = **§INSET-LOOP-DECOMPOSE**: split the self-intersecting offset ring into simple loops, discard loops wound opposite the CCW input on ORIENTATION (never a size threshold), match crossings by edge-pair identity so no tolerance governs topology. **Pau Claris 155: `12.0 m · min-floor · degenerate` → `15.7 m · interior-ratio · freeRatio=0.300`.** The exact bisection in `blockDerivedDepth.ts` was deliberately NOT replaced — Art. 242's *"figura similar a la illa"* means sharp mitered corners, so the miter is the legally faithful construction and a distance-field solve served only as the verification oracle (they bracket: 15.7 miter vs 17.4 field). ⚠ This primitive backs EVERY setback inset in every jurisdiction — non-Spain pilots not yet re-checked. | zoning / envelope geometry | ✅ SHIPPED v256 — regression test uses the REAL 58-vertex block ring (synthetic shapes did not reproduce it); confirmed to fail pre-fix, pass post-fix |
| **L-530 — 3D-Site boundary vs context misalignment.** Founder-reported on v256. L-529 ruled out on logic (it only touches the inset behind the purple envelope; the boundary never passes through it) AND by measurement (old-vs-new byte-identical on a cross-shaped drawn plot at every setback 1–6 m). Three candidate mechanisms needing OPPOSITE fixes: **(1) origin/translation** (anchor-vs-parcel-centroid, the L-521/L-524a family — the founder's log shows two context fetches ~800 m apart), **(2) θ/rotation** (`readProjectNorthRad` latching 0; ~45° error in Barcelona), **(3) neither** (Catastro parcel vs OSM footprints genuinely disagreeing). | geospatial / site frame | 🔍 **PROBE SHIPPED v257** (`§SITE-FRAME-PROBE`, diagnostic only) — awaiting one console line to select the mechanism. DO NOT fix blind. |
| **pack min-floor** — set `minDepth_m` = **12 m** (not 11 m) in `esBarcelonaEnsanche.ts`; add the 8 m inscribed-circle interior check (Art. 242). | zoning / pack | ✅ min-floor SHIPPED v254; 8 m inscribed-circle check still TODO |
| **L-525a height** — encode the **Art. 327.2 alçada table** (`<8→8.55/PB+1 … 20–30→20.75/PB+5 … ≥30→23.80/PB+6`) keyed by **OFFICIAL street width** (ample oficial, NOT GIS-measured); drive the massing height from it. ⚠ reconcile 20.75 vs **22.40 m** PB+5 (Barcelona certificate via Arts. 238/240/327) before shipping the number. | zoning / height + street-width source | 🟡 **TABLE SHIPPED v257, DELIBERATELY UNWIRED.** `resolveAlcadaReguladora` + `BCN_ALCADA_REGULADORA_TABLE` (19 tests). **The street-width source was PROBED and does not exist machine-readably**: Barcelona's open-data CKAN returns four `vial` datasets, the only relevant one (`mapa-base-de-vialitat`) being a **WMS RASTER** of façade/pavement contours with no width attribute; `amplada`/`secció`/`alineació` return zero. And a MEASURED width cannot substitute where it matters — the bands are STEPS and the Cerdà grid sits ON one: a standard street is 20.00 m, exactly the PB+4/PB+5 edge, so 19.99 m ⇒ 17.70 m and 20.00 m ⇒ 20.75 m. **One centimetre of cadastral noise moves the building a full storey.** So the resolver REFUSES (`reason:'band-edge'`) a measured width within `BAND_EDGE_GUARD_M` (0.5 m) of a boundary and accepts it only when flagged `trustedOfficialWidth`. Founder chose (2026-07-21) to supply the declared figures via a **curated Cerdà street-width allow-list** (`bcnOfficialStreetWidths.ts`, shipped v257, every entry tagged `curated-cerda-nominal`, unlisted street ⇒ null ⇒ no fabricated height). **REMAINING TO WIRE:** resolve address → official width → alçada → `envelope.maxHeightM`, and **delete the hardcoded `: 9` fallback in `CesiumViewport.ts` (~L4073)** which currently extrudes a fabricated ~PB+2 in the same purple study volume as a real height (the L-459 defect class). Held back pending L-530 — do not stack a new height source on an unexplained frame problem. ✅ **WIRED + SUPERSEDED AS THE WIDTH SOURCE BY L-537 (v265):** the `: 9` fallback became a 0.5 m footprint slab (§ENVELOPE-NO-FABRICATED-HEIGHT), and Phase 2 replaced the allow-list with a MEASURED, quantisation-aware construction — see the L-537 row below. |
| **L-526 citation** — depth = **Art. 242** (DROP Art. 322.1 as the depth authority — it's *edificabilitat*); height = **Arts. 238 + 240 + 327**; re-cite the source as the **current Barcelona consolidated NUMAMB / RPUC** (drop the anachronistic "AMB Dec 2010 / 31-12-2009" — AMB formed 2011); repair/remove the stale "2008 §2 not reflected" caveat (that mod is a HEIGHT change). Update `esBarcelonaEnsanche.ts` `ordinanceRef` + the panel; note in RISK-REGISTER R1; reopens L-449. | zoning / provenance (C58) | ✅ SHIPPED v255 — founder re-signed the L-449 gate 2026-07-21; `BCN_ORDINANCE_REF` + header updated |
| **L-537 (L-525a Phase 2) — the *amplada de vial* CONSTRUCTION.** Replace the ~26-street curated allow-list, which left `maxHeight: null` (⇒ 0.5 m footprint slab) on every unlisted Barcelona street. Art. 327.2 keys on the DECLARED width and no such dataset is published, so the width is CONSTRUCTED from cadastral geometry and cited as constructed — the ADR-0271 precedent. **GATED ON A PROBE FIRST** (`spain/SPAIN-STREET-WIDTH-DISTRIBUTION-PROBE.md`, 6,819 frontages / 697 blocks / 5 cities, production code reused): widths DO cluster (47.1 % within ±0.5 m vs a 13.0 % null) **but the quantum set is city-specific and NOT the intuitive one** — Barcelona has NO 10/15/25 m quantum (×0.63/×0.24/×0.00), only 20 m (×7.55) and 30 m (×7.51), and the nominal-50 m arteries measure 48 m (2 m off, 3× the error bar) so they are deliberately NOT snapped. Shipped: pure `geometry/streetWidth.ts` (ray-cast frontage measurement + per-façade edge selection, its own max−min error bar), §STREET-WIDTH-NEIGHBOURS on the block route (**zero extra network** — the opposing parcels were already in the bbox and were being discarded), and `rulepacks/ampladaDeVial.ts` (tiers declared > snapped > measured > none; only the first two disarm the band-edge guard; tolerance 0.60 m = the measurement's own p90 error). **Barcelona coverage 44.1 % → 86.4 %.** Cross-validates against the curated list (Aribau 60, Pau Claris 174 both measure→snap to 20 m ⇒ PB+5, the curated answer). ⚠ Inherits the L-535 dissolve blocker (64 % of manzanas nationally) and does NOT fix it; ⚠ does not certify any width as the legal *ample oficial* (L-528). | zoning / street-width construction | ✅ **SHIPPED v265** — 208 package tests (+30), 169 server tests (+2), root `tsc` clean, live acceptance on the two founder-reported streets |
| **L-539 — the block dissolve tolerates real cadastral geometry.** The single binding constraint on envelope coverage: `dissolveParcelsToBlockRing` produced a ring for only **63.5 %** of 956 real manzanas, and no ring ⇒ no depth ⇒ no width ⇒ no height ⇒ a 0.5 m footprint slab. **GATED ON A TAXONOMY FIRST** (`spain/SPAIN-DISSOLVE-FAILURE-TAXONOMY.md`, 33,865 parcels / 250,646 edges / 5 cities, production parsers+projection+dissolve reused): **there are no slivers and nothing to weld** — 0 edges below 0.083 m, 0 vertex gaps below 0.05 m, and a weld at ε ≤ 0.05 m moves 0.0000 m. The real cause is that **Catastro publishes coordinates rounded to 1e-6° (0.083 m E / 0.111 m N)**, so neighbours store one boundary with different vertex COUNTS; the leftover fragments form a hair-thin triangle and the chain finds two loops (66.8 % of failures multi-loop, 94.4 % of those nested at ~0 % area; 86.2 % carry a T-junction). Shipped §DISSOLVE-TJUNCTION-SPLIT: split an edge at an EXISTING neighbour vertex within **0.10 m** — a tolerance bounded by the publisher own quantum (0.111 m), the empirical valley (mass ends ~0.075 m, real geometry resumes > 0.3 m) and the success plateau (0.10 → 91.6 %, 0.20 → 93.4 %, 0.30 → 93.1 % falling). **No vertex is moved or constructed**, and the EXACT pass wins whenever it succeeds — repairing unconditionally would have altered 137 of 607 existing rings. **63.5 % → 91.4 %; Barcelona 81.5 % → 96.3 %, Madrid 75.8/77.2 % → 92.6/99.0 %, Córdoba 44.2 % → 90.8 %; 0 rings changed, 0 lost.** Independent check: repaired rings match the published cadastral areas (p50 0.13 %) slightly better than exact ones (0.15 %). ⚠ Removes the GEOMETRY blocker only — each city still needs its own tables + founder-signed source gate (L-449). | zoning / block geometry | ✅ **SHIPPED v266** — 219 package tests (+11, incl. two real Córdoba manzana fixtures), root `tsc` clean for these files, `quality{path,splitCount,maxOffset_m}` on every result |
| **L-528 — CERTIFY the numbers against the official planning GIS** (needs a real browser; not headless / not web search). Extract for parcel **0230904DF3803** (Pau Claris 155), OFFICIAL sources only (`dtes.gencat.cat/muc-visor`, `geoportalplanejament.amb.cat`, Barcelona Seu electrònica), **no estimates**: (1) profunditat edificable (m) + layer + screenshot; (2) ample oficial del vial (secció de vial layer, NOT map-measured) + screenshot; (3) alçada reguladora màxima (m / PB+N) + the 20.75-vs-22.40 resolution + screenshot; (4) whether 13a-only or an MPGM/PE/Modificació also applies + cited doc + screenshot. Deliver a final `Parameter · Value · Viewer · Layer · Screenshot` table. Ready-to-run prompt: `jurisdictions/es/es-ct/08019-barcelona/PAU-CLARIS-155-CERTIFICATION-PROMPT.md`. These certified numbers let L-525a/L-525b drop the constructed estimates → flip the badge toward green. | legal / verification | OPEN — founder-runnable in-browser; blocks the green badge, not the demo |

---

## L-540 — THE CI GATE (2026-07-21). Everything else in this plan was unverifiable until this landed.

Source: `STATUS-REPORT-2026-07-21.md` §5 — *"a red CI does not stop a production deploy"*, and
*"almost nothing in this report is 'tested' in the sense of being enforced."* Fixed FIRST, in
dependency order, because every other row's "FIXED-VERIFIED" is a claim about a command someone
ran by hand until a gate enforces it.

| Task | Phase | Status |
|---|---|---|
| **L-540 — deploy must depend on CI.** New `ci-gate` job in `deploy-fly.yml`; `deploy` now `needs: ci-gate`. Reads the Actions API for the CI run on THIS SHA and refuses unless the required jobs are green. Gated set = `lint · isolation · command-manager · test-server · test-unit · test-root · apex-gates` — deliberately NOT `build`, which `flyctl deploy --local-only` re-runs identically (waiting for a duplicate build would triple the push→prod loop for zero safety). Doc-only pushes (CI `paths-ignore`) pass only after the gate RE-DERIVES doc-only-ness from the diff; a missing CI run on a CODE commit FAILS; a poll timeout FAILS. Emergency `bypass_ci_gate` dispatch input, default false, recorded in the run inputs. | ci | ✅ **SHIPPED.** Gate script exercised locally against a stubbed `gh` on all four paths (green / red / bypass / no-run×{doc-only, code}); YAML parses; `bash -n` clean. |
| **L-541 — the 5 red editor tests.** All five fixed; 4 were real, 1 (`parcelBoundaryEnvelopeOrdering`) was ALSO real, not environmental — it read an async-emitted event synchronously. Root cause of 3 of them is a **layering violation**: `packages/geometry-slab/src/SlabTool.ts:3` statically imports the browser-only `@thatopen/ui` from an L2 geometry package. | ci / testing | ✅ **SHIPPED.** `219/219` files, `1881 passed / 0 failed` (was 5 failed). ⚠ The layering violation itself is NOT fixed — the stub hides it from tests. |
| **L-543 — the `--if-present` hole.** 166 workspaces, 18 enforced (10.8%), 130 silently skipped. New `check:testci-coverage` ratchet, hard-fail in CI: the unguarded set may only SHRINK, and a stale baseline also fails. Mass-adding `test:ci` to 130 workspaces was REJECTED (L-247 precedent: they will be red, and an all-red board gets switched off). | ci | 🟡 **PARTIAL — measured, visible, cannot widen. 130 still unenforced.** Broaden batch-by-batch, PRODUCT-FIRST. |
| **L-544 — the unwired suites.** Root `vitest` measured green (23 files / 263 tests) → landed HARD-FAIL as the new `test-root` job. `test:pryzm1` measured RED (2 of 13) → landed advisory per the L-247 ladder, with both root causes named. Playwright E2E: **still wired to nothing.** | ci / testing | 🔴 **OPEN — 2 real failures + E2E unwired.** Flip `test-pryzm1` hard-fail when they close. |
| **L-542 — the advisory-gate backlog.** `ga-gate` (the P1–P8 principle gate CLAUDE.md calls merge-blocking) is `continue-on-error`; `a11y` is PR-only AND advisory; `docker-image` (Fly-parity boot smoke) is PR-only in a repo that does not use PRs, so it never runs. Deliberately NOT upgraded here. | ci | 🔴 **OPEN — documented, not fixed.** |

**Did making the gate real reveal more red? YES, and that is the point:** (a) two suites that had
never once executed contained 10 stale assertions against a function that became `async`; (b) the
`test:pryzm1` suite is red with a test pointing at a module deleted in the PRYZM-3 migration. Both
...
> `docs/04-reference/jurisdictions/es/SPAIN-GEODATA-SOURCE-COVERAGE.md`
>
...
`docs/04-reference/jurisdictions/es/SPAIN-GEODATA-SOURCE-COVERAGE.md`. *Audit row: L-584.*
...
## L-607 — 3D-Site context tiles for every jurisdiction city (not Barcelona only) · Phase 5 · P1 · OWNER: UNASSIGNED · TARGET: TBD

⚠ **This is a DATA/INFRA gap, not a client bug.** The tile pipeline (`bake.mjs` → PMTiles → R2 →
`contextTiles.ts`) is built and correct; it was only ever pointed at the Barcelona bbox. Do NOT
"fix" it by re-enabling live Overpass off-Barcelona — that reintroduces the exact unreliability
L-513 removed. Reuse the existing pipeline; widen its coverage. See audit **L-607**.

| # | Task | Notes |
|---|---|---|
| 1 | **Probe first — confirm the reader's off-region result** | Before any fix: load a Madrid site, watch `contextTiles.ts` `readContextTileFeatures`. Does a wholly-out-of-Barcelona bbox return `ok`-empty (no fallback fires → "No surrounding building data") or `unavailable` (fallback to Overpass fires)? This decides whether the honesty bug (audit L-607 layer 2) is real. §L-579 discriminator; **do not assume — probe** (probe-can-be-wrong-three-ways) |
| 2 | **Decide the coverage shape** | (a) **PREFERRED — big-extract single tileset:** bake per-country Geofabrik extracts (`spain-latest`, `portugal-latest`, `norway-latest`, …) into ONE planet-scalable tileset served from the existing flat `s3://pryzm-assets/tiles/…` URL. PMTiles scales to planet → **zero client change**, and it is the "even higher standard" the founder asked for. (b) **Alternative — per-city prefixes** `tiles/<city>/…` + a client city→prefix resolver in `contextTiles.ts`. (a) is less code and less drift; pick it unless tile size forces (b) |
| 3 | **Generalise `bake.mjs` beyond the Barcelona bbox** | README §Scope already says "add a `REGION` + bbox". Turn the single `REGION` into a list driven by the jurisdiction set (BCN, Madrid, Córdoba, + every `docs/04-reference/jurisdictions/*` demo city). Keep the deterministic osmium→tippecanoe pipeline unchanged; only the extract bbox/source list grows |
| 4 | **Generalise `context-bake.yml`** | The `layer` input stays; add a `region`/`city` input (or bake-all). Same Docker toolchain, same R2 sync + the 206-range verification step. Long immutable TTL means a re-bake must change the PATH — honour that if per-city prefixes are chosen |
...
## L-640 — unified parcel-metadata model (per-domain confidence + honest unknown-reason) · Phase (post-launch) · P2 · OWNER: UNASSIGNED · TARGET: TBD (gated on founder sign-off)
See audit **L-640**. Litehaus-inspired but PRYZM-shaped (NO listing/valuation). NOT greenfield — aggregates the confidence/provenance/explainability models C58/C55/ADR-0277/C23 already own onto C57 `ParcelFeature`, plus a typed `UnknownReason` enum. **Phase 1 (free, no new upstream, honesty-safe):** fix the C57 §2.1 KV-3 area-collapse (`parseParcelGml` return official `areaValue` + SIG ring separately), un-discard the click→parcel distance + candidate margin (`parseReverseGeocode`), add geometry diagnostics (perimeter/centroid/bbox/compactness/frontage/validity/self-intersections), thread a `ParcelFeature.confidence { areaSource, areaOfficialM2, areaSigM2, areaDeltaPct, pointToParcelM, candidateMarginM, geometryComplete, match:'high'|'medium'|'low' }` through the Spanish + European cadastral adapters; footprint-fallback providers auto-`match:'low'`. Later phases populate per-domain metadata as sources land, each gated by unknown-reason. Multi-layer (L0 schema C57 + server proxy + client adapter + docs). Do NOT build until sign-off; amend CITY-REPLICATION §1 + C57 §2 in place after approval.

## L-641 — parcel-select boundary offset (wrong-parcel resolution and/or θ frame-split) · P1 · OWNER: UNASSIGNED · TARGET: TBD
See audit **L-641**. DIAGNOSE FIRST (do not blind-fix). Two candidate roots: (A) `parseReverseGeocode` picks the NEAREST Catastro parcel (`_Distancia`) → a click near a boundary resolves to the adjacent parcel (offset, intermittent, everywhere); (B) L-629 Barcelona θ_write≠θ_read frame-split. Diagnostic instrument already shipped by L-640 (`pointToParcelM` + a point-in-ring test of the click vs the returned ring). Fix per root: (A) prefer the parcel CONTAINING the click, reject/re-query when the click falls outside the returned ring (C57 provider, `parcelZoningProxy.js`); (B) the L-629 siteDispatch θ fix. Overlaps L-629 — do NOT duplicate; cross-linked. Queue: geospatial/parcel-data (C57) + C12 §9 SiteFrame.

## L-642 — production-ready layered radius-LOD 3D-Site context (globe-like, Forma-styled) · P2 (founder-prioritised) · OWNER: UNASSIGNED · TARGET: TBD
See audit **L-642** + the new **`docs/03-execution/specs/SPEC-3D-SITE-PRODUCTION-CONTEXT.md`** (authored this pass). EXTEND the existing `§FEAT-FORMA-CONTEXT-EXTENT-LOD` near/far LOD (do NOT fork): (client) radius (not square) culling + 4× radius + a new ultra-low-poly/wireframe outer tier + elevate sea to an always-on standing layer (like terrain); (bake) add rail (`w/railway`) + trees (`natural=tree`) layers + widen the context extent. BINDING CONSTRAINT: the ADR-0094 large-scene perf budget + WebGPU device-loss risk — the wireframe/low-poly outer tier + instancing are the mechanism, never 4× solid buildings. Multi-layer (bake data + client render). Needs the SPEC + a ratifying contract (the §GAP L-642 in MISSING-CONTRACTS) BEFORE implementation. Depends on: L-637 (sea, done) + L-639 (terrain, done). Queue: geospatial / 3D-Site context render + context bake.

## L-643 — Barcelona buildable-envelope overstates (footprint = whole parcel, covers non-buildable pati d'illa) · P1 · OWNER: UNASSIGNED · TARGET: TBD
See audit **L-643**. Known overstatement family on the REFERENCE city (siblings L-525/L-529/L-581/L-616). **PROBE FIRST** (per [[inset-collapse-was-the-depth-rootcause]] — two rival theories were both documented-as-confirmed and both wrong before): is it inset-collapse (L-529/L-581 signature: depth inset drains to full parcel) OR a missing pati-d'illa (block-interior-free) exclusion in the BCN Art-242.2 construction? Fix in the Barcelona solver so `footprint = depth-band ∩ parcel ∩ interior-free`; identical principle to L-616 (envelope = intersection of all derived constraints). ⚠ Changes every shipped BCN parcel's footprint → before/after measurement (L-581 caution), never a blind edit. Queue: geospatial / buildable-envelope — BCN rule-pack + solver (`ZoningRulesEngine.ts`, `insetPolygonPerEdge`), C58 §1.4/§10, ADR-0271.

## L-644 — deploy to Fly without paid GitHub Actions (local flyctl remote-only) · P2 · OWNER: UNASSIGNED · TARGET: TBD
See audit **L-644**. RESOLVED-IN-PRACTICE: `flyctl deploy --remote-only -a pryzm` (Fly Depot remote builder, zero Actions minutes, zero GCM sign-in pop-ups). Make it the documented default in `DEPLOYMENT-RUNBOOK.md`; keep Actions as the CI-gated fallback. Tracking: `flyctl releases/status/logs` + `https://fly.io/dashboard`. Queue: infra/deploy.

## L-646 — building HEIGHTS honesty + per-city standard (P1) · OWNER: UNASSIGNED · TARGET: TBD
See audit **L-646** + the new **`docs/04-reference/standards/BUILDING-HEIGHT-REPLICATION-STANDARD.md`** (authored this pass — the heights analogue of the envelope standard). Root (code-confirmed): `resolveContextHeight` bottoms out in a fabricated `DEFAULT_BUILDING_HEIGHT_M = 9` (`contextBuildings.ts:234`); OSM tags are sparse so most buildings fall to it (the "9 m carpet"); no measured (LiDAR DSM−DTM) fallback; massing prism has the sibling L-525 ordinance-height gap. PLAN-FIRST (done): honest source ladder (LiDAR → OSM height → levels → ordinance → typed-unknown) × C62 `DomainConfidence`; per-city sourcing (GEO-DATA-SOURCING-MASTER); CI fidelity gate (no fabricated height rendered as authoritative). Implementation sequenced AFTER sign-off — never a blind default-tune. Queue: geospatial / context-buildings + massing height (C62, C12, C58 §1.2, §CONTEXT-DATA-HONESTY). Family: L-459, L-525.

## L-647 — height-fidelity RENDER: tagged=solid LOD200, estimated=amber wireframe (P1) · OWNER: UNASSIGNED · TARGET: SHIPPED (render), gate live
See audit **L-647** + BUILDING-HEIGHT-REPLICATION-STANDARD §3. **DONE this pass (main loop):** context buildings whose `heightProvenance === 'tagged'` render SOLID (LOD200 true boundary + true height); derived-levels/assumed/untagged render as a see-through amber WIREFRAME (`CesiumViewport.ts` §CTX-HEIGHT-FIDELITY-RENDER). Locked by the new CI gate `tools/ga-gate/check-height-fidelity.ts` (#23, PASS). C62 data half wired via `contextHeightConfidence()` (contextBuildings.ts) mapping provenance→DomainConfidence. Deploying via flyctl. Queue: geospatial / 3D-Site render (C62, C12). Pairs with L-646 (data) + L-459 (provenance).

## L-648 — city-completion scorecard + dossier standard (P2) · OWNER: UNASSIGNED · TARGET: TBD (gated on founder weighting sign-off)
See audit **L-648** + **C63** (`docs/02-decisions/contracts/C63-CITY-COMPLETION-AND-DOSSIER.md`) + **ADR-0281** + **SPEC-CITY-COMPLETION-SCORECARD**. **DONE this pass (governance):** the 7-axis scorecard contract (PARCEL/LEGISLATION/DATA-SOURCES/ENVELOPE/TERRAIN/HEIGHTS-LOD/CONTEXT, each a total function of state per C63 §1.1, composing C62); the per-country `_TEMPLATE/COUNTRY-COMPLETION.md` (renamed `COUNTRY-RATE.md` under L-649) + per-city `_TEMPLATE/_CITY/` dossier template (its `COMPLETION.md` face renamed `RATE.md` under L-649); `docs/04-reference/_ORGANIZATION.md`; the per-city completion matrix + master implementation plan in `master-execution-tracker.md §CITY-COMPLETION` (every cell `not-assessed` — the honest current state). **SEQUENCED (implementation, after founder sign-off of `CITY_COMPLETION_WEIGHTS`, C63 §4):** (1) the L0 schema `packages/schemas/src/site/completion/CityCompletionScorecard.ts` (P5-pure); (2) the impure scorecard function (`tools/`) — cheap first computes DATA-SOURCES/TERRAIN/CONTEXT (state already inspectable), then PARCEL/HEIGHTS-LOD (one sampling run), then LEGISLATION/ENVELOPE (per-clau audit + L-449 gate); (3) populate each dossier's `COMPLETION.md` + country roll-ups; (4) the `tools/ga-gate/check-city-completion.ts` CI gate (fails hand-typed cells + fabricated renders + dossier-shape gaps, C63 §6). Fast-but-wrong REJECTED: hand-populate the matrix. Queue: geospatial / city-replication governance. Governs C63, C62, C57, C58, C60.

## L-649 — master-RATE structure (per-city + per-country + tracker template) + audit→map→plan sequence (P2) · OWNER: UNASSIGNED · TARGET: STRUCTURE DONE (naming + weighting ratified 2026-07-30); Phases 1–3 (audit→map→plan) remain
See audit **L-649** + **C63** §5/§8.1. EXTENDS L-648. **NAMING DECIDED (founder, 2026-07-30): Option B** — composite master = `RATE.md` (city) / `COUNTRY-RATE.md` (country); legislation sub-rate renamed `RATE.md`→`LEGISLATION-RATE.md` (semantics unchanged; rule: `RATE.md` = composite master, `<AXIS>-RATE.md` = per-axis detail). Weighting **RATIFIED** (C63 §4: LEG 25/ENV 20/PAR 15/SRC 15/HGT 10/TER 10/CTX 5). **STRUCTURE — DONE (docs-only pass):** authored `_TEMPLATE/NAMING-CONVENTION.md` (naming master) + `_TEMPLATE/MASTER-RATE-TRACKER.md` (copyable global-matrix face); added INDEX **Dossier index** cross-ref sections to the city composite master (`_TEMPLATE/_CITY/RATE.md`) + country roll-up (`_TEMPLATE/COUNTRY-RATE.md`) — every dossier file with its one-line purpose + axis fed; renamed the `_TEMPLATE/` files + the 4 shipped Catalan dossiers; updated C63 §5/§8.1 + `_CITY/README.md` + jurisdictions README + master-execution-tracker + ADR-0281 + SPEC + contracts README. (2) **Phase 1 — AUDIT** every tackled country+city: inspect real state and record it CELL-BY-CELL WITH CITED DERIVATION — cheap axes first (DATA-SOURCES from `heightSources.mjs`/`registry.ts`/`bake.mjs`; TERRAIN from `layer.json`+verify; CONTEXT from the 9-layer PMTiles probe), then PARCEL/HEIGHTS (one sampling run each), then LEGISLATION/ENVELOPE (per-clau audit + L-449 sign-off). This is honest ahead of the L-648 automated function BECAUSE each cell cites the state read (not a guess). (3) **Phase 2 — MAP** the audit into each dossier's composite master `RATE.md` + the country `COUNTRY-RATE.md` roll-ups + the global `master-execution-tracker.md §CITY-COMPLETION` matrix. (4) **Phase 3 — PLAN** a per-axis, per-city implementation plan to drive each RATE section toward 100% (this feeds back into THIS plan as new L-items per city×axis). Fast-but-wrong REJECTED: hand-type %s, or pick a naming side silently. Queue: geospatial / city-replication governance. Governs C63, C58, C62, C57, C60.

## L-664 — ONE confidence ontology: the ENVELOPE axis was UNSCOREABLE (P0) · OWNER: UNASSIGNED · TARGET: TBD
See audit **L-664** + **C63** §3.2/§3.3/§4.1/§8.3 + **C58** §1.2 + **C62**. **THE DEFECT:** C63 §3 scored Axis 4 with tier names (`certified`, `constructed-amber`) that **no code has ever implemented** — `EnvelopeConfidenceSchema` declares six entirely different tiers. So (a) contract compliance was unprovable and (b) the ENVELOPE axis could not be scored **for any city**, which is why Barcelona's Axis 4 reads `not-assessed` and its headline renormalises over 3 of 7. Ratified 2026-08-01: **fix the ruler BEFORE expanding coverage.** **VERDICT (argued in C63 §3.2, not asserted):** the SCHEMA is authoritative and the CONTRACT was stale — the code encodes distinctions the contract lost (`structured` = PUBLISHED vs `authoritative` = ISSUED; `block-constructed` = a CONSTRUCTED determination whose badge is contractually forbidden from reading "certified"; `pipeline-extracted-unverified` = a legal control with no contract name at all). Collapsing them would let a published-but-undetermined value read as a compliance fact. **DONE this pass:** (1) C63 amended **in place** (no derivative `*-AUDIT.md`) — §3.2 the ordered ladder + weights + the historic names as READ-ONLY PROSE (no runtime alias — a translation table was the forbidden "paper over the mismatch"), §3.3 the reachability finding, §4.1 the two weight vectors, §8.3 the known-violation record; (2) the ordered ladder **hoisted from L2 to L0** — `ENVELOPE_CONFIDENCE_ORDER` / `envelopeConfidenceRank` / `isStrongerEnvelopeConfidence` in `ProvenanceFlags.ts`, with `@pryzm/ordinance-extraction` now delegating (order byte-identical); (3) `EnvelopeAxisWeight.ts` — a **total** `EnvelopeConfidence → axis weight` map, **no default branch** (unmapped ⇒ `tsc` error / hard `throw`), monotone in the ladder, tested for totality; (4) a frozen 16-pack manifest proving **no pack's published confidence changed**, itself total over `src/rulepacks/`; (5) `computeScorecard.mjs` v1.1 scores ENVELOPE from a supplied coverage breakdown and stays honestly `not-assessed` without one. **⚠ THE CEILING ANSWER:** `authoritative` is **not reachable by any production path** — so a CONSTRUCTED determination (Barcelona Art. 242.2 and every city copying it) is capped at `block-constructed` = **0.70** on ENVELOPE, and "certify to 1.0" is not an engineering lever: it needs an ISSUED municipal determination as a data source. **SEQUENCED / ROUTED OUT:** (a) **founder** ratifies the 3 PROVISIONAL tier weights (`structured` 0.90 / `estimated-ruleset` 0.40 / `pipeline-extracted-unverified` 0.10) — C63 §8 open decision 2; (b) the per-clau × buildable-land-share MEASUREMENT (the only remaining ENVELOPE blocker — the ruler exists, the reading does not; never synthesise it, C63 §1.1); (c) **C58 defect, P1, needs its own row:** `ZoningRulesEngine` never reads a pack's `defaultConfidence`, so Madrid PGOUM-97 + Córdoba would badge machine-extracted numbers violet "Estimated" instead of the ⚠ red unverified chip — a silent promotion; (d) C58 §1.2 still calls the enum "complete" at five members; it has six. Fast-but-wrong REJECTED: invent a `certified`→`structured` translation table (would have promoted DK to a perfect axis), or re-label pack confidences to fit the contract. Queue: geospatial / city-replication governance. Governs C63, C58, C62, C57, C60.
...
## L-663 — viewport crash-recovery is an UNBOUNDED pipeline-rebuild loop (freeze on a furniture dimension edit) · P1 · OWNER: UNASSIGNED · TARGET: TBD (instrument first — do NOT blind-fix)
See audit **L-663**. **Phase mapping: Phase 1 — stability / demo-blockers** (a routine property edit must never wedge the editor). Contracts/ADRs touched: **C04 §RECOVERY** (Known Violations, added this pass) · **C04 §SHADOW.2** rules 7/8 · **C13 §2/§3.7** (`pryzm-project-switch` *defines* a project session — the render-side `onProjectSwitch()` is being invoked outside one) · **C10** (an error the guard swallows is an unobservable error) · **P3** single-rAF (recovery must not introduce a second drive loop; today it is macrotask-`setTimeout`-driven, which is correct and must stay so) · **P8** (any new exported fn ⇒ ≥1 OTel span) · **ADR-0089** §Decision (retrying the SAME graph against the SAME fault "cannot help — it only delays the same fatal outcome" ⇒ **downgrade**; the hard dialog is reserved for genuinely unrecoverable states) · **ADR-0257** (openings-only branch — *an edit rebuilds the element, not the world*) · **ADR-0111** (freeze the shadow map, never toggle `castShadow`) · **ADR-0076 / ADR-0094** (WebGPU perf budget).

**DONE this pass — diagnosis + pinning ONLY. Nothing fixed; no contract flipped to ACTIVE.**
- Loop mechanism code-confirmed: `RenderPipelineManager.onProjectSwitch()` sets `_retryCount = 0` (`:1851`) — the counter whose exhaustion (`MAX_RETRIES = 3`) is what *creates* `phase='error'` → `ViewportCrashGuard.handlePipelineError()`; `ViewportCrashGuard.onRetry` additionally clears `_hasCrashed` **and** `_consecutiveFailures` (`:236-239`); **no recovery-attempt counter exists anywhere**, so crash → recover → same fault → crash never escalates to the hard reload, and every turn pays a full guarded pipeline dispose + `createScenePass()` recompose.
- Diagnostic hole code-confirmed: `initScene.ts:2756` calls `handlePipelineError()` **with no argument**, and `PipelineStatus` (`:334-343`) carries **no `lastError`** ⇒ the crash card *and* the Sentry report both receive the synthetic `"Render pipeline retries exhausted — phase=error"`, and the real throw is unrecoverable. `§I3-USEDTIMES-SUPPRESS` separately discards the stack (`message.slice(0,80)`, `ViewportCrashGuard.ts:103`).
- Added `packages/renderer-three/__tests__/RenderPipelineManager.recoveryLoopUnbounded.test.ts` (4/4 green). ⚠ It pins the **defective** behaviour on purpose so the fix must consciously rewrite it rather than drift past it.
- Recorded C04 **§RECOVERY — Known Violations (open)** and `§GAP (L-663)` in `docs/02-decisions/MISSING-CONTRACTS-AUDIT-2026-06-01.md` — the crash-recovery subsystem's only cited authority, `08-ERROR-RESILIENCE-CRASH-RECOVERY`, **does not exist anywhere in the repo**.

**SEQUENCED (fix — strictly in this order):**
1. **INSTRUMENT BEFORE FIXING.** Per [[context-data-honesty-family]] / [[probe-can-be-wrong-three-ways]]: the founder's log shows the *recovery*, never the *cause*, so the trigger is **NOT established**. Add `lastError?: Error` to `PipelineStatus`, retain it wherever `_phase = 'error'` is set (`RenderPipelineManager.ts:757`, `:1092`), pass it through at `initScene.ts:2756`, and make `§I3-USEDTIMES-SUPPRESS` log the full stack at `console.debug` instead of an 80-char slice. Ship this **alone** and re-run the repro. It is both the fast fix and a prerequisite of the correct one.
2. **CORRECT FIX — recovery gets its own bounded primitive.** Mint `RenderPipelineManager.recoverFromRenderFault()` (≥1 OTel span, P8) and point `ViewportCrashGuard.onRetry` at it. It MUST NOT reset `_retryCount`, MUST count its own attempts, and MUST escalate on the **ADR-0089** ladder: attempt 1 → guarded rebuild; attempt 2 → `_downgradeToLightweightPipeline()`; attempt 3 → hard reload. `onProjectSwitch()` reverts to a pure **C13** lifecycle method. **Fast-but-wrong REJECTED:** wrapping a bare counter around the existing `onProjectSwitch()` call — that leaves lifecycle and error-recovery coupled and leaves the primitive unscoped to the fault.
3. **De-duplicate the double rebuild on the furniture path.** `UpdateFurnitureParametersCommand.execute` emits `bim-furniture-updated` (`:222`) on top of the emit `store.update()` already performs (`FurnitureStore.ts:34`), so `FurnitureFragmentBuilder.updateFurniture()` runs **twice per Apply** — doubling the mid-submit dispose exposure on a *selected, outline-composited* root (the family `SelectionManager.ts:651-668` already documents). **Spans layers** (L0/L3 store + L1 renderer + L5 editor wiring): one owner, one PR.
4. **Apply ADR-0257 to the furniture path.** A dimension change currently fans out to a whole-scene `pascalSceneLighting.onGeometryAdded(scene)` traversal plus a shadow freeze/thaw (`initScene.ts:3350-3365`). Classify the delta (dimension-only ⇒ rebuild that element's children only) exactly as the openings-only branch does for walls.
5. **Contract close-out (separate pass, only after 1–4 land with a passing test):** rewrite the characterisation test to assert the *bounded* behaviour, then either author the missing crash-recovery contract or promote C04 §RECOVERY to NORMATIVE.

**Queue: rendering / viewport crash-recovery** — routed by architecture, NOT by where the symptom appeared. The furniture panel is the trigger surface; the breach is in the render-pipeline recovery model. Dependencies: none blocking; touches the same `RenderPipelineManager` shadow/rebuild machinery as L-231 / L-316 / ADR-0111 — coordinate with any in-flight shadow work. **Do not conflate:** the same log's blank-thumbnail read is the pre-existing non-fatal `§THUMB-BLANK-REUSE` swapchain race, and `siteCapture.status='degraded'` is the independent L-489/L-545 georeference family.

## L-668 — the residential SETUP panel fires before the user has seen the parcel + envelope (P2 UX/flow gap) · PHASE 1 (interaction correctness) · OWNER: UNASSIGNED · TARGET: TBD

See audit **L-668**. **This is the SECOND report of the same defect — L-435 (2026-07-20) logged it and stayed OPEN.** Founder, re-escalating on live prod: *"I don't need the massive window for building residential at this stage. It should be like the apartment / house workflow — the user should see first the PARCEL, then the ENVELOPE, then click GENERATE BUILDING, then this screen comes."*

**Classification: a GAP, not a bug.** The flow the founder is asking for is not a new idea — it is the flow apartment and house already travel. `renderGenerateConfirmStep` (`apps/editor/src/ui/onboarding/OnboardingStepController.ts:1114`) renders a COMPACT, NON-BLOCKING confirm card over the live parcel (`--confirm` restores the vertical layout; pointer-events fall through to the map). For `residential-multifamily` it branched at `:1129` straight into `renderResidentialProgramStep`, so that card was REPLACED by the full-screen landscape setup panel. The parameter surface was simply built on the wrong side of the opt-in and never moved.

**IMPLEMENTED — PENDING VERIFICATION (§RESI-SETUP-AFTER-GENERATE, `8094c3fb`).** The residential branch no longer short-circuits the confirm step; it sets a flag, the generic card renders, and the "Generate residential building" button opens `renderResidentialProgramStep`. The panel's Back now returns to that card rather than `backFromConfirm(source)`, which discarded the drawn C19 boundary — re-drawing is one click further via the card's own §L-384 Back. **No third flow was invented:** the card is the existing generic one, and the setup panel is byte-unchanged apart from when it opens.

**NOT closed by this pass:**
- **L-435 stays OPEN** for its remaining clause (b): the no-fit state should not merely explain but *offer the nearest feasible band*, reusing the L-401 `capStoreysToEnvelope` `binding`/`explanation` machinery. L-669 makes the message TRUE; it does not yet auto-suggest.
- **The OFFICE step has the same shape.** `renderOfficeProgramStep` was written as "the sibling of `renderResidentialProgramStep`" (ADR-0092 §OFFICE-PREVIEW-STEP) and branches from the same `:1138` site. Deliberately NOT changed here — one typology per pass, and office has its own preview semantics — but it will re-report unless it follows.
- **The ordering is UN-CONTRACTED** (L-670), which is why this needed a founder to catch it twice.

Queue: onboarding / ux. Governs ADR-0069 (wizard pattern explicitly rejected), ADR-0092, C06 §7, C50 §5.

## L-669 — the residential generator refuses feasible plots, quoting a threshold it violates (P1 correctness) · PHASE 1 (interaction correctness) · OWNER: UNASSIGNED · TARGET: TBD

See audit **L-669**. Founder: *"check why a plot of 674 m² is too small? What is the ceiling? Reduce the ceiling — you could even build a resi building in a plot of 150 m²."* The modal said *"This plot (~674 m²) is too small… need roughly ≥400 m² of plate"* — a message that states its own threshold and then violates it.

**THE MEASUREMENT CAME FIRST** (probe against the real orchestrator, before any change). There is **no plot-area gate anywhere in the engine**. The binding quantity is the plate's WIDTH:
...
A 720 m² plate refused while a 272 m² plate built. The `≥400 m²` was `RESIDENTIAL_MIN_PLATE_M2`, declared in `apps/editor/src/ui/residential-building/residentialError.ts:38` and read by no engine — a **plot** area quoted against a fabricated **plate** threshold. Two distinct defects, as the founder said: the gate was wrong AND the copy was wrong, about different quantities.

**Mechanism.** `platePartition` carves a vertical circulation SPINE at the core's X-centre out of *every* apartment row (the spine is what links the horizontal corridor bands back to the core), splitting each row into two X-runs. `packRow` skips a run narrower than the demand's area-floor width — `MIN_ENGINE_FEASIBLE_AREA_M2 / MAX_APARTMENT_DEPTH_M` = 72/9 = **8 m**. On a narrow plate both runs fall under that floor → zero cells → refuse.

**Why the earlier fix could not close the class** (asked explicitly; answered from the code, not assumed). §RESI-SMALL-PLATE-CORE-SCALE (founder's FIRST report, ~352 m², 2026-06-24) had one lever — shrink the core — and no fallback. `effectiveCoreSize` computed `wByRuns = plateW − 2·MIN_SIDE_RUN_M`, then returned `max(MIN_CORE_DIM_M, min(requested, wByRuns))`. Once `wByRuns < MIN_CORE_DIM_M` — i.e. once `plateW < 2·8.5 + 2.6 = 19.6 m` — **the floor wins and the two-usable-runs goal the function's own docstring promises is silently abandoned**: it returns a core it has already proven cannot leave usable runs. A real stair + lift has a size floor, so **no additional tuning of that constant could ever have reached this class.** The missing piece was an alternative ARRANGEMENT, not a smaller number. That is why a second constant tweak was refused here.

**IMPLEMENTED — PENDING VERIFICATION (`8094c3fb`).**
1. **§RESI-NARROW-PLATE-SIDE-CORE — the missing arrangement, added as a FALLBACK, never as a new threshold.** The centred (R-CENTRE) plan is attempted FIRST on every plate, so nothing that builds today changes; only when it places zero apartments does the engine retry with the standard narrow-plot residential typology — a SIDE core flush to the x0 plate edge serving a SINGLE-LOADED apartment run. One run instead of two drops the **derived** width floor from `2·MIN_SIDE_RUN_M + MIN_CORE_DIM_M` = **19.6 m** to `MIN_CORE_DIM_M + MIN_SIDE_RUN_M` = **11.1 m**, exported as `MIN_PLATE_WIDTH_M`. R-CENTRE is preserved wherever it still works and relaxed only where the alternative is refusing to build.
2. **RIBBON GUARD — the fallback may not buy a build with junk.** Measured: on a 40 m × 8 m plate the single-loaded plan emits 34.0 m × 3.6 m cells (**9.4 : 1**), which is not an apartment. The fallback result is discarded if any cell exceeds the partition's own `MAX_RECT_ASPECT` (3.5 : 1), and the original refusal is surfaced. A plate that refuses today and would only gain ribbons **keeps refusing** — the fallback strictly adds real buildings.
3. **§RESI-REFUSAL-TRUE — the refusal is now true.** `RESIDENTIAL_MIN_PLATE_M2` deleted. The orchestrator emits the MEASURED plate width and its DERIVED threshold in the reason; the UI **parses both back out and states no number the engine did not measure**, so it is structurally incapable of repeating the defect. `residentialError.ts` stays PURE (a value import of the `@pryzm/ai-host` barrel would drag the AI host into the editor's first-paint chunk and break its plain-Node tests) — the constant is echoed, not imported.

**MEASURED AFTER:** founder's 674 m² narrow plate → 4 apartments/floor at 81–93 m²; **12 × 12.5 m = 150 m² builds** a 123 m² unit (the founder's stated target); 11 m wide still refuses at any area; 40 × 8 m still refuses; every pre-fix-building plate keeps its centred plan and its exact apartment count.

**Tests (fail without the fix):** new `packages/ai-host/__tests__/residentialNarrowPlate.test.ts` (11) + rewritten `apps/editor/__tests__/residentialError.test.ts` assertions (17 pass). ⚠ `residentialLargePlate.test.ts`'s "genuinely TOO-SMALL plate" case was **falsified, not loosened**: its 12 × 10 m premise ("cannot host a single ≥72 m² engine-feasible apartment") is untrue — the plate hosts a contained 98.5 m² apartment the engine lays out. Replaced with 9 × 10 m, infeasible by derivation.

**NOT fixed here, needs its own item:** `platePartition`'s §RESI-T3-FIT keep-band scales by `MAX_APARTMENT_DEPTH_M / depth` with **no upper bound**, so at shallow row depths it authorises cell widths far past `engineMaxCellWidth` (the 34 m ribbon came from a `depthScale` of 2.5). The ribbon guard contains the symptom; the unbounded scale is the defect. **ADR-0092 §OFFICE-PLATE-AUTOFIT must also be reconciled** — it asserts as fact that *"the residential building never hard-rejects (it degrades)"* and changed the OFFICE typology on the strength of it; that premise was false when written.

Queue: generative / residential-engine. Governs C50 §1.7 (soft-fail), C53, ADR-0092, ADR-0075 (preview↔execution parity).

## L-670 — no contract owns the typology CREATION-FLOW ORDERING (P2 coverage gap) · PHASE 1 (governance) · OWNER: UNASSIGNED · TARGET: TBD

See audit **L-670**. Derived while fixing L-668. The founder's request is a *sequence* requirement and there is nothing normative to check it against: **C60** owns globe navigation up to parcel arrival (its own §8 says the flow "is not yet reachable in the product"); **C19 §1.3/§1.4** owns the boundary commit; **C58 §1.8** owns that the envelope *constrains* generation, not the sequence; **C50 §5** owns the L5 dispatch caller's duties inside the router; **C06** owns z-index and no-overlap, explicitly not step order; **C59** owns panes. The ordering exists only in `docs/03-execution/plans/onboarding-workflow-design-2026-06-03.md` §3.3, whose status line reads **"PROPOSAL for founder review. No code yet."** ⚠ L-593/C60 must not be read as covering this — that closure was for *arrival at a parcel*, narrower than its heading. Paid twice already (residential L-435/L-668; office copied the shape via ADR-0092).

**Proposed close (NOT done this pass — a contract amendment is a governance change):** add to **C50 §5**, beside the existing dispatch-caller responsibilities, a normative invariant — *a typology's parameter/setup surface MUST NOT precede the user's explicit opt-in to generate, and the site (parcel + buildable envelope) MUST be visible at the moment of that opt-in* — plus a step-order table naming the shared seams so a new typology inherits the sequence instead of re-deciding it. Recorded in `MISSING-CONTRACTS-AUDIT-2026-06-01.md`. Queue: contract owner + onboarding/ux.

## L-671 — nothing requires a generator's REFUSAL to name the constraint the engine evaluated (P1 coverage gap) · PHASE 1 (governance) · OWNER: UNASSIGNED · TARGET: TBD

See audit **L-671**. Derived while fixing L-669, which is the paid instance: `RESIDENTIAL_MIN_PLATE_M2 = 400` lived in the editor UI, was read by no engine, contradicted no test, and shipped a self-falsifying sentence for weeks. **C50 §1.7** requires generative stages to fail SOFT (`{ok:false, reason}`) but is silent on what `reason` must contain and on what the UI may add to it, so a soft-fail may be rendered against a number the failing code never computed. Same family as §CONTEXT-DATA-HONESTY (L-422/457/467/469 — "failure and empty are the same value"), one layer up: here *refusal* and *a fabricated reason for refusal* are the same value to the user.

**Proposed close (NOT done this pass):** extend **C50 §1.7** — *a soft-fail `reason` MUST carry the measured value and the threshold of the constraint that actually bound, both sourced from the engine; UI copy MUST NOT introduce a numeric feasibility threshold not present in the reason* — plus a CI grep gate in `tools/ga-gate/` failing on a `MIN_*`/`MAX_*` numeric feasibility threshold declared under `apps/editor/src/ui/**` and interpolated into user-facing copy (mirrors `check:commandmanager`, ~½ day). Recorded in `MISSING-CONTRACTS-AUDIT-2026-06-01.md`. Queue: contract owner + generative.
