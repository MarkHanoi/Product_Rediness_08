...
| **L-287 dimensions must be ASSOCIATIVE — move the line, NEVER the references** | **3.2 annotations — CRITICAL (blocks L-286)** | **OPEN — CRITICAL, and it is a HARD PREREQUISITE for L-286 (Set Out), not a sibling.** *"Dimensions can now be selected and moved — brilliant — but the REFERENCE LINES SHOULD ALWAYS BE BOUNDED TO THE ELEMENTS THEY REFERENCE."* **He is drawing the line that separates a CAD drawing from a BIM model.** A dimension has **two kinds of state that must never be confused**: **(1) WHAT IT MEASURES** — the references (which element, which face/plane) — **immutable by drag; a QUERY against the model, re-evaluated on every change**; and **(2) HOW IT IS SHOWN** — the line's offset (his *"forward and backward"*) and the text position — **freely movable**. **Dragging edits (2). It must NEVER edit (1).** **The failure this prevents:** if a drag BAKES the witness lines into coordinates, the dimension **silently detaches**. It still *looks* right — until a wall moves, and then it measures **nothing**. **A detached dimension is worse than a missing one: it is confidently wrong, and a builder will build from it.** **Why it blocks L-286:** his own Example 5 there — *"walls move → dimensions move, no broken references"* — is **impossible unless the dimension stores REFERENCES rather than POINTS.** **You cannot make documentation live on top of dimensions baked into coordinates.** **First job: establish whether the record stores references or points — and report that BEFORE writing any code.** |
| **L-286 'SET OUT' — LIVE, self-maintaining documentation views** | **3.2 documentation — STRATEGIC (demo)** | **OPEN — STRATEGIC.** *"A Set Out view is a self-maintaining documentation view... the user should never think about regenerating tags or dimensions. This should become one of PRYZM's DEFINING FEATURES."* Add a door → tagged + dimensioned. Delete a window → tag vanishes, chain re-closes. Change elevation depth or crop → newly visible elements gain annotation, departing ones lose it. **No 'Update Documentation' command may ever exist.** **THE KEY FINDING — THIS IS NOT A NEW ENGINE, IT IS A BINDING OF THREE THINGS WE ALREADY HAVE:** **(1)** `ViewDependencyTracker` already watches the bus, computes which views a change dirties, and flushes them — **that IS his `detectChange()`, and it is shipped** (it's in every log he sends). **(2)** `RoomTagAutoPopulator` already reconciles idempotently — create / refresh / dedupe / **remove orphans** — which IS his synchronisation rule (*exactly one tag per visible element, no duplicates, no orphans*), **already solved for one element type**; **L-265 is generalising exactly that reconciler right now.** **(3)** `commitAnnotationSet` already commits creations **and** deletions as ONE undo entry. **So 'Set Out' = a view intent that turns the categories on and subscribes the reconcilers to the tracker's flush. That is why it is achievable.** **THE ONE DECISION HE HASN'T MADE, AND IT DECIDES USABLE vs UNBEARABLE: WHAT DOES CTRL-Z DO?** If each edit spawns its own annotation-reconcile undo entry, **moving one wall costs two undos** and Ctrl-Z becomes a lottery. Either the reconcile **rides the same undo entry as the edit**, or it is **excluded from undo and re-derived** (the drawing is a pure function of the model). **Needs an ADR before a line of binding code.** **Also: idempotence is the whole correctness argument** — if the reconciler is idempotent, *'live'* collapses to *'run it on every flush'*. And **visibility is the input, not the model**: his crop/depth examples break any implementation that reconciles against the LEVEL rather than against **what the view actually shows**. |
| **L-285 interior walls must draw LIGHTER than exterior/perimeter walls (plan + elevation + section)** | **3.2 drawing (Contract-23) — HIGH** | **OPEN — HIGH.** Every wall currently draws with the same pen, so a 100 mm partition and a 300 mm shell read as equally important — **the eye cannot find the building's envelope.** **THE WHOLE TICKET IS ONE DISTINCTION: the weight comes from the wall's FUNCTION, NOT its THICKNESS.** Keying the pen off `wall.thickness` is the obvious move and it is **wrong** — a 300 mm acoustic interior wall would draw as heavy as the shell. **The fact is already in the record**: the systemType carries the function (his own panel reads *Interior – Partition 100mm*). **What it really is: a NEW AXIS in the pen table.** L-277 shipped `pen = f(zone, category)` this morning; this makes it `pen = f(zone, category, FUNCTION)` — which is precisely what ISO 13567 sub-categorisation and Revit subcategory weights exist for. **The zone ladder is UNCHANGED and sacred: an interior CUT wall is still heavier than an exterior PROJECTION wall.** Function modulates *within* a zone; it never re-orders zones — **get that backwards and this silently undoes L-277.** **Guard the thickness shortcut explicitly:** a THICK interior wall must still draw lighter than a THIN exterior wall. And it resolves in the **shared** pen chain — he said *"but also in elevation"*, and a plan-only fix is the disease we have now diagnosed five times. |
| **L-283 elevation auto-dim is VERTICAL-ONLY — every horizontal dimension is missing** | **3.2 annotations (Gate G9) — HIGH** | **OPEN — HIGH.** *"The autodim elevation is working, but we are missing ALL the horizontal dims."* The vertical chains land correctly (sill 1000, opening 1200, floor-to-floor 2800); **not one horizontal dimension exists** — no window widths, no spacing, no façade length. **This is not a bug — it is the OTHER HALF of L-263, and it was never built.** The rule set that got written was the genuinely new one (the VERTICAL measurements an elevation needs and a plan does not). **The horizontal axis was left implicit, and 'implicit' means absent.** An elevation dimension set is **TWO chains**: vertical (have) and horizontal (don't). **And it is NOT the plan chain rotated** — it measures **along the FAÇADE PLANE**: opening widths, inter-opening spacing, distance to the façade ends, overall façade length outermost. **L-281's tier rule applies to both axes** — land that first and this inherits it. |
| **L-284 door symbol: handle is a crossbar; leaf not aligned to the wall face** | **3.2 hosted elements** | **OPEN.** *"Make the door handle correct — also make the door leaf aligned with the slab line."* The hardware draws as a **symmetric cross projecting from both faces** (a lever is an offset **L** on ONE face at the LATCH end), and the open leaf's hinge end **floats off the wall-face line** instead of pivoting flush with it. **Both are dimensional-truth defects (L-127), not styling: the arc's centre IS the hinge — if the leaf is offset from the wall face, the swing arc is a lie, and so is every clearance read off the drawing.** Corrections to the fresh L-266 geometry, not a legacy fix. |
| **L-282 a hosted DOOR promotes its whole HOST WALL to CUT** | **3.2 drawing (C09/C15) — CRITICAL** | **OPEN — CRITICAL. Breaks every elevation and section containing a door or window.** Founder: *"A hosted family MUST NEVER change the visibility classification of its host wall. Each object's geometry must be classified INDEPENDENTLY."* As the depth plane reaches a door, the door goes CUT **and the whole wall flips to cut graphics** — cut edges, cut profile, the opening exposed as if the wall were sectioned. **The rule:** classification is **PER GEOMETRY**, not per element hierarchy. `Wall → Door` does **not** mean `Door == Cut ⇒ Wall == Cut`. **No `if (door.isCut()) wall.setCut(true)` — or any equivalent — may exist.** Host/hosted, parent/child and assembly members must **never** inherit or propagate visibility state. **The only determinant of CUT vs PROJECTION is whether THAT SOLID intersects the plane.** **The corollary that will be missed:** the door's opening is a **cut** opening **only if the WALL is cut**. If the wall is PROJECTION, no cut edges, no cut faces, no opening edges — it renders like any other projection wall. **Why it's architectural:** L-277 built the right four zones; **this says the classifier runs at the wrong GRANULARITY — on the ELEMENT/host tree, not the SOLID.** Third granularity bug in the drawing layer today (L-275 layer-from-typename, L-266 mesh-edges-beside-own-symbol) — all the same shape: **a decision taken at the wrong level of the hierarchy.** **Guard the case that fails today:** door CUT + wall PROJECTION. A test that only pushes the plane into the wall passes already. |
| **L-281 the OVERALL dimension is drawn THROUGH the building** | **3.2 annotations (Gate G9) — HIGH** | **OPEN — HIGH.** *"The 'total' dims ALWAYS OUTSIDE and at the END of the dim pipeline — never through the building."* His **13000 mm** overall runs vertically **straight across the floor plate**. **Two rules in one sentence:** **(a) OUTSIDE** — a dimension line never crosses what it measures; **(b) OUTERMOST TIER** — chains stack outward (openings → exterior chain → **overall, furthest out**). **The real finding:** the planner takes the overall from the two extreme perimeter corners — **correct for the VALUE** — then draws it **on the axis those corners define**, which on an L-shaped/notched footprint crosses the plate. **The value is right; the OFFSET is missing.** **Fix it as a TIER MODEL, not a nudge:** each tier offset from the **FOOTPRINT BBOX** — not the nearest wall, because *'outside the nearest wall'* is still **inside** an L-plan, which is exactly his screenshot — scale-aware, each clearing the one inside it. **L-268's `partitionBuildings()` already supplies the footprints.** **Guard on an L-SHAPED plate; a rectangle would pass by luck.** |
| **L-280 window plan-symbol frame far thicker than the real window** | **3.2 hosted elements — HIGH** | **OPEN — HIGH.** *"The frame in plan view is much thicker than in reality."* **L-127 (dimensional truth) for the window.** `resolveWindowDimensions()` already resolves `frameThickness` through record → systemType → default **with no magic numbers** — so the resolver is right and **the symbol is not asking it**. **But TWO candidates look identical on screen and have OPPOSITE fixes, so REPRODUCE FIRST:** is the frame drawn at the wrong **WIDTH** (geometry — fix the geometry), or at the **right width with a CUT pen so heavy that the STROKE reads as a fat band** (pen — fix the pen)? L-277 has just made CUT the heaviest weight, so a correct 20 mm frame stroked heavily at 1:100 could be precisely what he is seeing. **A WEIGHT IS NOT A THICKNESS.** |
| **L-278 the WINDOW: 3D mesh edge-dumps into the plan symbol; 3D/elevation LOD still ZERO** | **3.2 hosted elements — HIGH** | **OPEN — carried forward by the door agent, which REFUSED to fake it.** The door's cure (`skipInPlan` — *an element with a plan symbol does not also emit its mesh edges*) **is NOT transferable, and copying it would be a real bug.** At the cut height **a door opening is EMPTY** — but a window's **frame and glazing ARE genuinely cut** by the plane. Tagging a window `skipInPlan` would **delete the very lines that make it a window.** **So the window is the element that PROVES L-277's zone model is necessary:** it must contribute its **CUT** section (frame/sash/glazing/mullion) while suppressing its **PROJECTION** edges (head, upper lights) — a real classification, not a blunt skip. **Sequence AFTER L-277.** Also owed: the window row of ADR-121's LOD matrix in 3D + elevation (the door row is now closed; elevation is no longer zero at 4/42), and L-274's untyped-record backfill. **No new fields** — the mullion comes from the record's existing `columnDividerThickness`/`columnRatios`. |
| **L-277 Revit line-type semantics — PROJECTION is being drawn as HIDDEN** | **3.2 drawing (C09/Contract-23) — HIGH** | **OPEN — HIGH. The founder handed us the normative model, and only ONE of the four zones dashes:** **CUT** (solid, heaviest, + poché — never dashed) · **PROJECTION** (solid, thinner — ***distance from the viewer does NOT make an edge hidden***) · **BEYOND** (solid, lighter — *not* hidden geometry; the lower run of a stair) · **HIDDEN** (**the only zone that dashes** — and it is an OCCLUSION fact, not a depth fact). **Why it is architectural:** we have a CUT/PROJECTION split and a **depth-based dash**. There is **no BEYOND zone at all**, and **HIDDEN is inferred from DEPTH rather than from OCCLUSION** — *"is it far?"* and *"is something in front of it?"* are different questions, and the drawing layer answers the wrong one. **This completes C09 §4.6 / ADR-121:** that rule says an element is CUT or PROJECTED and that nothing is drawn through a solid; it does not yet say WHAT PEN each zone gets, and has no BEYOND. **Model `DrawingZone` explicitly** — a zone that is not named cannot be styled correctly, and that is exactly how PROJECTION got dashed. **Any dash keyed on distance is the bug and must be deleted.** |
...
| **L-272 FACADE sun analysis renders terribly (ground heatmap is perfect)** | **Analysis / geospatial — HIGH** | **OPEN — HIGH. The founder isolated the variable himself, and that is the whole value of the ticket: same analysis, same model, same frame — GROUND is beautiful, FACADE is broken. So it is not the solar engine, not the ramp, not the sun model. It is the DRAPE.** **PROVEN (his log):** the ground path builds a **continuous resampled field** — `sunHours heatmap: smooth texture 512×512 (~0.9 m/texel) from 1623/1623 cell(s)`. One plane, one field, one interpolated texture. **REFUTED — and it was MY OWN first guess, recorded because it would have wasted an agent:** I assumed the façade had no texture and tinted meshes per-face. **False.** `CesiumViewport` already imports `prepareFacadeSunGrid`, `planFacadeSampling`, `rasterizeFacadeSunTexture`, `FacadeDrapeFace` — **the façade path already rasterises to a texture.** **Hypotheses to MEASURE, not pick:** per-face drape over a shattered surface (192 walls + balconies + partitions) giving per-face resolution and visible SEAMS · coarse sampling (smooth texture over a coarse field = smooth garbage) · draping onto INTERIOR partitions · **per-face normalisation** (each wall normalises its own min/max → patchwork even when every sample is correct). **Print the façade's texels/m and samples-per-face; the ground path's own log line is the benchmark and the gap will name the bug.** **A solar study is what an architect SHOWS A CLIENT — this is a credibility bug, not a cosmetic one.** |
| **L-270 loading screen for 3D-Globe / 3D-Site activation** | **UX / view lifecycle** | **OPEN.** **The overlay ALREADY EXISTS** — the founder's own log shows `[BatchCoordinator] ON-BATCH-END-DONE (overlay dismissed post-dual-PSO-compile)`. **So the work is not "build a loading screen"; it is: the overlay is wired to the BATCH lifecycle ONLY — teach it the VIEW-ACTIVATION lifecycle too. Anyone who writes a SECOND overlay has misunderstood the ticket** (the identical mistake L-262/264/265 keep punishing). **His real requirement is not cosmetic:** *"only when everything is loaded and ready can the user jump in and navigate"* — that is an **INPUT GATE tied to a READINESS SIGNAL**, not a spinner. **Dismiss on a real signal, never a timer: a spinner that lies is worse than no spinner.** The batch overlay already dismisses on a genuine signal (post-PSO-compile) — copy that discipline. **Directly related to L-259**, where the globe put the building underground *because it anchored before the terrain had loaded* — if that fix produced a terrain-readiness signal, **consume it, do not invent a second.** |
| **L-271 nested `runBatch` SILENTLY DROPS its batch guards** | **0.x commands/batching** | **🟢 FIXED (`cae813ec`, on `main`) — AND MY FRAMING WAS REFUTED, WHICH IS THE MOST VALUABLE PART.** **I claimed the nested batch fragments UNDO. MEASURED, NOT ASSUMED: `CommandBus.executeCommand()` pushes exactly ONE ring entry PER DISPATCH and NEVER READS `batchCoordinator`. So `runBatch()` is UNDO-NEUTRAL — nesting, opening or dropping a batch CANNOT change the undo count. The batch never merged those entries, so it could not fragment them.** **What actually buys 'one gesture = one undo entry' is dispatching ONE `*.batch.create` command** (one `produceCommand` → one patch pair → one ring entry). An executor dispatching N `foo.create` commands inside a `runBatch` produces **N** undo entries and the batch cannot merge them. **Now written into C16 §8.6 (B-6/B-7) so it stops being re-learned.** **THE REAL BUG SURVIVED AND IS FIXED:** `runBatch` exists for the **GUARDS** — builder pauses, ViewDependencyTracker suppression, room-redetect suppression, the CRDT blackout, the registration queue, the loading overlay — and it was **dropping them silently while announcing that it had** (*a warning is not a guard*). Nested `runBatch` now **JOINS by depth-counting**, following the precedent already in this codebase (`StoreEventBus.beginBatch` → *'depth now 1'*); nothing releases until depth returns to 0, and joining is **refused** once the sweep has started (the registration queue is draining; new work cannot safely join). **G10 EXTENDED:** 7 nesting + 4 undo-atomicity tests, G10 still green (20/20). **G10 pinned the invariant but NOTHING pinned it under NESTING — a gate that did not cover, the same lesson as G0. Closed.** |
...
| **L-274 untyped door/window records are silently A DIFFERENT ELEMENT (no backfill)** | **3.2 hosted elements — HIGH** | **OPEN — HIGH. MEASURED:** untyped door → `DEFAULT_DOOR_DIMENSIONS` **0.900 × 2.100**; typed `dt-solid-timber` → **0.926 × 2.040** (the founder's own log). **A legacy door is a DIFFERENT DOOR — drawn correctly, forever — because nothing backfills the record.** Same trap for windows (`wt-single-pane` vs `wt-timber-casement` ⇒ a mullion, or none). **HOW IT WAS FOUND, AND THE PROCESS POINT OUTRANKS THE BUG:** the founder said door parity was *still* wrong after L-260A. The reflex was to re-fix the creation path. **I read it first — it was ALREADY SOUND** (resolves via `getDoorToolConfig()` + `resolveDoorDimensions()`, commit AND preview, no literals, no P4 global). **Re-fixing it would have burned an agent on a working path and left the real defect standing.** That is now pinned as a test so nobody re-fixes it. **THE RESIDUAL OF THE SIGNATURE DISEASE: seven creation-path bugs have been fixed by converging the paths — and EVERY ONE left behind records the broken path had already written. A CREATION-PATH FIX IS ONLY HALF A FIX WITHOUT A BACKFILL.** The fix is a **MIGRATION** (one command, one undo entry), not another creation-path change — and the trap must be stated up front: if the backfill assigns a type whose dimensions differ from the defaults, **the founder's existing doors will visibly CHANGE SIZE.** That may be right, but it must be a DECISION, not a surprise. |
