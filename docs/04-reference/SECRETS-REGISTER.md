...
| | |
|---|---|
| **Distinct names** | **97** (86 read in scanned roots; floor 25) |
| Declared (have a row in secrets-declarations.json) | 85 |
| UNDECLARED (arm-a findings) | 12 |
| Declared but not found in scanned roots (disclosed, not failed) | 11 |
...
| `ALLOWED_ORIGIN` | CONFIG | runtime-env | optional | BFF | — | degraded-feature — CORS default applies (posture varies by NODE_ENV) | server.js:293 · server/corsPolicy.js:32 |
| `ANTHROPIC_API_KEY` | SECRET | runtime-env | optional | BFF | — | degraded-feature — with no CF_WORKER_URL, AI features hard-degraded | server.js:125 · server.js:295 · server.js:6228 |
| `ANTHROPIC_MODEL_ID` | CONFIG | runtime-env | optional | BFF | — | degraded-feature — model pin default used | server.js:147 |
| `APEX_ORIGIN` | CONFIG | runtime-env | optional | BFF | — | degraded-feature — apex/app split origin default (C51) | server.js:6019 |
...
| `APS_CLIENT_ID` | SECRET | runtime-env | optional | BFF | — | degraded-feature — Autodesk DWG conversion disabled | server.js:2698 · server/dwgConversionService.js:32 · server/dwgConversionService.js:71 |
| `APS_CLIENT_SECRET` | SECRET | runtime-env | optional | BFF | — | degraded-feature — Autodesk DWG conversion disabled | server.js:2698 · server/dwgConversionService.js:32 · server/dwgConversionService.js:72 |
...
| `BUILT_AT` | CONFIG | runtime-env | optional | BFF + CI + deploy | — | degraded-feature — /version provenance blank; one of the nine C77 §2.3 build-args | .github/workflows/deploy-fly.yml:442 · Dockerfile:201 · server.js:2444 · tools/deploy/fly-manual-deploy.sh:171 |
...
| `CF_WORKER_URL` | SECRET | runtime-env | required-in-prod-only | BFF | — | degraded-feature — AI proxying falls through to direct ANTHROPIC_API_KEY; with neither, the AI plane is dead | server.js:130 · server.js:295 · server.js:6227 |
...
| `DATABASE_URL` | SECRET | runtime-env | required-in-prod-only | BFF | sync-server | degraded-feature — BFF falls back to in-memory Postgres (non-durable); sync-server AUTHZ_MODE=pg refuses every request without it | server.js:282 · server.js:6224 · server.js:6234 · server/pgClient.js:52 · server/pgClient.js:54 |
| `DATAFORDELER_API_KEY` | SECRET | runtime-env | optional | BFF | — | degraded-feature — DK cadastre proxy fails | server/jurisdiction/dkMatrikelProxy.js:247 · server/jurisdiction/dkMatrikelProxy.js:272 |
| `DK_MATRIKEL_GEOM_TYPENAME` | CONFIG | runtime-env | optional | BFF | — | degraded-feature — DK proxy misconfigured | server/jurisdiction/dkMatrikelProxy.js:52 |
| `DK_MATRIKEL_TYPENAME` | CONFIG | runtime-env | optional | BFF | — | degraded-feature — DK proxy misconfigured | server/jurisdiction/dkMatrikelProxy.js:57 |
| `DK_MATRIKEL_WFS_URL` | CONFIG | runtime-env | optional | BFF | — | degraded-feature — DK proxy misconfigured | server/jurisdiction/dkMatrikelProxy.js:44 |
| `FAMILY_VIRUS_SCAN` | CONFIG | runtime-env | optional | BFF | — | degraded-feature — family upload scanning default | server/familyMarketplaceRoutes.js:43 |
| `FLY_IMAGE_REF` | CONFIG | runtime-env | optional | BFF | — | degraded-feature — image provenance blank | server.js:2446 |
| `GIT_BRANCH` | CONFIG | runtime-env | optional | BFF + CI + deploy | — | degraded-feature — /version provenance blank; one of the nine C77 §2.3 build-args | .github/workflows/deploy-fly.yml:441 · Dockerfile:200 · server.js:2443 · tools/deploy/fly-manual-deploy.sh:170 |
| `GIT_SHA` | CONFIG | runtime-env | optional | BFF + CI + deploy | — | degraded-feature — /version provenance blank; one of the nine C77 §2.3 build-args | .github/workflows/deploy-fly.yml:440 · Dockerfile:199 · Dockerfile:203 · server.js:2442 · tools/deploy/fly-manual-deploy.sh:169 |
| `GOOGLE_CLIENT_ID` | PUBLIC-TOKEN | runtime-env | optional | BFF | — | degraded-feature — Google OAuth disabled | server.js:2060 · server/oauthService.js:157 · server/oauthService.js:173 |
| `GOOGLE_CLIENT_SECRET` | SECRET | runtime-env | optional | BFF | — | degraded-feature — Google OAuth disabled | server/oauthService.js:174 |
| `INTERNAL_PLAN_SECRET` | SECRET | runtime-env | optional | BFF | — | degraded-feature — internal plan endpoint auth denies | server.js:1851 |
| `LOWMEM` | CONFIG | runtime-env | optional | CI + deploy | — | degraded-feature — low-memory build profile default; one of the nine C77 §2.3 build-args (CI passes LOWMEM=0) | .github/workflows/deploy-fly.yml:435 · Dockerfile:52 · tools/deploy/fly-manual-deploy.sh:164 |
...
| `MICROSOFT_CLIENT_ID` | PUBLIC-TOKEN | runtime-env | optional | BFF | — | degraded-feature — Microsoft OAuth disabled | server.js:2114 · server/oauthService.js:195 · server/oauthService.js:211 |
...
| `MIGRATION_BOOT_GATE_TIMEOUT_MS` | CONFIG | runtime-env | optional | BFF | — | degraded-feature — boot-gate timeout default | server.js:6275 |
...
| `OTEL_EXPORTER_OTLP_ENDPOINT` | CONFIG | runtime-env | optional | BFF | — | degraded-feature — spans not exported | server.js:2505 · server/telemetry.js:34 |
| `OTEL_EXPORTER_OTLP_HEADERS` | SECRET | runtime-env | optional | BFF | — | degraded-feature — telemetry export unauthenticated or off (may carry an auth token) | server/telemetry.js:87 |
| `OTEL_RESOURCE_ATTRIBUTES` | **UNDECLARED** | runtime-env | UNDECLARED | deploy | UNDECLARED | UNDECLARED — arm-a finding | fly.toml:80 |
| `OTEL_SERVICE_NAME` | CONFIG | runtime-env | optional | BFF + deploy | — | degraded-feature — default service name used | fly.toml:79 · server.js:2527 · server/telemetry.js:33 |
...
| `PORT` | CONFIG | runtime-env | optional | BFF + deploy | — | degraded-feature — default listen port used | fly.toml:76 · server.js:6156 |
...
| `PRYZM_OWNER_EMAIL` | CONFIG | runtime-env | required | BFF | — | degraded-feature — owner-grant resolves to free tier; no owner auto-grant | server.js:1891 · server.js:2872 · server.js:3764 · server.js:6226 · server/authStore.js:51 · server/oauthService.js:40 · server/planStore.js:283 · server/supabaseMigrate.js:192 |
...
| `PUBLIC_BASE_URL` | CONFIG | runtime-env | required-in-prod-only | BFF | — | degraded-feature — OAuth redirect URIs wrong; OAuth breaks | server.js:294 · server.js:448 · server/oauthService.js:138 · server/oauthService.js:139 |
| `REPLIT_DEV_DOMAIN` | CONFIG | runtime-env | optional | BFF | — | degraded-feature — legacy dev-domain detection off | server.js:132 · server/oauthService.js:141 · server/oauthService.js:142 |
...
| `RUN_NUMBER` | CONFIG | runtime-env | optional | BFF + CI + deploy | — | degraded-feature — /version provenance blank; one of the nine C77 §2.3 build-args | .github/workflows/deploy-fly.yml:443 · Dockerfile:202 · server.js:2445 · tools/deploy/fly-manual-deploy.sh:172 |
| `SESSION_SECRET` | SECRET | runtime-env | required-in-prod-only | BFF | sync-server | hard-exit (prod) / ephemeral-fallback (dev) | server.js:281 · server.js:6225 · server/authStore.js:33 · server/oauthService.js:36 |
...
| `STRIPE_SECRET_KEY` | SECRET | runtime-env | optional | BFF | — | degraded-feature — billing disabled | server.js:2169 · server.js:2522 · server.js:5664 · server/stripeService.js:44 |
| `STRIPE_WEBHOOK_SECRET` | SECRET | runtime-env | optional | BFF | — | degraded-feature — webhook signature verification fails; billing events dropped | server.js:307 · server.js:2170 · server.js:2522 |
| `SUPABASE_ANON_KEY` | PUBLIC-TOKEN | runtime-env | optional | BFF | — | degraded-feature — client-side Supabase disabled | server.js:6222 · server/supabaseClient.js:52 |
| `SUPABASE_DB_URL` | SECRET | runtime-env | optional | BFF | — | degraded-feature — alternate DB DSN; unset falls through to DATABASE_URL | server.js:282 · server.js:6223 · server.js:6234 · server/pgClient.js:57 · server/pgClient.js:59 |
| `SUPABASE_SERVICE_ROLE_KEY` | SECRET | runtime-env | optional | BFF | — | degraded-feature — privileged Supabase ops fail; a leak here breaks C13 project isolation | server.js:160 · server.js:6221 · server.js:6235 · server/supabaseClient.js:51 · server/supabaseClient.js:82 |
| `SUPABASE_URL` | CONFIG | runtime-env | optional | BFF | — | degraded-feature — Supabase client unavailable; auth/storage degraded | server.js:159 · server.js:2520 · server.js:6220 · server.js:6235 · server/supabaseClient.js:41 · server/supabaseMigrate.js:118 |
...
| `TRUST_PROXY_HOPS` | CONFIG | runtime-env | optional | BFF | — | degraded-feature — proxy-hop default | server.js:329 · server.js:330 |
...
| `VITE_CESIUM_TOKEN` | PUBLIC-TOKEN | build-time-inlined | required-in-prod-only | CI + deploy + editor-client | — | broken-bundle — ships tokenless; the 3D globe does not load. Baked into the public bundle BY DESIGN (C77 §1.2/§2.3) — scope + rotate at the ion dashboard | .github/workflows/deploy-fly.yml:436 · apps/editor/src/ui/geospatial/CesiumViewport.ts:262 · Dockerfile:118 · Dockerfile:119 · tools/deploy/fly-manual-deploy.sh:165 |
...
| `VITE_CONTEXT_TILES_URL` | CONFIG | build-time-inlined | required-in-prod-only | BFF + CI + deploy | — | degraded-feature — falls back to live Overpass instead of pre-baked R2 tiles | .github/workflows/deploy-fly.yml:439 · Dockerfile:144 · Dockerfile:145 · server/context-delivery/contextTilesProxy.js:45 · tools/deploy/fly-manual-deploy.sh:168 |
...
| `VITE_GLB_URL` | CONFIG | build-time-inlined | required-in-prod-only | CI + deploy + pkg:core-app-model | — | broken-bundle — furniture GLBs fall back to local /items/ (the prod-404 cliff, DEPLOY-CONTRACT-MANUAL-FLY.md §3.4) | .github/workflows/deploy-fly.yml:438 · Dockerfile:138 · Dockerfile:139 · packages/core-app-model/src/catalog/catalogAssetUrl.ts:79 · tools/deploy/fly-manual-deploy.sh:167 |
| `VITE_GOOGLE_MAPS_KEY` | PUBLIC-TOKEN | build-time-inlined | required-in-prod-only | CI + deploy + editor-client | — | broken-bundle — photorealistic tiles unavailable. Baked into the public bundle BY DESIGN — restrict by API + referrer | .github/workflows/deploy-fly.yml:437 · apps/editor/src/ui/geospatial/CesiumViewport.ts:270 · Dockerfile:127 · Dockerfile:128 · tools/deploy/fly-manual-deploy.sh:166 |
...
| `VITE_OTEL_EXPORTER_OTLP_ENDPOINT` | PUBLIC-TOKEN | build-time-inlined | optional | UNKNOWN | — | broken-feature — the PUBLIC OTLP ingest URL the browser posts spans to. Baked into the public bundle BY DESIGN (same posture as VITE_CESIUM_TOKEN) and therefore MUST be an ingest endpoint that is safe to publish and CORS-enabled; without it VITE_PRYZM_TRACING=otlp REFUSES rather than dropping spans | NONE-FOUND-IN-SCANNED-ROOTS |
| `VITE_PRYZM_TRACING` | CONFIG | build-time-inlined | optional | UNKNOWN | — | degraded-feature — the BROWSER half of PRYZM_TRACING. Read by vite.config.ts at BUILD time and baked in as __PRYZM_TRACING__; unset means the browser's 345 tracer sites stay no-ops. Flipping it requires a REBUILD, not a restart | NONE-FOUND-IN-SCANNED-ROOTS |
...
| `VITE_PRYZM_TRACING_SAMPLE` | CONFIG | build-time-inlined | optional | UNKNOWN | — | degraded-feature — browser head sample ratio; baked at build time as __PRYZM_TRACING_SAMPLE__ | NONE-FOUND-IN-SCANNED-ROOTS |
