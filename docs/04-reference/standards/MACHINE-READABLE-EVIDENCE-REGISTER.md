...
7. **The three questions are separate.** *Can we read it?* → this register. *Have we already tried?* → this
...
|---|---|---|---|---|---|
| **Murcia** | *Alineaciones* (municipal alignment geometry) | **Yes** | **Yes** — SIG-MU2 (methodology, 4 conditions) | **Use** | Implement the **bbox** neighbour fetch — the resolver API resolves a point. *Owner: Murcia agent.* |
...
| **València** | Layer 212 *PGOU Alineaciones* (`PGOU_AL`) — 21,210 polygons, keyless | **Yes** | ⛔ **Pending — await `altura` semantics** | **Use** | Await `altura` semantics. ⚠ The **movement geometry is the depth datum** (Art. 6.18.1) — R1 **Superseded**. *Owner: the founder (R5 email).* |
