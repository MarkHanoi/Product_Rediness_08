...
| **🇫🇮 Finland** | Elevation Model 2 m/10 m — WCS `avoin-karttakuva.maanmittauslaitos.fi/ortokuvat-ja-korkeusmallit/wcs/v2` (401 without key) | MML WCS | **free self-service API key** (`MML_API_KEY`) — email registration, **no approval** | CC BY 4.0 | **INSPIRE Buildings WFS `inspire-wfs.maanmittauslaitos.fi/inspire-wfs/bu_mtk_polygon` — ANONYMOUS/keyless** (footprints, no height) + derive DSM−DTM | **none** for footprints; keyed OGC-API-Features for attributes | INSPIRE WFS | CC BY 4.0 | **OK — VERIFIED live 2026-07-25** — split into 2 connectors | **Two connectors:** (1) terrain WCS + rich attributes = free self-service `MML_API_KEY` (repo secret, no approval); (2) national building footprints = **anonymous INSPIRE WFS, zero auth**. Heights = derive DSM−DTM (Helsinki LoD2 later). Much easier than Sweden — no org onboarding. |
...
| **🇧🇪 Belgium — Flanders** | DHMV II — **WCS `geo.api.vlaanderen.be/dhmv/wcs` — KEYLESS** (coverages `DHMVII_DTM_1m` + `DHMVII_DSM_1m`) | Digitaal Vlaanderen | **none (anonymous — VERIFIED 2026-07-25)** | Open Data | **DSM1m − DTM1m (both keyless)** + 3D GRB object heights + Basisregisters Buildings REST `api.basisregisters.vlaanderen.be/v2/gebouwen` (anonymous 200) | **none** for footprints / DSM−DTM | Digitaal Vlaanderen | Open Data | **OK — VERIFIED Tier-1, NO credentials for terrain or buildings** | Direct WCS + REST connectors, zero auth. DSM−DTM gives heights keyless; 3D GRB for ready-made LoD1. Easier than SE/FI/PT — no account at all. |
...
| **🇦🇺 Australia — Victoria (City of Melbourne LGA)** | *(no keyless national raster located 2026-09-05)* — ELVIS `elevation.fsdf.org.au` is a bulk-download HTML portal (200, 13,250 B text/html), no keyless WCS/COG; PRYZM drapes AU from the global Mapterhorn terrain (`terrain.mjs` AU state rows) | elevation.fsdf.org.au (bulk only) | — | *(not read)* | **City of Melbourne 2023 Building Footprints** — LoD1 roof-component stacks with AHD elevations (41,701 records; extent 144.898–144.991 E / −37.851–−37.776 S) | `data.melbourne.vic.gov.au/api/explore/v2.1/catalog/datasets/2023-building-footprints/exports/geojson` (Opendatasoft Explore v2.1; `/records` REFUSES `offset+limit > 10,000` — HTTP 400 verbatim, so page-through is impossible; export per 0.01° cell instead) | *none (keyless; X-RateLimit 10,000/day anonymous)* | CC BY 4.0 | **OK — Melbourne LGA only**; every other state BLOCKED-keyless 2026-09-05: ACT `ACTGOV_BUILDING_FOOTPRINTS/FeatureServer/0` (64,674) has NO height field · NSW `portal.spatial.nsw.gov.au` no footprint/height service · Vicmap `building_polygon` no height · ELVIS no keyless raster | **WIRED 2026-09-05** (lane HEIGHTS-AU, d93adb9e · 3a35dd22 · a3028748): `victoria` row `heightJoin:'au_open'`, `heights/auOpenHeights.mjs` + `stampAuOpenHeightsOnGeojsonseq`; height = max(footprint_max) − min(structure_min) over the component stack (Eureka 815210 = 11 components → 297.5 m, live smoke); CI row `melbourne victoria -37.8136,144.9631 500` in both workflows; first real Victoria bake NOT yet run (ISSUE-LOG L-12925). Marker `pryzm:height_src=measured-lidar` is a misnomer for photogrammetric LoD1 — per-footprint `heightSource:'melbourne-cc-lod1'` carries the method. |
...
| 2 | 🇫🇮 **Finland** | Get the free **self-service** NLS open-data key (email, no approval) for the **terrain WCS only**; add as a **repository** secret. VERIFIED 2026-07-25: terrain WCS = 401 without key; national **building footprints via INSPIRE WFS are ANONYMOUS** (no key at all); heights derive DSM−DTM. | `MML_API_KEY` (terrain only) | asiointi.maanmittauslaitos.fi (NLS open data) |
...
| 5 | 🇧🇪 **Belgium — Flanders** | ~~Register~~ **NO ACTION — VERIFIED keyless 2026-07-25.** DHMV WCS + Basisregisters Buildings are anonymous; terrain + DSM−DTM heights need no credentials. Pure engineering. | *(none)* | — |
