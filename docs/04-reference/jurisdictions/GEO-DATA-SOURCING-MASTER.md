...
|---|---|---|---|---|---|---|---|---|---|---|
| **🇸🇪 Sweden** | Nationell Höjdmodell (NH) — Download/WCS | Lantmäteriet Geotorget | **free account + API key** (`LANTMATERIET_API_KEY`) | CC0 | *(none nationally)* | — | — | — | **OK** — terrain solved; no national height source | Register once at Geotorget; store `LANTMATERIET_API_KEY` (repo secret); bake terrain only. Heights: nDSM-derive later (LiDAR exists) — not a sourcing gap. |
...
|---|---|---|---|---|
| 1 | 🇸🇪 **Sweden** | Register once at Lantmäteriet Geotorget; add the consumer key as a GitHub **repository** secret | `LANTMATERIET_API_KEY` | Lantmäteriet Geotorget |
