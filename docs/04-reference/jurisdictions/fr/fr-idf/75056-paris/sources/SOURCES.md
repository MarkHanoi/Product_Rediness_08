...
| **Paris height GIS layer 1** | `opendata.paris.fr` dataset `plub_filet` — "PLU bioclimatique - Filets gabarit-enveloppe" — **20,644 records** — schema: `n_sq_fil`, **`haut`** (coded letter: M/K/C/etc. mapping to height-by-street-width per PLU bioclimatique règlement), `cour`, `n_sq_ca` (arrondissement), `c_sec`, `n_pc`, `c_asp` (parcel ref), `st_length_shape`, `geo_shape` (geometry) | PLU bioclimatique de Paris | `https://opendata.paris.fr/api/explore/v2.1/catalog/datasets/plub_filet/records` | `verified` — live probe 2026-07-23; 20,644 records confirmed; **field `haut` is coded, NOT absolute metres** — decoding table in PLU bioclimatique règlement UG.10 |
