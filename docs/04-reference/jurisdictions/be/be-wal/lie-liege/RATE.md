# City RATE — master completion scorecard — Liège (be-wal, lie · NIS 62063)
...
bake-covered this pass; legacy naming `lie-liege` pending Phase-0 rename to `62063-liege`). The legislation
detail (the legacy Wallonia/CoDT structured-fill prior) lives in
...
| 2 | **LEGISLATION** | 25 % | `not-assessed` | `not-checked` | `pending-implementation` | Legacy structured-fill prior only — see [`LEGISLATION-RATE.md`](./LEGISLATION-RATE.md) (Wallonia CoDT / plan de secteur). No rule pack (`rulepacks/registry.ts` is ES/DK/NL/SA-only), no signed `sources/VERIFICATION.md`, no clau-inventory scorecard run. |
...
| 5 | **TERRAIN** | 10 % | `not-assessed` | `not-checked` | `outside-coverage` | No `terrain.mjs` TERRAIN_CITY row for Liège → no baked quantized-mesh to verify. |
| 6 | **HEIGHTS/LOD** | 10 % | `not-assessed` | `not-checked` | `not-queried` | Wallonia PICC building height is unconfirmed; `heightSources.mjs` has no `liege` REGION_SOURCE key. No provenance histogram probed. |
...
> standard files (C63 §5); Liège is not bake-covered, so they are deliberately omitted rather than shipped
> as empty placeholders. A future pass that adds a `bake.mjs` REGION + `terrain.mjs` row for Liège should
> scaffold them from `_TEMPLATE/_CITY/` (see `COUNTRY-RATE.md` §B). The folder rename `lie-liege` →
> `62063-liege` is orchestrator-owned Phase-0 (C63 §8.2).
