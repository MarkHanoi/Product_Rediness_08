# City RATE — master completion scorecard — Antwerp (be-vlg, ant · NIS 11002)
...
bake-covered this pass; legacy naming `ant-antwerp` pending Phase-0 rename to `11002-antwerp`). The
legislation detail (the legacy Flanders/VCRO structured-fill prior) lives in
...
|---|---|---:|---|---|---|---|
| 1 | **PARCEL** | 15 % | `not-assessed` | `not-checked` | `not-queried` | No Belgian cadastre provider is wired (`parcelProviders/registry.ts` has no BE entry) → universal footprint-fallback; a click resolves an OSM footprint, not a legal parcel. Unsampled (C57 §2.4). |
| 2 | **LEGISLATION** | 25 % | `not-assessed` | `not-checked` | `pending-implementation` | Legacy structured-fill prior only — see [`LEGISLATION-RATE.md`](./LEGISLATION-RATE.md) (Flanders VCRO / gewestplan + RUP). No rule pack (`rulepacks/registry.ts` is ES/DK/NL/SA-only), no signed `sources/VERIFICATION.md`, no clau-inventory scorecard run. |
| 3 | **DATA-SOURCES** | 15 % | `not-assessed` | `not-checked` | `outside-coverage` | Not bake-covered: no `bake.mjs` REGION (only `brussels` for Belgium), no `terrain.mjs` row, no wired city height `REGION_SOURCE` key. The 5-slot checklist is not computable. |
...
| 5 | **TERRAIN** | 10 % | `not-assessed` | `not-checked` | `outside-coverage` | No `terrain.mjs` TERRAIN_CITY row for Antwerp → no baked quantized-mesh to verify. |
| 6 | **HEIGHTS/LOD** | 10 % | `not-assessed` | `not-checked` | `not-queried` | Flanders GRB carries structured building height, but `heightSources.mjs` has no `antwerp` REGION_SOURCE key wired. No provenance histogram probed. |
...
(no rule pack), a real parcel (no BE cadastre wired → footprint), a measured height, any scorecard number it
...
> standard files (C63 §5); Antwerp is not bake-covered, so they are deliberately omitted rather than shipped
> as empty placeholders. A future pass that adds a `bake.mjs` REGION + `terrain.mjs` row for Antwerp should
> scaffold them from `_TEMPLATE/_CITY/` (see `COUNTRY-RATE.md` §B). The folder rename `ant-antwerp` →
> `11002-antwerp` is orchestrator-owned Phase-0 (C63 §8.2).
