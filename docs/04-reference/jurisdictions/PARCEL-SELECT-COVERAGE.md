...
| **Portugal** (`pt`) | **WIRED** | Loule (37.1378, -8.02) → HTTP 200 application/json 9380 B → **AAA000336903**, 195 m², 9-pt WGS84 ring, nearest-centroid pick (click on ROW/plaza), 39 ms | 200 empty | id: national cadastral reference · area: SERVED (areavalue) · address: served (município) | `/api/parcel/pt` → snicws.dgterritorio.gov.pt inspire:cadastralparcel (DGT SNIC) | ⚠ COVERAGE DECLARED-INCOMPLETE: per-município cadastro predial — Lisboa, Porto, Évora, Faro, Portimão, Beja, Setúbal all `empty` today; only Loulé resolved. The leg carries a coverageNote on every empty so the card can say so. ⭐ **Setúbal is no longer `empty` — it is WORSE, and it now has a source hunt and a reachability measurement** (`pt/findings/belverde-lot-sources-2026-09-05.md`, §L-12897 / §L-12912 / §L-12942). At Belverde (Seixal) SNIC answers **every** click in the urbanisation with ONE prédio — `AAA000091722`, **7 662 344 m²** (766 ha), 500 vertices, DICOFRE 151002 — so the leg is not silent, it is confidently wrong-grained. Six publishers, twenty-nine probes: **nobody publishes the lot polygon** (DGT `bdncp:predios_dico_26` is empty nationally; DGT OGC API `cadastro` → 0 at the point; BUPi RGG → 0; no AML geoportal exists; CM Seixal publishes the loteamento PERIMETER, the lot NUMBER as a point and the BUILDING outline, never the lot). The card therefore leads with the OSM footprint (§L-12912 `parcelCandidateChoice`) — **which fires for 20.1% of the município's own buildings there** (91/453 `Edificado` centroids against baked `buildings.pmtiles` v=L662a; Seixal-centre control 87.4%), so four clicks in five land on Draw-primary. The 80% filler is CM Seixal `Edificado` — keyless, by point, 100% by construction — and is **licence-blocked, not technically blocked**: `copyrightText: ""` on the service |
...
| **Qatar** (QA) | ✅ **WIRED** — Table A | keyless | `services.gisqatar.org.qa/.../CadastrePlots/MapServer/0` → HTTP 200, `PIN 1010028`, `PDAREA 183494` (surveyed), 137-vertex ring. ⭐ The layer is **HIDDEN from the folder listing** and was addressed directly ([[getcapabilities-is-not-an-inventory]]). Licence **UNREAD (YELLOW)**. **Re-probed 2026-09-05 as the control for the ME sweep:** `…/CadastrePlots/MapServer/0?f=json` → 200 5 441 B; point query @ 51.531,25.286 → 200 269 B `{"PIN":1010028,"PDAREA":183494}` in 0.67 s. The webmap `d0c262cb…/data?f=json` (55 557 B) lists 27 layers and **no building/height layer**; `Vector/Buildings` and `Vector/Building` → 404 |
...
| **Saudi Arabia** (SA) | **CREDENTIAL-GATED** (was geo-blocked) | **NO** | `umaps.balady.gov.sa` 301→`umaps.momah.gov.sa`; `/server/rest/services` → `{"currentVersion":11.5,"folders":["Hosted","umaps","Utilities"],"services":[]}`; **every DATA folder** → `{"error":{"code":499,"message":"Token Required","details":[]}}` (stable ×3); `/portal/sharing/rest` → `enterpriseVersion 11.5.0`. The viewer authenticates against `ssoapp.balady.gov.sa`. ⭐ **The L-606 class CHANGED**: an in-SA proxy alone no longer suffices — the gate is Balady SSO / Nafath identity, or a MOMRAH agreement. `balady.gov.sa` itself → HTTP 200, 202 277 B. **Re-probed 2026-09-05 (curl -m 15) — STILL CREDENTIAL-GATED:** `/server/rest/services?f=json` → HTTP 200 78 B `{"currentVersion":11.5,"folders":["Hosted","umaps","Utilities"],"services":[]}`; `/umaps?f=json` → 200 62 B `{"error":{"code":499,"message":"Token Required"}}`; `/Hosted?f=json` → same 499; `umaps.balady.gov.sa` → 301. GASGI (the national survey authority) has **no resolvable host** from here: `www.gasgi.gov.sa`, `gasgi.gov.sa`, `geoportal.gasgi.gov.sa`, `portal.gasgi.gov.sa`, `ngp.gasgi.gov.sa`, `nsdi.gasgi.gov.sa`, `www.saudinsdi.gov.sa` → all `curl (6) Could not resolve host`; `umapsudp.momrah.gov.sa` → NXDOMAIN; `open.data.gov.sa` → TCP timeout 15 s (×2). **Verdict: still blocked; nothing keyless to wire** |
| **UAE · Dubai** (AE-DU) | **GEO-BLOCKED** (vantage TCP fence) | n/a | `gis.dubai.gov.ae` → NETWORK-ERROR; `www.dubaipulse.gov.ae` → NETWORK-ERROR after 10.4 s; `geodubai.dm.gov.ae` RC=28 at 21.3 s. ⭐ **The disambiguating control**: corporate `www.dm.gov.ae` answers **HTTP 200** via Azure CDN — the **data tier** is fenced, not the domain. AGOL enumerated past the viewer: org `dubaimunicipalityitd` exposes one operational-layer-less "HQ" webmap; **no official DM/DLD keyless parcel FeatureServer exists**. **Re-probed 2026-09-05 (curl -m 15) — STILL GEO-BLOCKED:** `gis.dubai.gov.ae` → `curl (6) Could not resolve host`; `www.dubaipulse.gov.ae/` → `curl (28) Connection timed out after 15013 ms`; `…/api/3/action/package_search?q=parcel` → timeout 15017 ms; `geodubai.dm.gov.ae/arcgis/rest/services?f=json` → timeout 15007 ms; `gis.dm.gov.ae` → HTTP 302 → `https://dm.gov.ae/` (corporate); control `www.dm.gov.ae` → HTTP 200 95 298 B in 0.97 s. Same shape as 09-02: the data tier is fenced, the corporate tier answers. **Verdict: still blocked; nothing keyless to wire** |
| **UAE · Abu Dhabi** (AE-AZ) | **GEOMETRY-ONLY TILES, QUERY TOKEN-GATED** (class CHANGED 2026-09-05; was "GEO-BLOCKED (F5 WAF)") | **NO** (for the query) · keyless tiles carry no id | 09-02: `data.abudhabi/opendata/data.json` → F5 `<title>Request Rejected</title> … Your support ID is: 11709459134665974889`; DKAN metastore → TCP timeout 25 s; **all five** AD-SDI hosts (`geoportal.dmt.gov.ae`, `sdi.gsec.abudhabi`, `geoportal.abudhabi.ae`, `www.abudhabimaps.ae`, `adgeospatial.gov.ae`) → HTTP 000. `sdi.abudhabi.ae` answered **HTTP 200 / 87 934 B HTML** today — a portal shell, not a service. UAE-PASS credential class **unmeasured, not assumed**. ⭐ **Re-probed 2026-09-05 (curl -m 15) — the shell has a LIVE server behind it.** `data.abudhabi/opendata/data.json` → still F5 `Request Rejected` (200, 247 B); `geoportal.dmt.gov.ae` / `sdi.gsec.abudhabi` / `www.abudhabimaps.ae` / `adgeospatial.gov.ae` → `curl (6)` NXDOMAIN. But `sdi.abudhabi.ae` → 301 → `sdi.gov.abudhabi` → 302 → `/sdi/` (200, 88 433 B) whose HTML names **`arcgis.sdi.abudhabi.ae/portal`** (Portal `portals/self` → 200, name "Abu Dhabi Spatial Data Infrastructure", `helperServices` on **`arcgis.sdi.abudhabi.ae/agshost/rest/services`** → HTTP 200 129 B, ArcGIS Server 10.9.1, folders `[adp, Basemap, GeoHUB, Hosted, SpecialOlympics, Stage, TAMM, Utilities]`, `authInfo.isTokenBasedSecurity: true`). Anonymous inventory: `GeoHUB` → 499 Token Required; `Hosted` → 200, 2 611 B, **`Hosted/Plots_AD` is a `VectorTileServer`** ("AD Plots", pbf 512 px, LOD 0–19); `Hosted/Plots_AD/FeatureServer` → **499 Token Required**; portal search `q=cadastr` → total 0, `q=parcel` → 6 Esri stock apps. **Tile fetched keylessly:** `Plots_AD/VectorTileServer/tile/14/7043/10666.pbf` → HTTP 200, 15 201 B (gzip → 22 950 B), one MVT layer **`AD_Plots_NoZayedCity`**, string table = `_symbol`, `_minzoom` only — **geometry with NO plot id and NO attributes**. Also `Hosted/ZayedCity_Master_Plots` + `ZayedCityMasterPlotPlan2023` (VectorTileServer). ⛔ **NOT WIRED, deliberately:** the point-QUERY endpoint (the `qa` leg's mechanism) is token-gated, and a leg built on the tiles would need a new MVT-decode provider class and could serve only an id-less ring — the L-616 family if presented as a cadastral parcel (C57 §id). Licence **READ 2026-09-05 (lane ME-ABUDHABI-I3S)** — the SDI Data Catalog Terms and Conditions (`sdi.gov.abudhabi/sdi/locales/en/translation.json`, 66 946 B; clauses verbatim in `tools/context-bake/__tests__/fixtures/ae-adsdi-sdi-terms-and-conditions-clauses-2026-09-05.json`) confer no licence by implication (§8.2) and forbid copying/downloading without DGE's written consent (§8.3); reuse is permitted ONLY for catalogue-classified **"Open Data"** (download, use, integrate; credit the SDI Data Catalog). The `Plots_AD` vector tiles and the `abu_dhabi_3d_city_model` I3S are **uncatalogued** portal items (licenseInfo null) → **reuse NOT permitted; the I3S height stamp is REFUSED**. The catalogue DOES classify `BUILDING` (sid 1046, DMT — `OpenData/ADSDI_OpenData/MapServer/353`, keyless, 65 714 footprints in the context bbox, `BUILDINGNUMBEROFFLOORS`, no height) and `50CM_AD_DSM_DTM` (sid 2012, DGE) as Open Data; the latter's keyless ImageServers are now the wired Abu Dhabi HEIGHT channel (`tools/context-bake/heights/abudhabiNdsm.mjs`). No cadastral/plot layer is classified Open Data (`q=cadastr` → 0; the plot tiles carry no id). **Verdict for the qa-mirror leg: still blocked, now with the licence reason** |
| **Kuwait** (KW) | **UNMEASURABLE from this vantage** | n/a | `kuwaitfinder.paci.gov.kw` → `curl (35) schannel … SEC_E_CERT_EXPIRED` — **the server's TLS certificate has expired**; `services.paci.gov.kw/arcgis/rest/services` → HTTP 302 → error page; `mapapi` / `geoportal` / `data.paci.gov.kw` NXDOMAIN. ⛔ Deliberately **not** classified absent. **Re-probed 2026-09-05 (curl -m 15) — unchanged:** `kuwaitfinder.paci.gov.kw` → `curl (35) schannel … SEC_E_CERT_EXPIRED` (still expired); `services.paci.gov.kw/arcgis/rest/services?f=json` → HTTP 302 → `https://services.paci.gov.kw/error/404`; `kuwaitportal.paci.gov.kw/arcgisportal/sharing/rest?f=json` → `curl (28)` timeout 15 005 ms. **Verdict: still unmeasurable; nothing to wire** |
| **Bahrain** (BH) | **CREDENTIAL-GATED** (national eKey) | **NO** | `slrb.gov.bh` → HTTP 200; SLRB names cadastral services routed through the **national eKey login**. Every guessable GIS host (`bahrainmaps.bh`, `gisbahrain.bh`, `gis.slrb.gov.bh`, `bsdi.bh`) NXDOMAIN. `data.gov.bh` `q=parcel` → `{"nhits": 4 …}` — all **statistics tables**, no geometry; `q=cadastral` → 0. Fee schedule **UNREAD**. **Re-probed 2026-09-05 (curl -m 15) — unchanged:** `www.slrb.gov.bh` → HTTP 200 188 953 B (portal); `www.benayat.bh` → 200 7 308 B (permits portal, login); `www.data.gov.bh/api/datasets/1.0/search/?q=parcel` → 200, `nhits: 4`, first hit `02-parcels-subdivision-regulations` (a table); `www.bahrainmaps.bh` → NXDOMAIN. **Verdict: still credential-gated; nothing keyless to wire** |
| **Oman** (OM) | **GEO-BLOCKED or WAF — UNKNOWN** | n/a | `onsdi.ncsi.gov.om` → TCP timeout 20 s (NSDI drops foreign SYNs); `housing.gov.om` — the krooki/parcel authority — → **HTTP 403 WAF**; `nsdi.gov.om` / `geoportal.gov.om` / `maps.gov.om` NXDOMAIN. Class left **UNKNOWN** rather than guessed. **Re-probed 2026-09-05 (curl -m 15) — unchanged:** `onsdi.ncsi.gov.om` → `curl (28)` timeout 15 017 ms; `www.housing.gov.om` → **HTTP 403** (83 B application/json — WAF); `data.gov.om` → 200 48 002 B (statistics portal). **Verdict: still unknown-class; nothing to wire** |
...
### B3 · Oceania (lane NZ-EVERYWHERE, probed 2026-09-05)

| Jurisdiction | Class | Do we hold the key? | Verbatim evidence 2026-09-05 |
|---|---|---|---|
| **New Zealand** (NZ) | **WIRED-KEY-PENDING** (= CREDENTIAL-GATED with the leg BUILT) | **NO** — `LINZ_API_KEY` declared (SECRETS-REGISTER), not yet set on Fly | LINZ Data Service, layer **50772 "NZ Primary Parcels"**: `services/api/v1/layers/50772/` → **HTTP 200** application/json 10 827 B — `2,796,502` features, licence **CC BY 4.0**, native `EPSG:4167` (NZGD2000), fields `id, appellation, affected_surveys, parcel_intent, topology_type, statutory_actions, land_district, titles, survey_area, calc_area, shape`. **Every service is key-gated**: WFS GetCapabilities keyless → `HTTP 401` (Jetty `Error 401 Unauthorized`, text/html 486 B) — identical with a bogus key; WMTS `WMTSCapabilities.xml` keyless → `HTTP 401`. GetFeature with a **bogus** key (`/services;key=BOGUSKEY…/wfs`, typeNames `data.linz.govt.nz:layer-50772`) → `HTTP 400 · ows:ExceptionReport InvalidParameterValue locator="typeName" · "Feature type data.linz.govt.nz:layer-50772 unknown"` — **the layer list is key-scoped, so a bad key reads as an unknown layer, never as a parcel absence**; the leg classifies both non-OK answers `unreachable`. ⭐ The leg is **built and tested** (`server/jurisdiction/euCadastreProxy.js` `nz`, the FIRST keyed leg: `requiresEnv:'LINZ_API_KEY'`; `server/__tests__/nzLinzParcelLeg.test.ts`), the client row routes Auckland/Wellington/Christchurch to `/api/parcel/nz` (`nzRegistryWiring.test.ts`). **Without the key the leg answers `HTTP 503 {parcel:null, outcome:'unconfigured'}` + `Cache-Control: no-store`** — the C57 §1.5 amendment-3 shape, never `200 {parcel:null}` — and the client falls to the honest OSM footprint. ⛔ **No parcel has been served through this leg**: the test fixture is field-verbatim from the API record with a synthetic ring, and the CQL axis order (LAT,LON — the AU-VIC/GeoServer precedent) is **unverified live**; the first keyed probe must confirm both. Key is free, self-service (`data.linz.govt.nz/my/api/`); no public demo key was found in the catalogue API and none is committed. Height: layer 101290 "NZ Building Outlines" (3,236,141 features) has **no height field**; national LiDAR 1 m DEM/DSM (121859/122082) exist but are key-gated — see `heightSources.mjs` REGION_SOURCE `newzealand` |
