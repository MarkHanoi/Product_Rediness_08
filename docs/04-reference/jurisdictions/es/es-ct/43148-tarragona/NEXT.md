# NEXT — Tarragona (INE 43148, Cataluña)
...
## 2 — BLOCKERS
- 🔴 **No rule pack** → no envelope. Unblock: source the governing instrument + zones (§LEGISLATION-RATE), author `es-43148-tarragona`, sign `sources/VERIFICATION.md` (L-449).
- 🔴 **Heights unbaked** → see `HEIGHT.md`.

## 3 — SMALLEST NEXT STEP
Enumerate the distinct zones over the 43148 extent from the competent planning authority (one query),
...
- Terrain bake row `terrain.mjs` TERRAIN_CITY `tarragona` (PNOA MDT).
