# HEIGHT — Badalona (INE 08015) — THE TRUE-HEIGHTS PLAN
...
Badalona's context buildings are baked from **OSM footprints** via the whole-country `spain` bake
...
fabricated height rather than the real skyline. **The tiles carry no MEASURED height for 08015** (see
§3 for why), so the 3D render extrudes them at an estimated/fabricated height, not their true one.
**The split for 08015 is not-queried** — it is the output of the H1 probe (§4), not a number to guess.
...
- **Grid:** native EPSG:3042 (ETRS89/UTM30N), **one projection covering all of Spain** including the
  Badalona/Barcelona metro area.
- **Provenance it yields:** `tagged` (a real MEASURED height) — a strict upgrade over Catastro's floor
  count (`derived-levels`).
- **Coverage over 08015: NOT-QUERIED.** The source is national and *exists*; whether a clean MDS sample
  resolves over Badalona's footprints has not been measured. Do NOT state a coverage %.
...
1. **The per-city sample path** (`heightSources.mjs`, `fetchSpainBuildingHeights` / `REGION_SOURCE`):
   ...
   `barcelona, madrid, cordoba, valencia, sevilla, malaga, zaragoza, bilbao`. **Badalona is NOT in it.**
2. **The whole-country OSM-footprint JOIN** (`bake.mjs` `spain` region declares `heightJoin: 'mds'` →
   ...
   intended whole-country real-height path and it *geographically* spans 08015 — but whether it has run
   in the shipped tiles and stamped clean heights over Badalona specifically is **NOT-QUERIED**.

**THE PLAN to reach true heights (LoD1 real-height render):**

1. Run the **H1** coverage probe for 08015 (measure the tagged/derived/assumed split — the
   "default-fire rate").
2. **Add Badalona's per-city bbox to the MDS height-join** — either a new per-city row in
   `heightSources.mjs REGION_SOURCE`, or confirm the whole-`spain` `heightJoin:'mds'` join already
   covers 08015 with clean samples.
   - ⚠ **The bbox is a TODO — do NOT fabricate it.** A **candidate lower bound** is the router core box
     `BADALONA_BBOX` = `2.228,41.420,2.268,41.470` (`[minLon,minLat,maxLon,maxLat]`, cited from
     `providers/badalonaBbox.ts`), **but that box deliberately UNDER-COVERS** the municipality (a tight
     proximity gate kept east of the Besòs). The true height bbox must be derived from Badalona's
     **full municipal boundary** and sanity-checked on the first bake.
...
1. **H1 — OSM coverage probe.** Measure the `tagged` + `derived-levels` vs `assumed` split for 08015's
   baked buildings. *(NOT STARTED.)*
2. **H2 — Authority height source.** CNIG MDS Edificación (`mdsn_e025`) — identified (§2); wire/confirm
   the per-08015 join (§3). *(source identified; join for 08015 NOT STARTED.)*
3. **H3 — Ordinance height (massing prism).** Badalona's own *alçada reguladora* (street-width →
...
5. **H5 — VERIFICATION sign-off.** A human confirms the source ladder + coverage for 08015 and signs
