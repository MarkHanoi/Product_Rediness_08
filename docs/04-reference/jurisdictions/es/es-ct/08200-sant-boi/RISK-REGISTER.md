# Sant Boi de Llobregat (INE 08200) — Envelope + Heights RISK REGISTER

**Purpose.** Sant Boi is ROUTED and WIRED but its envelope gate is CLOSED
(`SANT_BOI_ENVELOPE_VERIFIED = false`) and its heights are ESTIMATED. This register tracks the risks of
...
**Risk.** Someone flips `SANT_BOI_ENVELOPE_VERIFIED` to `true` (or reuses `ES_BARCELONA_ENSANCHE_PACK`
directly) to "make a demo work," stamping Barcelona's height table, FAR and citations onto 08200 land —
a confident mis-citation.
**Mitigation.** The flag is typed `boolean` and documented as a **legal act, not a code change**
(`esSantBoi.ts`); the checklist ([`ENVELOPE.md`](./ENVELOPE.md)) requires a signed
...
**Risk.** `SANT_BOI_BBOX` (2.020–2.058 E / 41.328–41.358 N) is a CONSERVATIVE core; a Sant Boi parcel
...
tight (east bank of the Llobregat, west of L'Hospitalet) to avoid the worse error of eating a Barcelona
or L'Hospitalet parcel. Fails safe-ish.
**Residual.** An edge parcel could receive a Barcelona clau answer that does not hold in 08200. Widening
...
**Risk.** `santBoiUnverifiedRefusal` uses `code:'no-rule-pack'`; a user could read it as "the ordinance
...
`SANT_BOI_ROADMAP_LINE`). Fails safe.
...
08200-specific has been probed yet** — do not treat absence as measured.

---

## Sign-off & review
- **Owner:** UNASSIGNED. **Review cadence:** before any change to `SANT_BOI_ENVELOPE_VERIFIED`, the
  router box, or the heights ingest for 08200.
