# Rate Implementation Plan — Sant Boi de Llobregat (`es-ct`, INE 08200)

**Current rate:** NOT MEASURED (see [`LEGISLATION-RATE.md`](./LEGISLATION-RATE.md)) · **Envelope gate:**
`SANT_BOI_ENVELOPE_VERIFIED = false` (cited refusal) · **Model reference:** Barcelona =
`../08019-barcelona/RATE-IMPLEMENTATION-PLAN.md` (the pilot climb this file mirrors) ·
**Last updated:** 2026-07-30 · **Owner:** UNASSIGNED

> Sant Boi does not yet have a rate to *climb* — it has a rate to *establish*. Phase 0 is the
...
Unknown until measured. Sant Boi is ordinance-bound exactly like Barcelona (the PGM-1976 *Normes* are
prose; the AMB municipalities layer their own *modificacions*), so a realistic ceiling **cannot be
assumed equal to Barcelona's ~48%** — it must be re-derived from 08200's own clau distribution and
...
| **0** | **Clau-coverage audit** — enumerate the distinct claus over the 08200 extent from the MUC; record each clau's PGM article + whether a municipal *modificació* alters it | the first honest denominator; the measured rate | **NOT STARTED** | UNASSIGNED |
| **1** | **Source Sant Boi's own height / street-width tables** (municipal *text refós*, Ajuntament de Sant Boi de Llobregat) | the height rung Barcelona's tables cannot supply | **NOT STARTED** (BLOCKED on human sourcing) | UNASSIGNED |
| **2** | **Author `es-08200-sant-boi` pack** for the most-common verified clau (or a cited per-clau equivalence ruling to the PGM construction) | the first non-refusal envelope | **NOT STARTED** | UNASSIGNED |
| **cert** | Sign `sources/VERIFICATION.md` (L-449 gate) → flip `SANT_BOI_ENVELOPE_VERIFIED` for the certified clau(s) only | moves the certified clau amber→green; re-derives the LEGISLATION-RATE.md number | **NOT STARTED** | UNASSIGNED |
| **H** | Heights: H1 probe → per-08200 MDS bbox → re-bake (see [`HEIGHT.md`](./HEIGHT.md)) | measured context heights (LoD1) instead of the 9 m assumed carpet | **NOT STARTED** | UNASSIGNED |
...
tables, and at least one authored pack (13a shipped). Sant Boi reuses Barcelona's **engine** for free
