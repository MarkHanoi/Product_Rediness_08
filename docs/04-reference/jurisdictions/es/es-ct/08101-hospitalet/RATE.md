# City RATE — master completion scorecard — L'Hospitalet de Llobregat (es-ct, 08101)
...
value; every unknown typed). L'Hospitalet is a **Phase-2 AMB refusal city**: ROUTED + WIRED, envelope
gate CLOSED (cited refusal). Legislation detail = [`LEGISLATION-RATE.md`](./LEGISLATION-RATE.md)
...
|---|---|---:|---|---|---|---|
| 1 | **PARCEL** | 15 % | `not-assessed` | `not-checked` | `not-queried` | National Catastro parcel provider IS wired (shared with Barcelona, S1). No `computeParcelConfidence` run; block-ring dissolve success **not-queried** for 08101 (`ENVELOPE.md`). |
| 2 | **LEGISLATION** | 25 % | `not-assessed` | `not-checked` | `not-queried` | [`LEGISLATION-RATE.md`](./LEGISLATION-RATE.md) is **NOT MEASURED** — no per-clau audit has been run for 08101. Borrowing Barcelona's ~48 % would be a fabrication about another municipality's land (the honesty gate in `esLHospitalet.ts` forbids it). |
| 3 | **DATA-SOURCES** | 15 % | **90%** | `not-checked` | — | 5-slot checklist: cadastre-parcel **live** (Catastro national, S1) · regional-zone-GIS **live** (Generalitat MUC, per-parcel clau, S3 — shared with Barcelona) · building-height nDSM **documented** (national CNIG MDS `mds_edificacion` exists but 08101 is **NOT** in the per-city `REGION_SOURCE` list; per-08101 join not-started) · terrain DEM **live** (`terrain.mjs` TERRAIN_CITY `lhospitaletdellobregat`, source `es` = PNOA MDT) · context-OSM **live** (`bake.mjs` REGIONS `spain`). Mean = (1+1+0.5+1+1)/5 = 0.90. |
| 4 | **ENVELOPE** | 20 % | `not-assessed` | `not-checked` | `pending-implementation` | ROUTED + WIRED (S1–S5 present, `esLHospitalet.ts`) but the honesty gate `LHOSPITALET_ENVELOPE_VERIFIED = false` → `lhospitaletUnverifiedRefusal` (`code:'no-rule-pack'`) for **every** parcel. This is a PRYZM verification-status statement, NOT law (the PGM-1976 does grant an envelope). Barcelona's tables do NOT transfer. See [`ENVELOPE.md`](./ENVELOPE.md). |
| 5 | **TERRAIN** | 10 % | **50%** | `not-checked` | — | Terrain bake row present: `terrain.mjs` TERRAIN_CITY `lhospitaletdellobregat` (source `es` = PNOA MDT). Rung **50 = baked-but-unverified** — no `terrain.verify.mjs` round-trip recorded; same rasant caveat as Barcelona (L-584). |
| 6 | **HEIGHTS/LOD** | 10 % | `not-assessed` | `not-checked` | `not-queried` | Heights ESTIMATED (OSM `levels`×3.2 m or the 9 m `assumed` default); the split for 08101 is **not-queried** (H1 probe output). The CNIG MDS source exists nationally but 08101 has **no per-city bbox configured** + no confirmed join. See [`HEIGHT.md`](./HEIGHT.md). |
