# HEIGHT — L'Hospitalet de Llobregat (INE 08101) — THE TRUE-HEIGHTS PLAN
...
L'Hospitalet's context buildings are baked from **OSM footprints** via the whole-country `spain` bake
...
fabricated height rather than the real skyline. **The tiles carry no MEASURED height for 08101** (see
§3 for why), so the 3D render extrudes them at an estimated/fabricated height, not their true one.
**The split for 08101 is not-queried** — it is the output of the H1 probe (§4), not a number to guess.
...
- **Coverage over 08101: NOT-QUERIED.** The source is national and *exists*; whether a clean MDS sample
  resolves over L'Hospitalet's footprints has not been measured. Do NOT state a coverage %.

## 3 — Why the measured height is not on the tiles today, and THE PLAN

There are two MDS ingest paths in the ES pipeline, and **neither is confirmed to stamp measured heights
over 08101**:
...
1. **The per-city sample path** (`heightSources.mjs`, `fetchSpainBuildingHeights` / `REGION_SOURCE`):
   ...
   resolves exactly** — but the ready per-city bbox list in `heightSources.mjs` is
   `barcelona, madrid, cordoba, valencia, sevilla, malaga, zaragoza, bilbao`. **L'Hospitalet is NOT in
   it.** So no 08101-specific MDS sample is configured.
2. **The whole-country OSM-footprint JOIN** (`bake.mjs` `spain` region declares `heightJoin: 'mds'` →
   ...
   intended whole-country real-height path and it *geographically* spans 08101 — but whether it has run
   in the shipped tiles and stamped clean heights over L'Hospitalet specifically is **NOT-QUERIED**.

**THE PLAN to reach true heights (LoD1 real-height render):**

1. Run the **H1** coverage probe for 08101 (measure the tagged/derived/assumed split on the city's
   baked buildings — the "default-fire rate").
2. **Add L'Hospitalet's per-city bbox to the MDS height-join** — either as a new per-city row in
   `heightSources.mjs REGION_SOURCE`, or by confirming the whole-`spain` `heightJoin:'mds'` join
   already covers 08101 with clean samples.
   - ⚠ **The bbox is a TODO — do NOT fabricate it.** A **candidate lower bound** is the router core box
     `LHOSPITALET_BBOX` = `2.085,41.335,2.125,41.385` (`[minLon,minLat,maxLon,maxLat]`, cited from
     `providers/lhospitaletBbox.ts`), **but that box deliberately UNDER-COVERS** the municipality (it
     is a tight proximity gate kept clear of central Barcelona). The true height bbox must be derived
     from L'Hospitalet's **full municipal boundary** and sanity-checked on the first bake.
...
1. **H1 — OSM coverage probe.** Measure the `tagged` + `derived-levels` vs `assumed` split for 08101's
   baked buildings. Honest number, per city. *(NOT STARTED.)*
2. **H2 — Authority height source.** CNIG MDS Edificación (`mdsn_e025`) — identified (§2); wire/confirm
   the per-08101 join (§3). *(source identified; join for 08101 NOT STARTED.)*
3. **H3 — Ordinance height (massing prism).** L'Hospitalet's own *alçada reguladora* (street-width →
...
5. **H5 — VERIFICATION sign-off.** A human confirms the source ladder + coverage for 08101 and signs
