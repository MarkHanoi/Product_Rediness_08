# RATE-IMPLEMENTATION-PLAN — Santa Cruz de Tenerife (INE 38038)
...
| P2 | TERRAIN (10) | run `terrain.verify.mjs` round-trip for `santacruztenerife` → lift rung 50→100 | decoder pass |
| P3 | HEIGHTS/LOD (10) | wire an MDS join for this bbox + probe | `tagged` fraction |
| P3b | CONTEXT (5) | add a `canarias` bake row (outside the `spain` clip) | 206 + non-empty |
| P4 | PARCEL (15) | run `computeParcelConfidence` over an N-parcel sample in the 38038 bbox | sample distribution |
| P5 | LEGISLATION (25) | source the governing instrument + zones → cite `sources/SOURCES.md` → sign `VERIFICATION.md` | L-449 |
| P6 | ENVELOPE (20) | author `es-38038-santa-cruz-de-tenerife` rule pack once P5 lands | C58 certifiability |
