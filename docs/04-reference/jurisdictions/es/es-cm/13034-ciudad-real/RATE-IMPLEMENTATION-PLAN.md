# RATE-IMPLEMENTATION-PLAN — Ciudad Real (INE 13034)
...
| P2 | TERRAIN (10) | run `terrain.verify.mjs` round-trip for `ciudadreal` → lift rung 50→100 | decoder pass |
| P3 | HEIGHTS/LOD (10) | wire an MDS join for this bbox + probe | `tagged` fraction |
| P4 | PARCEL (15) | run `computeParcelConfidence` over an N-parcel sample in the 13034 bbox | sample distribution |
| P5 | LEGISLATION (25) | source the governing instrument + zones → cite `sources/SOURCES.md` → sign `VERIFICATION.md` | L-449 |
| P6 | ENVELOPE (20) | author `es-13034-ciudad-real` rule pack once P5 lands | C58 certifiability |
