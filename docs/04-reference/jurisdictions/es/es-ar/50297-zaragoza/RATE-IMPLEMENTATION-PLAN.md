# RATE-IMPLEMENTATION-PLAN — Zaragoza (INE 50297)
...
| P2 | TERRAIN (10) | run `terrain.verify.mjs` round-trip for `zaragoza` → lift rung 50→100 | decoder pass |
| P3 | HEIGHTS/LOD (10) | land the per-city MDS re-bake + probe the provenance histogram | `tagged` fraction |
| P4 | PARCEL (15) | run `computeParcelConfidence` over an N-parcel sample in the 50297 bbox | sample distribution |
| P5 | LEGISLATION (25) | source the governing instrument + zones → cite `sources/SOURCES.md` → sign `VERIFICATION.md` | L-449 |
| P6 | ENVELOPE (20) | author `es-50297-zaragoza` rule pack once P5 lands | C58 certifiability |
