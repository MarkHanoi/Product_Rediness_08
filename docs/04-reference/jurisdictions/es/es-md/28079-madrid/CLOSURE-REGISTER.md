...
| **13** | **NZ 3 — 60.458 % of the city: the ordinance INTENTIONALLY states no municipal envelope** | **A** | **P1** | EXACT *(a refusal publishes no number)* | ✅ **LEGALLY CLOSED 2026-08-02 (L-678) — Art. 8.3.1 GOVERNS. A DETERMINATION, NOT A GAP.** | ⚠⚠ **THIS ROW CORRECTS ITSELF FROM YESTERDAY, AND THE CORRECTION IS THE POINT.** On 2026-08-01 (`717bee6e`) I re-bucketed NZ 3 **A → A+B**, *"DATA-gated, not law-gated"*, on Art. 8.3.5.3.a).i) alone. **That was WRONG** — it read one sub-sub-paragraph of an *obras* article as a zone-envelope grant, without reading its own section heading («**Sección Primera: Condiciones de edificación del Grado 1º**» / «Artículo 8.3.5 **Obras admisibles**») or the four other places in the same chapter that anchor every quantity to the building that already stands. **Re-bucketed back to A, and CLOSED.** **THE VERDICT, from the filed primary source:** Art. 8.3.1 — «*se ha **agotado el aprovechamiento urbanístico***» … «*asume la consolidación … **sin imponer un nuevo modelo***» (p. 397). Art. 8.3.5 does not contradict it; it PRESUPPOSES an existing building and adopts **that building's own envelope and built area** as the measure — 8.3.1 restated operationally. **Five verbatim anchors, no free parameter anywhere in the chapter:** 8.3.5.1 «*la superficie edificada total del edificio existente*»; 8.3.5.3.a).i) «*la envolvente exterior del edificio existente … respetará la superficie total edificada del mismo*»; the *Estudio de Detalle* route «*No sobrepasar la edificabilidad permitida, entendida ésta como la superficie edificada total del edificio existente*»; 8.3.5.4 «*salvo cuando el planeamiento antecedente hubiese establecido expresamente una situación vinculante*»; and **8.3.3.1.b)** «*los suelos vacantes tendrán la consideración de **espacios libres SIN APROVECHAMIENTO URBANÍSTICO***». Grado 2º routes identically (8.3.2.3, «*conforme a las referidas ordenaciones específicas*»). The ONLY new-build route on vacant NZ-3 land (8.3.5.3.b) is «*con carácter excepcional*», for infrastructure and garages, «***en edificación enteramente subterránea***». ⇒ **There is no NZ-3 parcel for which the PGOUM-97 states a buildable volume of its own.** Entitlement comes from the antecedent instrument. **`madridNZ3Refusal` is correct, legally grounded and FINAL**, and `not-determined` is the right C63 score (§1.5 / L-656) — **60.458 % of Madrid ANSWERED, permanently.** ⚠ **THE GEOMETRY FOUND MUST NOT BE BOUND.** `PG_ANALISIS_EDIFICACION` layer 2 *«Fondo máximo…»* is real (1 785 features) but covers **≈4.9 %** of the municipality, and layer 12 *«Fondo»* is **ONE feature spanning the whole term** — a cartographic background, not a *fondo edificable* (row 19). Under **ADR-0283** *("…or **legally insufficient** … PRYZM returns Unknown rather than inferring entitlement")* **a legal refusal is not curable by geometry, however well published.** ⚠ **AND THE L-616 TRAP SURVIVES THE VERDICT:** 8.3.5.3.a).i) does let a substituted volume borrow NZ 5 / NZ 8 grado 4º *ocupación y altura* — beneath the existing-GFA ceiling quoted above. Packing that without the ceiling is mechanism-A across 60 % of Madrid. **REMAINS OPEN, and it is now one sitting's work:** a human signing Cap. 8.3 — *(i) the refusal is correct* and *(ii) the rules remain unknown*, as **two separate statements** — plus the **BOCM texts of MPG 00/335 (19.05.2016) and MPG 00/343 (27.11.2023)**, which amend the very articles this verdict rests on and are **not held**. Full evidence: [`findings/L-678-NZ3-ARTICLE-VERDICT-AND-LAYER-EXTENTS.md`](./findings/L-678-NZ3-ARTICLE-VERDICT-AND-LAYER-EXTENTS.md). |
...
| **19** | **The `PG_ANALISIS_EDIFICACION` geometry is NOT city-wide, and one layer is not what its name says** | **A** | P2 | ⚠ would have **OVER-STATED coverage ~12×** had extent been assumed | ✅ **MEASURED AND CLOSED 2026-08-02 (L-678)** | The ADR-0283 precondition — *prove the extent before claiming any coverage* — run as `returnCountOnly` + `returnExtentOnly`, EPSG:25830, all HTTP 200. **Layer 2 «Fondo máximo para nueva planta o reestructuración general»: 1 785 features over 4.70 × 6.24 km ≈ 4.9 % of the ~604 km² municipal area.** Layers 8/9 (*Condiciones de protección*, *Otras condiciones*): same ~4.7 × 6.3 km central window — consistent with **APE.00.01** scoping, NOT city-wide. ⭐ **Layer 12 «Fondo»: COUNT = 1**, one polygon spanning the entire municipal envelope (31.3 × 36.8 km) ⇒ it is the cartographic **background/mask**, not *fondo edificable* — which is why it sits beside *Cartografía*, *Malla 2000* and *Línea de Término*. **The "layer 12 is the fondo published as geometry" hypothesis dies on a count of 1.** ⚠ `returnDistinctValues` returned `[]` for `TIPOANEDIF` / `PROTECCION` / `PLANO_AE` / `ENLACE` on layers declaring those fields with `supportsDistinct: true` — that is **UNKNOWN (a query-interface limit), NOT "the fields are empty"**; no conclusion is drawn from it. **Consequence:** even had the law permitted an NZ-3 envelope, Doctrine B would have capped the claim at **≈5 %** of Madrid, never 60 %. Layer 2 remains a genuine lead for **NZ 4** (row 7), on its own extent. |
