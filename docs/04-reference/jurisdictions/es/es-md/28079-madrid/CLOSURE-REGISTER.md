...
| **13** | **NZ 3 — 60.458 % of the city: the refusal is CORRECT, and it is now DATA-gated, not law-gated** | ~~A~~ → **A+B** | **P1** | EXACT *(a refusal publishes no number)* | 🟠 **TERMINAL, UNSIGNED — and RE-BUCKETED 2026-08-02 on a PRIMARY-SOURCE read (L-677)** | `madridNZ3Refusal` ships **today**, gate-independent and `legallyGrounded: true`. **⚠ THE REGISTER PREVIOUSLY CALLED THIS "the only structurally impossible category" — that is now shown to be WRONG, and the correction matters because it is 60.458 % of Madrid.** Cap. 8.3 was read **from the primary document** (now in-repo, row 15), not from the machine summary. **Art. 8.3.5.3.a).i), verbatim:** «*La nueva edificación deberá inscribirse dentro de la **envolvente exterior del edificio existente**, constituida por sus cubiertas, fachadas exteriores y, en su caso, medianeras, y respetará la **superficie total edificada del mismo***» (PDF p. 399, printed 397). ⇒ **NZ 3 DOES state a computable envelope for its only new-construction pathway (*obra de sustitución*): the envelope of the building that already stands, capped at its existing GFA.** The rule is stated, bounded and citable; what PRYZM lacks is the **datum**, and that is a DATA problem with a named source. **⚠ PROBE P3 RUN 2026-08-02 — MEASURED NEGATIVE.** `PGOUM97/PG_ANALISIS_EDIFICACION/MapServer` is **live (HTTP 200, 13 layers)**, the published *Plano de Análisis de la Edificación*. But layer **4 `Edificaciones`** is `esriGeometryPolyline` with **`OBJECTID` as its ONLY field** — no storeys, no height, no built area, not even a polygon. Same for layer **2 `Fondo máximo para nueva planta o reestructuración general`** (polyline, `OBJECTID`) and layer **12 `Fondo`** (**polygon**, `OBJECTID`). It is a CARTOGRAPHIC drawing, not an attribute table — the identical shape as V18's `PG_ORDENACION/4`. ⇒ *«la superficie total edificada del edificio existente»* **cannot be obtained from the municipal plane.** Catastro carries built area per refcat and is the obvious next candidate; that join has not been attempted. **⚠⚠ AND HERE IS THE TRAP, WHICH IS THE MOST DANGEROUS SENTENCE IN THE MADRID DOSSIER.** Art. 8.3.5.3.a).i) *also* says the substituted volume «*podrá variar su posición respetando los parámetros correspondientes establecidos por la **Norma Zonal 5** para edificación aislada y por la **Norma Zonal 8, Grado 4º**, para la edificación en vivienda unifamiliar*», and the *Estudio de Detalle* route lets it «*acogerse a la **ocupación y altura** de este grado*». **Read alone, that looks like an invitation to pack NZ 3 with NZ 5 / NZ 8 numbers and book 60.458 % of Madrid.** It is not. Every one of those borrowings sits under an explicit governing condition stated immediately above it: «*– **No sobrepasar la edificabilidad permitida, entendida ésta como la superficie edificada total del edificio existente***». **Applying NZ 5 grado 2º's ocupación and altura WITHOUT the existing-GFA ceiling is the L-616 mechanism-A over-statement, across 60.458 % of the city** — a 2-storey villa would be issued a NZ-5 bulk envelope. **A SOLID must intersect ALL derived constraints (founder, L-616); an envelope that ignores one ceiling is worse than no envelope.** ⇒ the cited refusal is the CORRECT answer and scores correctly as `not-determined` (C63 §1.5). **Closes when** (i) a human signs Cap. 8.3 — *the refusal is correct* and *the rules remain unknown* as **two separate statements** — and (ii) a per-parcel existing-GFA + existing-envelope source is found and joined. ⭐ Still the highest verified-land-per-reading-hour item in the dossier: **one chapter, already extracted AND now re-readable in-repo, for 60.458 % of Madrid's zoned land.** |
