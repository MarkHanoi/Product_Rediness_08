...
|---|---|---|
| **`PG_ANALISIS_EDIFICACION`** | existing building footprint / floors / volume | ✅ **PROBED 2026-08-02 (P3) — LIVE, HTTP 200, 13 layers, and it carries NO ATTRIBUTES.** Layer 4 `Edificaciones` = polyline, `OBJECTID` only; layer 2 `Fondo máximo para nueva planta o reestructuración general` = polyline, `OBJECTID` only; layer 12 `Fondo` = **polygon**, `OBJECTID` only. It is the published *Plano de Análisis de la Edificación* — a DRAWING, not a table (same shape as V18). ⇒ it **cannot** supply Art. 8.3.5.3.a).i)'s *«superficie total edificada del edificio existente»*, which is the datum NZ 3 (60.458 % of the city) needs. Catastro built-area is the next candidate, untried. Prior text follows: **NET-NEW, UNPROBED.** If real it changes the product question from *"what may be built on an empty parcel"* to *"what may be done given what already stands"* — new build vs extension vs rehab vs replacement |
