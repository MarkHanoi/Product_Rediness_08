...
| ⭐ **Evidence — #2 (a): ONE municipal answer to "what does `altura` encode?"** (R5). **This is now the whole of #2** | **one email + a reply** | Low, but not ours to schedule |
| Engineering — #2 (b) the spatial join | ✅ **DONE 2026-08-01** | — |
| Engineering — #2 (c) `altura` unit-discriminating parser ⚠ **worthless until (a) lands, and STILL insufficient alone: `profundidad edificable` is published nowhere** | ~2–3 days | Medium |
...
| **R5** | ⭐ **Municipal confirmation of what `MapServer/212.altura` encodes** — is it Art. 6.19.1's *número de plantas grafiado en el Plano C*, and does it store the graphed count or Np (= count − 1)? | ⚠ **PROMOTED — ASK NOW.** The join measured the prize at **52,6 % of buildable land**, ~2× what the register assumed, and #2's engineering half is done. This one question IS the rest of the row. ⚠ Even a *yes* unlocks at most half: `profundidad edificable` is published nowhere | Servicio de Planeamiento |
