# RATE-IMPLEMENTATION-PLAN — Sevilla (INE 41091)
...
| P2 | TERRAIN (10) | run `terrain.verify.mjs` round-trip for `sevilla` → lift rung 50→100 | decoder pass |
| P3 | HEIGHTS/LOD (10) | land the per-city MDS re-bake + probe the provenance histogram | `tagged` fraction |
| P4 | PARCEL (15) | run `computeParcelConfidence` over an N-parcel sample in the 41091 bbox | sample distribution |
| P5 | LEGISLATION (25) | source the governing instrument + zones → cite `sources/SOURCES.md` → sign `VERIFICATION.md` | L-449 |
| P6 | ENVELOPE (20) | author `es-41091-sevilla` rule pack once P5 lands | C58 certifiability |
