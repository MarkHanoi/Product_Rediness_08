...
> regression.
>
> ### ⛔ RETRACTED 2026-08-04 — THE "41 OF 49 CUS SHEETS ARE DEAD" AND "NO ALIGNMENT LAYER EXISTS"
> ### FINDINGS (THIRD PASS, ABOVE) WERE BOTH WRONG. WRONG URL PATH, NOT WRONG CONCLUSION.
>
> The third-pass six-probe search (line 24 above, and `findings/MACHINE-READABLE-SOURCE-SEARCH-
> 2026-08-02.md`) tested `https://visor.pgou.coacordoba.org/doc/planos/cus/CUS{NN}W.jpg` — a
> **different domain** from the municipality's live GIS host. That domain path is dead. It is not
> the only path. **The correct, currently-live path is
> `https://www.gmucordoba.es/documentos/Gerencia_de_Urbanismo/imagenes_planos/planos/cusw_jpg/
> CUS{NN}W.JPG`** (uppercase `.JPG`, zero-padded 01–49) — live-verified this session (2026-08-04)
> and then **all 49 of 49 sheets bulk-fetched and saved**, including three (CUS01, CUS33, CUS49)
> this file's own dead-list explicitly named as dead. See
> [`corpus/MANIFEST.md`](./corpus/MANIFEST.md) for full provenance and per-file sizes.
>
> **The alignment ("alineaciones y rasantes") finding is retracted on the same grounds.** Row 25
> below concluded (Part 3) that a `/aline\|rasant\|.../i` sweep of both publishers' WFS/WMS
> inventories returns zero alignment layers. That sweep tested machine-readable vector *services*
> (WFS/WMS) — it did not test the GMU's own static document tree, which publishes 49
> pre-rendered PDF sheets at `https://www.gmucordoba.es/documentos/Gerencia_de_Urbanismo/
> imagenes_planos/planos/PDF_ar/ar{NN}.pdf`. **All 49 of 49 have now been bulk-fetched** (see
> `corpus/MANIFEST.md`). **This does NOT reopen Part 3's narrower claim** — no WFS/WMS vector
> alignment *service* has been found, so `w=2A/P` street-polygon derivation is still forbidden
> and MC's height-band measurement basis is still unresolved (Part 1/2 of row 25 stand). What
> changes is that a **published, citable alignment SOURCE now exists to georeference and read
> Art. 13.5.3.1's measurement basis FROM**, where before this pass concluded none existed at all.
> Vectorising/georeferencing these 49 PDFs into a usable layer is separate, larger follow-up work,
> **not done as part of this correction** — see blocker 22 and row 25 below, both individually
> corrected in place.
>
> ### ⬆ 2026-08-04, LATER THE SAME DAY — MC HEIGHT CLOSED, FOOTPRINT NOW THE SOLE MC BLOCKER
>
> Full arc: [`findings/SESSION-SUMMARY-2026-08-04.md`](./findings/SESSION-SUMMARY-2026-08-04.md).
> Same day, same pattern as the retraction above: a THIRD wrong-URL finding was caught and fixed —
> `idecordoba:manzana` (the block-ring layer blocker 8/25 needed) was cited at `idecordoba.
> cordoba.es`, which does not resolve. The real host is **`ide.cordoba.es`**, confirmed live,
> `totalFeatures: 20,730`. A resolver (`resolveCordobaStreetWidth.ts`) was built against it,
> primary over a pre-existing (previously undiscovered by this dossier) Catastro-dissolve
> fallback, and wired into dispatch. **MC's height blocker (row 25 / blocker 8, 16.86 % of pilot
> buildable land) is CLOSED.** MC still does not render an envelope — the footprint ground
> (`CORDOBA_MC_FONDO_UNRESOLVED_RING`) is untouched and independently still refuses, confirmed by
> test. MC's remaining blocker is now singular, not dual. PT-CV (`PTC`) was also registered this
> session (ocupación packed, height stays a verified `null` — a per-parcel graphical map, not a
> missing document). See the session summary for the full corpus/blocker table.
...
## ⭐ THE COVERAGE LOSS MATRIX — 100 % of the non-envelope land, in four buckets

> Standing rule (founder, 2026-08-02). **Córdoba is the city where this matters most, because
> "ENVELOPE 0.0 %" hides four completely different causes with four different owners** — and for
> three of the four the answer is *correct*, not missing. Denominator: **SUELO URBANO
> 33 341 928.156 m²** (SIU, in force). Derivation:
> [`findings/LAYER2-GEOMETRY-RECOVERY-2026-08-02.md`](./findings/LAYER2-GEOMETRY-RECOVERY-2026-08-02.md).

| Bucket | Share | What it is | Owner · exit |
|---|---:|---|---|
| **① Legally impossible** | **2.962 %** | 2.185 % delegated to a Plan Parcial / PERI / ED / Plan Especial · 0.667 % lucrative `usos_globales` (48/48 carry an `actuacion`) · 0.109 % legally-grounded refusals (Arts. 13.4.1 · 13.3 · 13.12.2) | ⛔ **nobody — permanent.** A cited refusal IS the answer (C63 §1.5 / L-656) |
| **② Data unavailable** | **94.448 %** | no machine-readable calificación — the **41 unpublished CUS sheets** | **founder** · GMU serves them or refuses in writing (**D-002**) |
| **③ Engineering not yet implemented** | **1.702 %** | PGOU-direct **and** packed, blocked only by blockers **3** + **4** | **the Córdoba agent** · resolver called, UAD depth packed |
| **④ Awaiting authoritative interpretation** | **0.888 %** | 0.880 % **MC** — Art. 13.5.3.1's measurement basis is unread and no alineación is published (row **25**) · 0.008 % **UAS** — `O_UAS1.pdf` is a 69-byte dead page | **COACo / GMU** · publish alineaciones or state the basis; restore the document |
| **Σ** | **100.000 %** | | |

⚠ **THE ONE CATEGORY MIGRATION THIS PASS, WITH ITS EVIDENCE** (the standard demands both):
**MC's 0.880 % moved ③ → ④.** Blocker 8 sat in Engineering on the belief that a width source merely
*"must be CONSTRUCTED"*. Measured: **no alineación layer exists at either publisher**, the ordinance's
measurement basis is **unreadable from the served PDF** (vector paths, no text layer), and the only
candidate proxy fails ADR-0285 part 3 outright and would be voided on 22–45 % of streets by ADR-0287.
**It is not ours to build.** No other row migrated.

⚠ **③ did not shrink, it got CHEAPER — and that is the whole Layer-2 result.** Every geometric input
blockers 3 and 4 need is now confirmed supplied: parcel (95 %), parcel area (5 721/5 725), **block ring
(⭐ `idecordoba:manzana`, 88.0 % of ordenanza land, published geometry — row 26)**, front alignment
(setback is *zero*, so it needs adjacency, not a published alignment line), and the UAD depth (a stated
scalar). **③ is now pure code with no data dependency.** It is **not** a coverage gain: the ENVELOPE
axis stays a **measured 0.0 %**.
...
| **8** | MC per-street-width height table | ⬆ **Awaiting authoritative interpretation** *(was Engineering — migrated 2026-08-02 with evidence, row 25)* | **GMU / COACo** | alineaciones published, **or** Art. 13.5.3.1's measurement basis stated |
| **26** | block ring from published municipal blocks | **Engineering** | the Córdoba agent | `idecordoba:manzana` bound as primary, dissolve as fallback |
...
| **22** | ~~41 of 49 urban CUS sheets are unpublished~~ ⛔ **RETRACTED 2026-08-04 — all 49 are published, at a different URL path than tested; all 49 now fetched, see `corpus/MANIFEST.md`.** Remaining work is raster→vector, i.e. **Engineering**, not Data acquisition. | ~~Data acquisition~~ **Engineering** | ~~the founder (request to GMU)~~ the Córdoba agent | ~~GMU serves the 41 sheets, or answers that it will not~~ all 49 sheets georeferenced + vectorised into calificación polygons, bound through the resolver |
...
| **26** | ⭐ **The block ring is RECOVERED — on published geometry, not a reconstruction** | **B** | UNDER-states | **P2** | 🔵 **NEW 2026-08-02 — Layer-2 win** | `idecordoba:manzana` publishes **20 730 city blocks municipality-wide**, and **421 of 453 ordenanza polygons (92.9 %) = 88.0 % of pilot ordenanza land** fall inside one (measured live, centroid test, EPSG:4326 axis order verified). ⚠ **This outranks our own cadastral dissolve under ADR-0283**: it is *published* geometry rather than a ring we reconstruct, and it covers the whole municipality rather than the pilot. Keep `dissolveParcelsToBlockRing` (Catastro 76.9 % / COACo 88.5 %) as the **fallback** where no manzana is published — the two are complementary. **Closes when:** the resolver takes `idecordoba:manzana` as its primary block-ring source with the dissolve behind it. |
| **25** | ⛔ **Can `ide.cordoba.es` street layers drive the MC height table? — TESTED, NO** | **A** | n/a | **P1** | ✅ **CLOSED 2026-08-02 AS TESTED-AND-NEGATIVE — do not re-open** | Probed under **ADR-0285's four-part test** ([`findings/LAYER2-GEOMETRY-RECOVERY-2026-08-02.md`](./findings/LAYER2-GEOMETRY-RECOVERY-2026-08-02.md)). **Part 1 HOLDS** (Art. 13.5.3.1 states the criterion, bands transcribed). **Part 2 UNVERIFIED** — the dossier never recorded *what the width is measured between*, and `O_MC2.pdf` cannot be read: it is **3 230 vector-path ops, zero text ops, zero image XObjects** (⚠ which also corrects `OCR-EXTRACTION-RESULTS.md §1`, filing the MC docs as *"clean-raster"* — they are **vector**). **⛔ PART 3 FAILS, and it is fatal:** an exhaustive `/aline\|rasant\|retranq\|fondo\|frente\|fachada/i` sweep of **both** publishers' complete inventories (ide.cordoba.es 105 WFS + 119 WMS, COACo 15) returns **ZERO alignment layers**. ⚠ **This is precisely where Córdoba differs from Murcia** — SIG-MU2 worked because Murcia publishes `Murcia:pgou_alineaciones` and its PGOU measures *between alineaciones*. Córdoba publishes none. `sup_viales` is the municipal **callejero** (`ine_via`, `dgc_via`, `competenci`, `fuente`) — the **physical street surface**, a different object from a legal alignment; substituting it is derived **law**, forbidden by **ADR-0284**. **And the proxy was measured anyway:** `w=2A/P` over all 6 529 street polygons gives a plausible-looking median **9.12 m** — the trap — but MC's bands are only 2 m apart, so **45.4 % of streets sit within ±1 m of a band edge** (±0.5 m → 22.5 %, ±2 m → 64.3 %) and **ADR-0287 would refuse them all**. `max = 5 836 m` also shows some features are whole street networks, not frontages. ⇒ **MC's 16.86 pp is NOT ours to build.** It migrates to **④ Awaiting authoritative interpretation** (owner: GMU/COACo — publish alineaciones, or state the measurement basis).  ⚠ **PARTIAL CORRECTION 2026-08-04 — “Córdoba publishes none” was too broad; Part 3's WFS/WMS-service scope was correct but incomplete.** Part 3 swept machine-readable **services** only. The GMU also publishes 49 static PDF *Alineaciones y Rasantes* sheets (`ar01.pdf`…`ar49.pdf`) at `https://www.gmucordoba.es/documentos/Gerencia_de_Urbanismo/imagenes_planos/planos/PDF_ar/` — live-verified and **all 49 bulk-fetched** this session, see [`corpus/MANIFEST.md`](./corpus/MANIFEST.md). **This does not overturn the row's verdict**: no vector WFS/WMS alignment layer exists, `w=2A/P` derivation from `sup_viales` is still forbidden (ADR-0284), and Art. 13.5.3.1's measurement basis is still unconfirmed pending someone reading these 49 PDFs against Part 2's open question. What changes is the row's exit criterion — “GMU/COACo publish alineaciones” is **partially met** (a published, citable source now exists to read the measurement basis from); “or state the measurement basis” remains open and is the narrower, now-only, remaining ask. |
| ~~**25-old**~~ | ~~"A Córdoba street-width source exists"~~ | — | — | — | ⛔ **RETRACTED 2026-08-02 — superseded by row 25.** | The lead (`ide.cordoba.es` serving `sup_viales`/`red_viaria`/`tramo_vial`) was real and was recorded as *"the cheapest remaining ENVELOPE point"*. **It did not survive testing.** Row 25 has the ADR-0285 evidence. Kept as one line so the optimism is on the record next to its refutation, and so nobody re-files it as a new discovery. |
