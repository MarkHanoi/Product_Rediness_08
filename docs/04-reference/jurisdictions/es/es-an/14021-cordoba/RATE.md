...
|---|---:|---|---|---:|---|---|
| LEGISLATION | 25 | `not-assessed` (sub-rate ~8 %) | ⬆ **NO LONGER `not-composable` — MEASURED 2026-08-01.** The census was run: `coaco:ordenanzas` = **453 polygons / 10 distinct `ordenanza` families / 15 distinct `link` documents / Σ 1 628 616 m²**, and PRYZM holds transcribed rules for **5 of the 10 families**, resolving **94.33 %** of ordenanzas land to a packed subzone code (residue 5.67 %: COMERCIAL 2.28 · PTC 1.31 · **bare `O_MC` 1.14** · EP 0.67 · UAS-1 0.17 · INDUSTRIAL 0.12). The **municipal denominator** comes from the national SIU `OGC_Clases_Suelo` (INE 14021, all in force): **SUELO URBANO 33 341 928 m²**, developable 72 396 079 m², municipal term 1 254.3 km². ⇒ published calificación = **4.88 % of SUELO URBANO** | remainder is publisher-gated | `STRUCTURAL-DATA` | ⚠ **94.33 % is a ROUTING fraction, not an ANSWER fraction** — after delegation + the MC height table it is ≈ 31 % / ≈ 16 % of pilot buildable land, ≈ 1.7 % / ≈ 0.9 % municipality-wide. See [`CLOSURE-REGISTER.md`](./CLOSURE-REGISTER.md) |
