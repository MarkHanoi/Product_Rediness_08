# City RATE — master completion scorecard — Málaga (es-an, 29067)
...
| 5 | **TERRAIN** | 10 % | **50%** | `not-checked` | — | Terrain bake row present: `terrain.mjs` TERRAIN_CITY `malaga` (source `es` = PNOA MDT, §ES-ALL-CAPITALS L-636). Rung **50 = baked-but-unverified** — no `terrain.verify.mjs` round-trip nor deployed `layer.json` 200 independently re-probed in this audit. |
| 6 | **HEIGHTS/LOD** | 10 % | `not-assessed` | `not-checked` | `not-queried` | MEASURED-CAPABLE: `heightSources.mjs` maps this city to `mds_edificacion` (impl:`live`, per-city ready bbox listed for `malaga`). But no baked-PMTiles `heightProvenance` histogram has been probed and the per-city MDS re-bake is not confirmed landed (heights currently OSM/assumed) → score not-assessed pending a bake + provenance probe. |
