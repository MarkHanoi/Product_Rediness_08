...
**Murcia is comparable to Barcelona (62.8 % delegated), not clearly better.** But 33 % of 75.1 km²
remains a real prize, and it is exactly what `esMurciaPgou2012.ts` transcribes.
...
- 🟠 **No Murcia street-width / frontage-class source.** Blocks `RC`, `RM` (base), `RN`, `RD1`'s
  third storey, `MZ`'s FAR and `MX`'s frontage rule — ~4 % of buildable land, one resolver.
