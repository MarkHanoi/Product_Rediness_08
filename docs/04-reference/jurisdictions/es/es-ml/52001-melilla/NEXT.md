# NEXT — Melilla (INE 52001, Ciudad Autónoma de Melilla)
...
## 2 — BLOCKERS
- 🔴 **No rule pack** → no envelope. Unblock: source the governing instrument + zones (§LEGISLATION-RATE), author `es-52001-melilla`, sign `sources/VERIFICATION.md` (L-449).
...
Enumerate the distinct zones over the 52001 extent from the competent planning authority (one query),
...
- Terrain bake row `terrain.mjs` TERRAIN_CITY `melilla` (PNOA MDT).
