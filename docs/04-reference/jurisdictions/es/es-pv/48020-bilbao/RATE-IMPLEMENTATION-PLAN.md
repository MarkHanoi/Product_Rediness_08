# RATE-IMPLEMENTATION-PLAN — Bilbao (INE 48020)
...
| P2 | TERRAIN (10) | run `terrain.verify.mjs` round-trip for `bilbao` → lift rung 50→100 | decoder pass |
| P3 | HEIGHTS/LOD (10) | land the per-city MDS re-bake + probe the provenance histogram | `tagged` fraction |
| P4 | PARCEL (15) | run `computeParcelConfidence` over an N-parcel sample in the 48020 bbox | sample distribution |
| P5 | LEGISLATION (25) | source the governing instrument + zones → cite `sources/SOURCES.md` → sign `VERIFICATION.md` | L-449 |
| P6 | ENVELOPE (20) | author `es-48020-bilbao` rule pack once P5 lands | C58 certifiability |
