# City RATE — master completion scorecard — Bilbao (es-pv, 48020)
...
|---|---|---:|---|---|---|---|
| 1 | **PARCEL** | 15 % | `not-assessed` | `not-checked` | `not-queried` | National Catastro parcel provider IS wired (`parcelProviders/registry.ts` `isInSpain`→`catastro`, cadastral · keyless · live), but Axis 1 measures the parcel-quality distribution over an N-parcel sample and **no `computeParcelConfidence` run has been executed** for this bbox (C63 §8). ⚠ FORAL cadastre: national Catastro does NOT serve this municipality — Basque Country / Navarra keep their own *foral* cadastres (`es/priority_318.csv` region_tier `T3-FORAL-CADASTRE-BLOCKER`), so a Catastro route misses here and falls to the OSM footprint. |
...
| 5 | **TERRAIN** | 10 % | **50%** | `not-checked` | — | Terrain bake row present: `terrain.mjs` TERRAIN_CITY `bilbao` (source `es` = PNOA MDT, §ES-ALL-CAPITALS L-636). Rung **50 = baked-but-unverified** — no `terrain.verify.mjs` round-trip nor deployed `layer.json` 200 independently re-probed in this audit. |
| 6 | **HEIGHTS/LOD** | 10 % | `not-assessed` | `not-checked` | `not-queried` | MEASURED-CAPABLE: `heightSources.mjs` maps this city to `mds_edificacion` (impl:`live`, per-city ready bbox listed for `bilbao`). But no baked-PMTiles `heightProvenance` histogram has been probed and the per-city MDS re-bake is not confirmed landed (heights currently OSM/assumed) → score not-assessed pending a bake + provenance probe. |
...
DOES: terrain (PNOA MDT, unverified) + national Catastro parcel routing + baked OSM context (5/9 layers) + measured-height CAPABILITY (MDS, unbaked). REFUSES: an envelope (no rule pack) — never a borrowed/invented number. UNKNOWN (typed): PARCEL quality (`not-queried`), LEGISLATION + ENVELOPE (`pending-implementation`), HEIGHTS (`not-queried`); cadastre foral-blocked. `honestyOk: true`.
