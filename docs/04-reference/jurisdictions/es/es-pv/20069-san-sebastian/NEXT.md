# NEXT — Donostia / San Sebastián (INE 20069, País Vasco)
...
## 2 — BLOCKERS
- 🔴 **No rule pack** → no envelope. Unblock: source the governing instrument + zones (§LEGISLATION-RATE), author `es-20069-san-sebastian`, sign `sources/VERIFICATION.md` (L-449).
...
Enumerate the distinct zones over the 20069 extent from the competent planning authority (one query),
...
- Terrain bake row `terrain.mjs` TERRAIN_CITY `sansebastian` (PNOA MDT).
