# NEXT — Burgos (INE 09059, Castilla y León)
...
## 2 — BLOCKERS
- 🔴 **No rule pack** → no envelope. Unblock: source the governing instrument + zones (§LEGISLATION-RATE), author `es-09059-burgos`, sign `sources/VERIFICATION.md` (L-449).
- 🔴 **Heights unbaked** → see `HEIGHT.md`.

## 3 — SMALLEST NEXT STEP
Enumerate the distinct zones over the 09059 extent from the competent planning authority (one query),
...
- Terrain bake row `terrain.mjs` TERRAIN_CITY `burgos` (PNOA MDT).
