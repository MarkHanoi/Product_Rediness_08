# NEXT — Odense (kommune 0461, Region Syddanmark)
...
- 🔴 **No terrain bake row** → `terrain.mjs` registers only `copenhagen`; add a odense bbox row to bake DHM
  terrain here.
- 🔴 **L-449 VERIFICATION PENDING** (national) → gates LEGISLATION + ENVELOPE `human-reviewed` (C63 §1.6).

## 3 — SMALLEST NEXT STEP
Run the byzone click-weighted dimensional-fill probe scoped to the 0461 bbox (reuse the L-609 harness) —
