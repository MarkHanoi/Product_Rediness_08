...
| **Zaragoza** | BLOCKED — no rule pack exists; path now substantially engineering-only for A1/3.1, A1/3.2, A1/4.1, A1/4.2 (2026-08-03 transcription) | 0% | 5% | 60% | 40% | Arts. 4.1.1/4.1.2/4.1.3/4.1.12/4.1.13/4.1.15/4.1.17 transcribed VALIDATED (verbatim, page-cited, 2024 texto refundido). **Correction to `esAragon.ts`'s own prior note**: depth is NOT graphic for these 4 subgrados (670 of 7,967 calificación polygons) — it's a flat scalar (15m upper-floor cap, 7.50m floor, Art. 4.1.3), confirmed by independently checking every candidate polygon layer (no Valencia-style movement-polygon exists). Height: 3.1/3.2 reuse existing `streetWidth.ts` unchanged (band table needed, ~2 bands + travesía case); 4.1/4.2 height is a flat B+2/10m constant, no measurement needed. Grado-1/2 (A1/1, A1/2 — the remaining ~40% of A1) still needs the graphic `Textos_Altura_Edificable` point-join + Roman-numeral parser + Art. 4.1.8's H=5+(n−1)×3.30 formula (98% of 7,856 labels are bare Roman numerals, VALIDATED full census) — a materially different, unbuilt pipeline. | For Grado 3/4 only: write a Zaragoza band table (~50-100 LOC, reusing `streetWidth.ts`), a flat-depth `alignment` rule (~50 LOC, ADR-0270 shape, NOT `blockDerivedDepth.ts`), registry wiring. No new geometry primitive needed. Then a founder decision to build `esZaragoza.ts` and sign it — same L-449 shape as every other jurisdiction. |
