...
| **Canarias** | BLOCKED — sentinel semantics fixed and LANDED (`8019d622`) | 5% | 60% | 60% | 30% | Signature only for the grammar/parser layer — `T`/`TP`/`TS` now correctly route to alignment-depth (was a real bug: silently discarded on every such row), `COM`/`GRF` citations corrected against the VALIDATED primary codebook (Manual ITPU-SIPU v02, 2012 — "SIPU 2.6.A" never existed, that string doesn't appear in the manual at all). **Vigencia CONFIRMED not automatable (2026-08-03)**: exhaustively checked GeoBDP (HTML-only, self-labeled "no vinculante"), IDECanarias WMS (no WFS, `GetFeatureInfo` confirmed live-returns zero results, empty bounding boxes — administratively dead for this purpose), Registro de Planeamiento (submission form + PDF only, explicitly disclaims legal validity), new Portal de Planeamiento (dashboard only, no independent data). No stable key links any of these to SIPU's CKAN packages — only fuzzy free-text date+BOC-citation matching, which produced an unresolved citation mismatch during the investigation itself. This is a genuine founder-policy question, not a research gap. `EDIF_L` shapefile ingestion (GRF-tagged rows point at structured GIS geometry PRYZM doesn't read yet) remains a separate, unsized engineering task | Founder ruling on the 46 multi-instrument municipalities (routing policy, not automatable) + `EDIF_L` shapefile ingestion (engineering) + eventual signature |
...
| **Huesca** | BLOCKED, correctly (refusal STRONG-to-VALIDATED, doubly exhaustive) | 5% | 20% | 30% | 20% | alignment — genuinely unpublished. `gis.huesca.es` re-tested 2026-08-03 with 3 independent unreachability signals (sandbox timeout, separate WebFetch infra ECONNREFUSED, zero Wayback/Google crawl history — likely geo-fenced to Spanish/EU consumer ISPs). New finding: even if reached, the one candidate layer (`/POLIGONOSPGOU1980`) is the SUPERSEDED 1980 plan, not current PGOU 2008 — reaching it would not have closed this regardless. Bespoke Leaflet SPA, not a standard OGC/ArcGIS stack. National IDEE CSW catalogue has zero Ayuntamiento de Huesca service records. | A Spain-based residential/mobile IP test is the one remaining realistic step — and even that would need to find a DIFFERENT, current-PGOU layer, not just reach the host |
| **Málaga** | BLOCKED — registry empty, but corpus is real and READ (2026-08-03) | 0% | 0% | 60%+ | 30% | **EXTERNAL AUTHORITY, not engineering.** `muralPGOU:POLCALIF_T` (its GIS layer) returns `ORA-28000: la cuenta está bloqueada` — the Oracle account backing the datastore is locked, confirmed with a control (3/3 unrelated layers served rows same run, 0/8 muralPGOU layers did). This is NOT Córdoba's blocker-shape (dead infra vs. Córdoba's live-but-unsigned). Rule corpus is actually MORE complete on paper than Córdoba's: altura 90% VALUE, retranqueo 60%, alineación 70% — and PDFs are natively text-extractable (12/12 readable), so Málaga needs NONE of Córdoba's OCR/raster-verification pipeline. CTP has a direct match to Córdoba's pattern: "Profundidad Máxima Edificable... 15 metros" (Art. 12.2.14) — same `GeometricRule.alignment` shape as Córdoba's CTP-1 (16m, Art. 13.8.2.4), different city, different number. MC defers to PERI/Estudio de Detalle — same delegation shape as Córdoba's `actuacion` override. | Nothing PRYZM can do until the Oracle account unlocks (someone else's infrastructure). Once unlocked: a Málaga-specific subzone-key resolver (new code, different filename convention, different field names — Córdoba's resolver does NOT port) |
