...
- **ADR-0063** `docs/02-decisions/adrs/ADR-0063-house-generative-layout-doctrine.md` — the house doctrine
  (stair-as-room, hall-singleton, the storey-loop reuse).
- **ADR-0061** `docs/02-decisions/adrs/ADR-0061-building-graph-bidirectional-edit-substrate.md` — purity /
...
| `§DIAG-RECTIFY-PROJECT` | `runDeterministicLayout.ts` (after `emitGeometry`, idx 0) | **The §RECTIFY-SHELL-PROJECT health line (v196, 2026-06-13).** Per shipped candidate: `rectifyFired` (did §RECTIFY-QUAD trigger → bbox tiling + projection active?), `projected` (count of interior endpoints moved onto the real shell), `maxResidual` (m — the largest distance to the real shell of any interior endpoint STILL on the bbox edge after projection). A non-zero `maxResidual` is the open seam that floods RoomDetection (merged rooms); `0.000m ✓` = every perimeter partition reached the shell. The first place to confirm whether a rotated/sheared merge is mechanism (C) — and whether the shell even rectifies. |
...
| **ADR-0061** | `docs/02-decisions/adrs/ADR-0061-building-graph-bidirectional-edit-substrate.md` | The bidirectional-edit substrate + the byte-identical-baseline (I2) invariant. |
...
| `§RECTIFY-PROJECT-CAP` | `rectDecomposition.ts:234` (`maxMoveM`) | **(v196, 2026-06-13) The fix that made §RECTIFY-SHELL-PROJECT actually close strongly-sheared seams.** The projection cast is AXIS-PERPENDICULAR inward; its move was rejected past a fixed `maxMoveM=3.0 m` sized for the NEAREST-point gap (~2.2 m). The PERPENDICULAR gap is larger (measured 4.75 m on a 0.78-fill parallelogram) → the move was rejected → the endpoint stayed ~2.3 m off the shell → open seam → room merge. The cast already returns the nearest forward ring hit (the correct near edge for a convex quad however far), so the cap is purely a degeneracy guard → now sized to the bbox extent (`max(bboxW, bboxH)`). Proven by `§DIAG-RECTIFY-PROJECT` 2.294 m → 0.000 m + `rectShellProject.test.ts` (>3 m gap closes). Only convex-quad shears (fill ≥ 0.5) reach it → byte-identical elsewhere. |
...
**Canonical home: [C53 §13](../02-decisions/contracts/C53-GENERATIVE-LAYOUT-ENGINE-ARCHITECTURE.md).** Governing ADR: [0067](../02-decisions/adrs/ADR-0067-graph-ir-intent-first-building-graph-bim3.md) (intent-first building graph); concretizations in flight: [0068 §FG7](../02-decisions/adrs/ADR-0068-five-graph-model-circulation-first-building-graph.md) (circulation-first) + [ADR-0126](../02-decisions/adrs/ADR-0126-graph-authoritative-room-identity-at-execution.md) (graph-authoritative rooms; renumbered from 0069).
