...
> **Status**: ratified 2026-04-29.  Single-page synthesis of `06-PRYZM-IDENTITY-AND-RECOUNT`, `08-VISION`, `09-AS-IS-VS-TO-BE`, `10-MASTER-IMPLEMENTATION-PLAN-36M`, `12-BIM-2-AND-3-POST-GA-ROADMAP`, `01-TARGET-ARCHITECTURE`, `02-ORCHESTRATION`, `CONFLICT-ANALYSIS`, all 41 ratified ADRs (+ 4 queued: ADR-0241..044), all 39 SPECs, and the 25 chunks under `phases/audits/PRYZM2-WIREUP-PLAN-S72/`.
>
...
adrs/ADR-0201..044   +   specs/SPEC-01..48 + SPEC-FAMILY-EDITOR               ← Tier 6
...
```
   ...
   ║  · cost meter, prompt pinning, BYOK custody (ADR-0238), spend limits (ADR-0214)       ║
   ╚════════════════════════════════════════════════════════════▲════════════════════════╝
   ...
   ║  · sandboxed (ADR-0209) · marketplace-distributable (apps/marketplace-*)             ║
   ╚════════════════════════════════════════════════════════════▲════════════════════════╝
                                                                │
   ╔════════════════════════════════════════════════════════════╧════════════════════════╗
   ║  L5  VIEW STATE & VISIBILITY-INTENT  (the verbatim PRYZM 1 system, re-housed)       ║
   ║  packages/view-state · visibility · picking · stores                                ║
   ║  · 11-wave VI engine (ADR-0215 places L4/L5/L7 split)  · per-element soft-locks      ║
   ...
   ║  · 264 Command handlers · ULID-stamped · MessagePack-encoded events (ADR-0204)       ║
   ...
   ║  · event log + CRDT bridge (ADR-0202) · MessagePack wire (ADR-0204) · R2 (ADR-0203)    ║
   ║  · soft-lock semantics (ADR-0219) · migration & rollback (SPEC-27)                   ║
   ╚════════════════════════════════════════════════════════════▲════════════════════════╝
   ...
   ║  · per-element robustness budget (ADR-0220) · runs in workers (Comlink-wrapped)      ║
   ╚════════════════════════════════════════════════════════════▲════════════════════════╝
   ...
   ║  · WebGPU primary, WebGL2 fallback (ADR-0206) · three.js 0.183 pinned (ADR-0225)      ║
   ║  · ONE rAF · ONE canvas · 58 legacy rAF owners → 1 by Phase G (ADR-0223)             ║
   ╚════════════════════════════════════════════════════════════▲════════════════════════╝
                                                                │
   ╔════════════════════════════════════════════════════════════╧════════════════════════╗
   ...
   ║  · beta-signup · ai-host (substrate) · OTel + Tempo (ADR-0207)                       ║
   ╚═════════════════════════════════════════════════════════════════════════════════════╝
```
...
- L1 owns the **single** rAF and the **single** canvas.  No other layer owns either.  Any library that wants its own rAF (Cesium / OBC) is rAF-quarantined per ADR-0223.
...
│   │   ├── api-rbac/                     role/permission matrix · ADR-0211
│   │   ├── oauth2-pkce/                  browser auth · ADR-0221
│   │   ├── rate-limit/                   token-bucket · per-tier
│   │   ├── webhooks/                     outbound delivery · retries
│   │   ├── perf-budgets/                 17 NFT bench targets · ADR-0207
...
│   │   ├── constraint-solver/            SPEC-48 · ADR-0224
...
│   │   ├── visibility/                   11-wave VI engine · ADR-0215
│   │   ├── picking/                      hit-testing · selection-set algebra
│   │   └── stores/                       27 typed stores · zero global state
│   │
│   ├── L6  Plugin SDK
│   │   ├── plugin-sdk/                   contribution interfaces · sandbox · ADR-0209
...
│       └── ai-spend/                     spend limits · ADR-0214
│
├── plugins/                              ← 38 PLUGINS
...
│   ├── ai-worker/                        LLM gateway · BYOK · ADR-0238
...
│   ├── adrs/ADR-0201..ADR-0244             44 ratified/queued
...
│
└── docs/operations/                      v2 candidate cuts (ADR-0218 Tier 1/2/3)
```
...
|---|---|---|---|
| L0 | `packages/api-spec/src/openapi.yaml` | api-spec | ADR-0221, SPEC-15 |
| L0 | `packages/perf-budgets/src/budgets.ts` | perf-budgets | 17 NFTs from `08-VISION §6` |
| L1 | `packages/frame-scheduler/src/Scheduler.ts` (the **single** rAF) | frame-scheduler | ADR-0222, ADR-0223, ADR-0225 |
| L1 | `packages/render-runtime/src/RuntimeBackend.ts` | render-runtime | ADR-0222 Part B |
| L2 | `packages/geometry-kernel/src/Producer.ts` (`produce<T>(dto, ctx) → IR`) | geometry-kernel | SPEC-01, ADR-0220 |
| L2 | `packages/scene-committer/src/Committer.ts` | scene-committer | SPEC-21 |
| L3 | `packages/file-format/spec.md`  +  `…/Codec.ts` | file-format | Tier 2 authority + ADR-0204 |
| L3 | `packages/persistence-client/src/Manifest.ts` + `EventReader.ts` | persistence-client | SPEC-02, ADR-0213 |
| L3 | `packages/sync-client/src/SyncSession.ts`  +  `SoftLock.ts` | sync-client | SPEC-03, ADR-0202, ADR-0219 |
...
| L4 | `packages/types-builtin/src/Catalog.ts` | types-builtin | ADR-0217 |
| L5 | `packages/visibility/src/VIEngine.ts` (the 11 waves) | visibility | ADR-0215, SPEC-FAMILY-EDITOR |
| L5 | `packages/stores/src/store.ts` (typed store contract) | stores | SPEC-24 |
| L6 | `packages/plugin-sdk/src/Contribution.ts` | plugin-sdk | ADR-0209, SPEC-09 |
| L6 | `plugins/<each>/plugin.json`  +  `index.ts` (manifest + entry) | each plugin | SPEC-09 |
| L7 | `packages/ui/src/RuntimeBinding.ts` | ui | ADR-0226 (vanilla TS, Path A) |
| L7 | `src/ui/**` (untouched) | n/a | `06-PRYZM-IDENTITY §1` |
| L7.5 | `packages/ai-host/src/AIHost.ts` | ai-host | ADR-0214, SPEC-07, SPEC-28 |
| L7.5 | `packages/ai-host/src/ApprovalQueue.ts` | ai-host | ADR-0214 |

Every contract above has at least one property test (SPEC-11).  Every contract has at least one OTel span (ADR-0207).
...
export interface ComposeOptions {
  ...
  sandbox: SandboxPolicy;              // ADR-0209
...
│      → calls constraint-solver (ADR-0224) for joins/openings/dimension chains      │
...
**Latency budget** (per ADR-0207 + perf-budgets): from `MouseUp` to first frame containing the moved endpoint is **≤ 80 ms p95** at the medium-fixture scale (1 K walls).  Each layer has its own OTel span; `pnpm bench` regression-checks this on every PR.
...
**ADR-0202 bridge rule**: the event log is the **source of truth**; the Yjs CRDT is a **transport and conflict-resolver**.  Snapshots are derivative and rebuildable from any point in the event log.  R2 chunks are derivative of snapshots.
...
─── BROWSER PROCESS ────────────────────────────────────────────────────────────────
  Main thread (UI)
    ├── frame-scheduler (THE rAF — ADR-0223)
    ├── packages/ui adapter
    ├── src/ui/ React render
    ├── command-bus dispatch
    └── stores subscriptions (typed signals)

  Worker pool (size = hardwareConcurrency - 1, ADR-0205)
...
─── SERVER PROCESSES (per tenant region — ADR-0237) ─────────────────────────────────
  apps/sync-server      Yjs WebSocket + soft-lock TTL daemon (ADR-0219, ADR-0221)
  apps/ai-worker        LLM gateway (BYOK custody — ADR-0238); cost meter persists to PG
...
─── INFRASTRUCTURE ─────────────────────────────────────────────────────────────────
  ...
  OTel/Tempo            traces + metrics (ADR-0207)
```

---

## §10  Deployment topology (per ADR-0212, ADR-0221, ADR-0231, ADR-0237)
...
```
                   ...
                   └─────────────┬───────────────────────────────┘
                                 ...
                                 ▼
        ┌───────────────────────────────────────────────────────────────┐
        │ Edge / Region (default: closest to tenant — ADR-0237)          │
  ...
  Self-host minimum (ADR-0212): one box, docker-compose with all five apps + PG + MinIO.
  Enterprise (ADR-0221):       SSO (SAML/OIDC) + SCIM + audit-log streaming + BYOK.
```
...
T0+800 first paint (medium fixture)  — gate: ≤ 1 s p95 (ADR-0207 + 04-PRODUCTION-PARITY §5)
...
**Soft-lock blast radius** (ADR-0219): the lock covers the **single element** being edited (the wall), not the level, not the project.  TTL 30 s, refreshed on every dispatch.  If User B tries to edit the same wall, the L4 `canDispatch` check returns false and the UI surfaces a "User A is editing this wall" badge.
...
**Invariant** (ADR-0214): step 6 — AI **never** writes to stores, **never** mutates events, **never** bypasses RBAC.  Every AI mutation is auditable as a normal user-dispatched Command, attributed to the AI agent via the actor field on the event envelope.
...
debounce 250 ms (ADR-0210, per-element) →  flush snapshot delta
...
The plugin SDK enforces (ADR-0209):
...
| Q4 | M10–M12 | S19–S24 | Bake worker + alpha (PHASE-1D) | Alpha gate — ADR-0218 T1 cuts |
...
| Single rAF + library quarantine | ADR-0222, ADR-0223, ADR-0225 |
| Persistence + CRDT bridge | ADR-0202, ADR-0213, SPEC-02 |
| Visibility-Intent placement | ADR-0215 + `packages/visibility` |
| AI as L7.5 (above the stack) | ADR-0214, SPEC-07, SPEC-28 |
| Plugin sandbox & marketplace | ADR-0209, SPEC-09, `apps/marketplace-*` |
| The white UI binding rule | this doc §16 + ADR-0226 |
| Customer migration (PRYZM 1 → 2) | **ADR-0244 (queued)** + chunk 25 §25.5 |
