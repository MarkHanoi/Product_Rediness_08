# ADR-0230 — Lifecycle Subsystem Placement
...
| Element instantiation hooks (`onCreate`, `onUpdate`, `onDelete`) | per-family handlers + `[strategic ADR-0202]` event log; plugins subscribe via `extension_points` per SPEC-09 |
...
- **Project-wide validation passes**: e.g., "after every commit, run the structural-loadpath validator if structural elements are present." Per `[strategic ADR-0220]` robustness budget.
...
- **Not the rendering scheduler**. That's `packages/render-runtime/` per ADR-0222.
...
- `pryzm.lifecycle.cross_family_check.<rule>.failed` (counter; > threshold = release blocker per ADR-0220).
...
- S31 — ADR-0230 land (Phase 2B start; Phase 2A holds no gap-closure); cross-family rule list audited from legacy `src/lifecycle/`; `plugins/lifecycle/` skeleton + first three rules ported.
