...
- **Authors**: main agent
- **Strategic anchors**:
  - `[strategic ADR-0214]` — AI L7.5 operational placement
  - `[strategic ADR-0206]` — idle budget (sub-µs no-op tracer when no SDK is set)
  - `[strategic ADR-0218]` — Tier-1 cut list (T1.7 + T1.8 stay cut per S47 D9 retro)
  - `SPEC-28` §4 — Cloudflare Worker relay for Anthropic
  - `ADR-0228` Part E — per-workspace AI Spend view + workspace-admin override (S65, 3C)
...
The AI subsystem in PRYZM 1 is 31 files of LLM orchestration, CV pipeline, generative workflows, rules engine, and voice input. Per `[strategic ADR-0214]` the AI host runs at architectural layer **L7.5** — above the editor shell (L7), below the user (L8). The cold-start contract is:
...
**Per spec D3** the approval-queue UI reads `pendingCount()` for the sidebar count badge. **Per SPEC-28 §4 + ADR-0228 Part E** the per-project budget enforcement and the per-workspace AI Spend view live SERVER-SIDE; this store is the client-side projection only. Workspace-admin override for plan/role per ADR-0228 Part E ships at **S65 (3C)** unchanged in S47.
...
### §2.6 — Cut-list checkpoint (S47 D9 retro per `[strategic ADR-0218]`)
...
| Per-project budget enforcement + AI Spend view | **S65 (3C)** per ADR-0228 Part E | Server-side; workspace-admin override + plan/role ratified at S65 unchanged. |
