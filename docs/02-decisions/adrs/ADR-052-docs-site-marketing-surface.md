...
3. **Pricing page never drifts from entitlements**: `pricing.astro` imports `@pryzm/entitlements` at build time and re-runs on every push. Marketing content sync is automatic.
4. **404 build no longer breaks**: pre-existing Zod 4 / Starlight 0.30 incompatibility is bypassed.
