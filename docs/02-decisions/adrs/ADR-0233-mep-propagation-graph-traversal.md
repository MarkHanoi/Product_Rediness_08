# ADR-0233 — MEP System Propagation: Graph Traversal (not Constraint Solver)
...
2. **Constraint solver** (e.g. integration of `[strategic ADR-0224]` solver) — sizing is a global-optimisation problem.
...
- Composes with worker pool (per ADR-0205) trivially.
...
- **Hybrid (traversal-then-refine)** — deferred to Phase 7; the hybrid composes naturally with ADR-0224 solver.
