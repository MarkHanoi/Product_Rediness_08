# ADR-0217 — Element Type Catalog Scope
...
- **Custom curtain-wall mullion families with arbitrary profiles** — needs constraint solver (per ADR-0224) which is post-GA-only at the v1 cut.
...
- Persists via the event log (`type.create.v1`, `type.update.v1`, `type.delete.v1`, `material.*.v1`) like any other L1 entity (per ADR-0202 / ADR-0213).
...
- S49–S54 — Component Editor for loadable families (Phase 3A; depends on ADR-0224 constraint solver).
