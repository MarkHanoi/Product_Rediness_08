# ADR-0256 — Supabase Auth migration (Phase A.5 of ADR-0255)

| Field | Value |
|---|---|
| Status | **ACCEPTED 2026-06-02** — sequenced AFTER ADR-0255 Phase A close; implementation begins Sprint 4 |
| Owner | Platform / server-infrastructure · `@MarkHanoi` |
| Supersedes (in part) | **C08 §1.1** (auth identity issuance — the "custom JWT only" clause) · **ADR-0245 §3** (the JWT mechanism row in the split-backend table) |
...
| Defers to | **ADR-0255** (parent — one PRYZM hosting architecture) · **C48 §1.1** RTO/RPO targets (Supabase Pro PITR remains the floor) |
...
| Contracts amended (follow-up PR, not this one) | C08 §1.1, §1.3 · ADR-0245 §3 superseded text block |

---

## Context

PRYZM today runs a hand-rolled auth stack: bcrypt password hashing (`server/authStore.js:28` — `BCRYPT_ROUNDS = 12`), HMAC-SHA256 JWT issuance against `SESSION_SECRET` (`server/authStore.js:99, 135, 183, 219`), a 30-day token TTL (`server/authStore.js:29` — note: **C08 §1.1 declares 7 days; the implementation is at 30 days**; this divergence is closed by §3 of this ADR), and a manual PKCE OAuth2 flow for Google + Microsoft (`server/oauthService.js:155-231`). User-identity rows live in Supabase (`pryzm_users` table) via the service-role REST client (`server/supabaseClient.js:68-119`) per **ADR-0245**.
...
2. **No SSO (SAML, OIDC).** Customer IT departments at the C4 enterprise tier (`docs/01-strategy/STR-09-personas.md`) gate purchase on SSO integration; building this against our own JWT issuer is months of work.
...
- We already run Supabase (`server/supabaseClient.js`) for the `pryzm_users` row store per ADR-0245.
...
This ADR sequences the migration as **Phase A.5 of ADR-0255**, deliberately AFTER Phase A close so the cutover does not block the apex/app split production deploy. Phase A ships with the custom JWT path unchanged; ADR-0256 implementation begins Sprint 4 once production is stable.
...
**This is the most important shape constraint in the migration**: `SESSION_SECRET` is NOT retired by ADR-0256. It survives indefinitely for `EntitlementBundle` signing because:
...
The new redirect URI is **per-Supabase-project**, not per-app-environment. Supabase Auth then forwards to whichever `redirectTo` we pass in `signInWithOAuth`, which honours `app.pryzm.so` for production and `<branch>.pryzm-staging.pages.dev` for branch previews per ADR-0255 §4.
...
Supabase Auth defaults to **localStorage** for client session persistence. For PRYZM at `app.pryzm.so` (per ADR-0255 §0 apex/app split), localStorage is acceptable on Phase A but **httpOnly cookies are required** for the long-term Pages-Functions deploy in Phase C per ADR-0255 §3.
...
The cookie cutover is a separate operation from the JWT cutover; it ships in Sprint 5 (ADR-0255 Phase C boundary) and reuses the dual-stack pattern (read Supabase cookie OR Supabase localStorage token, accept either).
...
- [ ] **Zero users locked out during cutover** — verified in staging branch preview (ADR-0255 §4) by signing in as 5 test users immediately before Sprint 4.3 flip, then verifying they can still issue API calls 1 hour, 7 days, and 29 days later without re-sign-in.
...
- [ ] **Discrepancy resolved: token TTL** — C08 §1.1 says "7 days, configurable via `SESSION_SECRET_TTL`"; `authStore.js:29` says `'30d'`. ADR-0256 §1 fixes this on the new path: 1h access token + 30d rotating refresh token, matching Supabase Auth defaults. C08 amendment in the follow-up PR adopts these values.
...
- Branch-preview testing (ADR-0255 §4) tests against BOTH JWT paths on every PR.
...
- **ADR-0255** — Parent. One PRYZM hosting architecture. This ADR is Phase A.5.
- **ADR-0245** — Mixed-Auth Architecture. ADR-0256 §3 supersedes the "JWT issuance" row of the ADR-0245 split table. The "user identity in Supabase" row survives (now via `auth.users` + `public.profiles` instead of `public.pryzm_users`).
...
- **2026-06-02** — Authored. Sequenced AFTER ADR-0255 Phase A close. Implementation begins Sprint 4.
