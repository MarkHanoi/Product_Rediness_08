...
| Extends | [ADR-046](./ADR-046-instanced-mesh-coalescing.md) (instancing), [ADR-006](./ADR-006-default-render-mode.md) (WebGPU default), [ADR-022](./ADR-022-renderer-topology-backend-runtime.md), [ADR-057](./ADR-057-realtime-geometry-and-view-interactivity.md) |
...
Under **WebGPU** (now the default boot backend per ADR-006 / C04 §1.4 once the P2
...
| 4 | **ADR-046 coalescer is scoped to curtain walls only.** `InstancedMeshCoalescer` merges per-wall CW `InstancedMesh` by `(levelId, geo, mat)` post-batch; it does not touch the per-element fragment meshes that make up the mesh-count bulk. | partial coverage | `packages/scene-committer/src/InstancedMeshCoalescer.ts` |
...
- Axis 3 geometry fingerprinting can mis-group visually-distinct shapes that share a
  ...
  default-off flag with a visual-diff check (ADR-006 P10 corpus).
...
- **C04 §1.4 / §3.5 / ADR-006:** WebGL2 fallback stays first-class; LOD/quality
