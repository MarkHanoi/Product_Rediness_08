...
> verification, two zero-`git-blame` sprints, ADR-0218 cut-list status) cannot
...
4. ADR-0218 hasn't fired Tier-3 T3.5 (date slip).
...
- Gate 4: ADR-0218 cut-list state at the time of the physical deletion is
...
- **S70 (90 days later)** — physical deletion of `src/engine/EngineBootstrap.ts`
  ...
  ADR-0218 status known, parity bench history accumulated).
...
- The 90-day window aligns with ADR-0221 enterprise customer comms (no
  surprise deletion).
