# ADR-0201 — Pascal Adoption Strategy

| Field | Value |
|---|---|
| Status | **Accepted** — 2026-04-27 |
| Closes | `05-IMPLEMENTATION-PLAN.md §17` row ADR-0201 |
...
- The **type/material schema shape** (Contract 17), expanded per ADR-0217.
...
- Centralised per-frame cache invalidation patterns — replaced by L1 store subscriptions and chunk-level bake (ADR-0210).
- The Pascal renderer pipeline — replaced by ADR-0206 (WebGPU-when-available, WebGL2 fallback).
- Pascal's persistence model — replaced by event-log + R2 chunks (SPEC-02, ADR-0213).
...
- S11 — Type/material schema (per ADR-0217) lands; supersedes Contract 17.
