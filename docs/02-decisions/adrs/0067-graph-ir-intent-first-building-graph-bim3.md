...
> **North-star realisation (2026-06-16):** the founder's 7-layer intent-first engine brief — *a building IS a set of relationships; geometry is one realisation derived last* — is documented canonically in **[C53 §13](../contracts/C53-GENERATIVE-LAYOUT-ENGINE-ARCHITECTURE.md)** (7 layers + the binding gap-leverage order). This ADR is the governing graph-IR; [ADR-0068 §FG7](0068-five-graph-model-circulation-first-building-graph.md) (circulation-first invariants) + [ADR-0069](0069-graph-authoritative-room-identity-at-execution.md) (graph-authoritative rooms) are its first concretizations.
