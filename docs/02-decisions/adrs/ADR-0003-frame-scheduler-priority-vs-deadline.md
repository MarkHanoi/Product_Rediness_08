# ADR-0203 — Frame scheduler priority vs deadline API
...
| `idle`        | only inside idle budget (ADR-0206) | TRAA accumulation, BVH refit, post-effects |
...
- We can layer a deadline on top later (e.g. `idle` lane gets an explicit
  budget per ADR-0206) without breaking the API.
...
(0..30, ADR-0206), and `pryzm.frame.idle_throttled` (boolean).
