# ADR-064 — Open-old-project: ceiling restore mapping + WebGL2 view-unstick (OBC loop hardening)
...
- **References:** `§OPEN-OLD-CEILING-RESTORE`, `§WEBGL2-VIEW-UNSTICK`, ADR-061 (WebGL2
  render-on-move — the frame-scheduler park/wake mechanism), ADR-060 / ADR-0084
...
after 30 idle frames per ADR-006) so `RenderPipelineManager.render()` repaints on the WebGL2
