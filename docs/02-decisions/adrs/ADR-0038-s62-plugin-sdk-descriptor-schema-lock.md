...
2. **Cross-origin isolation** — `<iframe sandbox="allow-scripts">` (no
   ...
   by Safari per [strategic ADR-0207]).
3. **Browser support parity** — every browser PRYZM targets per
   `[strategic ADR-0207]` ships the iframe `sandbox` attribute with the
...
- `[strategic ADR-0207]` browser support matrix (sandbox attribute parity)
- `[strategic ADR-0209]` plugin sandbox audit gate (the D7 audit referenced
  in Decision D)
