...
**Related:** [ADR-0058](./ADR-0058-unified-building-graph.md) (UBG — specialised graphs are projections of one node/edge model), [ADR-0060](./ADR-0060-living-design-parameters.md) (Living Design Parameters bind to existing substrate, not a parallel scorer — the global-slider sibling of this per-node decision).
...
> **Numbering note:** the next free number above both ADR series (`0060` is taken by Living Design Parameters; `ADR-0257`/`ADR-0055A` are the 3-digit/sub-letter strategic entries) is **0061**, in the 4-digit code-level series per the README §2 going-forward rule.
