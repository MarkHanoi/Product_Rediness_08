# ADR-0239 — Export Worker Architecture (Sheets + Schedules)

| Field | Value |
|---|---|
| Status | **Accepted** — 2026-04-28 (retroactive for S40 + S42) |
| Closes | `phases/PHASE-2C-Q3-M19-M21-SHEETS-SCHEDULES.md` §3.1 row "ADR-0226 Export worker architecture" (the spec referenced an as-yet-unallocated number — ADR-0226 was meanwhile spent on `ui-binding-vanilla-ts`; this ADR carries the export-worker decision under the next free number) |
...
The phase-2C spec (§0, §S40 D1, line 39, §5 line 1082) calls for an `apps/export-worker/` BullMQ worker following the same pattern as the bake worker (ADR-0205). At commit time of this ADR, the implementation has converged on a slightly different layout — see *Decision* below — but the spirit (server-side capable, headless, dependency-isolated to a few well-bounded packages) is preserved.
