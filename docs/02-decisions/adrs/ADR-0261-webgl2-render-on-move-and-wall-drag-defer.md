# ADR-0261 — WebGL2 continuous render-on-move & wall-drag rebuild deferral
...
- **References:** [C04](../contracts/C04-RENDERING-AND-SCHEDULING.md) (single THREE owner P2,
  single rAF P3), [ADR-0257](./ADR-0257-realtime-geometry-and-view-interactivity.md)
...
ADR-0257 established the door-move principle: during a live edit, do the minimal visual update and
defer the authoritative geometry rebuild. ADR-0257 P1 made an *openings-only* offset edit O(1), but
...
on release** instead of blocking the interaction frame — the door-move principle (ADR-0257) applied
...
- **Jamb-local wall rebuild during drag (ADR-0257 P3).** The proper structural end-state, but a much
