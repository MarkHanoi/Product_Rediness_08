# ADR-0205 — Worker Pool Policy

| Field | Value |
|---|---|
| Status | **Accepted** — 2026-04-27 |
| Closes | `05-IMPLEMENTATION-PLAN.md §17` row ADR-0205 |
...
1. **Browser** — geometry baking, edge projection, IFC/DXF parse, snapshot serialisation, plugin sandboxes (per ADR-0209). Under-provisioned: kills frame-rate. Over-provisioned: thrashes the main thread context-switching.
...
`08-VISION §3` mandates "Workers everywhere by default; main thread is render-only." `10-MASTER-IMPLEMENTATION-PLAN-36M.md` row ADR-0205 mandates "Browser: Web Worker per task, max 4 concurrent. Server: BullMQ + Node `worker_threads`, R2-backed durable queue."
...
- **Plugin sandbox workers** (per ADR-0209) are **separate** from the kernel pool; their concurrency is per-plugin (heap-isolated) and not counted against the kernel ceiling.
...
- BullMQ adds Redis to the self-host stack (already present per ADR-0212, so net-new is zero).
