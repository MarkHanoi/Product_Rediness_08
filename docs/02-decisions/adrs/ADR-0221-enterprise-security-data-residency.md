# ADR-0221 — Enterprise Security & Data Residency
...
**v1 ships SAML 2.0 + OIDC SSO, TOTP MFA (mandatory for Enterprise admin), SCIM 2.0 provisioning, audit-log streaming to webhook (Splunk/Datadog/SIEM), per-tenant region pinning across EU-W and US-E (AP-SE Phase 4), and the D7 self-host bundle (per ADR-0212). All deferred items are listed explicitly.**
...
- WebAuthn / passkeys — *deferred to v2* (acknowledged in §B8; Tier-1 cut per ADR-0218).
- SMS MFA — explicitly **not** offered (phishing risk).

#### Authorization (per SPEC-08 §3 + ADR-0211)
...
- AP-Southeast (Singapore) — *deferred to Phase 4* (Tier-1 cut per ADR-0218 if velocity slips).
...
- Air-gap install supported (Tier-2 cut candidate per ADR-0218).

#### Threat model (per SPEC-08 §9)
- Top threats addressed in v1: credential theft (MFA + planned WebAuthn), IDOR (RLS + signed URL prefix isolation), plugin escape (Web Worker sandbox per ADR-0209), AI prompt injection from project content (sanitisation in AI plugins; CI test asserts), supply chain (`pnpm audit` + signed lockfile + Renovate; gate on M12), insider threat (audit-log streaming + per-tenant data residency).
...
- Self-host (ADR-0212) plus residency pinning covers the *strict* end of the regulated market.
...
- S43 — Role/permission matrix live (per SPEC-08 §3 + ADR-0211).
