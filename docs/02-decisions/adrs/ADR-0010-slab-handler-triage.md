# ADR-0210 — Slab handler triage (12 → 8) + cross-element coupling lift
...
PRYZM 2's `CommandHandler<TPayload, TStores>` (ADR-0202) is parametric
...
per ADR-0212's `CascadeRule` registry.
...
PRYZM 2 lifts this branch into a `CascadeRule` per ADR-0212:

* **File**: `plugins/cross/slab-wall.ts`
* **Rule key**: `cross.slab-wall`
* **`appliesTo`**: `slab.move`, `slab.setBaseOffset`, `slab.setThickness`
* **`extractEntityId`**: `payload.slabId` (overrides ADR-0212 default
...
Per ADR-0212 §"Decision" the cascade registry is L4 (cross-cutting),
...
producing `slab_<ulid>` per the typed-id brand contract (ADR-0201).
...
inputs (ADR-0209 §"Pure-function signature").

### C. `affectedStores: ['slab']` (not `['slab', 'level']`)

Same rationale as ADR-0208 §D for walls — the bus' multi-store patch
...
to the L7 presentation layer per ADR-0202 §3.
...
* `code-level ADR docs/02-decisions/adrs/ADR-0008-wall-handler-triage.md` — the wall-side companion triage
* `code-level ADR docs/02-decisions/adrs/ADR-0012-cross-element-cascade-rule-registration.md` — the cascade contract this rule conforms to
