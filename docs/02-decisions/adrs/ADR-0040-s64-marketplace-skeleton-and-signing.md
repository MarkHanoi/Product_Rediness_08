...
- **Cross-references**: ADR-0038 (plugin SDK descriptor schema lock),
  ADR-0039 (S63 OpenAPI + docs site), ADR-0218 (rate-limit policy),
  [strategic ADR-0209] (sandboxing + plugin governance),
...
### E — Rate-limit policy: ADR-0218 verbatim

The marketplace API uses `@pryzm/rate-limit` with the ADR-0218 free-tier presets (60 r/m read + 20 r/m write). Read endpoints (browse, detail, versions list, revocations) use the read bucket; write endpoints (publish, revoke) use the write bucket. Buckets are per-(subject, kind, tier).
