# ADR-0255 — One PRYZM: hosted on Cloudflare + Supabase
...
| Supersedes | **ADR-0252** (the docs-site marketing surface — retired) |
| Defers to | **ADR-0256 (planned)** — Supabase Auth migration (Phase A.5, sequenced AFTER Phase A close so it doesn't block production) |
| Owner | Platform infrastructure · `@MarkHanoi` |
| Constraint reference | C08 §1.1 (auth identity) · C22 §1.1/§1.3 (PII tier + region) · C39 pricing surfaces · C43 a11y tokens · C48 §1.1 (RTO/RPO) · C49 §1.2/§1.13 (EU residency + GDPR) · ADR-0245 (Supabase mixed backend) |
| Touches | `server.js` · `apps/editor/` · `apps/docs-site/` · `package.json` · `fly.toml` (new) · GitHub Actions · Cloudflare DNS · ADR-0252 (deprecates) · `docs/01-strategy/STR-02-product-vision.md §8` (amend domain canonical) |
...
1. **§5 region** — pinned `iad` (US East) when **C22 §1.3** + **C49 §1.2** mandate EU primary for the GDPR customers STR-09-personas.md targets. **Corrected → `fra` (Frankfurt) for Fly, `eu-central-1` for Supabase.**
2. **§5 domain** — hardcoded `pryzm.so` when `STR-02-product-vision.md §8` declared `pryzm.app` canonical (with `pryzm.so` flagged "legacy with cutover planned"). User confirmed pryzm.so is the actual canonical going forward. **Corrected → pryzm.so canonical, `STR-02-product-vision.md §8` amended in same PR.**
3. **§2 auth** — claimed "Supabase Auth · JWT issuance" when **C08 §1.1 (CANONICAL Wave A19)** + **ADR-0245 §3** are emphatic: PRYZM uses custom JWT + bcrypt with `SESSION_SECRET`, NEVER Supabase Auth sessions. **Corrected → custom JWT preserved through Phase A. ADR-0256 (planned) will sequence Supabase Auth as a deliberate Phase A.5 migration with C08 §1.1 superseded explicitly.**
...
PRYZM's first deployment attempt (IP-A5, ADR-0252) introduced a **second codebase** for the customer-facing surface: an Astro Starlight project at `apps/docs-site/` deployed to Cloudflare Pages, with hand-mirrored copies of the editor's landing page, pricing data, manifesto narrative, and trust pillars.
...
ADR-0252's "marketing surface as separate codebase" decision is therefore replaced. There is no marketing codebase. There is one PRYZM, and it is the editor.
...
| Postgres | **Supabase** (`eu-central-1` Frankfurt — C22/C49 mandated EU residency) via `server/supabaseClient.js` per ADR-0245 | SOC 2 + ISO 27001. Read replicas. **PITR requires Supabase Pro ($25/mo)** to meet C48 §1.1 RPO ≤ 5 min; free tier is daily-snapshot only. ADR-0245 §3 mandates service-role REST + custom JWT — see Auth row below. |
| Auth | **Custom JWT + bcrypt + OAuth (preserved as-is for Phase A)** per **C08 §1.1** + **ADR-0245 §3** | `SESSION_SECRET`-signed JWTs, bcrypt rounds = 12 (`server/authStore.js:28`), PKCE OAuth flow (`server/oauthService.js`). Supabase REST acts ONLY as the user-row reader via service-role per ADR-0245 §3. **Phase A.5 (ADR-0256 planned)** migrates to Supabase Auth's JWT issuance — sequenced AFTER Phase A close so it doesn't block production. |
...
| **A.5 — Auth migration to Supabase Auth** | Sprint 4 (sequenced AFTER A close) | **ADR-0256 (planned)** authors the Supabase Auth migration: supersedes C08 §1.1 + ADR-0245 §3, identity reconciliation, JWT cutover with 30-day TTL overlap, OAuth flow rewrite, RLS policies wired to `auth.uid()`. Production-grade enterprise auth — SOC2/ISO27001 audited IdP, JWKS, refresh-token rotation, MFA-ready, SSO-ready. | (a) ADR-0256 ACCEPTED before any code; (b) 30-day overlap window between old + new JWT issuance; (c) zero-downtime cutover verified in staging branch preview; (d) `auth.uid()` RLS verified across every PII-bearing table. |
...
| **ADR-0252** | Marked SUPERSEDED-BY-ADR-0255; kept for historical record | Phase A close |
...
- **C39** Pricing surfaces — pricing page now renders directly from `@pryzm/entitlements` inside the editor (the docs-site JSON-snapshot pattern from ADR-0252 §1.4 is retired).
- **C43** Accessibility — `PRYZM_TOKENS` registry is the single source for colour values across all editor routes; the duplicated hex literals from ADR-0252 §3 are retired.
- **C48** Backup & DR — Supabase PITR + Cloudflare R2 versioning replace the previous Replit-volume backup story.
- **ADR-0252** — SUPERSEDED BY THIS DOC. Retained for historical context.
- **ADR-0253** Lockfile-drift policy — unchanged.
- **ADR-0254** Reference-only repos as gitignored — unchanged.
