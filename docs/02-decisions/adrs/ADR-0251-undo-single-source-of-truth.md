# ADR-0251 — Undo as single-source-of-truth + derived mesh (patch-inverse on one store)
...
two-part undo = the next ADR-0251 slice) and `level` (Path-A). This is the per-type bridge; the
