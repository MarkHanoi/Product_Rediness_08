...
> **Naming note.** `phases/PHASE-2B` uses the number ADR-0224 informally for "Section view cut algorithm." That phase-doc usage is sprint-scoped working notation; the **canonical** ADR-0224 in the strategic series (`docs/02-decisions/adrs/`) is this constraint-solver ADR, per SPEC-01 §4 and SPEC-01 §10's authoritative cross-reference.
...
- **Phase 2A (S25–S30)** — Light per-element parametric expressions: `length = a + b`, `angle = 90°`. Evaluated by a small expression evaluator (per ADR-0227) — **not** the constraint solver. No multi-body coupling.
...
- NURBS-based constraints (deferred with the OpenCASCADE.js kernel-swap, per ADR-0220).

### Performance budget
- Solve a 50-constraint sketch in < 16 ms (one frame).
- Solve a 200-constraint sketch in < 100 ms.
- Solve in a Web Worker (per ADR-0205) so the UI thread is free.
...
- Edits during solve: solver runs reactively on every constraint addition/edit; debounced via the same 250 ms policy as bake (ADR-0210).
...
- Considered as a Tier-2 cut (per ADR-0218 T2.2). If exercised, D10 is deferred with it. Default plan keeps both in v1.
...
- Rejected — overkill; expression evaluator (per ADR-0227) is simpler, faster, and has no solver-state to worry about.

---

## Phase rollout
- S25 (Phase 2A) — light expression evaluator lands (per ADR-0227); **no** solver yet.
