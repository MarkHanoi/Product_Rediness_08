# ADR-0206 — Idle-continuation N-frame budget
...
The `idle` lane (ADR-0203) needs a budget — without one, an aggressive
...
- **Wall-clock ms budget** — lost: identical complaints to the deadline-
  based scheduler (ADR-0203) — non-portable across throttled tabs.

## References

- ADR-0203 (priority lanes)
