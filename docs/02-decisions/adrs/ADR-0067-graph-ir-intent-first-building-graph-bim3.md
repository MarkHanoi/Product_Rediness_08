...
**Related contracts**: [C52 — Editable Building Graph](../contracts/C52-EDITABLE-BUILDING-GRAPH.md), [C53 — Generative Layout Engine Architecture](../contracts/C53-GENERATIVE-LAYOUT-ENGINE-ARCHITECTURE.md) (§1 topology-before-geometry; **§13 — the intent-first 7-layer target engine + gap-leverage order, the canonical statement of this ADR's realisation**), [C50 — Typology Pipeline](../contracts/C50-TYPOLOGY-PIPELINE.md), [C19 — Site Model & Parcel](../contracts/C19-SITE-MODEL-AND-PARCEL.md)
> **North-star realisation (2026-06-16):** the founder's 7-layer intent-first engine brief — *a building IS a set of relationships; geometry is one realisation derived last* — is documented canonically in **[C53 §13](../contracts/C53-GENERATIVE-LAYOUT-ENGINE-ARCHITECTURE.md)** (7 layers + the binding gap-leverage order). This ADR is the governing graph-IR; [ADR-0068 §FG7](ADR-0068-five-graph-model-circulation-first-building-graph.md) (circulation-first invariants) + [ADR-0069](ADR-0069-graph-authoritative-room-identity-at-execution.md) (graph-authoritative rooms) are its first concretizations.
