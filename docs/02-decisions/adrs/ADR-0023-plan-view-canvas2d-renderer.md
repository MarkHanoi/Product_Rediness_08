...
- Subordinate to: **`[strategic ADR-0216]`** (drawing engine architecture; Canvas2D is the
...
(Reaffirms `ADR-0025 §1` for S31 onward; cites `[strategic ADR-0216]` §"Alternatives
...
- SVG was rejected for the **screen** back-end (text-anchor + line-join inconsistency
  ...
  for the **export** back-end (per `[strategic ADR-0216]`; ships parallel S31 SVG +
  S33 PDF).
...
**Positive:**
- One vector model, three back-ends per `[strategic ADR-0216]` — bug fixed once
...
Rejected as in `[strategic ADR-0216]` — no shading need, second WebGL context
...
- `docs/02-decisions/adrs/ADR-0216-drawing-engine-architecture.md`
- `docs/03-execution/plans/legacy/phases/PHASE-2/2B-Q2-M16-M18-PLAN-VIEW.md` §S31
- `docs/02-decisions/adrs/ADR-0028-plan-view-canvas-architecture.md` (S29 skeleton; superseded for
...
`docs/02-decisions/adrs/ADR-0031-s61-staged-legacy-deletion.md`.  Until then,
