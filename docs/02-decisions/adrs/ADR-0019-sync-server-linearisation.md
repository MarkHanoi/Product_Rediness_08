...
- **Related ADRs**: ADR-0202 (CRDT ↔ event-log bridge), ADR-0210 (bake
  debounce), ADR-0218 (`.pryzm` ZIP envelope)
...
- **CRDT semantics** — ADR-0202 deferred Yjs to Phase 2D (S43).  Today
...
binary cost is in the **chunks** (ADR-0218), not the events.  The per-
...
JWTs from the auth-server.  ADR-0221 (enterprise security) tracks the
upgrade.
...
the bake-debounce window in ADR-0210 work cleanly.
...
> has happened" — the bake is async, debounced (ADR-0210), and observed
...
via the same env vars as the rest of PRYZM 2 (ADR-0207); the sync-server
...
- Yjs document loading requires the **whole** doc, which clashes with
  the chunk-streaming model (ADR-0218).  The Yjs ↔ event-log bridge
  (ADR-0202) is non-trivial and not yet specified.
...
same tick, which defeats the per-project debounce in ADR-0210.  Per-
...
- **Auth is trust-the-query-string** — explicitly v0; ADR-0221 tracks
  the JWT upgrade.
...
- **S38**: replace query-string auth with signed JWTs from the
  auth-server (ADR-0221).
- **S43**: Yjs CRDT replacing LWW; ADR-0202 will be amended.
