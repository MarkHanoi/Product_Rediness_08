# ADR-0228 — Authority Unification (one permission model)
...
| Spec dependency | `SPEC-08-SECURITY.md` (extended); `SPEC-15-DEPLOYMENT-TOPOLOGY.md` §5; `[strategic ADR-0221]` |
...
Roles are project-, view-, or element-class-scoped per `[strategic ADR-0211]`. v1 = three roles: `owner`, `editor`, `viewer`. Per-element instance is post-GA.
...
### Part G — audit trail (per ADR-0221)
...
These rows become the SOC2 audit trail (per `[strategic ADR-0221]` §7), retained for the customer-visible audit window.
...
- S31 — ADR-0228 land (Phase 2B start; Phase 2A holds no gap-closure); `Action × Resource` taxonomy v0 in `packages/authz/`; service-role-key removal; CI gate `pnpm spec:audit-secrets` lit; service-token issuer in gateway.
...
- S55 — audit-log schema lit per ADR-0221.
