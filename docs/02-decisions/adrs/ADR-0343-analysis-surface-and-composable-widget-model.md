...
- **H9 — A SCOPE and a TRUNCATION are different facts and MUST NOT render identically.**
  *Added 2026-08-22 · L-3620 · founder's words: "'filtered to Level 1' is not the same statement as
  'truncated at 60 nodes' and they must never render identically."*
  A **cap** is the tool failing to deliver the universe the reader asked for: every count under it
  is a **FLOOR**, `complete` is `false`, and it renders on the warning plate. A **filter** is the
  reader choosing a smaller universe: every count inside it is **EXACT for that universe**,
  `complete` stays `true`, and it renders on a neutral scope plate naming the universe.
  ⛔ Printing `≥` over a scoped count puts doubt on a number that is not in doubt, which teaches the
  reader to discount the one prefix that must never be discounted (the L-3303 lesson, inverted).
  ⛔ **One exception, and it must be counted separately:** an element a filter drops because it
  cannot be PLACED on the filtered axis at all — as distinct from being placed elsewhere — might
  belong to the scope being shown, so it does make the scoped figures a floor. "Excluded because you
  asked", "excluded because it has no value on this axis" and "excluded because we could not place
  it" are **three** answers, never one.
- **H10 — Emphasis may DIM, never HIDE.**
  *Added 2026-08-22 · L-3610 · founder's words: "when the user selects a part of the graph it should
  highlight this and the rest be a bit dormant."*
  A widget that responds to a pick by removing the unpicked marks leaves its own denominator, legend
  totals and percentages describing a population the reader can no longer see — a filtered chart
  under an unfiltered caption, which is H2 failed at the presentation layer. Dimming changes
  emphasis; it does not change the answer.
  ⛔ **Dimming moves ALPHA ONLY, never hue.** A re-tint can turn one category's colour into
  another's and — the case that matters — can promote the named neutral (`unassigned` / `untyped` /
  `unmeasured`, D.5) into the categorical rotation by accident. Those are ANSWERS about the model,
  not categories, dimmed or not.
  ⛔ **There must be a way back to the whole population by the same gesture that left it**, or the
  reader is stranded on an emphasised subset of a card whose caption describes the whole.
...
- **§U.3 — ~~Which measured-area standard PRYZM adopts~~ — RESOLVED 2026-08-22 (lane ANLZ3,
  L-3640). SIA 416 is the default, it is SWITCHABLE, and the surface names it on the card's face.**
  The declaration is `apps/editor/src/ui/analysis/areaStandards.ts` (SIA 416 · IPMS · RICS CoMP ·
  plus a fourth, non-normative "PRYZM room-boundary sum" that is the only one that measures).

  ⚠ **The count that nominally chose it is SELF-REFERENTIAL, and that is recorded rather than
  smoothed over.** Measured: `SIA 416` **16** · `RICS` **13** · `IPMS` **4** — but every one of the
  sixteen is this product *saying it does not have it* (the refusal card, this ADR, the SPEC row, a
  test). A count generated by a refusal card is not adoption. The brief's alternative branch — *"the
  one the code already leans on"* — has **no referent**: the only implemented area rule in the
  product is `ScheduleExtractor.ts:567`, `actualGFA = Σ rooms[].computed.area`.

  ⛔ **AND THE SENTENCE THIS BULLET USED TO END WITH IS NOW FALSE.** It read *"There are **zero**
  `SIA` occurrences in `packages/` or `apps/editor/src`."* That was true when written and is not any
  more. **Re-measure, do not re-transcribe.**

  ⛔ **RESOLVING §U.3 DID NOT UNBLOCK THE GFA/NIA/SIA WIDGETS, AND THE REASON IS §U.7 BELOW.** They
  were blocked on TWO things and only one of them was a decision.
- **§U.7 — Which MEASUREMENT PLANE PRYZM's areas are reported on** — NEW, opened 2026-08-22 (lane
  ANLZ3, L-3641), and it is the blocker §U.3 was hiding.

  ⛔ **MEASURED, C01 §6 rule 6 — cite the command:**
  ...
  **Every `Room.area` in this product is the area enclosed by the wall CENTRELINES.** That is exact
  for that definition and it is **not the plane any published standard measures on**: SIA NGF /
  IPMS 3 / RICS NIA and GIA are measured to the internal dominant face (centreline **OVERSTATES**);
  SIA GF / IPMS 1 / RICS GEA to the external face (centreline **UNDERSTATES**).

  So no class of any adopted standard can be MEASURED today, whichever standard is selected. The
  surface therefore reports the centreline sum **as** a centreline sum, with the plane in its basis
  string, and brackets the conversion — `[P·t_min/2, P·t_max/2]` over each room's bounding walls,
  a **bound**, never a midpoint estimate.

  **What would close it** is per-edge wall-thickness attribution on the room boundary: the producer
  knows which wall each half-edge came from (`RoomAnalytic.boundingWallIds`) but the cache does not
  keep the edge→wall pairing, so the exact correction `Σ(edge length × that edge's thickness ÷ 2)`
  cannot be formed. It is one field on the producer's output, not a new subsystem — but it changes
  a geometry-kernel contract and belongs to a geometry lane, not a UI one.
