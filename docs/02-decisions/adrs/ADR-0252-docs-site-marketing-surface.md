# ADR-0252 — Docs-site marketing surface (Cloudflare Pages + 404-as-page + token reuse)

| Field | Value |
|---|---|
| Status | **SUPERSEDED 2026-06-02 by [ADR-0255](./ADR-0255-one-pryzm-cloudflare-supabase.md)** |
...
| Superseded by | **[ADR-0255 — One PRYZM: hosted on Cloudflare + Supabase](./ADR-0255-one-pryzm-cloudflare-supabase.md)**. Authored same day after the user flagged the drift trap ("there is only one app — one solution — PRYZM"). The decisions below remain historically accurate but the architecture they describe is being retired. See ADR-0255 §7 for what gets deleted + when. |
...
- **ADR-0253** (forthcoming) — lockfile-drift policy + CI gate
- **ADR-0254** (forthcoming) — reference-only repos as gitignored subtrees
