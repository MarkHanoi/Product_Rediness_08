...
- `docs/02-decisions/adrs/ADR-0051-s69-largest-fixture-bench-policy.md` §A + §C (warn-only landing + reversal contract S71 fulfils).
- `docs/02-decisions/adrs/ADR-0052-s70-browser-matrix-wcag-selfhost-publish-pdf-preview-lifecycle-deletion.md` §C (publish-prep dry-run rationale that left S70 D8 without an isolated CI runner).
- `docs/02-decisions/adrs/ADR-0048-s67-self-host-docker-compose.md` §B (code-stability invariant preserved at S71 close).
