# ADR-0207 — Telemetry Backend

| Field | Value |
|---|---|
| Status | **Accepted** — 2026-04-27 |
| Closes | `05-IMPLEMENTATION-PLAN.md §17` row ADR-0207 |
...
`05-IMPLEMENTATION-PLAN.md §17` proposed "Honeycomb / self-hosted Tempo / both → Both — self-host primary, Honeycomb option for hosted customers." `10-MASTER-IMPLEMENTATION-PLAN-36M.md` row ADR-0207 amended this to "OTel SDK + self-hostable Tempo + Honeycomb dev account." This ADR ratifies the amended position.
...
- **Self-host (D7 enterprise):**
  - Tempo in the customer's docker-compose (per ADR-0212).
...
**Mitigation:**
- Tempo runs on the same R2 bucket layout as event chunks (per ADR-0203); no new storage primitive.
