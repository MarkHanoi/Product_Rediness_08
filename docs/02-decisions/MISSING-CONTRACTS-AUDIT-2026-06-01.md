...
**Proposed resolution (post-launch, needs founder/architect decision — do NOT auto-resolve):** the L-414 spike determines whether the geospatial view stays **context-only** (C55 as written) or becomes a **first-class interactive BIM/IFC surface**; if the latter, author a governing contract (or amend C55 §1.2 + extend C27/C28 to the geospatial surface) **before** any code — spanning the BIM data model (C03), IFC/ThatOpen interop (C25/C26), the Cesium render + bridge (C12/C55/CesiumViewport), and the C10 perf budget. Cross-referenced from C55 §4.1 + the L-414 spike stub (`docs/03-execution/spikes/spike-bim-ifc-data-interrogation-in-cesium.md`). Adjacent to the L-355/L-357/L-359 C-CONTEXT-ENGINE gap.
