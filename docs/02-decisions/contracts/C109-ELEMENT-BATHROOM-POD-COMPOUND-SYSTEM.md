...
| each member's **position / rotation** | **STORED on the member record** | The member is a real `plumbing` record that the move tool, the plan symbol builder and the IFC exporter all read directly. A position that lived only on the parent would be invisible to every one of them |
| `childrenIds` | **STORED** | The C104 §2.2 / §3.2 rule requires it: it is the member list, and it must survive a level that has not been built |
