...
> **Stamp**: 2026-07-18 · **Status**: CANONICAL (Phase 1a landed — pure model; live PaneHost wiring phased)
...
- **Single rAF (P3).** No `PaneHost` calls `requestAnimationFrame`. All panes subscribe to the ONE frame bus (`runtime-composer` frame scheduler, C04). A Canvas2D pane paints on a `pre-render` tick (as SVP does today); Cesium uses request-render mode driven by the same scheduler.
...
2. **Single rAF (P3).** Panes never own a frame loop; they subscribe to the composition-root frame bus.
...
|---|---|---|
| `SplitViewManager` fixed Canvas2D right pane | `#svp-secondary-pane` fixed at 40%, Canvas2D-only | Becomes a **`PaneHost`** whose default view is `bim-plan-2d`; can host any view. Its Canvas2D renderer becomes the `canvas2d` mounter. |
...
- **Phase 1b — One non-Canvas2D view hostable in a pane.** A minimal `PaneHost` that can mount the **3D Site (Cesium)** into either the left or right pane element (re-parenting the single `#cesium-viewport-container`, reflow via `reflowContainer()`), driven by the pure model. Proves renderer-agnostic hosting end-to-end for ONE hard case (the singleton Cesium). Verify: 2D map LEFT + 3D Site RIGHT during site authoring, envelope live (the original founder scenario) — now via the general host, not a hard-wire.
...
- **Phase 4 — Consolidation.** Retire the three legacy owners' bespoke container logic once every flow routes through `PaneHost`.
