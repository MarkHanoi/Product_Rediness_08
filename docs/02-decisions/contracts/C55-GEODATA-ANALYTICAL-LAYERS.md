...
> **Decision record**: [ADR-0065](../adrs/ADR-0065-geodata-analytical-layers-pluggable-provider.md) — the decision to provide geodata as a first-class **pluggable-provider** layer subsystem draped on Forma/Cesium (vs hardcoded per-country layers vs none).
...
### §4.1 — Forward note: interactive BIM/IFC data in the geospatial view is OUT of current scope (L-414 spike)

**§1.2 makes analytical drape layers read-only context — they never become BIM geometry.** A separate, forward-looking founder ask (2026-07-18) explores making the Cesium / Forma geospatial view **interactive with BIM/IFC data** — i.e. selecting / querying / manipulating the BIM massing (today placed as a passive whole-scene glTF `Cesium.Model`, with no per-element feature IDs) *inside* that view. That capability is **explicitly out of the current scope of this contract** and is tracked as the **[L-414](../../04-reference/ISSUE-LOG.md) SPIKE** (QUEUED, NOT priority; stub: [spike-bim-ifc-data-interrogation-in-cesium.md](../../03-execution/spikes/spike-bim-ifc-data-interrogation-in-cesium.md)).

**This is a coverage gap, not a present violation of §1.2**: §1.2 governs analytical *layers* (which must stay context), and does **not** speak to per-element picking of the BIM model already rendered in the view — **no contract currently governs interactive BIM/IFC interrogation in the geospatial surface** (C27/C28 cover BIM inspect + Pset data but only on the editor canvas). The gap is logged in `MISSING-CONTRACTS-AUDIT-2026-06-01.md` (audit removed 2026-08-09 — recoverable from git history).

**Founder decision (2026-07-18) — DIRECTION CHOSEN, DEFERRED.** The founder resolved the direction: the geospatial view **should eventually become a first-class interactive BIM/IFC surface** (*"definitely nice to have"*), **not** stay permanently context-only — but this is explicitly **queued, not-priority (post-September)**. Consequences for this contract: (a) **C55 stands as-written** — the geospatial surface remains context-only *until* a governing contract lands; (b) when L-414 is picked up, the **first deliverable is that contract** (an amendment to this §1.2 + a C27/C28 extension, or a new geospatial-interrogation contract) **before any code**; (c) nothing changes now. Recorded here so this contract's context-only framing is an explicitly time-bounded stance, not silently contradicted later.
