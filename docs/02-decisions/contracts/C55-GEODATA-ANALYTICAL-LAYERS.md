...
**This is a coverage gap, not a present violation of §1.2**: §1.2 governs analytical *layers* (which must stay context), and does **not** speak to per-element picking of the BIM model already rendered in the view — **no contract currently governs interactive BIM/IFC interrogation in the geospatial surface** (C27/C28 cover BIM inspect + Pset data but only on the editor canvas). The gap is logged in [`MISSING-CONTRACTS-AUDIT-2026-06-01.md`](../MISSING-CONTRACTS-AUDIT-2026-06-01.md). Whether the geospatial view stays **context-only** (this contract's stance) or becomes a **first-class interactive BIM/IFC surface** is a **founder decision** the L-414 spike must surface; the interactive end-state would require a governing contract (or an amendment here / to C27/C28) *before* any code. Recorded here so this contract's context-only framing is explicitly cross-referenced, not silently contradicted later.
