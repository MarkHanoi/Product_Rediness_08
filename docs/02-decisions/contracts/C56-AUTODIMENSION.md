...
> **Amendment (ADR-0119, 2026-07-07).** Completeness shortfalls are recorded as **non-blocking `AutoDimReport.warnings`** (e.g. `opening-undimensioned`) which the executor surfaces to the user (toast + console) — satisfying C24.1 §1.3 "no silent omission". They are NOT a hard reject: the executor still creates whatever the engine did produce. A hard-`errors` **reject** channel remains deferred to P2 (conflict/stacking). DI-6 below is restated to match the shipped intent.
>
> **Amendment (§FIX-AUTODIM-ORTHO-COMPLETE-CHAINS, L-147, 2026-07-07).** Two items are now **shipped**, not deferred: (1) the **orthogonal-only invariant DI-7** below, and (2) the run-partition **`chain-gap` / `chain-overlap` detection (QA-2)** — `detectChainCoverageGaps` runs in Stage-8 QA over every exterior run and records non-blocking warnings (a full-façade run deduped into the `overall` per DI-4/§4.3 is credited via its end-to-end ref-pair, so a façade dimensioned only by the overall is NOT a false gap).
...
- **DI-6 QA surfaces incomplete documentation (no silent omission).** An SET missing any of DI-1…DI-4 surfaces a typed diagnostic (`opening-undimensioned`, `overall-mismatch`, `chain-gap`, `chain-overlap`, …) in `AutoDimReport.warnings`, which the executor reports to the user. Silent omission is forbidden (C24.1 §1.3). A hard-reject `errors` channel remains deferred to P2 (ADR-0119); `chain-gap`/`chain-overlap` (QA-2) is **shipped** (§FIX-AUTODIM-ORTHO-COMPLETE-CHAINS, L-147).
- **DI-7 Auto-dimensions are ORTHOGONAL-ONLY (§FIX-AUTODIM-ORTHO-COMPLETE-CHAINS, L-147).** No emitted dimension may render as a diagonal across the footprint. Every `overall` string measures a **cardinal bbox extent** — the X-extent (horizontal) and the Y/Z-extent (vertical), each drawn as an offset chain OUTSIDE the perimeter — never a point-to-point distance between two non-axis-aligned extreme corners. Because `overall` deliberately references the two **extreme perimeter corners** (SPEC §4.2) to stay live (INV-4), and those corners are NOT collinear on the cross-axis for an L/notched footprint, the pure `cardinalMeasurementAxis(orientation)` helper supplies the measurement axis (world +X for `horizontal`, +Z for `vertical`, `null` for `aligned`/`angular`); the L5 executor stamps it onto `geometry2D.measurementNormal` so the plan renderer's §DIM-ORTHO branch draws an axis-aligned line and labels the **axis extent**, not the corner-to-corner hypot. `aligned` chains on genuinely angled façades measure along their own direction and are exempt. QA records a defensive `non-orthogonal-string` warning if an `overall` ever arrives non-cardinal (never expected).
