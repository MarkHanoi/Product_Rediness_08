...
CI gate: ~~existing `check-spans.ts` extended (planned)~~ — ⚠ **`check-spans.ts` NEVER EXISTED, so nothing was "existing" to extend.** The real gate is [`tools/ga-gate/check-otel-spans.ts`](../../../tools/ga-gate/check-otel-spans.ts), **RED at RC=3**, and it counts handler FILES rather than exported functions. **This clause is NOT-YET-TRUE.** See †PHANTOM-GATE.
