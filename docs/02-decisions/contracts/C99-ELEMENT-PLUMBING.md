...
### THE VERB — `plumbing.changeLevel` ✅ LIVE

| | |
|---|---|
| **Payload** | `{ plumbingId, levelId }` — declared at `packages/command-bus/src/levelChangeVerbs.ts` |
| **Handler** | `plugins/plumbing/src/handlers/ChangePlumbingLevel.ts` |
| **Registered** | `plugins/plumbing/src/handlers/index.ts:26` |
| **Legacy move** | `packages/geometry-plumbing/src/PlumbingStore.ts` — `changeLevel(id, newLevelId)` |
| **Mirror row** | `apps/editor/src/engine/elementLevelChangedMirror.ts` — `LEGACY_LEVEL_MOVERS.plumbing` |
...
`PlumbingStore.update()` is a **WHOLE-RECORD REPLACE with NO EXISTENCE CHECK**
(`PlumbingStore.ts:27`, measured 2026-08-19). It is therefore worse than slab's: handed
`update('does-not-exist', {levelId})` it does not fail — it **MINTS A PHANTOM RECORD** consisting of
nothing but a `levelId`, under an id no create ever produced. An undo that took the generic branch
here would not merely destroy a fixture; it could **manufacture** one.

`changeLevel` checks existence itself and returns `undefined` when there is nothing to move, which
the mirror reports as a NAMED refusal rather than logging success over a no-op.
...
| | Measured 2026-08-19 |
|---|---|
| **`element.changeType` tag(s)** | `plumbing` · `plumbingfixture` · `plumbing_fixture` |
| **Branch** | `apps/editor/src/engine/initBusHandlers.ts`:1802 |
| **Type catalogue** | ⛔ **NONE.** `PlumbingFixtureType` plus `toiletVariant` / `showerVariant` / `accessoryVariant` are bare unions (`PlumbingTypes.ts:13-36`) with no display names. |
| **Type field on the record** | `fixtureType` + the variant fields |
| **Executor the chat must use** | ✅ `element.changeType` `:1802` → `UpdatePlumbingParametersCommand` → legacy `plumbingStore` → `PlumbingFragmentBuilder.updateFixture()`, ring-parity (L-622 + L-623) |
| **Chat capabilities published TODAY** | ⛔ **NOTHING** beyond the probe set. |
| **Retiring condition** | **MINT an `{id, name}` plumbing-fixture catalogue** (C65), covering the fixture type AND the variant axes. |
...
- ⭐ **This family already paid for the lesson the whole audit is about, and its branch comment is the receipt.** `initBusHandlers.ts:1802-1820` records that the panel used to dispatch `plumbing.setSystem`, *"which could NEVER have worked, on two counts that TypeScript could not catch because the verb is absent from the command-bus payload map"* — a **payload mismatch** and a **DETACHED STORE** the 3D tool never populates. **A chat capability routed at a plausible-looking plugin verb would have reproduced exactly that.** It is why ADR-0334 requires the route to be `element.changeType`, measured, and never a `*.setType` handler.
- ⚠ The variant axes (`toiletVariant`, `showerVariant`) mean *"change the plumbing type"* is **not one question**. A minted catalogue must decide whether a fixture type and its variant are one name or two, **before** a grammar claims the sentence — otherwise the refusal cannot state what it is refusing.
- ⛔ **MUST NOT narrow the vocabulary.**
