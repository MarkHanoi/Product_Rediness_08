...
4. **⛔ A PLACE PHRASE IS READ BY THE ONE SHARED PARSER, AND A SCOPE MAY NEVER SILENTLY WIDEN** (added rev 4 — L-10220, 2026-08-24). Two obligations, and the second is the destructive one.

   **(i) One parser.** `SpatialScopeTail.SPATIAL_TAIL_SRC` + `readSpatialTail` is the only thing that may decide whether a preposition phrase names a LEVEL or a ROOM. A grammar that spells its own tail encodes a rule English does not have — *"on level 2"* and *"in level 2"* are the same sentence. This has now been the same defect **five times**: L-1201 (dimension + creation grammars), L-1261 (wall finish), L-1372 (rake), and L-10220 — which found the fifth spelling inside `makeHostedTypeParser`, **the factory that generates the grammar for every catalogue family at once**. Measured there before the fix: *"change all lightings in ground level to recessed downlight"* → `typeRef: "in ground level to recessed downlight"`, and window / door / slab / ceiling / stair / stair-railing were **all** broken identically, unreported, because nobody had typed the sentence.

   **(ii) ⛔ An unresolvable place DECLINES; it never falls back to a wider scope.** The tail's `unusable` reading exists for exactly this, and a mass-edit grammar must honour it. The first half of the L-10220 defect was a wrong refusal; the second half was that the sentence **kept its `'all'` scope**, so had the type ref resolved it would have retyped every fixture in the building. **A level scope that silently widens is worse than no level scope** — the user restricted the ask and was not told it had been ignored.

   **(iii) 👁 And a place NOUN inside a TYPE NAME is not a place.** Three of the twelve named lighting fixtures are FLOOR lamps, and `set-floor-finish` claimed *"change all lights to brass arc floor lamp"* as a finish ask with `finishRef: "brass"` — repainting every floor in the project. This is §3.d's self-referential-name rule with ANOTHER family's noun doing the damage: **when a family's element noun can appear inside another family's catalogue names, the other family's noun must be in its decline list, and the catalogue's own claim decides.** ⚠ Decline on the NOUN, never on an adjective the other family legitimately uses ("light oak" is a real finish).
