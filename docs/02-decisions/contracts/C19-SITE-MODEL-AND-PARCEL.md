...
| Jurisdiction-specific building code databases (UK Approved Documents, IBC, NCC) | **No longer "TBD" for the HABITABILITY axis** (amended 2026-08-22, lane JURIS11): room-level habitability minima are owned by [ADR-0352](../adrs/ADR-0352-habitability-minima-are-jurisdiction-keyed-and-carry-their-instrument.md) + [SPEC-HABITABILITY-MINIMA](../../03-execution/specs/SPEC-HABITABILITY-MINIMA.md), keyed on the jurisdiction id `resolveRegisteredJurisdictionAt` already returns — so C19's `Parcel.zoning.jurisdictionRef` **is** the hook, exactly as this row predicted, and no second resolver was minted. ⚠ Only that axis. Fire, structural, thermal, egress and accessibility code lookup remain unowned, and the audit's finding is that **14 of the 15 country folders hold no primary legal text of any kind** — so this row is a SOURCING backlog, not a wiring one. |
