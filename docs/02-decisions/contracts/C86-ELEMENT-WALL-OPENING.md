...
| **R-17** | An opening that does **not fit inside its host's authored elevation outline** refuses, naming **which edge** and **by how many metres** | `§FEAT-WALL-PROFILE-OPENINGS` (L-7400); `WallProfile.wallProfileRectFit`, consulted by `WallOccupancyStore.canPlace` (`OCC_OUTSIDE_HOST_PROFILE`), by `profileAuthorability`, and by `buildWallHoleBodyGeometry` | ✅ **BUILT 2026-08-23 (lane OPEN38).** ⭐ **ONE predicate, three askers** — *may this opening go here*, *may this ring be applied*, *can this solid be built* are the same question from three sides, and answering them in three places is how a gate and its geometry drift (C84 EI-9). The reason is dimensional, never categorical: *"the outline falls to 1.600 m across this opening and the head is at 2.100 m — 0.500 m would be outside the wall"*, with `overshootM` on the result so a caller can offer the nudge without re-deriving it. ⛔ **MUST NOT be "fixed" into a clamp or a nudge-on-commit** — an opening silently moved to fit is an element the author did not select, edited to honour one they did |
| **R-18** | Applying a wall outline that would **remove the material an EXISTING opening sits in** refuses — the OUTLINE edit is rejected, the opening is **never** moved or clipped | `profileAuthorability` on `WallStore.update`, against the MERGED wall | ✅ **BUILT (L-7400), and this is a DECISION, not a fallback.** The alternative — relocating or trimming the opening to suit the new ring — was considered and rejected under C84 EI-2 (refuse, do not narrow) and the founder's standing direction that a spatial conflict is always **ASKED**, never silently resolved. ⚠ The bidirectional half is the one that is easy to omit: a gate wired only into the create path lets an author reach the same bad state by doing the two operations in the other order, which is the identical trapdoor `WallStore.addOpening` already exists to close |
| **R-19** | A **floor-reaching opening (a door)** on a wall whose outline is **not level across the door's own span** refuses (`uneven-foot`), naming the span and the metres | `wallProfileRectFit`; the notch walk in `buildWallHoleBodyGeometry` | ✅ **BUILT (L-7400).** The reason is structural rather than stylistic and is worth stating because it is not obvious: a door is **carved out of the outer boundary** — it is a notch, not a closed hole, which is what gives it continuous reveals with no T-junction — so the boundary has to *be* where the door's foot is. On a wall over an archway there is nothing at floor level to carve, and the walk would emit a jamb starting in mid-air. A window in the same place is fine: a `THREE.Path` hole is valid inside any simple outline |
| **R-20** | A **curved** host that carries **both** an authored outline **and** openings refuses (`curved-hosted-openings`) | `profileAuthorability` | ⚠ **NOT-YET, and each PAIR is already built** — which is exactly why it needs saying. A curved wall takes an outline (`§FEAT-WALL-PROFILE-CURVED`, per-station heights) and a curved wall hosts openings (`_buildCurvedWallWithOpenings`, radial bands at stations along the arc). They are **different builders**, and the router sends a wall with openings to the second one, which never reads the ring — so the outline would be **silently discarded**. Refused rather than approximated. **Machinery for the lift:** teach the radial-band carve a per-station top/bottom, the way `CurvedProfileHeights` taught the layer builder |
| **R-21** | ⚠ The **plugin-side** occupancy port cannot see a host outline at all | `plugins/wall/src/occupancy.ts` (a second `WallOccupancyStore` with its own `CanPlaceRefusalCode`) | ⛔ **DECLARED, NOT CLOSED (L-7412).** That port is documented as **deliberately** divergent from `geometry-wall`'s (GE-04), has no rake arm and now no outline arm, so `plugins/wall/src/handlers/CreateWallOpening.ts` places against length alone. Lane OPEN38 threaded the five geometry-wall-backed create sites and **reverted** its edit here rather than widen a duplicate the file asks not to be matched. The store write boundary still refuses, which is what protects the model — but the plugin path's *pre-flight* is weaker than the commit it precedes, and a pre-flight that is weaker than its commit is a crash waiting to be reported as a bug. Closing it is GE-04's collapse of the family, not a patch |
