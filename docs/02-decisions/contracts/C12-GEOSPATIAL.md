...
### §12.3 — The seat is an ABSOLUTE scalar height, PER FEATURE, and every layer MUST be re-seatable
...
⭐ **Extended 2026-09-05 (§GROUND-DRAPE-ON-RELIEF, L-12924) — ONE scalar per LAYER was the defect; the scalar is per FEATURE.** The founder, Lisbon Baixa, terrain ON: *"the grey layer (urban landuse) and probably others is CUTTING the buildings — not set on the correct height."* The console carried the whole mechanism, in two lines that disagree with each other:

- context BUILDINGS — `per-footprint terrain: sampleTerrainMostDetailed resolved 6349/6349 real ground heights … per-footprint seat, centroid base 68.9 m, relief ON`
- context GROUND FEATURES — `ground-features re-seat: 1413 road/park/water entity(ies) lifted onto settled ground (base 71.0 m)`

**One number for 1413 features, beside 6349 individually-seated buildings on the same hill.** Baixa sits ~10 m orthometric and Chiado ~90 m, so the single scalar is a plane that floats tens of metres ABOVE the ground downhill — i.e. through the buildings extruded from that ground, which is exactly the founder's picture — and sits UNDER it uphill, where the layer simply vanishes. Madrid, Zürich and Sète carry the same latent shape.

**THE RULE.** Every ground-context feature seats on the terrain sampled at **ITS OWN representative point** — polygon centroid (the same `ringCentroidLatLon` the buildings sample at) or corridor half-length point — with its §12.4 ladder offset applied **relative to THAT height**. `reseatContextGroundFeaturesForBase` does the same **per entity**, never one base for all: each entity's seat point is recorded at load (`contextGroundSeatPoints`) so the re-seat reproduces the load-time decision from the same terrain cache.

**A feature that spans more relief than the ladder can hide is SPLIT**, not tilted: above `GROUND_DRAPE_RELIEF_SPLIT_M` (3 m, MEASURED by 3–5 probes across the feature) a polygon becomes grid cells and a corridor becomes length-bounded segments, each on its own seat, bounded by `GROUND_DRAPE_MAX_PIECES_PER_FEATURE`. A drape is then a staircase of flat pieces on the hill rather than one plane through it.

**The flat / keyless path is UNCHANGED and MUST stay so** — no relief attached means no sampling and one scalar `base + ladder` for the whole layer, byte-identical to the pre-L-12924 seat. That is pinned by `groundFeatureSeat.spec.ts`, not merely intended.

**Relief UNKNOWN is not relief ZERO** (§CONTEXT-DATA-HONESTY / C84 EI-6): a point the terrain sampler did not resolve seats on the SAFE base, never on a stray 0, and is counted separately in the diagnostic line — which now reports per-feature / fallback counts and the seat RANGE, so a reader can see at a glance whether the layer actually followed the hill.

⛔ **`CLAMP_TO_GROUND` was RE-EVALUATED and is still NOT wired — but this contract's stated REASON for that was wrong.** §12.3 blamed `globe.depthTestAgainstTerrain = false`. MEASURED against the shipped `node_modules/cesium/Build/CesiumUnminified/Cesium.js` (1.143.0): line **248276** gates the depth CLEAR that flag triggers on `globe.show`, and lines **247764–247781** run the `TERRAIN_CLASSIFICATION` pass BEFORE that clear. The flag therefore does not starve classification. What starves it is a **HIDDEN globe** — which is what Forma held when L-635 was measured, and is the same condition that made `globe.getHeight` unusable there (§CTX-PERFOOTPRINT-SAMPLE). §TERRAIN-NORMALS now shows the globe under relief, so clamping is plausible **by code path**; L-11840 (heatmap invisible with the globe shown) is not explained by that reading, and no browser session has tested it. **Clamping stays unwired until someone measures it in a browser** — recorded as an open option with its citation, not as an impossibility.
...
The per-layer offsets are one ordered set, owned by `reseatContextGroundFeaturesForBase` and stated there: landuse `+0.005` < parks `+0.01` < roads / sea `+0.02` < rail `+0.022` < water `+0.03`. ⭐ **Corrected 2026-09-05 (L-12924): they are offsets above the FEATURE'S OWN ground (§12.3), not above one settled base** — and `rail` is now IN the ladder. It was in neither: the rail loader seated tracks at the layer base and `reseatContextGroundFeaturesForBase` did not lift them at all, so on a city whose terrain settles upward the tracks stayed at the load-time height while every other layer rose. A ladder that omits a shipped layer is not a ladder; §12.4 exists precisely so a new layer takes its place EXPLICITLY, and rail never did. The literals live in `groundFeatureSeat.ts` (`GROUND_LAYER_OFFSET_M`) and `groundFeatureSeat.spec.ts` asserts they equal the ones the re-seat carries, so the load-time seat and the re-seat cannot drift apart silently. A new layer MUST take a place in that ladder explicitly. Prose at a call site that contradicts the number is a defect in its own right — the water loader carried *"sit water just BELOW the road hair-line"* beside a seat that has always been **above** roads.
...
- **Reference (read-only):** `apps/editor/src/ui/geospatial/groundFeatureSeat.ts` (the pure seat decision + splitters) · `apps/editor/src/ui/geospatial/CesiumViewport.ts` (`resolveGroundDrapePieces`, `loadContextWater`, `loadContextRoads`, `loadContextRail`, `loadContextSea`, `reseatContextGroundFeaturesForBase`, `paintMetricTexture`) · `apps/editor/src/ui/geospatial/contextWater.ts` (`waterwayKind`, `waterwayDuplicatesArea`) · `apps/editor/__tests__/formaWaterwayGroundRibbon.test.ts` · `apps/editor/src/ui/geospatial/__tests__/groundFeatureSeat.spec.ts` · `apps/editor/__tests__/CesiumViewportGroundDrapeReseat.test.ts`.

---

## §13 — The 3D-Site SCOPE: one value, every layer cut to it, complete inside it (§SITE-SCOPE, L-645)

> Added 2026-09-07. Founder, verbatim: *"I want to have a slide of the scope on 3D Site view — like cityweft does — basically we have a scope — could be circular or rectangular whatever is easier for you — and then we crop everything — absolutely everything — but within the scope should be sound — really detailed and completed."* Decided by [ADR-0382](../adrs/ADR-0382-site-scope-one-value-cut-per-layer-class.md); grounded in [`AUDIT-3D-SITE-SCOPE-CROP`](../../03-execution/plans/AUDIT-3D-SITE-SCOPE-CROP.md). **Status: Phase 1 (value, polygon, geometric clip, command) and Phase 2 (render-path wiring, the per-pane slider, and the ARMED cut) BOTH LANDED — `d602e1bf` · `c76fef51` · `26396bb8` · `9784a3e8` · `bd76a346` · `bce1d42c` (carries this lane's per-layer pre-clips) · `0f4bfc8d`. ⚠ NOT BROWSER-VERIFIED: the globe clip's WebGL-2 feature detect and the flat-shaded slab side cannot be asserted headlessly (AUDIT §7.1/§7.2), and two named limits stay OPEN — sea island holes (§AUDIT F-3) and the tree read past ~891 m.**
...
|---|---|---|
| Globe surface (baked terrain, flat fallback, base colour) | `globe.clippingPolygons`, ONE polygon, `inverse:true`, feature-detected; a failed clip leaves the full terrain AND draws no slab side | `CesiumViewport.applySiteScopeClip` (ARMED 2026-09-07, `0f4bfc8d`) |
| Slab side + floor | a `Primitive` (`WallGeometry` on the ring + floor cap), `PerInstanceColorAppearance({ flat: true })`, neutral grey `#E6E6E3`, shadowless. **A LIT material on the slab side is FORBIDDEN** — that is the "red ring" | same |
...
- **Reference (read-only):** `packages/schemas/src/site/SiteScope.ts` · `packages/stores/src/site-commands/siteSetScope.ts` · `apps/editor/src/ui/geospatial/siteScope.ts` · `apps/editor/src/ui/geospatial/scopeClip.ts` · `apps/editor/src/ui/geospatial/__tests__/siteScope.spec.ts` · `apps/editor/src/ui/geospatial/__tests__/scopeClip.spec.ts` · `apps/editor/src/ui/geospatial/contextExtentBudget.ts` (the range + the runtime radii) · `apps/editor/src/ui/geospatial/CesiumViewport.ts` (`clearContextEarthSlab` — the retired teardown; `applyParcelClipToPhotorealTiles` — the §7 sibling) · `apps/editor/src/engine/views/SiteScopeSlider.ts` (the per-pane control) · `apps/editor/src/ui/site/siteScopeDispatch.ts` (`dispatchSiteScope` — the ONE write path — and the parcel floor; re-exported from `siteDispatch.ts`) · `apps/editor/src/engine/views/SiteAuthoringPaneShell.ts` (`defaultSiteScopePorts`, mounted per pane) · `apps/editor/src/ui/geospatial/__tests__/siteScopeSwapNeverBlank.spec.ts` (the §13.4 swap order) · `apps/editor/src/engine/__tests__/siteScopeSlider.spec.ts`.
...
| Date | Change |
| 2026-09-07 | **§13 Phase 2 LANDED and the CUT is ARMED (lane SCOPE-SLIDER).** The per-pane slider (`SiteScopeSlider.ts`, mounted inside each pane beside the view picker) previews on drag and commits on release through `site.setScope`; its track value is the CIRCUMSCRIBING radius in both shapes, it carries the §13.5 complete mark, and it prints the biting cap's own line with its numbers. Every layer class now pre-clips (rings, polylines, footprints, instanced points), so `SITE_SCOPE_CLIP_ARMED` flips true and the globe cut + flat-shaded neutral slab side run. ⛔ The teardown that would have silently undone it — a `clearContextEarthSlab()` under a "RETIRED" comment, ~400 lines AFTER the same method raises the cut — is replaced by an idempotent re-assertion; it would have read as "the clip does not work". §13.4's swap-never-blank order is now PINNED by a spec that the source had cited for a day without it existing. Default shape is a RECTANGLE at the SAME circumscribing radius (√2 half-extents), so no derived radius or tile count moves. **Still NOT browser-verified; sea island holes and the >891 m tree read stay OPEN and are named at the arming constant.** |
...
| 2026-09-05 | **§12.3 extended to a PER-FEATURE seat, §12.4 corrected (§GROUND-DRAPE-ON-RELIEF, L-12924).** The founder on Lisbon's Baixa: *"the grey layer (urban landuse) … is CUTTING the buildings — not set on the correct height."* One scalar for a whole layer is a plane through a hillside; the buildings beside it had been seated per footprint since L-635. Every ground feature now seats on the terrain at its OWN representative point, a feature spanning >3 m of measured relief is SPLIT into per-seat pieces, and `rail` — which the re-seat had never lifted — takes its explicit place in the ladder. The flat / keyless path is unchanged and pinned. `CLAMP_TO_GROUND` was re-evaluated against cesium 1.143 and stays unwired, but §12.3's stated reason (`depthTestAgainstTerrain=false`) is REFUTED with line citations — the real starver was the HIDDEN globe. NOT browser-verified. |
