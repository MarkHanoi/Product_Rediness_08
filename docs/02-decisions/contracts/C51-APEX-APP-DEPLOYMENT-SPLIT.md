...
Each gate is hard-fail on the production branch. **Four** gates are **LIVE** as of 2026-06-02 (the three apex-output gates via `npm run check:apex` + `check-no-product-routes-in-docs-site` via `npm run check:docs-site`; all run by the `apex-gates` CI job). The remaining three are **planned** — each gated on the migration step it protects (the strict-CSP gate would false-fail until the Fly CSP lands; the route-surface static wrapper and the DNS probe follow their phases).
