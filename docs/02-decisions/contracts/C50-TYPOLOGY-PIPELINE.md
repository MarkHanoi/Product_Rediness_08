...
> Tracker: O.12.a · SPEC: [SPEC-TYPOLOGY-BRIEF-SCHEMA.md](../../03-execution/specs/SPEC-TYPOLOGY-BRIEF-SCHEMA.md) · ADR: [0056](../adrs/ADR-0056-typology-declared-brief.md).
...
- **§2.6.5 — Living design parameters are a Stage-4 generative INPUT, not a parallel scorer.** A *design parameter* (a live slider re-influencing the generated layout) is the re-runnable sibling of a brief field: where the brief declares *what* you want, a parameter declares *how to prioritise* and re-runs Stage 4 LIVE. Parameters are typology-declared (same footing as `briefSchema`) and **bind to existing generative substrate** — the deterministic scorer's `ScoringWeights`, the program-rules permission matrix, and the climate `SolarBias.weight`/`siteLatitudeDeg` inputs — re-running the deterministic engine. They MUST NOT introduce a parallel scoring path or a hard-coded per-typology knob, and they re-rank/regenerate within the existing Stage-4 dispatch (no stage reorder). Design of record: [SPEC-LIVING-DESIGN-PARAMETERS](../../03-execution/specs/SPEC-LIVING-DESIGN-PARAMETERS.md), [ADR-0060](../adrs/ADR-0060-living-design-parameters.md). Tracker: A.25.
