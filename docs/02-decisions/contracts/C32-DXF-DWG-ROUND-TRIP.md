...
CI gate: ~~`tools/ga-gate/check-commandmanager.ts` (existing — extended)~~ → ⚠ **that path does not exist. There are TWO real gates plus a rival npm script, disagreeing — see †GATE-ALIAS** — flags any direct store mutation in `plugins/dxf/src/`.
...
CI gate: existing ~~`tools/ga-gate/check-three-import-boundary.ts`~~ → [`tools/ga-gate/check-three-imports.ts`](../../../tools/ga-gate/check-three-imports.ts) extends to `plugins/dxf/`. ⚠ **The cited path does not exist; see †GATE-ALIAS.**
...
| ~~`check-three-import-boundary`~~ → [`check-three-imports`](../../../tools/ga-gate/check-three-imports.ts) — ⚠ **†GATE-ALIAS** | `plugins/dxf/` MUST NOT `import * as THREE` | extends existing P2 gate. |
