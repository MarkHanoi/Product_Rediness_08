...
| **CRDT collaboration at design-tool fidelity** | Yjs and Automerge are mature enough, *as a technology*, to hold a BIM scene with hundreds of concurrent edits; explicit-conflict semantics solved. ⚠ **This is a statement about the industry's capability, not about PRYZM's measured capacity.** PRYZM's own tiers (50 / 300 / 1,000 concurrent) are all **CLAIMED, none HELD** — see [C66 §1](../02-decisions/contracts/C66-CONCURRENCY-AND-SCALE.md), whose §1.1 forbids describing a CLAIMED tier as supported. **Exit condition:** a recorded k6 run at tier VU count passing the C66 §6 thresholds. | Architects working with consultants, clients, and contractors in the same model — not in a chain of WeTransfer'd IFCs |
