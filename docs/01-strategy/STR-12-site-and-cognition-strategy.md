...
### §2.6 — Geodata analytical layers: the site substrate made VISIBLE from day one

The PG0 substrate (§2.4) makes the site *known to the model*. The **geodata analytical layers** make the site *legible to the designer* — the "WHERE-IT-LIVES" axis surfaced on the view the designer already uses. Governance: [C55 — Geodata Analytical Layers](../02-decisions/contracts/C55-GEODATA-ANALYTICAL-LAYERS.md) (DRAFT), [SPEC-GEODATA-ANALYTICAL-LAYERS](../03-execution/specs/SPEC-GEODATA-ANALYTICAL-LAYERS.md), [ADR-0065](../02-decisions/adrs/ADR-0065-geodata-analytical-layers-pluggable-provider.md) (PROPOSED).

**The thesis.** Early-stage land analysis is dominated by *constraints*, and constraints are *spatial*: where does it flood (100-year / 200-year / calculated-max), where is the landslide risk, how steep is the slope, what soil sits under it, where is the noise, where are the protected (Natura 2000) habitats, how dense is the surrounding population, what statutory detail-plans and heritage monuments bind the parcel. The reference tool (**Hektar**, a Nordic land-analysis tool) puts exactly these on a **country-grouped Layers panel** beside a 3D massing+terrain view — each layer a **toggle + opacity slider** draped over the terrain — and credits its sources (Lantmäteriet, SGU, Mapbox, Nimbo). PRYZM adopts that capability on its **Forma 3D view**: the constraints that decide whether a site is even buildable are visible *from day one*, not discovered in a post-hoc planning report.

**How it extends, not duplicates, the existing Forma overlays.** PRYZM's Forma view already drapes **climate** analysis — sun scrubber, soft shadow, wind rose + 3D streaks, comfort heat field (A.21.D24, [SPEC-FORMA-SITE-VIEW](../03-execution/plans/master-execution-tracker.md) §6) — and the [C54](../02-decisions/contracts/C54-IN-BROWSER-WIND-CFD.md) wind-CFD field is the high-fidelity sibling of those streaks. The geodata layers are the **third family on the same surface**: where climate overlays answer *sun/wind/heat*, geodata layers answer *flood/landslide/slope/soil/noise/habitats/population/statutory*. They share the substrate — the Site/parcel bbox ([C19](../02-decisions/contracts/C19-SITE-MODEL-AND-PARCEL.md)) in the [C12](../02-decisions/contracts/C12-GEOSPATIAL.md) coordinate frame, draped through the same Cesium/THREE owner. PRYZM already even carries a `HEKTAR_PALETTE` 2D cartography style (`siteMap2DStyle.ts`) inspired by the same reference — the analytical-layers panel is the 3D completion of that thread.

**The data-source strategy: a pluggable provider model, not a hardcoded country.** National open-data registries differ in coverage, CRS, format, licence, and rate limit. The strategy is a single **`GeodataProvider` abstraction** so each registry plugs in as an adapter without touching core (C55 §1.3): the **reference adapter is Sweden (Lantmäteriet cadastre/DEM + SGU soil/landslide)**; a generic **OGC (WMS/WMTS/WFS) fallback** reaches any standards-compliant registry; Mapbox/Nimbo-style imagery is a provider too. Norway and Denmark are *new adapters*, never core edits — exactly the per-country grouping the reference shows. Every source is **credited** (mandatory attribution, C55 §1.5 / [C23](../02-decisions/contracts/C23-PROVENANCE-AND-AI-AUDIT.md)), and sensitive layers (population, noise) carry the [C22](../02-decisions/contracts/C22-PRIVACY-AND-PII-TIER.md) privacy tier and respect [C49](../02-decisions/contracts/C49-MULTI-REGION-AND-SOVEREIGNTY.md) sovereignty.

**Discipline-neutral, like the rest of the substrate.** The layers are *site context*, not a residential feature — a hospital, a school, a commercial tower, an industrial shed all need to know the flood line, the slope, the noise contour, the protected habitat. Geodata layers benefit **any typology** (the §2.4 PG0.12 discipline-neutrality principle), which is why they live at the site substrate and not inside the apartment pack.

**Where it goes next.** GL.1–GL.5 (the SPEC §11 plan) are *visualisation only* — drape, toggle, opacity, attribution. The compounding move (reserved, not built) is to let a flood / landslide / slope layer feed an **existing** site-constraint / suitability input the generator already reads — a build keep-out surface — never a parallel objective (C55 §1.8). That is the §2.5 lesson applied: ship the substrate (the layers) first; the AI workflow that reasons over flood keep-outs compounds on it for free.
...
**Year 2**: L5 perceptual simulation expands (acoustic, sightlines, thermal-comfort). L7 typology priors expand to office + retail + hospital (each typology gets its own rules pack — possibly community-authored per [STR-10-platform-strategy.md §3](./STR-10-platform-strategy.md)).
...
The platform's value is partly the editor + the AI + the marketplace; but the **constraint database is the cognitive heart**. Marketplace authors can extend it (region-specific rules; typology-specific rules per [platform-strategy §3.3](./STR-10-platform-strategy.md)). PRYZM curates the open-source core.
...
6. **Per-typology constraint database authorship**. Currently PRYZM-curated. Should marketplace authors be able to publish their own typology-rules packs? Yes, per [STR-10-platform-strategy.md](./STR-10-platform-strategy.md); the curation surface gates this for regulated content.
...
| Doc | Relationship |
|---|---|
| [STR-01-manifesto.md](./STR-01-manifesto.md) | The "design intelligence" claim depends on these substrates |
| [STR-07-positioning.md](./STR-07-positioning.md) | The moats §4.1 derive from these substrates |
| [STR-02-product-vision.md](./STR-02-product-vision.md) | The user journey runs across the substrate (esp Step 3 site definition) |
| [STR-10-platform-strategy.md](./STR-10-platform-strategy.md) | Marketplace authors extend these substrates with typology / regional content |
| [STR-15-risks-and-assumptions.md](./STR-15-risks-and-assumptions.md) | Site + cognition risks (e.g. climate-data licensing) |
