# Astro retirement plan — `apps/docs-site/` post-ADR-0255

> **Stamp**: 2026-06-02 · **Status**: CANONICAL · **Authority**: [ADR-0255 §7](../../02-decisions/adrs/ADR-0255-one-pryzm-cloudflare-supabase.md) · **Owner**: @MarkHanoi
>
> **Purpose**: ADR-0255 declares "there is one PRYZM, and it is the editor." This plan enumerates every artefact in or referencing `apps/docs-site/`, classifies it (RETIRE / KEEP / MOVE), prescribes the exact retirement sequence, and gives a verification checklist. Future engineers should be able to redo this retirement step-for-step without re-derivation.
>
> **Timing**: Steps execute at **Phase A close** (after the `pryzm.so` apex DNS flips from `pryzmapp.pages.dev` → `app.fly.dev`, per ADR-0255 §3 Phase A). Each step is individually reversible until the final deletion commit.
>
...
| `apps/docs-site/src/pages/manifesto.astro` | **MOVE → editor** | Editor route. Source content already lives in `docs/01-strategy/STR-01-manifesto.md`; the editor reads that at build time (Vite import) instead of via JSON snapshot. |
...
| `apps/docs-site/src/content/docs/selfhost/STR-04-architecture.md` | **KEEP** | `docs.pryzm.so/selfhost/architecture` |

### §1.3 — `apps/docs-site/src/data/` (build-time data snapshots)

| Path | Classification | Post-retirement target |
|---|---|---|
| `apps/docs-site/src/data/pricing.json` | **RETIRE** | The whole snapshot pattern (ADR-0252 §1.4) is retired — the editor reads `@pryzm/entitlements` directly at runtime / build time. No JSON intermediate. |
...
| `apps/docs-site/package.json` | **KEEP** (edited) | Description rewritten: "PRYZM developer docs site (Astro Starlight) — docs.pryzm.so only per ADR-0255 §1." Dependencies unchanged. |
| `apps/docs-site/tsconfig.json` | **KEEP** | Unchanged. |
| `apps/docs-site/INVENTORY.md` | **KEEP** (edited) | Append a "2026-06-02 / ADR-0255" entry noting the customer-facing pages were retired here; the file's S63 D1 history is preserved. |
...
| `replit.md:23` + `:62` — references docs-site as a workspace app and as the home of headless mode documentation | Replit-platform readme. | **KEEP (edit)** | Update line 62 to clarify "developer docs at `docs.pryzm.so` (consumer-facing surfaces moved into the editor per ADR-0255)." |
...
| `docs/02-decisions/contracts/C31-DOCUMENTATION-AUTHORING-PROTOCOL.md` + C39 + C40 + C41 + C43 + C45 + C46 + C49 (mentions) | Contract docs reference docs-site as the publication target. | **KEEP (light edit)** | Each contract that points at the Astro marketing pages needs a one-line "post-ADR-0255 the surface moved into the editor" note. Many contracts mention `docs-site` only for developer docs — those stay verbatim. Audit each contract individually in step §7 below. |
...
| `docs/02-decisions/adrs/ADR-0039-s63-public-api-and-docs-site-foundation.md` | The ADR that created the docs-site workspace. | **KEEP (no edit)** | ADR-0039 created the developer-docs portion (which survives). The customer-facing layer was added by ADR-0252, retired by ADR-0255. Do not edit ADR-0039 — its scope is the surviving portion. |
| `docs/02-decisions/adrs/ADR-0042-s65-ai-public-api.md` · `ADR-0048-s67-self-host-docker-compose.md` · `ADR-0053-s71-perf-regression-hunt-and-hardfail-flip.md` · `ADR-0054-s72-m36-ga-launch-gate.md` | Phase-3 ADRs that reference the docs-site as a publication target. | **KEEP (no edit)** | They reference developer docs (Plugin SDK / API / Self-Host / GA gate). Surviving surfaces. |
...
| What if the developer docs (`docs.pryzm.so`) are retired later? | At that point: delete the `pryzmapp` Pages project AND the whole `apps/docs-site/` workspace. A future ADR would supersede ADR-0255 §1 to allow that. |
...
|---|---|---|---|
| `pryzm.so` (apex) | `CNAME-flat → pryzmapp.pages.dev` | `CNAME-flat → app.fly.dev` (Phase A) → later `pryzm.pages.dev` (Phase B per ADR-0255 §5) | Apex now serves the editor. |
...
| `app.pryzm.so` | reserved (no record) | (decision deferred; ADR-0255 §5 keeps it reserved) | Phase A serves the editor at the apex itself; `app.pryzm.so` reserved for Phase B+ if we want to separate marketing-vs-editor surfaces later. |
...
- Be removed if the editor doesn't pre-render those routes (Vite SPA — pre-render happens in Phase B per ADR-0255 §3).
...
## §6 — Documentation that needs a SEE ADR-0255 pointer

Each of the following must add a one-line "SEE [ADR-0255](../../02-decisions/adrs/ADR-0255-one-pryzm-cloudflare-supabase.md)" pointer at the top (or in the relevant section). The doc is otherwise preserved for historical accuracy.

| Doc | Where to add the pointer | Reason |
|---|---|---|
| `docs/02-decisions/adrs/ADR-0252-docs-site-marketing-surface.md` | Already done — ADR-0255 §0 stamps ADR-0252 as SUPERSEDED. Verify the stamp landed in the ADR-0252 file itself. | The original decision; reverse-link is mandatory. |
| `docs/05-guides/deployments/cloudflare-pages-setup.md` | Add top-of-doc banner: `> POST-ADR-0255: This runbook now applies to docs.pryzm.so only. The apex pryzm.so flip is documented in CLOUDFLARE-MIGRATION-RUNBOOK-2026-Q3.md.` Re-scope §7 wording from `pryzm.so` to `docs.pryzm.so`. | Was the apex-deploy runbook; is now the developer-docs runbook. |
| `docs/03-execution/plans/master-execution-tracker.md` (rows A.18 / A.18.a / A.18.b at lines 356-358) | Append `· **2026-Q3 (Phase A close)**: surface moved to editor per [ADR-0255 §7](../../02-decisions/adrs/ADR-0255-one-pryzm-cloudflare-supabase.md). Astro `pricing.astro` retired.` | Tracker is the working catalogue; needs accuracy. |
...
| `CLAUDE.md` | No mandatory edit (line 64 description still accurate). Optional: add a one-line note in the architecture section mentioning ADR-0255's "one-product-surface" rule. | The 8-layer model still names `docs-site` as an L5 app — true for the developer-docs surface. |
| `replit.md` lines 23 + 62 | Add "developer docs only per ADR-0255" qualifier to line 62. | Platform readme. |
| `docs/04-reference/architecture-detail/02-file-structure.md` | Add ADR-0255 cross-reference in the `apps/` section. | Reference doc must stay in sync. |
| `docs/04-reference/architecture-detail/03-final-map.md` | Same. | Same. |
| `docs/01-strategy/STR-04-architecture.md` + `STR-05-architecture-breakdown.md` + `STR-03-engineering-vision.md` | Add a one-line "see ADR-0255 for the one-surface rule" where the architecture is enumerated. | Strategy docs anchor newcomers. |
| `docs/01-strategy/README.md` | One-line `> 2026-Q3: customer-facing surfaces consolidated into the editor per ADR-0255.` | Index doc. |
...
| `docs/DOCUMENTATION-GAPS-AND-NEXT-PHASES.md` | Add a one-line "marketing-surface consolidation per ADR-0255" entry. | Gap-tracker. |
...
| `docs/01-strategy/STR-01-manifesto.md` | No edit (canonical source — the surface moved, the content didn't). | Source of truth for the manifesto page content. |
...
6.10. **Edit** `apps/docs-site/astro.config.mjs` — update header comment to "developer docs only per ADR-0255."
...
7.5. **Edit** `.dockerignore` — update line-72 comment to reference ADR-0255.
...
8.1. Edit each doc enumerated in §6 in a single PR titled "docs: post-ADR-0255 sweep — pointer updates."
...
- **2026-06-02** — Plan authored at ADR-0255 acceptance. Awaiting Phase A close to execute.
