# Phase A migration runbook — one PRYZM goes live (ADR-0255 Phase A)

> **Stamp** · 2026-06-02 · **Status**: AUTHORITATIVE — companion to [ADR-0255](../../02-decisions/adrs/ADR-0255-one-pryzm-cloudflare-supabase.md)
> **Owner**: @MarkHanoi
...
> **Companion documents**:
> - [ADR-0255](../../02-decisions/adrs/ADR-0255-one-pryzm-cloudflare-supabase.md) — the contract this runbook executes (§0 apex/app split, §3 Phase A row, §5 DNS map).
...
- [ ] **3 pre-flip security gates closed** (the three explicitly called out in ADR-0255 §3 Phase A row's risk gate (b)):
...
- [ ] **Supabase Pro upgrade decision logged**. Two paths:
  ...
  - **Path B (alpha-only deferral)**: stay on Free until first paying tenant. Add an explicit note in the launch ticket: *"Phase A on free tier; Supabase Pro to be enabled before first paying tenant per ADR-0255 §2."* Free tier means daily backup only — fails C48 audit, so this path is incompatible with selling subscriptions.
...
3. Form:
   ...
   - **Region**: **`Central EU (Frankfurt) — eu-central-1`**. This is mandated by **C22 §1.3** + **C49 §1.2** + ADR-0255 §5. Do not pick anything else.
...
- `PUBLIC_BASE_URL` is `https://app.pryzm.so` (NOT the apex `https://pryzm.so`). The apex never receives auth traffic per ADR-0255 §5; OAuth `redirect_uri` is built relative to `PUBLIC_BASE_URL` (`server/oauthService.js:137-151`).
...
The deletions land as a single PR titled `chore: ADR-0255 Phase A close — Astro retirement`. CI must be green; the new `check-no-duplicate-surfaces.mjs` gate (ADR-0255 §1 promised, may not yet exist — see ASTRO-RETIREMENT-PLAN §5.3) will reject any reintroduction.
...
Snapshot the actual cost surface 24 hours after the flip. If anything is off-projection, document + amend ADR-0255 §2.

| Resource | Free-tier headroom | Snapshot value (fill in) | ADR-0255 §2 projection | Verdict |
|---|---|---|---|---|
...
Monitor these every day for the first week. Document anomalies; major regressions trigger an ADR-0255 amendment.
...
- **2026-06-02** — Authored at ADR-0255 acceptance. To be executed at Phase A close (Sprint 3 / 2026-Q3). Companion to PRODUCTION-HARDENING-CHECKLIST + ASTRO-RETIREMENT-PLAN.
