# Production-hardening checklist — pre-Fly.io Phase A deploy (ADR-0255)
...
> **Companion to** · `docs/02-decisions/adrs/ADR-0255-one-pryzm-cloudflare-supabase.md`
...
|---|---|---|
| **Content-Security-Policy** | GOOD (enforced in prod) | `securityHeaders.js:148-156`. Report-only in dev, enforced in prod. `script-src` includes `'unsafe-eval'` (Three.js shader compilation — tracked for removal in ADR-0247). `'unsafe-inline'` is dev-only (`SCRIPT_SRC_PROD` at line 90 excludes it). `style-src` keeps `'unsafe-inline'` permanently (CSS-in-JS + Three.js CSS2DRenderer). Acceptable for Phase A. |
...
**Phase A setting**: `ALLOWED_ORIGIN=https://pryzm.so,https://www.pryzm.so` (the `www` redirects per ADR-0255 line 83 but its CORS will still preflight). Add `https://staging.pryzm.so` if/when that subdomain ships.
...
| Check | Verdict | Reference |
|---|---|---|
| Connection-pool sizing | WEAK for production | `max: 10` (`pgClient.js:62`). Fly's free-tier machine has 256MB RAM (ADR-0255 line 65); 10 pool slots × ~6MB / connection is fine. Supabase free tier limits to 60 direct connections — 10 is safe. **Bump only after scaling past free tier.** |
...
Per ADR-0255 §A (Sprint 3), when DNS flips:
...
Per ADR-0255 line 45 + line 147 (C48 reference):

| Target | Mechanism | Verdict | Notes |
|---|---|---|---|
| **Database** | Supabase PITR (Point-in-Time Recovery) | Plan-dependent — Free tier = daily backup only, no PITR. **Pro tier ($25/mo) is required for PITR.** | ADR-0255 budget assumes Pro tier "at scale" — confirm enabled **before** first paying customer. |
| **Object storage** | Cloudflare R2 versioning | Per ADR-0255 §C48 reference | R2 versioning is a per-bucket flag — verify ON before any user file is uploaded. |
