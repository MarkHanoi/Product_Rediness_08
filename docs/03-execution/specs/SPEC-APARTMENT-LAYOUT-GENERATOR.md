...
| **A6-core** (cmd set) ✅ | `buildLayoutCommands(option, opts, mintId)` extends A6-core: composes `buildLayoutPlan` then **PRE-MINTS** wall/door/opening ids (injected `mintId` = `createId`) so each door references its host wall directly — **no store read-back** (deterministic; the §12 step-4 "resolve from created walls" is satisfied by construction). Emits the ordered set: `wall.batch.create` (ids) + per-door `wall.createOpening` (`opening.elementId === door id`, the C15 cascade) + `door.batch.create`. 6 tests. | command set has real verbs + metres + cascade-correct ids |
| **A6-wire** ✅ | editor `ApartmentLayoutExecutor.attach` subscribes `apartment.layout-execute {optionIndex}` → reads the AIStore option → `buildLayoutCommands(…, createId)` → dispatches the three verbs through `runtime.bus.executeCommand` **inside** `batchCoordinator.runBatch({levelIds, totalElementCount, skipRedetectRooms:false})` → **ONE undo entry** + automatic room redetect → emits `apartment.layout-executed` + clears the store. P6 (bus-only mutation); typed `batchCoordinator` singleton (no window); wired in the AI panel next to the controller. | one undo unit; rooms detected ✅ |
...
SPEC-47 (precedent — read first), **SPEC-TGL-DETERMINISTIC-LAYOUT-ENGINE (the offline generative engine, A8)**, C16 §8 (batch), C17 (catalogue entry #51), C09 (AI L7.5), SPEC-28 (cost), SPEC-07 (approval queue), C15 (hosted doors), SPEC-SEMANTIC-DESIGN-ASSISTANT §10 (#51), PlanarTopologyEngine (room detection).
