...
7. **Single resolved site-store accessor (P1) — the Bug 2 lesson.** All site-state reads (parcel boundary, envelope, site model) MUST go through ONE resolved site-store accessor — a `runtime ?? window.runtime` resolution or a single shared `getSiteModelStore()` — never per-call-site direct `runtime.siteModelStore` reads that can diverge. Phase 1b's boundary-not-in-3D bug root-caused here: the boundary was committed into `window.runtime`'s store, but a reader that had captured a null/stale `runtime` saw an empty store → "no parcel boundary yet"; some call sites trusted `siteDispatch`'s module globals while others read `runtime.siteModelStore` directly, and the two silently diverged. One resolved accessor keeps every reader and writer on the same store instance. This is the P1 single-composition-root rule applied to site state — an architectural invariant, not just a bug note (see C59 §2 invariant 7).
