...
| Sprint | S85–S86 |
| References | `13-` §2; `[strategic ADR-0242]`; SPEC-30 (plan-view); ADR-0227 (schedule formulas) |
...
- Per-row formula expansion via ADR-0227 schedule formulas extended to look up `RateRef` from cost library (SPEC-45).
...
### §2.4 4D simulation playback per `[strategic ADR-0242]`

ADR-0242 ratifies: **client-side timeline replay** (not server-side video render) for in-app playback. Server-side MP4 export is a separate output (also ADR-0242) for share / report use.
...
| S85 D7 | BoQ sheet template; ADR-0227 formula extension to cost library refs |
...
| S86 D5 | client-side timeline replay (per ADR-0242); per-frame element-visibility map |
...
- Server-side per-frame rendering for in-app playback (ADR-0242 ratifies client-side).

## §6 Cross-references

- ADR-0227 schedule formulas
- ADR-0242 4D playback strategy
