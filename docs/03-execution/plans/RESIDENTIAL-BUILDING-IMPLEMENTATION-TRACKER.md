...
|---|---|---|---|---|---|---|---|
| P3.1 | `corePlacement:'centre'` additive path | TODO (deferred) | `houseLayout/stairCore.ts:275`, `stairPosition.ts:557`/`:686` (new `'centre'` candidate forced) | house path byte-identical (`'worst-aspect-corner'` default); building passes `'centre'` | `§DIAG-STAIR-RULE R-CENTRE` | ADR-0063 H3 (containment); ADR-0075 PC2 (no-op proof for house) | n/a (param) |
| P3.2 | `residentialBuildingOrchestrator.ts` (storey-loop skeleton) | **DONE** | NEW `packages/ai-host/src/workflows/residentialBuilding/residentialBuildingOrchestrator.ts` (PURE L2; mirrors `houseOrchestrator.ts:467`/`:762`; computes the centred core DIRECTLY — does NOT touch the house default; P3.1 wires the `'centre'` param INTO `chooseStairCorePosition` later) | N upper + 1 ground level minted; centred-core R-CENTRE invariant (core centroid ≈ footprint centroid on every level); ground = NO apartments (commercial stub); upper = N apts via packer→partition; per-cell D-TGL P7 seam; deterministic; soft-fail | `§DIAG-RESI-ORCHESTRATE levels=… coreCentre=(x,z) apartmentsPerLevel=[…]` | C53 (generative engine); audit §3 (divergences); C50 §1.7 (soft-fail); ADR-0063 H3 (centred core trivially contained); P8 span (`pryzm.ai.workflow.residentialBuilding.orchestrate`) | flag |
...
| id | title | status | files | accept | §DIAG | contract / ADR | gate |
|---|---|---|---|---|---|---|---|
| P6.1 | The packer (pure: net area → N apartments + typologies) | **DONE** | NEW `packages/ai-host/src/workflows/residentialBuilding/apartmentPacker.ts` (PURE L2, test-first); `packApartments(net, {min,max,typologies})` → `{typology,targetAreaM2,program}[]`; T1→1bed…T4→4bed map (`typologyBedrooms`); program built consistent with `scaleProgramToShell` (baths = clamp(1..3, floor(beds/2)); masterEnSuite when beds≥3) but PINS the count | every apt's targetAreaM2 ∈ effective band (intersection of user [min,max] with the §6 typology band); mix uses only enabled T1-T4; infeasible (net<min / no typology / empty band intersection / net≤0) → C50 soft-fail `{status:'rejected'}` (never throws) | `§DIAG-APARTMENT-PACK level=k N=… mix=[…] areas=[…]` | C50 §1.7; audit §6.1; T1-T4 map §6; P8 span (`pryzm.ai.workflow.residentialBuilding.apartmentPacker`) | flag |
