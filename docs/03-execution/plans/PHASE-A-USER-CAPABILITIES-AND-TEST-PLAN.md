...
| 7.4 | WCAG 2.2 AA contrast on every surface | All UI | A.32 (axe-core CI PLANNED) + A.34.a (token-pair audit DONE) + A.34.b (focus-ring + form border tokens PLANNED) | 🟢 IN PROGRESS | `pnpm --filter @pryzm/a11y-tokens test` → 0 failing pairs (audit gate); `pnpm test:axe` (E2E gate) still pending |
