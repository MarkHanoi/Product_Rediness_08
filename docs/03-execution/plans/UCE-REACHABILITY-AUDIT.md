...
| A8 | **Reference planes** (§14) | **WIRED — and, as of §82.4, CONSEQUENTIAL** | ⭐ **This row was true and IDLE, which is the more interesting fact.** `makeAddReferencePlaneMigrator` was reachable, but the bake refused any direction but +Y, so a plane was *a datum nothing read* — an author had no reason to make a second one. Now: `[data-cdw-add-plane]` mints a named plane with a chosen normal, the list prints each normal, and a per-shape `[data-cdw-shape-plane]` select re-bases a solid onto a plane through `makeSetExtrudeWorkPlaneMigrator` (which writes the profile's `planeId` and the solid's `direction` in ONE act). Gesture chain measured end to end in `apps/editor/__tests__/componentWorkPlaneGestureReach.test.ts` — the live preview's own mesh goes from 0.6 × **2.4** × 0.4 m to **2.4** × 0.6 × 0.4 m. ⭐ **RENAME landed 2026-09-06** (§82.1-NAME-A-DATUM): `[data-cdw-plane-rename]` → `makeRenameReferencePlaneMigrator`, which enforces name UNIQUENESS because the chooser picks a plane BY NAME and never by id, and which is safe on a plane already carrying shapes precisely because the **id never moves**. ⛔ Still absent from §82.1: **reorient**, **delete**, and *dimension to a plane*; `ReferencePlane.origin` is **not applied** and the work-plane op refuses an offset plane rather than half-honouring it |
...
| **82.1** | Reference planes — named, on any work plane, carrying parameters | ⚠ **PARTIAL — three of the acceptance's four clauses are now reachable** | The acceptance's own words: *"Create a reference plane in a 2-D view; it is visible in the 3-D view; rename it; dimension to it."* ✅ **create** — `[data-cdw-add-plane]` with a chosen normal. ✅ **visible in 3-D** — a shape re-based onto it swings the live viewport's mesh from 0.6 × 2.4 × 0.4 m to 2.4 × 0.6 × 0.4 m. ✅ **rename it** — `[data-cdw-plane-rename]` → `rename-reference-plane`, landed 2026-09-06. ⛔ **NOT MET: dimension to it** — there is no dimension object in the schema at all (see 82.3), so the fourth clause is not a UI gap, it is a MODEL gap. Reorient and delete are also still absent, deliberately. Whole chain in `componentWorkPlaneGestureReach.test.ts` |
