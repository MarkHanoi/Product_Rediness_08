...
| **R3** | **9 `wave-6` panels** — `ComponentHistoryPanel`, `ComponentParameterPanel`, `ComponentRelationshipPanel`, `ComponentValidationPanel`, `DetailComponentPanel`, `FamilyBrowserPanel`, `FamilyConstraintPanel`, `FamilyPreviewPanel`, `FamilyPropertiesPanel` | 9 files + 9 binding specs | **Test-only.** None appears in `PANEL_REGISTRY` (`panelDefaults.ts:193`, 15 ids, zero Component/Family) or in `PanelManager` (`PanelManager.ts:38-55`). Each binds `runtime.viewRegistry.activatePanel` inside a `show()` **nothing ever calls** — an internally consistent set that never met a composition root. ⭐ They are also the **superseded vocabulary generation** (ADR-0376 D5): they say *Family* on user-facing surfaces, which the live browser's own suite forbids (`componentPlacementFlowThroughComposedRuntime.test.ts:232` asserts the browser's text `.not.toMatch(/family/i)`) |
...
| **6** | **Chat-capability ratchet** — `check-chat-capability-coverage` **UNDECLARED 17, baseline 0** | ⛔ A **RED ratchet this family made worse**, deliberately and on the record | C113 §12.4: the declaration surfaces are Phase 4H's exclusive files and 4H's entry condition is *"4C's verbs registered"*. **OWED, not acceptable.** ⛔ Per §RATCHET-EXCEEDED-IS-NEVER-DEBT this may **not** be absorbed into `gate-debt.json` |
