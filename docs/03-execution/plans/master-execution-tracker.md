...
| **A.21.D24** | 🛋️ **Furnish-room command did NOTHING** (founder 2026-06-06: `pryzmFurnishAllRooms()` + the AI furnish button placed no furniture — must work for apartment AND house) | **ROOT CAUSE (house = primary):** the `FurnishLayoutExecutor` resolves each room's furniture archetype from `room.occupancyType` (e.g. `living-room`, `bedroom`). The **apartment** path set that via `ApartmentLayoutExecutor._nameDetectedRooms` (`room.rename` → `SetRoomOccupancyCommand`), but the **`HouseLayoutExecutor` never named its rooms or set occupancy at all** — so every house room had `occupancyType === undefined` + a generic `"Room 00-NNN"` name → `furnishRoom('')` → `archetypeFor('') === null` → `[]` for every room → zero `furniture.create` commands → silent no-op. (NOT a D20 signature drift — `furnishRoom(input, options={})` is back-compatible and the executor passes `furnishOptions` correctly; NOT a D15 regression; the event wiring + console fn reach the executor fine.) **FIX:** (1) extracted the apartment naming pass into a shared `nameDetectedRooms.ts` (P6 — `room.rename` command, self-polling for the deferred redetect) and made the apartment executor delegate to it; (2) the **house executor now names + occupancy-tags rooms on EVERY storey** (per-storey `nameDetectedRooms(...)`) after openings land, with a 600 ms settle, BEFORE `runHousePostGenChain` furnishes — so each storey's rooms carry a recognised `occupancyType`; (3) ROBUSTNESS — `FurnishLayoutExecutor` now resolves occupancy as `occupancyType` **OR name-derived** (new pure `furnishOccupancy.ts` `primaryOccupancy`/`occupanciesForRoom`), so a room whose naming pass hasn't applied yet (or a manually-drawn room with a recognised name) still furnishes; (4) the 0-items path is now LOUD — `§FURNISH-EMPTY` logs each room's resolved occupancy so the cause is never silent. **PROOF:** with occupancy set (or name-derived), the single-room path runs `furnishRoom(input, furnishOptions)` → `buildFurnishCommands(...)` → dispatches `furniture.create` inside one `runBatch` (unchanged). **Files:** `apps/editor/src/ui/apartment-layout/nameDetectedRooms.ts` (new shared), `ApartmentLayoutExecutor.ts` (delegate), `house-layout/HouseLayoutExecutor.ts` (per-storey naming + settle before finish chain), `furnish-layout/furnishOccupancy.ts` (new pure helpers), `FurnishLayoutExecutor.ts` (name-fallback occupancy + §FURNISH-EMPTY diagnostic), `apps/editor/__tests__/furnishOccupancy.test.ts` (new, 6 describe groups). **TYPECHECK:** worktree has no node_modules → not runnable in-agent; changes are type-aligned to `ScoredLayoutOption.rooms` (`LayoutRoom`: name/occupancy/area/centroid/polygon) + `FurnishRoomRef`. **NEEDS IN-BROWSER VERIFY:** generate apartment → `pryzmFurnishAllRooms()` → furniture appears; generate a house → auto-chain furnishes every storey (or stand on a storey → `pryzmFurnishAllRooms()`). | ✅ DONE (in-browser verify pending) |
