...
| **A.U.19** | **AI design-assistant panel: scrollable create-commands list** | The AI design-assistant right-panel's "create commands" list does NOT scroll when the catalogue overflows the viewport — items below the fold are unreachable. Fix: wrap the list in a scrollable container (`overflow-y: auto`) + keep the panel header + footer fixed (sticky positioning) + restore scroll position on panel re-mount per [C06 §4 panel UX](../../02-decisions/contracts/C06-UI-SHELL-AND-TOOLS.md). Touch surfaces: `apps/editor/src/ui/tools-panel/panels/CreateRailPanel.ts` + `apps/editor/src/ui/ContextualEditBar.ts`. | 🟡 NEXT UP (Sprint 2) |
