...
| A.8.c Polygon-draw tool (Hektar-style) 🟢 FIRST CUT 2026-06-03 (`SiteBoundaryDrawTool.ts` Cesium click/dblclick/Esc → `boundaryProjection.ts` equirectangular XZ + edge-class → `site.setParcelBoundary`; `pryzmStartBoundaryDraw()`). Browser-verify: pick on 3D-tiles, projection accuracy, frontage heuristic. Residual: proj4 LTP-ENU swap, drag-edit, >30 warn/>200 refuse (C19 §1.4) | IP-A2 (vertex-click + close-loop) · IP-A4 (drag-edit + OSM snap) |
