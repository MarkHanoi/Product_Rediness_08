...
| **A.21.D33(a)** | 🛡️ **Stairwell-void RAILING off / not aligned with the slab + floor void** (founder 2026-06-07) | The A.21.D26 guardrail RENDERS (A.21.D29 #2) but is OFFSET from the hole. ROOT CAUSE: `HouseLayoutExecutor._createVoidGuardrail` computed the void rect from `stair.rectMm` (the stair's allocated LAYOUT-frame CELL, rotated about the pivot), whereas BOTH the SLAB void (`CreateStairCommand.autoCreateOpening`) and the FLOOR/CEILING void (A.21.D29 #1, `recordStairVoid`) use `computeStairFootprintRect(...)` (sized to the stair's actual flight/landing BOUNDING footprint) — two different rectangles → the rail sat off the hole. ✅ **FIXED 2026-06-07 (editor-only, `HouseLayoutExecutor.ts`):** `_createStair` now hoists the SINGLE `voidRect = computeStairFootprintRect({shape,width,treadDepth,startPosition,flights:worldFlights,landings})` (the exact value already passed to `recordStairVoid` + identical to the slab opening's input) and passes it straight to `_createVoidGuardrail`, which rails THOSE world-XZ corners verbatim (no second rotation — `computeStairFootprintRect` is fed the already-rotated `startPosition`+`worldFlights`, so its corners are world-space). Step-off-open logic (max dot of edge-outward vs final-flight dir), height 1.050 m, baluster/rectangular, best-effort try-guard ALL preserved. Now all three surfaces (slab hole · floor/ceiling hole · railing) share ONE footprint polygon → rail edges coincide exactly with the void edges by construction. `_createVoidGuardrail` signature changed (now `(cm, voidRect, topLevelId, lastDir)` — dropped `rectLayout`/`angleRad`/`pivot`); single call site updated; `_rotateXZ`/`_rotateXZDir` still used by `_createStair`. No new deps; apartment/single-storey path unaffected (no auto-opening stairs → registry empty). **VERIFY (worktree has no node_modules):** verified by inspection (`voidRect` is the literal arg to `recordStairVoid` + matches `CreateStairCommand`'s `computeStairFootprintRect` input). **NEEDS IN-BROWSER VERIFY:** generate a 2-storey house → upper-storey plan + 3D → the 3 railed edges sit exactly on the slab/floor void edges (no offset), step-off side open. | 🟢 DONE (editor; in-browser verify pending) |
