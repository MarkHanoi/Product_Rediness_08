...
| **A.21.D21** | 🏚️ **House v35 prod defects** (founder in-browser test 2026-06-06, post-v35) | Five real defects from the first live `House` generation: **(1) NO design modal** — house builds the layout DIRECTLY; apartment shows a "Choose a layout" options modal, house must too (per-storey options, A.21.k). **(2) Roof doesn't match the layout** — the rendered roof floats/offsets off the footprint (screenshot: parallelogram roof shifted off the plan) → coordinate-frame / position bug in `_createRoof` (likely the principal-axis rotation or the world-vs-emit frame not applied to `RoofDescriptor.footprint`). **(3) Not all perimeter walls created** — the house shell doesn't close (open sides) → the per-storey perimeter-wall emission in `HouseLayoutExecutor` (upper storeys build their own perimeter; ground reuses the drawn shell) is incomplete. **(4) Stairs crash into walls** — the stair geometry intersects interior walls → the reserved stair-core rect isn't excluded from room subdivision / walls are built across it; the core must be a true keep-out. **(5) Graph must auto-cover ALL rooms + relationships** without node selection — the whole-project room graph should render by default (Living Graph already builds on open; ensure it shows every room + edges, and the static graph too). REVIEW → DOCUMENT → AMEND. Geometry defects (2/3/4) = `HouseLayoutExecutor` + `houseOrchestrator`/`stairCore`; modal (1) = A.21.k; graph (5) = living-graph/graph. | 🔴 HIGH (post-v35 demo defects) |
