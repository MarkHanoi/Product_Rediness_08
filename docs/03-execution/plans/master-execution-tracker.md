...
| **A.21.D21** | 🕸️ **Graph slice: whole-project room graph by default (ALL storeys, no selection)** (founder 2026-06-06: "the graph should cover ALL rooms in the project + their relationships, WITHOUT having to select a node first") | The Living + static Building-Graph overlays already build the UBG on open and render every ROOM node + relationship edge without a click — but the UBG was **silently active-level-scoped**: `resolveLevelIds()` in `buildBuildingGraph.ts` read ONLY `bimManager.getAllLevels()`, which is **undefined on the real BimManager** (its method is `getLevels()`), so it fell through to the active-level-only fallback and upper storeys of a multi-storey house never entered the graph. ✅ **DONE 2026-06-06** — **(1)** `resolveLevelIds` (§UBG-ALL-LEVELS) now tries EVERY level enumerator in turn — `bimManager.getLevels()` (canonical) → `getAllLevels()` (legacy alias) → `wallStore.getLevels()` → `projectContext.levels` — de-duplicated, first non-empty wins; the single active level is the LAST-resort fallback only. So one UBG now aggregates rooms from **every storey**. **(2)** §UBG-LEVEL-TAG — Living Graph `GraphNode` gained an optional `level` (humanised storey from the room node's `levelId` prop the roomGraph adapter stamps); the inspector line prefixes the storey so a multi-storey house's rooms are distinguishable. Static graph already exposes `levelId` in its detail-panel Properties. Both overlays already render all nodes+edges on `show()` (Living `§LG-BUILD-ON-OPEN`; static `rebuildFromGraph()` in `show()`) — no selection needed. Additive; D16/D17 untouched; P3-safe (frame-bus, no rAF); no `Math.random`. **Tests:** +2 describe blocks in `buildBuildingGraph.test.ts` — two-storey aggregation (all four rooms in one graph, tagged by level) + `resolveLevelIds` across all enumerator shapes (canonical/legacy/wallStore/projectContext/active-fallback/dedup/throwing). Files: `apps/editor/src/engine/buildBuildingGraph.ts`, `apps/editor/src/ui/living-graph/{livingGraphSchema,livingGraphData,LivingGraphOverlay}.ts`. **IN-BROWSER:** generate a 2-storey house → open the graph → see EVERY room from BOTH storeys + their relationship edges immediately, no click. (Tests not run locally — Bash/PowerShell denied in this worktree; verified by review + parity with existing test fixtures.) | ✅ DONE (engine + overlay; in-browser verify pending) |
