...
| **A.21.D23** | 🌦️ **Climate · wind · weather LIVE in the Forma site view** (founder 2026-06-06: "then climate, wind, weather, etc. in Forma view?") | PARTIAL today: sun/shadow ✅ astronomically correct; climate-driven WINDOWS ✅ (affect generation); but the **wind rose is empty** (`No wind data — set a site + load climate`) and there's no live weather/temperature overlay tied to the building in Forma. The DATA exists — `@pryzm/climate-host` parses EPW (wind speed/direction + temps) + NOAA normals (`degreeDaysBuilder`/`designTempsBuilder`/`monthlyNormalsBuilder`). **Implement:** wire the climate-host wind + temperature data into `geospatial/FormaSiteAnalysisControls` — populate the wind rose from EPW wind, show a temperature/comfort profile + the design-temps/HDD-CDD card, all keyed to the authored site. Ensure it loads on site-set (the `set a site + load climate` path actually fires). Needs in-browser verify (Cesium/Forma render). Refs: C19 climate substrate + [APARTMENT-COGNITION-STACK]. | 🔵 QUEUED (founder 2026-06-06) |
