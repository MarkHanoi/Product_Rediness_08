...
| **A.21.D60** | ⟂ **Boundary draw: orthogonal-to-previous-edge lock** (founder 2026-06-08: "the FIRST line is an arbitrary site rotation (fine), but the SECOND+ lines are normally orthogonal to the FIRST") | A RELATIVE right-angle draw aid on the 2D cream/Hektar parcel-boundary map (`SiteBoundaryMap2D.ts`): once >=1 edge is committed, the NEXT vertex DIRECTION snaps to the nearest 0/90/180/270 degrees RELATIVE to the PREVIOUS edge (NOT true north), projecting the cursor onto that ray, so the user draws a clean rectilinear plot at ANY base rotation. FIRST edge is free. ✅ **SHIPPED 2026-06-08** — NEW pure screen-pixel helper `apps/editor/src/ui/geospatial/orthoSnap.ts` (`resolveOrthoSnap(prevStart, prevEnd, cursor, tol=8 degrees)` → cursor projected onto the nearest 90 degree ray, reach-preserving perpendicular foot, clamped >=0; null when out of tolerance / degenerate / <2 verts; NEVER throws). Composes with the existing A.8.c.g building corner/edge/loop snap via `resolveSnap(pt) ?? resolveOrthoSnapTarget(pt)` so a real corner ALWAYS wins; ortho only fills the gap. New `'ortho'` `SnapTarget.kind` reuses the violet snap indicator; the draw→commit→`site.parcel-boundary-set` + cream-map lifecycle are UNTOUCHED (ortho only changes the resolved cursor lng/lat the existing `onClick` already commits). Compact on-map HUD: bottom-centre brand white+#6600FF `⟂ Orthogonal to previous edge` checkbox (default ON), shown only while drawing, hidden on commit (`freezeDraw`) + removed on cancel (overlay teardown); toggling recomputes the live snap immediately. +8 unit tests (`apps/editor/__tests__/orthoSnap.test.ts`: ~88 degrees→90, near-collinear→0, near-reverse→180, 45 degrees free, wider-tol engages, skewed base rotation, perpendicular-foot reach, degenerate→null). Editor-UI only; deterministic pure math; typecheck sandbox-blocked in the worktree (no node_modules) → verified by reading. **NEEDS IN-BROWSER VERIFY:** draw a boundary → first edge free at any angle → second+ edges lock to right-angles off the previous edge → toggle off frees them → commit sets the boundary as before. | 🟢 DONE (editor-UI; in-browser verify pending) |
