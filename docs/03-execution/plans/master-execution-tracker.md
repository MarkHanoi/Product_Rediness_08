...
| **A.21.D46** | 🚪 **Corridor physiognomy — narrow strip, never a fat cell (RE-DONE with the sealing fix)** (founder rule "a corridor is a RECTANGLE — one dimension 0.9–1.2 m, the OTHER ≈2–6 m"; corridors generated FAT — live screenshot 17.8 m²) | **First attempt `5b472cfb` was correct-in-intent but REVERTED** — it clamped the corridor strip's LENGTH (`leftoverRect`/length-trim) and donated the freed band, which DISCONNECTED a served room from the spine → `doorMinimums.test.ts §EVERY-ROOM-ACCESS` failed ("room r3 (dining) is sealed — no door, no open boundary"). **ROOT of the regression:** shortening/narrowing the corridor + redistributing the freed band can strip a room of its only shared wall with circulation → `wallsAndDoors` can place no door → §SEALED-ROOMS flags it. **RE-DONE FIX (sealing-safe by construction + a hard gate):** (1) `programRules.corridor` re-adds `maxShortSideM 1.2` + advisory `minLongSideM 2.0`/`maxLongSideM 6.0` (§CORRIDOR-PHYSIOGNOMY) — but maxLong is BEST-EFFORT, never enforced by shortening the spine. (2) `subdivide.reshapeCorridorStrip` narrows a NEAR-SQUARE (squarified) corridor along its SHORT axis ONLY, donating the freed band to neighbours that already fully TILE it — it NEVER trims the long axis (the §SINGLE-RECT carve corridor — a 1.2 m strip — is a no-op). (3) `subdivideWithReport` finalise runs the reshape behind a SEALING-SAFETY GATE (`roomsWithAnySharedWall` before/after): the reshape is accepted only when no previously-wall-connected room becomes an island, else the unreshaped placements ship — so §EVERY-ROOM-ACCESS is a HARD guarantee. The 2-bed fixture's corridor stays a 1.2 m strip and NO room is sealed. GATE: `doorMinimums.test.ts §EVERY-ROOM-ACCESS` PASSES; +`tglCorridorPhysiognomy.test.ts` (9, incl. the exact doorMinimums 2-bed fixture → no sealed room WITH the reshape); FULL ai-host suite **2015/2015** (2006 baseline + 9), touched-file tsc clean; no regression to D41 central-blob, the stair fix, A.25.3, §FEASIBILITY-ALLOC, per-room openings, D45/D47. Files: `rules/programRules.ts` · `tgl/subdivide.ts` · `tgl/bubbleGraph.ts` · `__tests__/tglCorridorPhysiognomy.test.ts`. | ✅ RE-DONE (sealing fix) — narrow strip + every-room-access HARD |
