...
| **🎯 0.5 — DRIVING NOW** | **IP-A5.X** | Sprint 5 (overlap) | **One PRYZM: apex/app split + EU residency + Cloudflare + Supabase (Phase A bridge)** | "pryzm.so apex serves pre-rendered marketing (Cloudflare Pages); app.pryzm.so serves the editor (Fly.io, EU `fra`) — both from the SAME codebase per the motif.io / linear.app pattern." | ✅ A.17.x.0 (ADR-055 authored 2026-06-02), ✅ A.17.x.1 (ADR-052 marked SUPERSEDED), ✅ A.17.x.2 (Phase A artifacts: Dockerfile + .dockerignore + fly.toml + .github/workflows/deploy-fly.yml — wired to `/api/health/{live,ready}`), ✅ A.17.x.3 (Production-hardening checklist authored — 15 sections cited file:line; 3 pre-flip gates identified: trust-proxy=2 · STRIPE_WEBHOOK_SECRET · 6-route err.message leak), ✅ A.17.x.4 (Astro retirement plan authored — 6 pages retire, 17 docs survive at docs.pryzm.so), ✅ A.17.x.5 (Strategic-docs audit caught 4 critical ADR-055 errors: region · domain · auth · Phase D conflict invariant), ✅ A.17.x.6 (**ADR-055 AMENDED**: §0 apex/app split per motif pattern · §5 EU region `fra` per C22/C49 · §2 custom-JWT preserved per C08+ADR-045 · §3 Phase D conflict invariant per C08 §3.2 · §2 honest cost trajectory ($25/mo Supabase Pro floor for C48 RTO/RPO compliance)), ✅ A.17.x.7 (`product-vision.md` amended: pryzm.so canonical domain; pryzm.app aspiration retired), ⚪ A.17.x.8 (**ADR-056 planned** — Supabase Auth migration sequenced as Phase A.5, AFTER Phase A close), ⚪ A.17.x.9 (Phase A migration runbook — exact Fly + Cloudflare dashboard clicks), ⚪ A.17.x.10 (close 3 pre-flip security gates), ⚪ A.17.x.11 (Fly app `pryzm` provisioned in `fra` + first deploy green), ⚪ A.17.x.12 (apex pre-render step: `pnpm build:apex` emits static HTML for /, /pricing, /manifesto, /trust → Cloudflare Pages → `pryzm.so`), ⚪ A.17.x.13 (DNS: `pryzm.so` → `pryzm.pages.dev` apex · `app.pryzm.so` + `api.pryzm.so` → `pryzm.fly.dev` · TLS issued), ⚪ A.17.x.14 (Astro deletion: 6 pages + `scripts/build/gen-docs-site-pricing.mjs` + `apps/docs-site/src/data/pricing.json`), ⚪ A.17.x.15 (Cloudflare Pages project re-scoped to `docs.pryzm.so` only OR deleted), ⚪ A.17.x.16 (Supabase project provisioned in `eu-central-1` Frankfurt; Supabase Pro upgraded for C48 PITR compliance before first paying customer) | (1) `curl -I https://pryzm.so/` returns 200 pre-rendered HTML from Cloudflare Pages globally ⚪; (2) `curl -I https://app.pryzm.so/` returns 200 from Fly `fra` ⚪; (3) Sign-in modal on app.pryzm.so ⚪; (4) Project Hub reachable post-auth ⚪; (5) Astro source tree deleted ⚪; (6) ADR-055 amended + ADR-056 planned ✅; (7) Custom JWT preserved per C08 §1.1 ✅; (8) EU region honored per C22 §1.3 + C49 §1.2 ✅; (9) **ALL 4 contract conflicts resolved BEFORE any code shipped** ✅ | 🎯 ~ 50 % — 7 of 16 sub-tasks DONE; **CRITICAL PATH**. Honors the architecturally-sound + no-shortcuts directive: 4 contract violations caught + corrected via strategic-docs audit pre-implementation. |
