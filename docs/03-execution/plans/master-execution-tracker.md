...
| **A.7.c.2** | **site.updateZoning + site.setFootprint + site.clearFootprint** | Mutable parcel fields (zoning/setbacks/FAR/maxHeight) + footprint authoring with §1.6 containment check. | ⚪ PLANNED (Sprint 2) |
| **A.7.c.3** | **site.addContextBuilding + site.removeContextBuilding + site.replaceContextBuilding** | Per §1.5 reference-only neighbour shapes. | ⚪ PLANNED (Sprint 2) |
...
| **A.7.d** | **Cross-schema validations** | Containment + FAR + edge-classification length + polygon-immutability hash (Zod refinements + L3 store guards) per [C19 §2.7](../../02-decisions/contracts/C19-SITE-MODEL-AND-PARCEL.md). | ⚪ PLANNED (Sprint 2) |
