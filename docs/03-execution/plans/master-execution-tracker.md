...
| **A.21.D24** | 🕸️ **Living Graph: richer + more meaningful inspector** (founder v36: the Living Graph is "too basic") | Make the D17 Living Graph ARCHITECTURALLY richer WITHOUT breaking D16/D17/D21.5 — additive only, scoped to `apps/editor/src/ui/living-graph/` (+ the engine area-stamp fix). ✅ **DONE 2026-06-06.** **(1) Rich inspector (§LG-RICH-INSPECT):** the one-line inspector became a scrollable multi-section card pulling the **D16 rationale depth** into the Living Graph — Header (type dot · label · `level` chip · REAL `occupancy` program chip), Metrics (📐 real area · ☀ · ♪ · type · 🔗 active links), **"Why it's here"** (`nodeRationale` + its data source), **"Spaces"** (`roomRelationshipSentences` — "connects to Corridor via a door"), and **"Openings"** (the founder's *element relationships* — the room's hosted **doors/windows each with their own rationale**, e.g. "Window · south façade — for the best daylight"). Reads the CACHED `BuildingGraph` through the pure `@pryzm/building-graph` helpers (`humanNodeLabel`/`nodeRationale`/`roomRelationshipSentences`); read-only, never rebuilds (binder re-entry contract). **(2) "— m²" FIXED (§LG-REAL-AREA):** the inspector showed "— m²" because `enrichRoomNodes` stamped only a top-level `area` (which the detected-room store does NOT expose — its area lives at `computed.area`). Now the engine stamps `computed.area` + `boundary.polygon` onto room nodes, AND the binder's `areaOf()` resolves a scalar metric (`area`/`areaSqm`/`floorArea`/`areaM2`/`computed.area`) then FALLS BACK to a **shoelace area from the boundary polygon** — so a bare "— m²" appears only when a room has neither. `GraphNode` gained an optional `occupancy` (humanised program tag). **Brand+P3+determinism preserved:** white + #6600FF, no rAF (sim still ticks the frame bus / guarded setInterval), no `Math.random`, no new deps. **Tests:** NEW `apps/editor/__tests__/livingGraphData.test.ts` (10 cases — scalar/computed/shoelace/no-area area resolution + occupancy + room-only filter + determinism). Files: `LivingGraphOverlay.ts` (rich inspector + `collectOpenings`/section helpers), `livingGraphData.ts` (real-area + occupancy mapping), `livingGraphSchema.ts` (+`occupancy`), `engine/buildBuildingGraph.ts` (`enrichRoomNodes` area/polygon stamp + `RoomRecordLike`). SPEC-LIVING-BUILDING-GRAPH v1.1 (§7.1). **VERIFY (worktree has no node_modules; tsc/vitest exec blocked in sandbox):** verified by parity with the sibling `BuildingGraphOverlay.ts` which uses the identical helper imports + graph API. **NEEDS IN-BROWSER VERIFY:** generate a house → open Living Graph → click a room → multi-section inspector with REAL area + "Why it's here" + "Spaces" + "Openings"; nodes stay legible. | ✅ DONE (in-browser verify pending) |
