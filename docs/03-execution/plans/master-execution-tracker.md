...
| **A.21.b** | **House program + room types + storey allocation** | Extend `RoomType` (stair/landing/garage/porch/terrace) + house program-rules (incl. vertical `accessFrom` for stair) + `allocateProgramToStoreys(program, storeys) → StoreyProgram[]`. Refs: [SPEC §3](../specs/SPEC-CASA-UNIFAMILIAR-TYPOLOGY.md), [SPEC-ARCHITECTURAL-PROGRAM-RULES](../specs/SPEC-ARCHITECTURAL-PROGRAM-RULES.md). | ⚪ PLANNED (Sprint 7) |
| **A.21.c** | **Storey orchestrator (reuse D-TGL per plate)** | NEW `packages/ai-host/src/workflows/houseLayout/` outer loop: reserve an aligned stair-core rect, run the single-plate D-TGL engine per storey with the core as a fixed obstacle. Refs: [SPEC §6](../specs/SPEC-CASA-UNIFAMILIAR-TYPOLOGY.md). | ⚪ PLANNED (Sprint 7–8) |
...
| **A.21.h** | **House validators + cognition** | Stair clearance, cross-floor circulation reachability, wet-stack preference; cognition evaluators. New ObjectiveVector axes as needed. | ⚪ PLANNED (Sprint 8–9) |
| **A.21.i** | **Post-gen chain fan-out across storeys** | floor / ceiling / furnish / lighting triggers iterate every created storey (today they act on the active level only). | ⚪ PLANNED (Sprint 9) |
| **A.21.j** | **Editor onboarding wiring + console** | `briefBootstrap.ts` typology gate (currently bails on non-apartment) gets a `casa-unifamiliar` branch; `pryzmGenerateHouse*` console commands (mirror apartment). | ⚪ PLANNED (Sprint 9) |
| **A.21.k** | **UI: per-storey modal + multi-level result + dollhouse** | Generation modal shows per-storey thumbnails; result view adds a level selector (2D plan per floor) + reuses `LevelExplodeController` for an exploded "dollhouse" axon. Tracked as `A.U.*` under §12.3. Refs: [SPEC §9](../specs/SPEC-CASA-UNIFAMILIAR-TYPOLOGY.md). | ⚪ PLANNED (Sprint 9) |
| **A.21.x** | **Reference projects + tests + ratify** | ≥3 reference houses (1/2/3-storey) + ≥50 pipeline tests + ratify; retire any apartment-coupling. Flips C50 toward CANONICAL for multi-storey. | ⚪ PLANNED (Sprint 9) |
...
|---|---|---|---|
| **A.21.D1** | **Windows never created** (`shell windows built — 0/0`) | The window engine is FULLY wired (windowEmission/emitWindows: windowable types + specs + isExternal walls + BOUNDS edges + baseLine all present, unit-tested). The live break is DOWNSTREAM: `executePlan.ts:648` builds shell-window commands only after `resolveAllShellWindows` (windowEmission/shellWallMatch.ts:117) maps the D-TGL's emitted windows onto the EXISTING drawn shell walls — and that match needs geometric alignment. On a non-orthogonal boundary the D-TGL's axis-aligned perimeter doesn't align with the drawn shell → match returns null → windows dropped. FIX: make shellWallMatch tolerant (project the emitted window onto the nearest collinear shell segment within an angle/offset tolerance), OR emit windows directly on the existing shell walls (not the D-TGL perimeter). | 🔴 DEMO BLOCKER |
| **A.21.D2** | **Non-orthogonal layouts misalign** (walls + modal-options vs wall execution diverge) | Same family as D1: `tgl/rectDecomposition.ts` decomposes the shell into AXIS-ALIGNED rectangles (+ one `principalAxisAngle` skew). A truly non-orthogonal parcel can't be tiled by axis-aligned rects, so the generated interior walls don't meet the angled shell cleanly (gaps / overshoot), and the modal preview (all walls) diverges from the build (shell skipped + WJR clamps). FIX: per-edge oriented decomposition (align partition runs to the nearest shell edge), or clip generated walls to the shell polygon before emit. | 🔴 DEMO BLOCKER |
...
| **A.21.D4** | **Style (modern/classic/minimal/warm) does nothing** | CONFIRMED: `briefToProgram.ts:48,70` CAPTURES `style` but NO layout or furnish engine consumes it. FIX: thread `style` into (a) furnish material/variant selection (D-FLE) and (b) optionally layout priors (open-plan-ness, room proportions). Decide the contract: does style drive geometry, or only finishes/furniture? | 🟡 HIGH |
| **A.21.D5** | **Room layout quality** (proportions, corridors, allocation) | Broad — the D-TGL subdivision/squarify quality. Overlaps the existing [single-apartment-fix-pass-spec] (§AREA-FRACTIONS / §KITCHEN-DISTINCT / §SEALED-ROOMS shipped; master over-allocation + corridor-links-all + proper programme remain) + the [dimensional + topology validators framework]. | 🟡 HIGH |
| **A.21.D6** | **Climate-driven design** (windows + layout respond to sun / wind / shadow / temperature) | NOT implemented — this is the Phase-2 [APARTMENT-COGNITION-STACK] (Environmental Intelligence → Spatial Intent Field). Window SIZE/orientation + room placement should respond to solar gain (S-glazing in cold climates, shading in hot), prevailing wind, daylight. Depends on D1 (windows must exist first) + the climate substrate (C19/C21, already real). Big feature; the demo differentiator. | ⚪ PLANNED |
