...
| **A.21.D21** | 🏚️ **House v35 prod defects** (founder in-browser test 2026-06-06, post-v35) | Five real defects from the first live `House` generation: **(1) NO design modal** — house builds the layout DIRECTLY; apartment shows a "Choose a layout" options modal, house must too (per-storey options, A.21.k) — 🟢 agent in flight. **(2) Roof doesn't match the layout** — rendered roof floats/offsets off the footprint → `_createRoof` frame/position bug — 🟢 agent in flight. **(3) Not all perimeter walls created** — the house shell doesn't close → per-storey perimeter emission in `HouseLayoutExecutor` — 🟢 agent in flight. **(4) Stairs crash into walls** — stair-core not a real keep-out → 🟢 agent in flight. **(5) ✅ Graph auto-covers ALL rooms + relationships** — DONE, see A.21.D21.5. Geometry (2/3/4) = `HouseLayoutExecutor`+`houseOrchestrator`/`stairCore`; modal (1) = A.21.k; graph (5) ✅. | 🟢 IN PROGRESS (4 agents; graph done) |
