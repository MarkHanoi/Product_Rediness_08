...
| **🎯 2** | **IP-A3** | Sprint 3 · 2026-08-11 | **RAC chatbot end-to-end + Climate substrate parses** | "I sign up, the chatbot asks who I am and what project type, I type 'apartment', it routes me to apartment generation in < 60 seconds." | ✅ A.5 (RAC L3 + parsers + summarizeCapturedState DONE; L5 panel A.5.b DONE — vanilla-DOM RACChatbotPanel, 20 tests), ✅ A.6 (TypologyPicker L3 model DONE; L5 panel A.6.b DONE — vanilla-DOM TypologyPickerPanel, 17 tests), 🟢 A.10 (climate EPW + NOAA L0+L2+L3+commands DONE; A.11 UI PENDING), ⚪ A.15 part 2 (3 more plugins), 🟢 A.M.1/2 (landing-page enriched on pryzm.so with stats + capabilities + interop matrix + invariants; richer media still pending), ⚪ A.U.5 (tool-registry refresh) | (1) RAC chatbot live in editor — L5 panel ✅, Layout wiring + console hook PENDING; (2) TypologyPicker shows N categories — L5 panel ✅, Layout wiring PENDING; (3) End-to-end signup → … → generate < 5 min — needs Layout wiring + first-run gate; (4) EPW parses 10 reference files ✅ | 🟢 ~ 80 % — both L5 panels DONE + 37 tests green; remaining = Layout wiring + console hook + climate UI |
