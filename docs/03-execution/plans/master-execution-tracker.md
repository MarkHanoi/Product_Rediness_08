...
|---|---|---|---|---|---|---|
| **IP-A1** | Sprint 1 · 2026-07-14 | **Regression baseline + first marketplace install** | "I can still create an apartment via the existing flow, AND I can install BCF plugin from the new marketplace UI in < 30 seconds." | ✅ A.1 (TypologyPipeline pkg), ✅ A.2 (TypologyManifest schema), 🔴 A.12 (`@pryzm/sdk` npm publish — token+2FA), 🟡 A.14 (`marketplace.pryzm.app` DNS — Cloudflare creds), ⚪ A.15 first BCF + IFC-Export plugins, ⚪ A.PR.B19 secrets rotated | (1) Existing apartment generation works unchanged ✅; (2) `npm view @pryzm/sdk version` returns 1.0.x — 🔴 blocked on npm token; (3) `marketplace.pryzm.app` resolves — 🟡 blocked on Cloudflare; (4) Customer installs BCF plugin from marketplace — ⚪ not yet | 🟡 ~ 30 % — engineering done, blocked on external infra |
| **IP-A2** | Sprint 2 · 2026-07-28 | **Apartment-as-TypologyPack + Site UI scaffold** | "I can still generate an apartment AND I now see a Site panel in the editor with my plot boundary loaded from address." | ✅ A.3 (TypologyRegistry slot in composeRuntime), 🟢 A.4 (apartment refactored as pack — bridge handlers DONE, full migration A.4.b pending), 🟢 A.7 (C19 Site schemas — L0+L3+13 commands DONE; A.7.f IfcSite round-trip pending), ⚪ A.8 part 1 (basic Site UI), ✅ A.20 (C50 contract DRAFT), ⚪ A.A.1 (signup polish) | (1) Apartment regression suite green ✅ (typology-pipeline 54/54 + ai-host 1444/1444); (2) New Site panel renders plot from address — ⚪ L5 UI pending; (3) IFC export from refactored pipeline passes 10-project reference suite — ⚪ A.27 nightly pending | 🟢 ~ 55 % — substrate done, L5 Site UI pending |
| **IP-A3** | Sprint 3 · 2026-08-11 | **RAC chatbot end-to-end + Climate substrate parses** | "I sign up, the chatbot asks who I am and what project type, I type 'apartment', it routes me to apartment generation in < 60 seconds." | 🟢 A.5 (RAC chatbot — L3 reducer + parsers + summarizeCapturedState DONE; L5 React A.5.b pending), 🟢 A.6 (TypologyPicker — L3 model DONE; L5 React A.6.b pending), 🟢 A.10 part 1 (climate EPW + NOAA — L0+L2+L3+commands DONE; UI A.11 pending), ⚪ A.15 part 2 (3 more plugins), ⚪ A.M.1/2 (landing-page rebuild), ⚪ A.U.5 (tool-registry refresh) | (1) RAC chatbot live in editor — ⚪ L5 component pending; (2) TypologyPicker shows 10 categories — ⚪ L5 pending; (3) End-to-end signup → … → generate < 5 min — ⚪ pending; (4) EPW parses 10 reference files ✅ (parser + 10-vs-9 field LOCATION fallback shipped) | 🟢 ~ 50 % — L3 + L2 layers complete, L5 React pending |
| **IP-A4** | Sprint 4 · 2026-08-25 | **Full pipeline router + Climate UI + privacy** | "I can see my project's sun-path + wind-rose, AND the editor has a privacy consent banner I can configure." | ⚪ A.9 (IfcSite round-trip), ⚪ A.11 (climate UI panel — sun-path / wind rose / temp profile), ⚪ A.U.12 (consent banner), 🟢 A.30 part 1 (DSAR — L0+L3+commands DONE; server worker A.30.d.2 pending), ⚪ A.B.5 (trial banner), ⚪ A.B.4 (quota meter widget) | (1) Climate sun-path correct vs published almanacs at 4 cities — ⚪ L5 panel pending; (2) Consent banner appears within 60s of EU signup — ⚪ L5 banner pending (L3 ConsentStore + consent.* commands ✅); (3) DSAR export endpoint returns user data — ⚪ server worker pending | 🟢 ~ 35 % — privacy substrate done, all UI surfaces pending |
| **IP-A5** | Sprint 5 · 2026-09-08 | **Brand cutover + C19/C21 canonical** | "I navigate to pryzm.app, see the new landing + pricing pages, and AI-generated apartment layouts show provenance details in the inspect panel." | 🟡 A.17 (pryzm.app DNS — needs Cloudflare creds), ⚪ A.M.3 (manifesto page), ⚪ A.M.4 (trust page), ✅ A.18 (pricing-page generator — L2 entitlements + L5 Astro page DONE), 🟢 A.31 (provenance — L0+L3+commands DONE; L5 inspect-tree tab A.31.e pending), ⚪ C19 + C21 ratifications | (1) pryzm.app + pryzm.so redirect work — 🟡 DNS blocked; (2) Pricing page reads from entitlement registry ✅; (3) Right-click element in inspect tree → "Show AI provenance" — ⚪ L5 tab pending (queryByProject ✅) | 🟢 ~ 50 % — pricing FULLY DONE, provenance backend DONE, DNS blocked |
| **IP-A6** | Sprint 6 · 2026-09-22 | **Phase A Q3 acceptance — full apartment workflow** | "First 10 paying customers complete the entire signup → apartment → IFC export workflow without help." | ⚪ A.40 part 1 (first 10 customers — marketing-led), ⚪ marketplace dashboard for developers, 🟢 A.31 provenance backend ✅ (UI pending), ⚪ IFC4X3 IfcSite round-trip nightly green | (1) 10 paying customers MRR > $250 — ⚪ pending; (2) Marketplace catalogue has ≥ 50 artefacts — ⚪ pending; (3) Apartment regression × 5 reference projects nightly green — ⚪ A.27 PLANNED | ⚪ ~ 10 % — marketing + ops gated |
| **IP-A7** | Sprint 7 · 2026-10-14 | **House typology MVP + Inspect tree axis + L5 daylight** | "I pick 'house' in the picker, fill in the brief (storeys + bedroom count), AND get a valid layout — AND I see daylight violations highlighted in the inspect tree." | ⚪ A.21 part 1 (house schema + workflow scaffold), 🟢 A.38 (L5 daylight — L2 rule A.37.β ✅ + report formatter A.37.δ ✅; L5 panel pending), 🟢 A.23 part 1 (C20 schemas — all 4 aggregates + 14 commands + stores ✅), ⚪ A.U.2 (property panel migration) | (1) House layout generates < 60s for 3 reference briefs — ⚪ house pack PLANNED; (2) Inspect tree highlights rooms without mandatory windows — 🟢 L2 rule DONE (windowless master / bedroom / living / kitchen HARD-rejected); L5 highlight pending; (3) IFC PSet coverage 80%+ — ⚪ A.25 PLANNED | 🟢 ~ 30 % — daylight rule + C20 substrate ahead of schedule; house typology pending |
| **IP-A8** | Sprint 8 · 2026-10-28 | **House polish + Inspect tree complete + first community family pack** | "I generate a house, navigate the Site→Building→Level→Apt→Room→Element tree, install a UK door catalogue from the marketplace, and drop a door from it." | ⚪ A.21 part 2 (house validators), ⚪ A.24 (Inspect tree fully wired with aggregates), ⚪ B.I.2 (per-element-type sub-panels start), ⚪ A.29 + A.28 part 1 (first community pack) | (1) House passes 5 reference projects — ⚪ pending; (2) Inspect tree navigates the 6-tier hierarchy — ⚪ L5 UI pending (C20 schemas ✅); (3) UK door catalogue family pack — ⚪ pending; (4) IFC4X3-RV variant exports — ⚪ A.26 PLANNED | ⚪ ~ 5 % — substrate exists, all surfaces pending |
| **IP-A9** | Sprint 9 · 2026-11-11 | **House SHIPPED + C20 canonical + axe-core green** | "House typology in production; 25 paying customers; axe-core CI gate passes all critical/serious." | ⚪ A.21 SHIP, ⚪ C20 CANONICAL (schemas DONE, ratification awaits A.21+A.22 production), 🟢 A.32 axe-core (A.32.α static gate ✅; A.32.β Playwright dynamic gate pending), ✅ A.34 colour-contrast sweep (FULLY DONE — 17 token pairs audited), ⚪ A.40 part 2 (25 customers) | (1) House typology in production with 5 reference projects nightly — ⚪ pending; (2) C20 contract ratified — 🟢 schemas + commands ✅; (3) CI all-21-gates green for 2+ weeks — 🟢 static a11y gate ✅, dynamic gate pending; (4) Stripe MRR > $750 — ⚪ marketing-led | 🟢 ~ 20 % — A.34 fully done, A.32 static-side done |
| **IP-A10** | Sprint 10 · 2026-11-25 | **Office MVP + first Revit round-trip** | "I pick 'small office', generate a layout, export to IFC, import into Revit, see the model carries through with > 95% data integrity." | ⚪ A.22 part 1 (office schema + workflow), ⚪ A.26 (Revit IFC4X3-RV variant), ⚪ Revit reference round-trip (1 project), 🟢 A.35 part 1 (4 backup runbooks ✅; A.35.b-d follow-ons + drill pending), ⚪ A.U.4 (settings panel restructure) | (1) Office layout generates for 3 reference briefs — ⚪ pending; (2) 1 Revit RVT survives the full loop — ⚪ A.26 PLANNED; (3) PG snapshot scheduler running every 5 min — 🟢 runbook ready (RUNBOOK-DB-PRIMARY-FAILURE ✅), implementation pending | ⚪ ~ 15 % — DR runbooks shipped, office + Revit pending |
| **IP-A11** | Sprint 11 · 2026-12-09 | **Office SHIPPED + DR drill complete + 35 customers** | "Office typology in production; first DR drill completes without data loss; provenance UI works for office layouts too." | ⚪ A.22 SHIP, ⚪ A.36 first DR drill (the 4 runbooks A.35.a are the drill's procedure source — DRY-RUN now possible), 🟢 A.31 provenance (graph backend ✅ for ALL typologies — works for office at no extra cost when A.22 ships), ⚪ A.40 part 3 (35 customers) | (1) Office typology in production — ⚪ pending; (2) DR drill: PG primary failover succeeds < 30 min RTO — ⚪ pending (runbook ready); (3) Stripe MRR > $1050 — ⚪ marketing-led; (4) NPS first measurement > 40 — ⚪ pending | ⚪ ~ 10 % — runbook + provenance ready, office pending |
| **IP-A12** | Sprint 12 · 2026-12-23 | **Phase A acceptance — 3 typologies + 50 customers + 100 cognition rules** | "All three typologies (apartment + house + office) tested end-to-end; 50+ paying customers; 252 of 248 spec rules enforced (exceeds spec)." | ⚪ All Phase A buckets close to acceptance criteria; 🟢 A.37 cognition substrate (5 validators DONE — G9 hierarchy + G8 daylight + L5 corridor width + L5 entry sightline + aggregator + formatter + summarizer; ~ 5 / 100 new rules) | (1) E1-E10 Phase 1 exit criteria all verified true — ⚪ pending; (2) Marketplace 50+ artefacts — ⚪ pending; (3) Stripe MRR > $1500 (50 customers) — ⚪ marketing-led; (4) All 21 CI gates stable 4 consecutive weeks — ⚪ pending | ⚪ ~ 8 % — cognition pack started, full closure end of Phase A |
| **IP-A13** | Sprint 13 · 2026-12-30 | **Phase A EXIT — ADR-NNN-phase-1-exit-alpha ratified** | "Phase A officially closes; the team moves to Phase B (Beta); first 2 Enterprise pilots already in flight." | ⚪ A.41 (Phase 1 exit ADR ratified) | (1) ADR-NNN-phase-1-exit-alpha.md merged + status ACCEPTED — ⚪ pending; (2) 2027 Q1 quarterly plan + Phase B plan ratified — ⚪ pending | ⚪ 0 % — terminal IP, awaits IP-A12 |
...
| **A.18** | **Pricing page from entitlement registry** | Multi-slice; broken into A.18.a (L2 `@pryzm/entitlements` package — DONE) and A.18.b (L5 `apps/docs-site/src/pages/pricing.astro` consuming `buildPricingPageData()` — DONE). Refs: [C39 §1.1 + §1.2 + §1.13](../../02-decisions/contracts/C39-PRICING-AND-PLAN-TIERS.md). | ✅ DONE (Sprint 2) |
...
| **A.37.γ** | **L2 aggregate dimensional validator (`validateAllDimensional`)** | New orchestrator `packages/ai-host/src/workflows/apartmentLayout/dimensions/validateAllDimensional.ts` that runs validateRoomShape (G1-G6) + validateRoomHierarchy (G9) + validateRoomDaylight (G8) + validateCorridorWidth (L5 perceptual) and concatenates findings into one `DimensionalReport`. `admissible` is the AND of every sub-validator; the `perValidator` field carries the raw per-sub-validator results so the L5 modal can render per-section badges. `skipDaylight: true` lets early pipeline phases (D-TGL pre-window) skip the daylight gate cleanly. The D3.1 enumerate.ts gate + the L5 modal both call this single function instead of 4 separate validators. 8 tests covering sound-fixture admissibility · per-validator breakdown shape · fault propagation from each sub-validator (windowless → G8 HARD, narrow corridor → corridor HARD, master < bedroom → G9 SOFT) · skipDaylight short-circuit · combined-finding-count consistency. | ✅ DONE (Sprint 2) |
...
| **A.39** | **L5 perceptual evaluator (corridor width · sightline · aspect ratio)** | Multi-slice; broken into A.39.a (L2 `validateCorridorWidth` — DONE) and A.39.b/c (sightline + L5 UI — PLANNED). Refs: [phase-1-alpha §10.3](./roadmap-phase-1-alpha.md). | 🟢 IN PROGRESS (Sprint 2) |
