...
| **🎯 2** | **IP-A3** | Sprint 3 · 2026-08-11 | **RAC chatbot end-to-end + Climate substrate parses** | "I land on pryzm.so, click 'Build something', enter a draggable canvas, answer 4 questions (role · team size · typology · brief), get routed to apartment generation in < 60 seconds." | ✅ A.5 (RAC L3 + parsers + summarizeCapturedState DONE; L5 panel A.5.b DONE — vanilla-DOM RACChatbotPanel, 20 tests), 🟡 A.5.c (company-size capture LOGIC done; its host was the Astro /start surface — now retired, see A.5.d; editor-side L3 racReducer extension + app.pryzm.so re-mount pending, A.5.f), 🔁 A.5.d (draggable+resizable RAC canvas was built on `apps/docs-site/src/pages/start.astro` — **that surface was RETIRED with the A.17.x.14 Astro deletion 2026-06-02**; the canvas pattern + RAC logic survive in `RACChatbotPanel.ts`, pending re-mount INSIDE app.pryzm.so per ADR-055 §5.2 / C51 §5.2), ✅ A.6 (TypologyPicker L3 model DONE; L5 panel A.6.b DONE — vanilla-DOM TypologyPickerPanel, 17 tests), 🟢 A.10 (climate EPW + NOAA L0+L2+L3+commands DONE; A.11 UI PENDING), ⚪ A.15 part 2 (3 more plugins), 🔁 A.M.1/2 (landing's purple-mesh hero + 'Build something' CTA: the editor's `LandingPage.ts` is canonical, served via the apex pre-render per ADR-055 §7 — the old Astro landing was deleted A.17.x.14), ⚪ A.U.5 (tool-registry refresh), ⚪ A.5.e (POST /api/leads handoff to app.pryzm.so), ⚪ A.5.f (re-mount RAC canvas inside app.pryzm.so: editor-side racReducer extension for company-size + browser-canvas wrapper around RACChatbotPanel) | (1) "Build something" → draggable RAC canvas → 4 questions captured 🟡 RAC logic + canvas DONE (`RACChatbotPanel.ts`, 20 tests) but the public `/start` SURFACE was retired with the Astro docs-site (A.17.x.14); per ADR-055 §5.2 RAC re-mounts INSIDE app.pryzm.so (landing 'Build something' → app.pryzm.so/signup), pending that deploy + in-app routing (A.5.f); (2) Captured payload logged + redirects to manifesto (placeholder for /api/leads) ⚪; (3) End-to-end signup → editor preload of brief ⚪ blocked on app.pryzm.so deployment; (4) EPW parses 10 reference files ✅ | 🟡 ~ 65 % — RAC L3/L5 logic DONE (RACChatbotPanel, 20 tests) but the public `/start` surface was RETIRED with the Astro deletion (A.17.x.14); per ADR-055 §5.2 it re-mounts on app.pryzm.so — pending app deploy + in-app routing (A.5.e/f) + lead-capture endpoint |
