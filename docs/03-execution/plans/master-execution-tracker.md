...
| **A.23.b** | **L3 stores + nullable apartmentId widening** | BuildingStore + LevelStore + ApartmentStore + RoomStore + cross-store invariants (active-Level uniqueness + unit-number uniqueness + Room.apartmentId ↔ Apartment.levelId consistency). Includes the `RoomParameters.apartmentId` nullable-widening to close the C20 §2.4 gap. | ⚪ PLANNED (Sprint 7–9) |
