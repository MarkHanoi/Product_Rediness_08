...
| **A.21.D43** | FORMA-view defects — SHIPPED in v52 | (a) context FLOAT ROOT = the globe path's `sampleHeightMostDetailed` wrote a non-zero height into the persistent `formaTerrainBaseHeight`; switching globe→Forma re-seated context+massing at that leftover height while the grey ground stayed at 0 → float; FIX `applyFormaMode` resets `formaTerrainBaseHeight=0` + clears sampledAt on entry. (b) larger context `CONTEXT_BBOX_HALF_DEG` 0.005→0.0125 (~2.8 km square; new cache key auto via toFixed(4), 7-day TTL + 4-mirror unchanged; pan-refresh 450→1100 m). (c) Forma window glass ROOT = the glazing already had `FORMA_GLAZING_ALPHA` but `outline:true` makes a Cesium polygon render fill OPAQUE; FIX windows `outline:false` (doors keep outline, stay opaque). Merged (stale-base 3-way merge verified: Cesium `photorealTilesActive`/`resolveMassFill` + D43 changes coexist, tsc clean). | ✅ MERGED (v52) |
| **A.21.D45** | FINALLY solve "windows placed on the EDGE of the perimeter wall" (founder recurring; NOT happening originally) | **HARD EVIDENCE** (live log): shell windows landed at `[WallOccupancyStore] canPlace OK … offset=0.100m width=1.800m wallLen=16.983m` — a 1.8 m window at offset 0.1 m on a 16.98 m wall hugs the corner with only a 0.1 m pier (reads as "window on the edge"). **REGRESSION CAUSE**: the original window engine (T1.W-A, `01d47d04`) placed ONE **centred** window (offset = (len−width)/2 ≈ mid-wall, miles from any corner). The D5.c multi-window rework (`98e02342`) switched to even distribution where the FIRST window's ideal offset is `minOff = WINDOW_CLEARANCE_MM = 0.1 m` (the bare end margin) — and the shell-frame resolver (`shellWallMatch.resolveShellWindow`) clamped the final span to `END_CLEAR_M = 0.1 m`, passing the 0.1 m corner-hug straight through to the live `canPlace` log. Both layers used the cosmetic 0.1 m as the wall-end margin. **FIX (§WINDOW-CORNER-SETBACK)**: replaced the 0.1 m wall-end role with a REAL wall-length-scaled masonry pier `cornerSetbackForWall(len) = clamp(0.10·len, 0.5 m, 1.2 m)`, reduced on a short wall (`min(scaled, (len−0.4)/2)`) so a hostable wall still gets a window, applied to BOTH ends in the resolver (final shell authority — the live-log value) AND the emitter distribution (`endSetbackMm` drives `clearOffsetMm`/`evenOffsetsMm`/`nearestClearOffsetMm`/`windowCountForWall`). VALUE RATIONALE: 0.5 m floor ≈ a half-brick + reveal return (the minimum that reads as a window set INTO a wall); 1.2 m cap so a long façade doesn't bunch glazing mid-wall; on the live 16.98 m wall the setback is 1.2 m so the first window now sits at ≥1.2 m (was 0.1 m). DISTRIBUTION: the full span `[offset, offset+width]` must sit inside `[setback, len−setback]` for EVERY window (first/last/middle); a window that can't fit is width-clamped then dropped (full-span-or-drop doctrine), never slammed to the corner. The §WINDOW-CORNER-FIT drop was NARROWED to TOLERANT (skewed-graze) matches only — an EXACT-match window near a corner is now SLID inward to the pier (keeps distributed windows on long orthogonal façades) rather than dropped; the skewed wrong-wall drop (D39/D34b) is preserved via a new `matchShellHost` `exact` flag. Inter-window + window↔door gaps still use the bare 0.1 m clearance (de-overlap D40 #2 and door avoidance unchanged); D6 solar orientation/sizing unchanged (sun-biased façade still wins). **TESTS**: `shellWallMatch.test.ts` — new §WINDOW-CORNER-SETBACK block (cornerSetbackForWall scaling; live-log reproduction pulls offset 0.1 m → 1.2 m pier; sweep along a long wall: no window within the setback of either corner; over-wide width-clamped windows stay inside both piers) + exact-slides-vs-graze-drops; existing D28/D34/D36/D39/D40 assertions retuned 0.1 → scaled setback; `windowEmission.test.ts` single-centred + multi-window + door/junction-avoidance re-verified against the new end margin. ai-host suite expected green (was 1990) — execution sandbox-blocked in the worktree (no node_modules; Bash+PowerShell denied) → STAGED with the exact run command. FILES: `windowEmission/shellWallMatch.ts`, `windowEmission/emitWindows.ts`, `__tests__/shellWallMatch.test.ts`. | 🟡 STAGED (sandbox-blocked test run) — `pnpm --filter @pryzm/ai-host test` |
