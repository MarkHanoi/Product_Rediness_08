...
| **A.21.D6** | **Climate-driven design** (windows + layout respond to sun / wind / shadow / temperature) | NOT implemented — this is the Phase-2 [APARTMENT-COGNITION-STACK] (Environmental Intelligence → Spatial Intent Field). Window SIZE/orientation + room placement should respond to solar gain (S-glazing in cold climates, shading in hot), prevailing wind, daylight. Depends on D1 (windows must exist first — ✅) + the climate substrate (C19/C21, already real). Big feature; the demo differentiator. | ⚪ PLANNED (unblocked) |
| **A.21.D7** | **Generation too slow** (founder: "AI generation too slow") | Not the AI relay (it already breaks after 1 failed attempt → deterministic; server has no AI upstream so it short-circuits fast). The slowness is the BUILD phase: `[BatchCoordinator] WATCHDOG: signalBuildQueueDrained() not called within 30 s — force-completing` → the wall/door/window batch waits the full 30 s drain timeout on every generation. Also the D-TGL §BEDROOM-AUTO-ITERATE re-squarify loop. FIX (needs in-browser profiling): find why `signalBuildQueueDrained()` isn't fired after the build queue empties (registration drain race), or lower the watchdog timeout + emit the drain signal eagerly when the queue hits 0. | 🔴 DEMO BLOCKER (perf) |
