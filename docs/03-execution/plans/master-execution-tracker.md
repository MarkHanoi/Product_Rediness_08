...
| **A.21.D48** | **Floor FINISH must not be COINCIDENT with the SLAB** (founder 2026-06-08) | **ROOT (confirmed coincident):** a floor finish was created with `thickness = 0.075` (75 mm) + `baseOffset = 0`. The `FloorPanelBuilder` rule is "top face at FFL = `level.elevation + baseOffset`; body extends DOWNWARD by thickness", so the finish occupied `[level.elevation − 0.075, level.elevation]`. The structural SLAB is anchored with its TOP face at the level datum (`SlabFragmentBuilder.resolveWorldY`: slab top = `level.elevation + slab.baseOffset`, default 0; body downward). → the finish occupied **EXACTLY the slab's top 75 mm** = coincident/overlapping volume → Z-fighting + meaningless clash detection. **FIELDS:** a finish-thickness field DID NOT exist; `baseOffset`+`thickness` existed on `FloorBoundary` but defaulted to 0 / 0.075 (never seated on the slab). **FIX (geometry/command, minimal):** new canonical `resolveFinishSeating()` + `DEFAULT_FINISH_THICKNESS_M = 0.015` (15 mm) in `core-app-model/stores/FloorTypes.ts` (the floor data-model home). A BARE finish floor is now seated ON the slab top — finish **bottom Y = slab top Y** (= `level.elevation + slabTopOffsetM`), stacking UP by the thin finish thickness, so `baseOffset = slabTopOffsetM + finishThicknessM`, `thickness = finishThicknessM`. Finish & slab volumes are now **disjoint** (finish strictly above slab top — touch only at the top face → no Z-fighting, clash-detectable). New OPTIONAL payload fields `finishThicknessM` + `slabTopOffsetM` added to BOTH creation paths (`CreateFloorCommand` legacy + `CreateFloorHandler` plugin). **NON-REGRESSION:** explicit `baseOffset` is respected verbatim (manual / IFC-pinned); explicit `thickness` or a layered build-up keeps its thickness but is seated bottom-on-slab; `ImportProjectCommand` rebuilds persisted floors verbatim (untouched). D-FLOOR finish colour/pattern (`floorFinish.ts`), the §VOID-FINISH stairwell-hole cut, and slab rendering are all untouched (only the input `baseOffset`/`thickness` changed; builder logic unchanged). Also corrected `FloorSlabBindingHandler`: its `_resolveSlabTopOffset` wrongly returned `baseOffset + thickness` (the slab top is `baseOffset`); on a slab move it now seats the finish at `slabTopOffset + floor.thickness` so it stays above the slab. **DATUM FOLLOW-UP (noted, not silently shifted):** the finish TOP now sits `finishThicknessM` ABOVE the level datum/slab top — wall/door bases + skirting still reference the level datum (slab top), so they meet the SLAB top, not the finished-floor TOP. A future tracker item should choose the walkable datum (FFL = finish top) consistently across doors/skirting/room elevation. **TESTS:** `FloorFinishSeating.test.ts` (core-app-model vitest) — asserts finish bottom ≥ slab top, finish/slab bands disjoint, legacy coincident placement gone, explicit-offset/thickness/layered/raised-slab/custom-thickness honoured (9 cases). Typecheck/test run SANDBOX-BLOCKED (worktree has no node_modules; Bash/PowerShell denied) — change verified by reading; diff is type-safe (new fields optional, helper pure). | ✅ SHIPPED (geometry+command; finish seated on slab top; datum-reference follow-up noted) |
