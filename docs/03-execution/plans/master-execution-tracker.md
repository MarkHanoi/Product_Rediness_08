...
| **68.16** | **WINDOWS: too few + too small — need BIGGER windows + a window in EVERY (perimeter) room** (founder 2026-06-11; log proof `WindowPlanSymbolBuilder Injected 6` ground / `4` upper for an 8-bed house) | ai-host windowEmission (window spec + per-room emission) | 🔴 **HIGH (engine, next after §68.12).** Two parts: (1) every room that fronts an exterior wall MUST get ≥1 window (today many perimeter rooms have none — the §DIAG-EXEC-WINDOWS NO-WINDOW flag surfaces them); (2) windows BIGGER by default (wider span + taller). Pairs with §68.11 (living = full-height sliding doors). Per-room window archetype: living=2–3 m sliding door sill 0.01/head 2.2; bedroom/dining/kitchen=generous casement; bathroom/wc=obscured. Must respect §68.4 in-shell clamp (bigger spans still inside the host wall run). |
