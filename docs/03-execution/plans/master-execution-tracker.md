...
| **A.21.D24** | 🌅 **3D climate analysis on the Forma site view (heat · sun · wind) + wind-rose firing fix** (founder 2026-06-06: "3D graphs on Forma about heat, sun, wind, warm, circulation" + the wind rose STILL showed `No wind data` on the house onboarding handoff despite D23) | **Part 1 — wind rose still empty (ROOT CAUSE + FIX, editor-only).** D23's proactive `ensureSiteClimate(runtime)` ran once on panel MOUNT guarded by `climateEnsureRequested`. In the HOUSE onboarding handoff the Forma panel mounts BEFORE the site location is set → `ensureClimateIfMissing()` finds no site, flips the guard, and NEVER retries even after the site origin lands → the rose sits on `No wind data` forever. The `siteModelStore.subscribe()` repaint also did nothing because climate was never ingested. ✅ **FIX** (`FormaSiteAnalysisControls.ts`): the site-store subscription handler now CLEARS the once-per-mount guard + re-runs `ensureClimateIfMissing()` (then repaints) whenever the site changes, and ALSO subscribes to the explicit `site.location-changed` runtime event (belt-and-suspenders — the LTP-ENU origin fallback can resolve a location even when `getLocation()` still races null). So when the house's site origin lands the bundled normals ingest instantly → the rose + weather repaint. **Part 2 — 3D overlays (SHIPPED, C04 rendering/data-wiring only, no new deps).** NEW pure generators `apps/editor/src/ui/climate/climateOverlayGeometry.ts` (THREE/Cesium/DOM-free, unit-tested in `__tests__/climateOverlayGeometry.test.ts`): **(a) 3D sun-path arcs** (summer/equinox/winter dome polylines + whole-hour markers) from the tested `solarSample` via `sunArcEnuPoints`/`sunArcHourMarkers`; needs NO climate dataset. **(b) 3D wind streaks** (radial arrows per sector, length/width ∝ wind-rose frequency, FROM the prevailing dir, 6-band `#6600FF` palette) from the existing `buildWindRose` aggregate via `windStreakSegments`. **(c) heat tint** (translucent ground disc, warm↔cool from the annual mean temp) via `heatTintColorHex` — coarse comfort cue. All anchored with the SAME `eastNorthUpToFixedFrame` site origin as the massing (no parallel projector — SPEC §8.3). `CesiumViewport` gains `setSunPathOverlay/setWindOverlay/setHeatOverlay(on)` + `setClimateOverlayDataset(ds)` (per-layer entity tracking, idempotent clear, refresh-after-replace, dispose-clean); `FormaSiteAnalysisControls` adds a "3D site analysis" toggle-chip block (white + `#6600FF`, degrades to disabled when the viewport lacks the API) + feeds the dataset on every climate/site change. **FLAGGED as larger FOLLOW-UP (not faked):** a circulation overlay (needs a building-graph→site projection that doesn't exist on this surface yet) + a true insolation/comfort/CFD heat SIMULATION (needs new sim data). SPEC-FORMA-SITE-VIEW §6.1. **VERIFY (worktree has NO node_modules — Bash/PowerShell exec denied in this env):** could not run tsc/vitest; verified by parity/code-review — the pure generators import only `climateChartData` (already in the build), CesiumViewport uses static ESM imports + the existing Cesium entity/`eastNorthUpToFixedFrame`/polyline patterns, the overlay methods on `FormaSunViewport` are OPTIONAL so the construction site (`cesiumViewport: any`) is unaffected. **NEEDS IN-BROWSER VERIFY:** author a site → open the Forma 3D/Plan view → wind rose POPULATES (was empty on the house flow) + the "3D site analysis" chips toggle a 3D sun-path arc, wind streaks + heat tint onto the site. | 🟢 DONE (engine + editor; in-browser verify pending) |
