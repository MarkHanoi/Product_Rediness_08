...
| **Wall-junction defects** | A.WJ.LCORNER + A.WJ.MULTICLUSTER + A.WJ.MCTRIM + A.WJ.STUBSWEEP + A.WJ.IWO (5) | B.WJ.ADR55P4A/B/C (3) | — | — | — | §16.13 |
...
| **Strategic ADRs pending (ADR-0236 → ADR-0250)** | — | B.ADR.36–38 (3) | C.ADR.39–42 (4) | D.ADR.43–50 (8) | — | §20.3 |
...
| A.17 brand cutover (SUPERSEDED by IP-A5.X / ADR-0255 → `pryzm.so` apex/app split) | IP-A5 |
...
| **A.5** | **RAC chatbot UI v1** | Split into A.5.a (L3 pure state-machine — DONE) and A.5.b (L5 React component). Role + typology + brief flow per [product-vision §5 Step 2](../../01-strategy/STR-02-product-vision.md). Refs: [typology-expansion §2](./typology-expansion-roadmap.md). | 🟢 IN PROGRESS (Sprint 1–3) |
...
| **A.8** | **Site authoring UI (Cesium-light)** | Cream/warm-white map aesthetic per [product-vision §5 Step 3](../../01-strategy/STR-02-product-vision.md). Hektar-style UX: address-search → satellite zoom → polygon-draw → auto-analyses. Broken into A.8.a-A.8.f below. Refs: [C19 §5](../../02-decisions/contracts/C19-SITE-MODEL-AND-PARCEL.md), [phase-1-alpha §4.1.4](./roadmap-phase-1-alpha.md). | ⚪ PLANNED (Sprint 3–4) |
...
| **A.8.b** | **Cesium-light tile layer** | Cream/warm-white satellite-imagery basemap (the "not the dark globe" aesthetic per [product-vision §5 Step 3](../../01-strategy/STR-02-product-vision.md)). Cesium ion + custom style. Default zoom-to-bbox on address commit. | ⚪ PLANNED (Sprint 3) |
...
| **A.17** | **Brand cutover — SUPERSEDED by IP-A5.X / ADR-0255** | `pryzm.so` is now the canonical domain (the `pryzm.app` aspiration is retired, A.17.x.7). The apex/app split (C51) replaces the old "single landing rebuild" approach; the work is tracked under IP-A5.X (A.17.x.*). Landing copy is the editor's `apps/editor/src/ui/platform/LandingPage.ts` per ADR-0255 §7. | 🔁 SUPERSEDED → see IP-A5.X |
| **A.18** | **Pricing page from entitlement registry** | Multi-slice: A.18.a (L2 `@pryzm/entitlements` package — DONE) + A.18.b (L5 pricing surface — **MOVED into the editor** `apps/editor/src/ui/marketing/` per ADR-0255 §7 / A.17.x.18; the old `apps/docs-site/src/pages/pricing.astro` was DELETED with the Astro docs-site, A.17.x.14. The editor's `PricingPage` + the apex prerender both render from `buildPricingPageData()`/`@pryzm/entitlements` — DONE). Refs: [C39 §1.1 + §1.2 + §1.13](../../02-decisions/contracts/C39-PRICING-AND-PLAN-TIERS.md) · C51 §6.3. | ✅ DONE (Sprint 2) |
...
| **A.19** | **Brand-voice content sweep** | Every customer-facing string audited against [manifesto §5](../../01-strategy/STR-01-manifesto.md). | ⚪ PLANNED (Sprint 5) |
...
| **A.30** | **C22 PII tier — partial ratification** | Multi-slice; broken into A.30.a (L0 schemas: DataTier · Region · DSAR · Consent · RetentionPolicy — DONE), A.30.b (L0 BreachIncident + StorageRoutingPolicy — ✅ DONE: `@pryzm/schemas/privacy` BreachIncident (§2.5, 22 tests) + StorageRoutingPolicy (§2.2 — per-tier region endpoints eu/us/ap + optional selfHosted, fixed aes-256-gcm/tls-1.3-min ciphers, positive retention ceiling, downcast no-self-target + unique-target invariants; 12 tests); 1093 schemas tests green, typecheck clean, L0-pure), A.30.c (L3 ConsentStore + RetentionScheduler — ✅ DONE: `@pryzm/stores` ConsentStore (user-scoped consent authority + grant/revoke/purge commands) + RetentionScheduler (pure clock-free decision core — per-tier RetentionPolicy registry + `planSweep`/`isExpired`/`firingTrigger`/`overdueSweeps` deciding which records purge by age §1.10 or early-trigger §2.3, surfacing the "missed three sweeps" Sev-2 signal; the `apps/retention-worker` timer+delete loop that calls it is A.30.d; 16 tests, 735 @pryzm/stores green, L3-pure), A.30.d (server DSAR worker + privacy UI + apps/retention-worker — PLANNED). Refs: [C22](../../02-decisions/contracts/C22-PRIVACY-AND-PII-TIER.md), [phase-1-alpha §7.1](./roadmap-phase-1-alpha.md). | 🟢 IN PROGRESS (Sprint 2) |
...
| **C.19** | **L6 behavioural simulation (pedestrian flow + occupancy)** | Per [site-and-cognition §3.4](../../01-strategy/STR-12-site-and-cognition-strategy.md). | 🔵 DEFERRED |
...
| **X.10** | Brand-voice content moderation per [manifesto §5](../../01-strategy/STR-01-manifesto.md) | Per-customer-surface | 🟢 IN PROGRESS |

---

## §8 — Immediate next 5 actions (the "what's next" answer)

The 5 actions to do FIRST, in priority order, as of **2026-06-02** (rewritten — the prior 2026-06-01 list is closed: A.1/A.2 typology-pipeline shipped `172fc8c`, A.7 site substrate shipped, A.17 superseded by the ADR-0255 apex/app split; `pryzm.app` retired for canonical `pryzm.so`). **All of IP-A5.X's code/config is now DONE on `main`; the critical path is now INFRA the user performs (1–4), then the Revit code-track (5).**
...
|---|---|---|---|
| **A.U.1** | **Editor shell redesign for typology routing** | Top bar shows typology badge + role badge; typology-switcher in user menu. Refs: [product-vision §5 Step 2](../../01-strategy/STR-02-product-vision.md). | ⚪ PLANNED (Sprint 4) |
...
| **A.PL.1** | **Project Hub redesign (landing for signed-in users)** | Recent projects · starred · typology shortcuts · sharing inbox. Refs: [product-vision §5 Step 1](../../01-strategy/STR-02-product-vision.md). | ⚪ PLANNED (Sprint 5) |
...
| **C.F.2** | **Family analytics for authors** | Install counts · review · earnings per pack. Refs: [platform-strategy §11](../../01-strategy/STR-10-platform-strategy.md). |
...
| **A.R.3** *(new)* | **Persistent IFC/Revit meta-store** | 🟡 **FOUNDATION SHIPPED** `1f3cea5` — canonical `IfcElementMeta` schema at `@pryzm/schemas/ifc` (L0) + durable reactive `IfcMetaStore` at `@pryzm/stores` (L3): get/getByGlobalId/add/updatePset/updateQuantity/delete + `serialize↔hydrate` (Zod-validated, the `.pryzm` path) + `reset`/`subscribe`/`dispose`; 13 tests green. **Completion sequence:** (1) ✅ **re-pointed both plugins to the canonical shape** via the plugin-sdk (L6) facade — `plugin-sdk` re-exports `@pryzm/schemas/ifc`; `ifc-export` + `ifc-import` alias `IfcElementMeta`/`Pset`/`Qset`/`PsetValue`/`IfcElementTier` from it (3 near-identical copies → 1; plugin-sdk + both plugins typecheck clean, 252 + 28 tests green); (2) ✅ **registered in `composeRuntime`** — `runtime.ifcMetaStore` (construct+expose+dispose; 132 runtime tests green); the reset into the *distributed* C13 project-switch path is still pending (no consumer needs it until import populates) — **but the `.pryzm` LOAD path now resets-on-load (step 5), so the persistence leg can no longer leak prior-project metadata even before the distributed reset lands**; (3) 🟡 **command surface built** — `@pryzm/stores/ifc-commands` `registerIfcMeta`/`deregisterIfcMeta` (the P6-clean mutation path; pure `(payload, store) → IfcCommandResult<Event>`, mirrors `consent-commands`; 5 tests green) — remaining: bus-registration + `ifc-import` dispatching `ifc.meta.register` after producing metas (depends on the S55 import-pipeline call-site); (4) ⚪ `ifc-export` reads via `get()`; (5) ✅ **`.pryzm` save/load calls `serialize()`/`hydrate()`** — `ProjectSnapshot.ifcElementMeta` field + `ProjectSerializer.serialize()` emits it (only when the store is non-empty → byte-identical snapshot for non-IFC projects) + `ProjectLoader` hydrates it (reaching `runtime.ifcMetaStore` via the established `window.runtime` typed-cast, the same pattern as `runtime.bus`) and **resets the store on a project WITHOUT the field** so an imported model's GlobalIds can't leak across a project switch; threaded through `initPersistence` (`serializeStores`); editor typecheck clean; +1 `@pryzm/stores` test pinning the real serialize→`JSON.stringify`→`JSON.parse`→hydrate cycle (719 stores tests green). **Remaining for the live round-trip: steps 3b/4 — but those depend on promoting the un-wired legacy import/export drivers (`ImportExportSlots` throws `RuntimeNotWiredError`, F.12.3/F.12.4).** **The single highest-leverage unlock** — converts the existing half-round-trip into a real one. |
...
| **D.R.4** | **Revit-aware conflict resolver** (Revit-user A vs PRYZM-user B edit same element) | Builds on the ADR-0249 CRDT resolver; NEW = per-parameter binding / deterministic merge strategy (C08 §3.2 explicit-conflict posture). ~1–2 sprints. |
...
| Phase | Goal | Description |
|---|---|---|
| **A.M.1** | Landing page (`pryzm.so`) rebuild | Aspirational hero (per [manifesto §5](../../01-strategy/STR-01-manifesto.md)) + 90-sec live apartment demo + clear CTAs. (Now the editor's `LandingPage.ts` served via the apex pre-render per ADR-0255 §7 / IP-A5.X — see A.17.x.12/18.) |
...
| **A.M.10** | Customer case studies | First 5 customer references; per [go-to-market §3.1](../../01-strategy/STR-08-go-to-market.md). |
| **B.M.1** | Developer site (`developers.pryzm.so` or `pryzm.so/developers`) | Per [platform-strategy §10.1](../../01-strategy/STR-10-platform-strategy.md). |
| **B.M.2** | Blog (engineering + design + customer stories) | Per [go-to-market §3.1 content marketing](../../01-strategy/STR-08-go-to-market.md). |
| **B.M.3** | Customer summit (annual; first edition) | Per [operating-principles §6.5](../../01-strategy/STR-06-operating-principles.md). |
| **C.M.1** | Plugin author conference (annual; first edition) | Per [platform-strategy §10.3](../../01-strategy/STR-10-platform-strategy.md). |
...
5. **Customer-managed integrations** (BIM360 · Procore · Bentley iTwin · Trimble Connect · Aconex · ArchiCAD's BIMcloud) — explicitly marketplace-plugin opportunities per [platform-strategy §2.5](../../01-strategy/STR-10-platform-strategy.md), not PRYZM-first-party builds.

These gaps are by design — pushing them to community + marketplace is the strategic moat per [platform-strategy](../../01-strategy/STR-10-platform-strategy.md).
...
| **A.OI.053f/g/h** | OI-053f/g/h | Apartment batch generator: drop redundant Phase-1 `REDETECT_ROOMS` (f, quick) · replace the 150 ms wall-poll `setTimeout` cadence with a store-subscription signal (g, quick) · openings-only single-wall rebuild in batch door creation (h, structural — shares ADR-0257 P1; collapses O(walls×doors)→O(doors)). `apps/editor/src/ui/apartment-layout/ApartmentLayoutExecutor.ts:157-367`. Analysis §4. **✅ OI-053f + OI-053g SHIPPED 2026-06-03** (`3500fab`): f = `skipRedetectRooms:true` on the wall batch (Phase-2 doors sweep is the meaningful one — final room set identical); g = subscribe to `storeEventBus` wall create/update → proceed the instant walls are ready, 150ms `setTimeout` kept as fallback + telemetry/iteration-cap preserved. Output identical (ai-host `apartmentLayoutCommands.test.ts` 27/27 green). In-browser confirm: `wall-poll-completed` `iters`→0-1, lower `elapsed_ms`, 1 redetect sweep. **✅ OI-053h / ADR-0257 P1 SHIPPED 2026-06-03** (`ea11e457`): `WallDeltaClassifier.classifyWallDelta` → `WallRebuildCoordinator._flushOpeningsOnly` rebuilds ONLY the edited wall (cached miter `JoinData`; skips `resolveLevel`/V2-cache/infill) when the delta is openings-value-only on baseline-stable walls; ANY other delta falls back to whole-level. Door-move O(walls/level)→O(1); also speeds batch door placement. Invariance proven (resolve reads endpoints/thickness/adjacency, never openings); 20/20 geometry-wall tests; WJR-NAN-GUARD preserved. **ADR-0257 → ACCEPTED, P1 implemented** (P2-P5 backlogged). | 🟢 f/g/h ALL DONE |
...
| **A.OI.054** | OI-054 | Hosted door/window two-part undo (followup-a); cross-stack redo / ADR-0251 single-store (followup-b) | Phase A |
...
| Sub-phase | ID | Title |
|---|---|---|
| **A.WJ.LCORNER** | — | Defect #3 — interior↔exterior L-corner produces black-triangle artefact. **Verdict (investigation 2026-06-18): the residual ~134mm open perimeter L-corner GAP (`§DIAG-PERIM-CORNER-WHOLE gappy(>5mm)=1, bothMitred=true`) is NOT a resolver bug — the resolver closes every corner (`jointGap=0.0mm closed=✓`). ROOT = PROJECT-NORTH frame mismatch in `HouseLayoutExecutor._weldGroundPartitions` (`apps/editor/.../HouseLayoutExecutor.ts:1964+`): perimeter built in the DRAWN-shell frame, partitions welded in the RECTIFIED PN frame → corner endpoints don't coincide. `§ONE-FRAME-MINT` (`0414f007`) rebased partition EDGES; the deeper PERIMETER ONE-FRAME MINT (adopt the rectified `shellWallsWorld` AS the perimeter — ADR-0073 §Decision-2) is the actual close. DEFERRED-HARD (re-snap fights the weld guards); next-session fix needs an end-to-end rotated-plate executor fixture asserting `gappy=0`.** | 🔵 root-confirmed, deferred (ADR-0073 §2) |
...
| **A.WJ.MCTRIM** | — | **§MULTI-CLUSTER-PARTITION-TRIM (✅ SHIPPED `7c050561`, 2026-06-18)** — interior PASS-THROUGH cluster: an angled partition arm whose off-end lay near a shell body was caught by `§SHELL-ANCHOR-PRESERVE` and deferred to the pair-wise shell T-join (over-ran 145–431mm off the junction); now square-caps to the junction consensus FIRST when the arm is not collinear with the through-pair. Gated `__pryzmMultiClusterPartitionTrim` (default-ON). Repro: free interior over-run → 0mm; the ~288mm shell-adjacent residual is a **MEASUREMENT ARTIFACT** (partition lands flush on the host inner face — correct; the metric reports distance-from-junction-centreline, not over-run). geometry-wall 79/79 green. | ✅ FIXED — browser-confirm pending |
| **A.WJ.STUBSWEEP** | — | **§RESOLVED-STUB-SWEEP (✅ SHIPPED `a0183898`, 2026-06-19)** — deep trace of the prod "wall malformed on execution" / down-spike (`wall_…RBZ9` t=0.100): a wall can be collapsed into the degenerate dead band `[minWallLength, DEGENERATE_STUB_LENGTH)` = **[0.05, 0.15) m** by the SUM of several individually-legal trims — a same-thickness corner crossing on its start + a SUCCESSFUL shell inner-face clamp (+99mm) + a multi-cluster consensus on its end → a ~0.10m residual. Every guard misses it: per-pass trims refuse only a result BELOW 0.05m; `§WJR-INVALID`'s 0.15m guard fires ONLY on the shell-clamp-**REFUSED** branch (a SUCCESSFUL clamp skips it); the mesh backstop is `< 1e-3 m`. So a 0.10m stub shipped `closed=✓ invalid=false` → `WallFragmentBuilder` extruded the down-spike below the building (+ `§DOOR-LIVE-CLAMP` dropped its door; `§DIAG-PERIM-CORNER-WHOLE` read **1580mm** because the wall's two ends were independently relocated — a *symptom*, not the root; the resolver's own `jointGap=0.0mm closed=✓` is correct). **Fix:** one post-resolve sweep in `resolveLevel` (after `_clampPartitionEndsToShellInnerFace`, before `return`) judges the FINAL resolved length and flags any wall the resolver ADJUSTED (`result.has`) whose length lands in the dead band invalid → builder skips it ("wrong-but-skipped beats a 0.1m spike"). Only ADDS a flag (never moves an endpoint, never widens a tolerance) → composes with all four shell/clamp passes, parity-safe. Unadjusted walls keep their upstream-validated source length. Does NOT raise per-pass `minWallLength` (would refuse legit 0.05–0.15m corner trims). Tests (e)/(f) (same-thk L-corner 0.10m arm flagged + rotated-plate parity invariant); 79/79 geometry-wall green. **The new §PARITY-GATE (`e4f97966`, `HouseLayoutExecutor`) now also catches any residual at execution.** | ✅ FIXED — browser-confirm pending |
...
| **A.SP.38** | — | MEP systems framework — per [SPEC-38-MEP-SYSTEMS](../specs/SPEC-38-MEP-SYSTEMS.md). PRYZM ships lighting + plumbing + structural at the architectural level; MEP detailing primary tool is out of scope (per [engineering-vision §8](../../01-strategy/STR-03-engineering-vision.md)). Phase A scope: MEP element typing + IFC4X3 export of MEP categories so consultants can take over downstream. | SPEC-38 |
...
|---|---|---|
| [status/intent-analysis/visibility-intent-implementation-plan.md](../status/intent-analysis/visibility-intent-implementation-plan.md) | Master plan analysis | informational; superseded by H2 phase roadmaps |
...
Per [cadence-and-planning-system §7](./cadence-and-planning-system.md), per-sprint retros are H5 cadence artefacts. The retro template + cadence is binding via [operating-principles §6.2](../../01-strategy/STR-06-operating-principles.md).
...
These are pushed to community + marketplace per [STR-10-platform-strategy.md](../../01-strategy/STR-10-platform-strategy.md):
...
| Customer-managed integrations (BIM360 · Procore · Bentley iTwin · Trimble Connect · Aconex · BIMcloud · ...) | Marketplace plugin opportunity per [platform-strategy §2.5](../../01-strategy/STR-10-platform-strategy.md) |
| Photoreal rendering primary tool | Out of scope per [engineering-vision §8](../../01-strategy/STR-03-engineering-vision.md) |
...
The discipline of saying no is the same as Phase 1 (per [positioning §6](../../01-strategy/STR-07-positioning.md)). These scope-cuts are not gaps in the tracker — they are deliberate.
...
| **D.AEC.47** | **ConTech Bridges** (Procore · Asite · OpenSpace · Dusty Robotics · Trimble Connect · Autodesk Construction Cloud bidirectional integrations) | D | SPEC-47 proposed. Per [platform-strategy §2.5](../../01-strategy/STR-10-platform-strategy.md), these are **marketplace plugin opportunities**, not PRYZM-first-party builds. |
...
### §20.3 — Strategic ADRs (ADR-0231 → ADR-0250) — decisions pending
...
|---|---|---|---|
| **B.ADR.36** | ADR-0236 | Stakeholder review pricing — free viewer per project vs metered | B |
| **B.ADR.37** | ADR-0237 | Hybrid data sovereignty default — cloud-default vs local-default | B |
| **B.ADR.38** | ADR-0238 | Enterprise BYOK key custody — KMS-backed vs HSM-backed vs both | B |
| **C.ADR.39** | ADR-0239 | Analysis bridge data contract — IFC4+JSON-LD vs gbXML vs MessagePack | C |
| **C.ADR.40** | ADR-0240 | Render-worker engine selection — Cycles only vs +Mitsuba vs +LuxCore | C |
| **C.ADR.41** | ADR-0241 | Cost rate library plug-in model — BYO vs marketplace-verified vs both | C |
| **C.ADR.42** | ADR-0242 | 4D simulation playback — server-side video render vs client-side replay | C |
| **D.ADR.43** | ADR-0243 | LCA database — open EC3+ICE vs commercial One Click LCA partnership | D |
| **D.ADR.44** | ADR-0244 | DfMA fabrication output — IFC4-Precast vs LandXML-CAM vs vendor-direct | D |
| **D.ADR.45** | ADR-0245 | ConTech integration topology — direct API per vendor vs unified ConnectorHub | D |
| **D.ADR.46** | ADR-0246 | Triple-store implementation — Apache Jena vs Oxigraph vs Postgres-AGE | D |
| **D.ADR.47** | ADR-0247 | SPARQL endpoint authn — anonymous public read vs project-token vs OAuth2 | D |
| **D.ADR.48** | ADR-0248 | IDS authoring UX — visual editor vs YAML vs both | D |
| **D.ADR.49** | ADR-0249 | bSDD sync policy — pull-on-edit vs nightly mirror vs hybrid | D |
| **D.ADR.50** | ADR-0250 | AI design partner constraint propagation — declarative vs imperative vs hybrid | D |
...
|---|---|---|---|
| **STR-01-manifesto.md** §2 promise: "one conversation, from raw site to coordinated building" | A (apartment + house + office); B (10 typologies); C (25 typologies) | IP-A6 (apartment full flow) → IP-B17 (10 typologies) → IP-C9 (25 typologies) | Demo runbook proves the one-conversation promise per typology |
| **STR-01-manifesto.md** §5 brand voice | A (content sweep + landing rebuild) | IP-A5 (brand cutover) | Quarterly brand-voice content audit per operating-principles §6.3 |
| **STR-01-manifesto.md** §7 trade-offs (open format · 70/30 marketplace · sovereignty default) | A (SDK + marketplace) · B (EU region) · C (4 regions + BYOK + self-host) | IP-A1 + IP-B9 + IP-C3 | C07 + C40 + C49 contracts CANONICAL |
| **STR-02-product-vision.md** §2 promise + §5 user journey 8 steps | A (Steps 1-8 wired) | IP-A3 (RAC chatbot Step 2) → IP-A6 (full journey) | E2E demo passes all 8 steps |
| **STR-02-product-vision.md** §4 element types (14) + AI workflows (7) | A (3 typology pipelines) → C (25 typologies) | All Phase A/B/C IPs cumulative | Per-typology reference projects nightly green |
| **STR-02-product-vision.md** §6 environments (local + CI + staging + prod) | A (CI gates) · B (EU region) · C (4 regions) | IP-A1 + IP-B9 + IP-C1/2/3 | All environments live + DR drills passing |
| **STR-02-product-vision.md** §9 phased roadmap (Phase 0-3) | All phases A/B/C close | IP-A13 · IP-B18 · IP-C9 | Exit ADRs raised at each phase |
| **STR-02-product-vision.md** §10 guiding principles (constraint DB is law · conversation before UI · etc.) | Continuous + X.4 + X.6 | Continuous | Per-PR review per operating-principles §6.2 |
| **STR-07-positioning.md** §3 D1-D13 differentiators | All phases | IPs cumulative — see §18.1 + §12.2 | Each D# traces to a contract → phase delivery |
| **STR-07-positioning.md** §4 moats (constraint DB · 49-contract suite · layered architecture · open format · marketplace) | A (foundation) · B (marketplace flywheel) · C (ecosystem moat) | IP-A6 · IP-B16 · IP-C9 | Marketplace 2000 artefacts + 200 devs by Phase C close |
| **STR-07-positioning.md** §5 two-sided positioning | A (demand side PLG + supply side SDK) | IP-A6 (demand) · IP-A8 (supply via first community pack) | C39 + C40 contracts CANONICAL |
| **STR-09-personas.md** C1-C5 archetypes | A (Solo + Studio PLG) · B (Mid-firm + first Enterprise) · C (full Enterprise) | IP-A6 · IP-B8 · IP-C1 | Per-tier MRR targets met |
| **STR-08-go-to-market.md** 4 acquisition motions | A (PLG · DR · enterprise prep) · B (Mid-firm sales) · C (Enterprise contracts) | IP-A6 + roadmap-enterprise-delivery | Per-tier customer counts met |
| **STR-10-platform-strategy.md** 3 pillars (Plugin SDK · Family Platform · Marketplace) | A (all 3 live) · B (flywheel) · C (moat) | IP-A1 + IP-A8 + IP-B16 + IP-C9 | C07 + C40 + family-platform infrastructure CANONICAL |
| **STR-12-site-and-cognition-strategy.md** site substrate + 7-layer cognition | A (L1-L4 + site/climate) · B (L5) · C (L6 + L7 + cognition API) · D (substrate as research benchmark) | IP-A4 + IP-A7 + IP-B11 + IP-C7 + IP-D1 | C19/C20/C21 + cognition rules + L5-L7 evaluators |
| **STR-06-operating-principles.md** O1-O10 + hiring bar + cadence + comp | Continuous (X.* + all phases) | Continuous | Quarterly + annual reviews per O8 + O9 |
| **STR-03-engineering-vision.md** P1-P8 principles | Continuous + CI gates | Per-PR | All 21 CI gates green |
| **STR-03-engineering-vision.md** D1-D13 differentiators | All phases | See §18.1 | Each D# → contract → phase delivery |
| **STR-03-engineering-vision.md** 5 customer archetypes C1-C5 | STR-09-personas.md row above | Same | Same |
| **STR-03-engineering-vision.md** 17 headline NFTs + 68 benches | Continuous (X.1) | Continuous per-PR baseline | apps/bench CI baseline regression gate |
| **STR-04-architecture.md** L0-L9 layered model + boundary lint | Continuous + C01-C18 ratifications | Per-PR | eslint-plugin-boundaries hard-fail gate |
| **STR-04-architecture.md** composeRuntime() 29+ slot interface | Continuous (slot additions per phase) | Per-PR + IP-A2/A4/etc. | Per-slot test in packages/runtime-composer |
| **STR-04-architecture.md** convergence booleans | A (most close) · B (region live) · C (multi-region) | IP-A12 (Phase A booleans) · IP-B17 · IP-C9 | check-pryzm3-exists.ts 9/9 then 13/13 then 17/17 |
| **STR-05-architecture-breakdown.md** 79 packages + 47 plugins + 13 apps | All phases (package count grows) | Continuous | STR-05-architecture-breakdown.md refreshed per PR |
| **STR-15-risks-and-assumptions.md** R1-R8 ongoing + mitigations | Continuous monitoring + per-incident response | Quarterly review | STR-15-risks-and-assumptions.md per-quarterly review per §9 |
...
| **Strategic ADRs ADR-0231 → ADR-0250 (PROPOSED in §20.3)** | DRAFT — ratify in Phase B/C/D | ✅ Tracker §20.3 |
...
| **adrs/ADR-0231-050 (strategic, proposed in §20.3)** | DRAFT → CANONICAL Phase B/C/D |
...
| **PANEL-ALLROOMS** | ✅ **DONE 2026-06-18** — `AREA_FIELDS` extended from the core 6 to ALL house room types (+ En-suite/WC/Study/Utility/Hall/Corridor; stair excluded as fixed circulation). The existing generic `§MODAL-PER-STOREY-REAL` hide logic shows each tab only its present rooms; the parse threads every `s{i}.area_t_<type>` into `roomAreas[type]` (the engine hook). houseModalHtml tests + typecheck green. Original ask: **every room on a level must appear in the per-level tool panel with its own size slider**, so the user can adjust EACH room's area. Today the panel hard-codes a fixed subset (Living/Kitchen/Dining/Bedroom/Master/Bath); rooms actually present on the plate — **Study, Corridor, Entrance Hall, Stair, En-suite, WC** — have NO slider, so they can't be sized (founder arrows: Study + Entrance Hall on the plan have no panel control). Goal: the per-storey slider list is **DRIVEN BY the rooms the engine actually produced** for that storey (dynamic), not a static list. | founder msg 2026-06-18 + img | extends §MODAL-PER-STOREY-REAL (per-storey readouts) → a per-storey room→slider list keyed off the chosen layout's `rooms`; `apps/editor/src/ui/house-layout/` (HouseLayoutModal + houseModalHtml) | ☐ **PRIORITY** |
| **WIN-LOC-SIZE** | **Window location + size still not correct.** TWO distinct roots: (1) **position** is downstream of the WALL — `§DIAG-PARITY-OPENINGS posDrift=0` means the window sits at the right offset ALONG its host wall, but a host wall still off-line (the §MULTI-CLUSTER / wall-residual work) carries the window with it → window appears mislocated until the wall lands on the previewed line; (2) **count/size** is a separate window-EMISSION concern (founder's earlier "kitchen 4 vs 3 windows") — the sub-engine choosing how many windows + what width per wall, NOT measured by posDrift. Also seen: a vertical wall stub poking ABOVE the wall head (wall-body over-extrusion). | founder msg 2026-06-18 + img | (1) resolves with the wall-join residual fixes (§ONE-FRAME-MINT/§WALL-BODY-INNER-FACE/§MULTI-CLUSTER-PARTITION-TRIM); (2) is the window-emission engine — audit count/width-per-wall vs the preview | ☐ |
| **GROUND-COUNT-CONSTRAINT** | ✅ **DONE 2026-06-18 (`84470b26`)** — root was NOT the min-area residual (founder hypothesis refuted): `buildBubbleGraph` re-ran `scaleProgramToShell` with an unconditional bedroom round-up `max(program.bedrooms, round(shellArea/130))`, and `bedroomsExplicit` never reached enumerate/bubbleGraph, so a plate ≥195m² bumped 1→2. Fixed by threading a `lockBedroomCount` flag (houseOrchestrator→runDeterministic→enumerate→buildBubbleGraph→scaleProgramToShell) gating only the bedroom-count term; area-convergence math untouched (apartment + AUTO byte-identical, landmine guards green). +2 tests, ai-host 2823. Original: **Ground-floor Bedrooms/Bathrooms count not honoured** — setting Ground **Bedrooms=1** still ships **2** bedrooms (Bedroom 1 + Bedroom 2). Founder hypothesis: the **MIN/MAX area constraints of the OTHER rooms** (panel shows Living 42 m² "min for this plot", Dining 22 "min", Kitchen 9 "max", Bedroom 18 "min") leave residual plate area the sizer **fills with an extra bedroom**, overriding the stated count. Investigate the interaction between the explicit count and the area min/max convergence. | founder msg 2026-06-18 + img | relates to §GROUND-COUNT-AUTHORITATIVE (`469cb1bb`) + `fillGroundPlate` bedCap + `houseProgramSizerConvergence`; check whether min-area floors on the public rooms leave area the sizer absorbs by re-adding a bedroom; `packages/ai-host/src/workflows/houseLayout/houseProgramFloor.ts` | ☐ |
| **FURN-DOOR-CLEAR** | ✅ **DONE 2026-06-18 (`c29ad42d` + verdict `96d8d06f`)** — the door swing is DETERMINISTIC (left-jamb hinge, inward; `DoorPlanToolHandler.ts:207`), and the existing `door.width × 0.9m` keep-out MATCHES that swing fan's span for a standard door → floor items already covered (Gap A adequate; the symmetric `+0.6m` widening over-covers and REGRESSED the wardrobe). Closed the real gap: integrated bed LAMPS bypassed the keep-out — now `placeIntegratedBedLamps` tests against `doorObstacles(input)` (Gap B). A wide-door / hinge-side-aware asymmetric sector still needs the door hinge-side in the payload (deferred). Original ask below: a door keep-out ALREADY exists (`doorObstacles`, `door.width × 0.9m`, all FLOOR paths test it). Widening it to cover the swing FAN (`+0.6m`) was empirically RE-CONFIRMED to regress (caps the big-bedroom wardrobe to the small-room length — `furnishRealism` realism-gap-2); a precise ASYMMETRIC swing sector (hinge side only) would avoid that but the door payload carries **no hinge side** → blocked. Lamp-in-doorway is low-frequency (integrated lamps at the head wall; doors aren't). NEXT: add a hinge-side flag to the door payload, then emit a one-sided swing sector. Original ask below. | **Furniture must clear DOORS — no furniture in a door's swing arc or blocking the leaf** (founder img: a corridor door's swing arc overlaps a furniture rectangle). The furnish solver places furniture against walls but does not subtract a **door keep-out** (the swing-arc sector + a clearance strip in front of each door/opening), so a cabinet/table can land in the doorway. Goal: each door contributes a keep-out (swing radius ≈ leaf width + ~100mm clearance) the placer must avoid; furniture aligns to leave door access clear. | founder msg 2026-06-18 + img | extends the D-FLE clash queue (bed↔wall · bedside-depth · carpets) — add the room's door/opening list as a placement keep-out in `furnishLayout/placeSolver.ts`; reuse the opening offsets the engine already has | ☐ |
| **DECOR-VARIETY** | **Wall hangings + soft decor need pattern + size VARIETY across rooms** — today every §OVERBED-WALL-TAPESTRY renders the same 4×5 colour-block grid. Founder refs: (a) horizontal-band weave with long fringe (cream/black/red), (b) kilim triangle/zigzag with tassels, (c) the current block grid. Goal: a SET of `WallTapestryBuilder` pattern generators (block-grid · horizontal-band+fringe · kilim-triangle+tassel · stripe) chosen DETERMINISTICALLY per room (different rooms → different pattern), with **per-room size variation** (not one fixed footprint). Pairs with the carpet-variety item (same "vary the variant per room" discipline). | founder msg 2026-06-18 + 3 imgs | extends `packages/geometry-furniture/src/builders/WallDecorBuilder.ts` (WallTapestryBuilder) with pattern variants + a deterministic per-room selector; size from a room-hashed range in `furnishLayout/footprints.ts` | ☐ |
...
**House-generator deep reference + audit** written to `docs/04-reference/architecture-detail/layout-generation-algorithm.md` (§ "Residential House Generator (Casa Unifamiliar)"). AUDIT FINDINGS (queued — NOT yet fixed):
| ID | Finding | Severity |
|---|---|---|
...
- **[STR-13-generative-layout-world-model-strategy.md](../../01-strategy/STR-13-generative-layout-world-model-strategy.md)**
...
1. **Vision/strategy** — [GENERATIVE-LAYOUT-WORLD-MODEL-STRATEGY](../../01-strategy/STR-13-generative-layout-world-model-strategy.md)
...
3. **Decisions** — [ADR-0062](../../02-decisions/adrs/ADR-0062-layout-engine-deterministic-graph-solver.md)
...
ADR-0216/ADR-0239. **Needs ONE new contract — C24.1 "Auto-Documentation Sheets Protocol"** (auto vs
...
| **§GROUND-ENGINE-PERIMETER** | ground storey closes rooms via the engine's authoritative perimeter ring (the §5 smallest-slice from the unification audit) when safe — the cure for the ground "merged room" / fragile weld. **Doctrine = [ADR-0063](../../02-decisions/adrs/ADR-0063-house-generative-layout-doctrine.md) H1.** | v90 (0567f456) | house |
...
| **§UPPER-SHELL-WELD** | upper storeys weld partitions to the minted engine perimeter on rotated plates. **Doctrine = [ADR-0063](../../02-decisions/adrs/ADR-0063-house-generative-layout-doctrine.md) H2.** | v91 (e061f263) | house |
| **§LANDING-NOT-HALL (G14)** | entrance hall is GROUND-ONLY; upper floors get a Landing (stair arrival = circulation). **Doctrine = [ADR-0063](../../02-decisions/adrs/ADR-0063-house-generative-layout-doctrine.md) H4.** | v91 (da48f3e1) | house |
...
- **ADR:** [ADR-0069 — dynamic program canvas as the primary authoring surface](../../02-decisions/adrs/ADR-0069-dynamic-program-canvas-as-primary-authoring-surface.md).
...
recomputes geometry on a window change (per the door/window setOffset/baseline rebuild path, ADR-0257).
...
| **DG.4** | **Live recompute on window-size change.** Subscribe the DF field to the existing window-edit hook (window create / resize / aperture / move) so the field + stats recolour LIVE as the user drags a window-size slider — reuse the realtime-edit rebuild path (ADR-0257), debounced; cancellable. |
...
- **ADR:** [ADR-0064 — Wind CFD runs client-side on WebGPU + LBM](../../02-decisions/adrs/ADR-0064-in-browser-wind-cfd-webgpu-lbm.md)
...
- **Strategy:** [`docs/01-strategy/STR-12-site-and-cognition-strategy.md` §2.6](../../01-strategy/STR-12-site-and-cognition-strategy.md)
...
- **ADR:** [ADR-0065 — Geodata analytical layers are a first-class pluggable-provider subsystem draped on
  Forma/Cesium](../../02-decisions/adrs/ADR-0065-geodata-analytical-layers-pluggable-provider.md)
...
- **Pipeline architecture (DOC, done):** [PIPELINE-ARCHITECTURE-APARTMENT-VS-HOUSE](../../04-reference/architecture-detail/pipeline-architecture-apartment-vs-house.md)
...
- **ADR-0063 — House generative-layout doctrine** (the stair is the multi-storey spine; reserved-core
  placement). [`docs/02-decisions/adrs/ADR-0063-house-generative-layout-doctrine.md`](../../02-decisions/adrs/ADR-0063-house-generative-layout-doctrine.md) ✅ exists.
...
- **Reference:** [`docs/04-reference/architecture-detail/stair-creation-pipeline-and-anchor-analysis.md`](../../04-reference/architecture-detail/stair-creation-pipeline-and-anchor-analysis.md) ✅ exists
...
- **§A.21.D29 entrance** — `resolveEntranceDoor` (ground-only, lands the front door on the entrance shell
  ...
  ground-only/Landing doctrine is **ADR-0063 H4** ([`ADR-0063-house-generative-layout-doctrine.md`](../../02-decisions/adrs/ADR-0063-house-generative-layout-doctrine.md) ✅).
...
**Governance assigned:** stair pipeline ref `docs/04-reference/architecture-detail/stair-creation-pipeline-and-anchor-analysis.md`;
...
- **ADR-0066 — Access-Graph-First Generative Layout Doctrine** (PROPOSED) —
  `docs/02-decisions/adrs/ADR-0066-access-graph-first-generative-layout-doctrine.md`. AG1–AG5 + the
...
- **Governance:** C07 §7 (PWA / service worker), ADR-0255 (deploy), `sw-stale-cache-fixed` memory.
...
- **NEW: [ADR-0067 — Graph-IR / Intent-First Building Graph (BIM 3.0)](../../02-decisions/adrs/ADR-0067-graph-ir-intent-first-building-graph-bim3.md)** (PROPOSED; GIR1–GIR5 + S1–S4 migration).
- Builds on: C52 (editable building graph) · C53 §1 (topology-first) · C50 (typology pipeline) ·
  ...
  unified-building-graph strategy (`STR-14-pryzm-building-graph-and-relational-ai-foundation.md`,
  `STR-13-generative-layout-world-model-strategy.md`).
...
- **NEW: [ADR-0068 — Five-Graph Model: Circulation-First Building Graph](../../02-decisions/adrs/ADR-0068-five-graph-model-circulation-first-building-graph.md)** (PROPOSED; FG1–FG6 + S1–S5 migration).
...
**Governance:** [ADR-0070](../../02-decisions/adrs/ADR-0070-project-north-vs-true-north-authoring-frame.md) (Accepted) +
...
Governed by **[ADR-0071](../../02-decisions/adrs/ADR-0071-room-and-module-rule-engine.md)** +
...
|---|---|---|
| **63.1** | **Mirror is black — should have a MIRROR material** | 🟢 **DONE v145** — new `FurnishFinish`/`FurnitureMaterial` `'mirror'` + shared `makeMirrorMaterial()` (MeshStandardMaterial metalness 1.0 · roughness 0.04 · silver #EEF2F4 · envMapIntensity 1.6, no emissive); `wall_mirror`/`bathroom_mirror`/`wc_mirror` routed to it (WallDecor/BathroomVanity/Wc builders + styleFinish). Was the shared root cause of the black mirror. **Also DONE: the founder's BEDROOM request — `curtain_panel → wall_mirror` (×2) in the bedroom archetypes + program (living-room curtains kept); the bedroom now places a reflective wall mirror.** |
...
|---|---|---|---|
| **65.1** | **"The kitchen is in the stair room"** — ground Kitchen detected **41.4 m²** with the STAIR drawn inside it (kitchen + stair-void flooded into one room; kitchen also way over its ~16%-of-plate cap) | ✅ **DONE v149 §65.1** (2026-06-11). New `clipRoomsOutOfKeepOut(...)` in `subdivide.ts` — a HARD invariant net wired into `subdivideWithReport.finalise` after the reflection: any non-stair room rect that intersects the stair keep-out is clipped back clear of it (+ `overlapAreaM2` helper). + `§DIAG-STAIR-OVERLAP` per-storey diagnostic (room-overlaps-keep-out YES/NO + offender). Before: a room could tile across the keep-out (the ~41 m² kitchen-in-stair); after: `roomOverlapsKeepOut=NO` on every dense plate, a simulated flood fully clipped. |
| **65.2** | **Big white/blank space on BOTH floors** — ground "Room 00-001 **63.9 m²**", first "Room 01-005 **48.2 m²**" + "Room 01-004 **30.2 m²**" (huge undivided cells) | ✅ **DONE v151 §65.2** (2026-06-11). ROOT (case b, leftover-rect): `subdivide.ts` `§STAIR-CARVE-NO-DROP` short-circuit (≈L2323) — on a large/dense plate the stair keep-out fractures the plate into a dominant rect + side fragments (`§DIAG-RECTS areas=[162.4, 51.0, 7.7]`); `trySingleRectCarve(dominant)` succeeds with 0 drops and returns immediately, placing the WHOLE programme in the 162.4 m² dominant rect and leaving the 51.0+7.7 m² fragments EMPTY (= the "Room 00-001 63.9 m²" cavern). FIX: new pure `claimResidualPlacements` (subdivide.ts) wired post-subdivide in `enumerate.ts` — computes residual (buildable − placements) then **GROW then MINT**: grow an adjacent circulation/habitable room into a fragment capped at its dimensional `areaHardMax` + re-validated on width/length/aspect (can't recreate the over-allocated blob); mint the genuine remainder as bounded NAMED `utility` "Store" cells (each ≤ utility hard-max ~7.5 m², aspect-guarded). **Rank-neutral**: the claim touches ONLY the emitted geometry (separate emit-graph), excluded from scoring/Pareto → never flips the winner. Gated: fires only when largest blank ≥ 48 m² AND rectilinear plate → 130/165 m² + sheared plates BYTE-IDENTICAL; stair keep-out honoured by construction (v149 preserved). + `§DIAG-FILL-RESIDUAL` log. Before→after (230 m² 6-bed 2-storey): largestBlank 50.2 m² → **0.0** on BOTH storeys, tiled 171→229.9/230. ai-host 2345/2345 (+14 houseResidualFill.test.ts), root tsc clean. Caveat: gate skips sub-48 m² blanks + sheared plates (byte-identity guard) + extreme ~500 m² single-floor squarified-blob (separate §PLATE-ROLE concern). |
| **65.3** | **Stair (first floor) STILL not connected to the corridor** | ✅ **DONE v149 §65.3** (2026-06-11). New exported `findCorridorStubToKeepOut(...)` in `subdivide.ts` finds a corridor-width stub through **empty space only** (carved clearance + empty bands beside the keep-out — never carves through a habitable room, so no overlap risk), clamped to the true shell bbox so it never protrudes past the façade. `enumerate.ts` §STAIR-SPINE-TOUCH extended: a stair the straight-grow can't bridge gets the stub **minted as a dedicated `corridor` room** (`corridorStub_…`) wired `stub↔corridor` (open) + `stub↔stair` (door). Gated `§STUB-ONLY-RESCUE-CLEAN` (fires only when the candidate has no pre-existing room overlap AND no OVERSIZE/blob finding → never flips the Pareto winner on plates v142 already handled, so 165/500 m² houses stay byte-identical). `wallsAndDoors.ts`: open-zone union now merges two abutting `corridor` rooms (stub + main read as one continuous L-corridor; §DIAG-MERGE-DIVIDER treats corridor↔corridor as legitimate). Before: `sharesStairWall=NO`, stair sealed / served through a bedroom; after: `stubFired=true`, `sharesStairWall=YES` (reach 1.20 m), `stairDoorPartners=[corridor]`, `stairSealed=false`. New `stairDensePlateIntegrity.test.ts` (18 tests). ai-host 2331/2331, root tsc clean. |
...
| **66.1b** | **Room LOOP breaks → blanks + merges + generic names** (the BUILD-vs-preview divergence) | room-topology tolerance vs the §PARTITION-SHELL-INNER-FACE clamp | ✅ **DONE v148 §ROOM-LOOP-BREAK** (2026-06-11). Root confirmed: opening creation → whole-level re-resolve → `WallJoinResolver._clampEndToShellInnerFace` (WallJoinResolver.ts:333) pulls each partition endpoint to `hostHalfT − INNER_EPS` from the shell centreline; `RoomDetectionEngine._splitAtTJunctions` measured guest-to-host-centreline against a **fixed** `SNAP = 0.20` → on a shell ≥ 0.40 m, `hostHalfT ≥ 0.20` → endpoint sat AT/BEYOND the snap → T-junction missed → loop floods. **FIX (data-driven per-host snap, chosen so WallJoinResolver rendering geometry is UNTOUCHED):** in `packages/room-topology/src/RoomDetectionEngine.ts` build `thicknessByBaseId` from the walls, thread it into `_splitAtTJunctions`, and replace the fixed snap with `snapForHost() = max(0.20, hostHalfThickness + 0.02)` per host. 0.40 m shell → 0.22 m snap, 0.60 m → 0.32 m → loop closes → 2 separate rooms (was 1 flooded). **Thin-shell/apartment byte-identical** (0.10 m partition → `max(0.20, 0.07)` = 0.20 floor wins) + unchanged when no thickness data. + **§DIAG-ROOM-LOOP** per-level log (`detectedRooms`, `thickShellTJunctionsRescued`, `unresolvedLoopBreaks`). New regression `thickShellTJunction.test.ts` (4 cases, fails when reverted); room-topology 27/27, geometry-wall 57/57, root tsc clean. Residual: shells > ~0.64 m exceed the old corner-snap scale — uncommon, surfaced by §DIAG-ROOM-LOOP. |
...
**Status: ✅ DONE v161 (2026-06-11).** The geometry already existed (ChevronCarpet/PatchworkCarpet, CornerSofa, BedFactory/JapaneseBed) — this was wiring + placement rules in `furnishLayout/`. **§67.1 RUGS:** new `rug` FurnitureType/kind (1.60×2.30×0.02, zero clearance) → PatchworkCarpetBuilder; new `'under'` anchor + `placeUnder` in placeSolver (centres on the group leader, inherits yaw, sizes to anchor the zone, polygon-clamp shrink-ladder); placed in bedroom (under bed), dining (under table), living (under/in-front-of sofa); **collision-exempt** (never overlap-tested, never on the obstacle set). **§67.2 BED VARIETY:** new `bedVariety.ts` (deterministic FNV-1a hash of roomId → `separate`|`integrated` + variant kind, no Math.random); integrated swaps bed→variant + places lamps inline and SUPPRESSES bedsideLamps.ts (single lamp source → exactly one lamp per nightstand, never both — the founder's consistency check); variant beds still get flanked nightstands + a centred rug. **§67.3 L-SOFA:** new `sofaVariety.ts` `preferCornerSofa(area,w,d)` (area≥16 m² AND L fits with ≥0.9 m circulation) → living `sofa`→`corner_sofa` (CornerSofaBuilder); coffee table + rug pair to the corner sofa; single-program living only (open-plan compound keeps the straight sofa). ai-host 2383 (+14 softFurnishings.test.ts + snapshot updates), geometry-furniture 20, root tsc 0. Limits: not in-browser-verified (render of rug/corner_sofa/variant beds); integrated bed = a coherent guarded SET (variant bed + own single lamp source), no mesh bakes nightstands in; L-sofa single-program only.
...
|---|---|---|---|
| **68.1** | **2 m² bedroom produced in AUTO mode** — must be FORBIDDEN (bedroom min 9 m², master 14). | ai-host `apartmentLayout` enumerate (Pareto hard gate) | ✅ **DONE v153** (2026-06-11). ROOT: `subdivide.ts runRebalance` `needers` filter checked SHORT-SIDE width vs `floorFor(type)`, never AREA — a bedroom could clear its 2.6 m width yet be 7.8 m² < 9 m² areaMin, so the drop never fired. FIX (at the Pareto level in `enumerate.ts`, subdivide.ts byte-identical): per candidate compute `underMinAreaRooms` (habitable rooms < `roomDimensions[type].areaMin`) → new HARD rule `minarea` → such a candidate is `hardValid:false` and ranks below clean ones; apartment path with no viable candidate returns `[]` → `generate.ts` `status:'rejected'` ("plate too small … at minimum room sizes"). + `§DIAG-MIN-AREA-GATE` log. Before (20×3.2=64 m², 2-bed): shipped living=12.5/master=10.5/dining=7.9 (all sub-min); after: REJECTS, no degenerate option; roomy plates unchanged. ai-host 2351/2351 (+6 tglMandatoryAndMinArea.test.ts), root tsc clean. Pairs with the v150 §DIAG-EXEC-MIN-AREA console proof. |
| **68.2** | **A generated option had NO kitchen and NO living** (photo 2: ground = Bedroom/Bath/Corridor only, score 55) | ai-host enumerate (Pareto hard gate) | ✅ **DONE v153** (2026-06-11). ROOT: `enumerate.ts evaluateHardTopology` checked only window/circulation/privacy/overlap — a candidate missing kitchen/living was merely topology-low-scored, so Pareto could still pick it. FIX: per candidate compute `missingMandatoryTypes` (requested mandatory absent from the realised placed set, read from the bubble's minted types so `roomTypesByName` re-types are honoured) → new HARD rule `mandatory` → hardValid:false. House path: public-mandatory (kitchen/living/dining) enforced, bedroom-COUNT check skipped (the house deliberately over-requests bedrooms) and never empties (no house rejection UX) — viability is a ranking preference. + `§DIAG-MANDATORY-GATE` log. ai-host 2351/2351, root tsc clean. Pairs with v150 §DIAG-EXEC-PROGRAM MISSING-MANDATORY console proof. |
| **68.3** | **Upper floor blank NEVER fills on live-resize** — on GROUND, resizing living/dining/kitchen fills the blank; on FIRST FLOOR the blank never gets covered (photo 3 red arrow) | ai-host upper-floor fill | 🟢 **MOSTLY DONE v151 §65.2** — `claimResidualPlacements` now tiles BOTH storeys to ~100% (upper largestBlank 50.2 → 0.0 m²), so the static upper blank is gone. RE-TEST the live-resize case on the green build; if the upper blank still fails to fill *only after a slider edit* (regen path), that residual goes to the §68.1/68.2 program-correctness follow-up. |
...
| **68.6** | **Stair room TOO BIG; stair should be CORNERED** (photo 1 "Stair 31 m²"; §DIAG-EXEC-STAIR-SIZE roomToFootprint 2.8×) | ai-host subdivide §STAIR-LANDING-SEAL + stairPosition §STAIR-CENTRAL-SPINE | ✅ **DONE v160** (2026-06-11). ROOT (confirmed b, not a): the engine ALREADY emits a tight stair room (~8.9 m² for a 5.6 m² U-core = 1.58×) — the keep-out is NOT oversized. The 2.8× / ~30 m² is a DETECTION FLOOD: §STAIR-OBSTACLE-CARVE leaves the stair-clearance slivers + an adjacent UNPARTITIONED band (~9–11 m²) flush against the keep-out with NO wall between, and v151 §65.2 residual-fill only claimed blanks above the 48 m² cavern gate → the sub-gate stair-adjacent band stayed open → editor detection floods keep-out+band into one ~30 m² cell. FIX `§STAIR-LANDING-SEAL` (subdivide.ts `claimResidualPlacements`, wired via enumerate.ts passing `input.keepOutRects`): a blank band that SHARES A WALL with the stair is claimed even BELOW the 48 m² gate → grown into an adjacent habitable room (capped at hard-max) or minted "Store"/"Landing" wired open to its neighbour → a real room borders the stair on every open side → buildWallsAndDoors seals it → no flood. Non-stair-adjacent blanks + apartment path byte-identical. Before→after: stair-adjacent unwalled blank 10.99→4.83 m² upper (0 on cavern-path ground) → detected stair ≈13–14 m² (~1.5×) not ~30 (~2.8×). + `§STAIR-CENTRAL-SPINE` placement bias (stairPosition.ts) — prefers the central-spine U-stair (room bank ≥2.4 m both flanks) but STRICTLY subordinate to wall-hugging (only when no perimeter candidate exists; a stronger central default regressed the subdivider — 60 m² merges / 143 m² blanks — so kept conservative). + §DIAG-STAIR-FOOTPRINT-RATIO log (cell÷footprint ≤1.6 + central-spine/cornered). Stays connected to circulation (§65.3/v149). ai-host 2369 (+7 houseStairTight.test.ts, fail-before/pass-after), root tsc 0. Limits: detected area is ultimately editor flood-fill (engine band-removal is the proxy — confirm in-browser via §DIAG-EXEC-STAIR-SIZE); fully central-spine-by-default needs the central carve hardened (separate larger piece). |
...
| **68.16 + 68.11 + 68.4** | **WINDOWS: bigger + one in EVERY perimeter room + living sliding doors + in-bounds** | ai-host windowEmission | ✅ **DONE v159** (2026-06-11). (a) §68.16 BIGGER: `WINDOW_SPECS` raised — living 2000×1500→**2400×2190**, dining 1800×1500→2100×1700, bedroom/master 1500×1300→1800×1500, kitchen→1500×1200, study→1500×1400, wet rooms 600×600→800×800; all heads ≤ `MAX_WINDOW_HEAD_MM 2300` (sit under the lintel of the 3.0 m-storey ~2.4 m clear; §WINDOW-HEAD-FIT test). (b) §68.11 LIVING = full-height sliding/patio door: **sill 10 mm, head 2200, width 2400 (minWidth 2000 — never degrades to a small window)**, + raised living budget (MAX_WINDOWS_PER_ROOM 4→6, PER_WALL 3→4, §WINDOW-LIVING-PATIO) so a corner living glazes both frontages. (Applied to ALL living rooms, not ground-only — storey-awareness would need a frozen-API change; acceptable per "max daylight".) (c) §68.16 GUARANTEE: new safety-net tier — any windowable perimeter room fronting an external wall ≥ MIN_WINDOW_MM now ALWAYS emits ≥1 window (retries every external wall with a min opening when the main loop is crowded out by doors/partitions); + §DIAG-WINDOW-GUARANTEE log (per room: emitted N ✓ / GUARANTEE VIOLATION / NO-USABLE-FRONTAGE). (d) §68.4 IN-BOUNDS preserved: bigger widths reuse §WINDOW-SPAN-FIT clamp + corner pier → never overrun (tests assert offset+width ≤ wallLen on 2.3–2.5 m walls; editor §DIAG-EXEC-WIN-BOUNDS stays 0). ai-host 2362 (+7, fail-before/pass-after), root tsc 0, apartment path signature-unchanged. Limit: living sliding-door is storey-agnostic; no down-scale if a project sets floor-to-ceiling < 2.3 m (no current path does). |
| **68.15** | **Stair STILL not connected to corridor UPSTAIRS** (recurs on this 4-bed plate despite v149 §65.3) | ai-host enumerate §STAIR-SPINE-TOUCH / houseLayout | 🟡 LIKELY DOWNSTREAM of §68.12 — when the upper corridor floods/merges (the build-flood root), the stair can't door onto a clean corridor cell. Re-test after §68.12 lands; if it persists, the §STAIR-SPINE-TOUCH bridge isn't firing on this plate (the §DIAG-STAIR-CIRC `sharesStairWall` / `§STAIR-SPINE-TOUCH stairsBridgedToCorridor` gen logs will say which). |
| **68.11** | **Living-room MAX DAYLIGHT** — increase living windows; GROUND-floor living windows: sill height **0.01 m**, head **2.2 m** (≈2.2 m tall, near floor-to-ceiling), large spans **2–3 m**, behaving like **sliding/patio doors** | ai-host `windowEmission/` (per-room window spec) | 🔴 QUEUED (engine, after §68.1/68.2). Per-room window archetype: living on ground = full-height glazed sliding door (sill 0.01, head 2.2, width 2–3 m), maximise count/area on the living's perimeter frontage. Cross-ref §68.4 (window-in-shell clamp — the larger spans MUST still sit inside the host wall run). Pairs with the AI-creation default door/window types queue. |
| **68.10** | **GROUND-FLOOR shell: openings (doors/windows) NOT present + perimeter wall CORNERS not joined** (project "aewt"; upper floor fine — has doors/windows/furniture) | geometry-wall (WallJoinResolver corner miter) + opening creation on the DRAWN ground shell | 🔴 **NEW (2026-06-11), needs the v150 §DIAG-EXEC-WALLS generate block to pinpoint.** The ground shell is the DRAWN boundary (hosts entrance door + façade windows); the upper floors are minted perimeter rings (render fine). Two symptoms: (a) openings absent ONLY on the ground shell → ground opening-creation/host-resolve failing; (b) ground perimeter CORNERS not mitred → WallJoinResolver not closing the drawn-shell corners. Possible interaction with v147 (§WALL-PLAIN-SEAM-MERGE-ATTR) or the drawn-vs-minted shell path. ACTION: get founder onto the GREEN v150 build → regenerate → paste the `§DIAG-EXEC-WALLS` (per-wall role/len/openings) + `§DIAG-EXEC-WIN-BOUNDS` block; that says whether the openings exist in the store (creation) vs are lost in render, and whether the shell walls report as joined. Then a focused geometry-wall agent. |
| **68.12** | **BUILD floods: detected room polygons OVERLAP → merges + generic names + giant flood cells** (the deep §66.1b root, worse than v148 fixed) | room-topology detection (robust to the WallJoinResolver trim) | ✅ **DONE v157** (2026-06-11). ROOT (headless repro): the D-TGL EMITTED graph is CLEAN (maxGap=0, 0 overlaps, detected==engine) — the ~961 mm gap is introduced AFTER emission by the editor's whole-level `WallJoinResolver` (geometry-wall), which clusters 3 coincident Y-junction endpoints, finds no pinnable pair, and TRIMS one member ~1 m along its axis (`§MULTI-CLUSTER pinned=0 trimmed=N`) → a dangling end the ≤0.20 m T-snap can't bridge → loop floods. FIX in `RoomDetectionEngine._reconnectDanglingEnds` (run between _snapNearbyCorners and _splitAtBodyCrossings): move ONLY a genuinely dangling endpoint onto the host body/corner it runs up to, guarded dangling+collinear(|cos|≥0.9)+bounded(≤1.25 m, interior) → clean plans are a strict no-op. + §DIAG-PARTITION-REACH log. Before→after: max gap 961→0 mm, detected 9-vs-10→10-vs-10, overlaps→0. room-topology 31 (+4), ai-host 2355 (+4), fail-before/pass-after, root tsc 0. NOTE: makes DETECTION robust to the trim, doesn't prevent the trim (WallJoinResolver out of scope — a follow-up §68.12b could length-clamp the resolver trim itself); §DIAG-ROOM-LOOP still audits any case > the 1.25 m reach cap.
| **70** | **STAIR move GIZMO in 3D** (founder: "all elements must move the stair on demand in 3D — do it the same way") | input-host + command-registry/bus + apps/editor/engine | ✅ **DONE v156** (2026-06-11). Stairs were deliberately skipped from gizmo attach (SelectionManager.select ~L1262 — baked world-geometry at local origin → gizmo lands at world origin). FIX (mirrors the WALL move path; ADR-0257 + C03/C11/C16, P6/P8): un-skip stairs; new `StairTransformController` anchors the gizmo to an invisible proxy at the stair bbox-centre + forwards the drag delta live; drag-end → new `stair.move` bus cmd → `MoveStairCommand` (MOVE_STAIR enum + PlanOrdering + barrel) translates startPosition + flight overrides + landing centres by the XZ delta → mesh rebuild. Persists across reselect + save/reload + undo; Y level-locked (translate only). +4 tests, root tsc 0. Limitation: translate only (rotate = separate cmd, queued). |
| **FORMA.6** | **REAL full-fidelity PRYZM building in the Forma (Site 3D) view** (founder: "same as the 3D globe — the WebGPU scene elements") | apps/editor geospatial (Cesium) + @pryzm/file-format GLB bridge | ✅ **DONE v158** (2026-06-11). Both globe+Forma re-extruded only the wall SHELL inside Cesium (globe real-coloured, Forma whitewashed) — windows/doors/roof/furniture/stairs absent. FORMA.6 mirrors the globe's existing real-model glTF bridge (`renderRealModelOnGlobe` §A.21.D49) for the Forma study: `exportFragmentsToGLB(scene.three)` (file-format, P2-safe) → `Cesium.Model.fromGltfAsync` at the SAME site ENU origin (`eastNorthUpToFixedFrame`, terrain clamp `formaTerrainBaseHeight`, upAxis Y/forward Z §A.21.D54). Toggle `setFormaBuildingFidelity('massing'\|'real')` default 'real' + segmented UI button + console cmd; massing stays reachable; floor-filter partial → massing fallback (single GLB has no per-storey nodes); dispose/blob-revoke; never-throws (graceful-degrade); geometry-signature-gated re-export. +10 tests, root tsc 0. Limitation: browser-unverified alignment (carries the globe axis convention). |
| **68.12-orig** | (superseded by the DONE row above) | | 🔴 **AGENT RUNNING (2026-06-11).** Green-v153 evidence from 2 separate 7-bed ~205–220 m² 2-storey builds: GROUND `§DIAG-ROOM-LOOP BREAK endpoint 961mm from host centreline > hostSnap 200mm` ×3 (a genuine ~1 m partition-to-host GAP, NOT the thick-shell case v148 fixed); UPPER `§DIAG-EXEC-OVERLAP overlaps=24–31`, `coverage=128%`, a `55.7 m² no-engine-match no-door` flood cell, `HABITABLE-ON-STAIR`. The modal PREVIEW is clean + the engine DESIGNED a divider for every pair (`§DIAG-MERGE-DIVIDER` all ✓) — so the divergence is that the EMITTED partition baselines don't reach their hosts (likely a room rect grown/shifted post-subdivide via §65.2 grow / §FEASIBILITY drop-refit / residual MINT, leaving a ~1 m wall stub), so detection floods + traces overlapping loops. Agent: re-derive partition baselines from FINAL room rects / snap-extend endpoints to host. Hard-gate: emitted graph has no >tol partition-to-host gap AND detectedRooms==engineRooms with 0 overlaps. |
| **68.13** | **~51 s main-thread stall during house finish** (`§BN-07 DEFERRED-RESUME-FLUSH delayed 51086ms`) | apps/editor post-gen chain + core-app-model BatchCoordinator | 🟢 **DONE v163 (2026-06-11) — NEEDS PROD-PROFILE CONFIRMATION.** ROOT (traced, not a single blocking call): `runHousePostGenChain` advanced each storey/stage by `await`ing the executor's `*.layout-executed` event — but the executors emit it SYNCHRONOUSLY the instant `runBatch()` RETURNS, long before the batch's async tail (the deferred resume-flush rAF + the FIRST-RENDER-POST-SUPPRESS WebGPU **PSO-compile longtask** + the post-batch **PBR whole-scene traverse**, none of the furnish/light/ceiling batches passing `skipPbrUpgrade`). So storey N+1's batch opened ON TOP of storey N's still-pending PSO+PBR tail → overlapping storms saturate the main thread → the just-registered resume-flush rAF can't get a slot for ~51 s (it's the victim, not the cause). Ruled out a single O(scene) offender (PBR scope already new-meshes-only/idle/chunked; redetect already coalesced). FIX: new `BatchCoordinator.onNextSettle(cb)` primitive (multi-subscriber fire-once "batch fully settled", flushed in onComplete + error path + forceReset so no awaiter is stranded) + `yieldBatchSettled()` in runHousePostGenChain awaits it after each batch-bearing stage + yields a few macrotasks + an idle slot so the prior batch's tail DRAINS before the next opens → storeys/stages no longer overlap. Behaviour-preserving (same batches/commands/flags/order/final scene — only temporal spacing). + §DIAG-POSTGEN-TIMING log (per storey/step elapsed). root tsc 0, runHousePostGenChain 4/4. Caveat: per-storey wall-clock is slightly longer (you now wait for each drain instead of overlapping) but the thread no longer freezes 51 s — the win is RESPONSIVENESS. Follow-on if a prod profile shows PBR dominates per-storey: new-meshes-only PBR scope on the furnish/light executors (those files were out of this agent's scope). |
| **68.14** | **Degenerate 0.10 m "shell" wall stubs** (`§DIAG-EXEC-WALL … shell len=0.10m` ×3 in one build; 0.48 m in another) | ai-host emit / WallJoinResolver | 🟡 QUEUED. Tiny 10 cm wall fragments classified as shell — junction/clip artefacts. Likely the same family as §68.12 (stub left by a rect change). Flag + drop sub-0.2 m walls at emit, or fix the producer. |
| **68.D** | **§DIAG-EXEC-NAMES false positive** — flagged EVERY detected room as generic (`genericNames=12/13`) because the diagnostic runs BEFORE the rename batch, when every cell still has its placeholder "Room NN" name | apps/editor houseExecDiagnostics | ✅ **DONE v155** (2026-06-11). Condition now keys off `!eng` (no engine match) — the true flood-remnant signal — not the name prefix. So `genericNames` now equals the real no-engine-match count (matches `noEngineMatch` in the rollup). Editor-only. |
| **68.P** | **§DIAG-PREVIEW console block** (founder "add many many more logs" + "ground modifies but never the upper floor" + "do those buttons work?") | apps/editor HouseLayoutController | ✅ **DONE v152** (2026-06-11). New `_logPreviewDiagnostics` in HouseLayoutController fires every modal regen (per slider/checkbox edit), logging: `§DIAG-PREVIEW-PROGRAM` (every flag the engine received — livingRoom/includeKitchen/openPlanKitchenDining/masterEnSuite/per-type area overrides/weights → answers "do the buttons work?"); `§DIAG-PREVIEW-STOREY` (per storey: rooms, covered vs footprint, BLANK m²+% → the "top floor empty by the stair" with a number); `§DIAG-PREVIEW-CHANGED` (per storey CHANGED vs ⚠ UNCHANGED since last regen → proves "ground modifies but upper never does"); `§DIAG-PREVIEW-FLAGS` (did masterEnSuite produce an ensuite / openPlan fuse kitchen+dining); ROLLUP changedSinceLastRegen / storeysWithBlank. Confirmed: the form→program plumbing for both checkboxes is CORRECT (HTML `name` matches the parser), so any no-op is engine-side — the logs will show which. Editor-only; root tsc clean. |
...
## New items logged post-2026-07-16 (owner + target date — per logging protocol)

> NOTE: this tracker still lacks Owner/Target-date COLUMNS (structural gap tracked as **L-346**); until L-346 lands, new items carry owner+date inline here.

| Item | Owner | Target date | Description | Dependencies |
|---|---|---|---|---|
| **L-352** projects-hub stale-flash | UNASSIGNED | TBD | First paint of `#/projects` shows stale/placeholder state then swaps to the correct server list (~1-2 s). Fix = persist last-known-good list+thumbnails (C05) + reconcile-in-place + skeleton-only-when-empty + confirm C02 §121 deferred-boot. | Independent of L-335 (sync). Touches C02/C05/C06/C10. Coverage-gap → SPEC-PROJECT-HUB or C02 clause (see MISSING-CONTRACTS-AUDIT). |

| **L-353** Gaussian Splatting strategy (R&D) | UNASSIGNED | TBD (post-V1) | Product-strategy + architecture review for 3D Gaussian Splatting as a reality-capture capability; extends spike-gaussian-splatting-photoreal-3d.md. 9-section research deliverable + core/module/workflow recommendation. | OFF the V1 critical path. Depends on a reality-capture-assets contract (MISSING-CONTRACTS-AUDIT). Related: geospatial-foundation, SPEC-FORMA-SITE-VIEW, ADR-0065. |
| **L-354** site-analysis credibility audit | UNASSIGNED | TBD | Validate/provenance the sun/wind/temp/population layers; per-layer metadata + confidence + honest decision-grade-vs-indicative UI labeling; advanced-simulation roadmap. P1 sub-component = provenance labeling. | Related C21/ADR-0074/@pryzm/solar-analysis, SPEC-GEODATA-ANALYTICAL-LAYERS. Coverage gap → analysis-provenance contract. |
| **L-355** Forma view perf + Context Engine | UNASSIGNED | TBD (perf=pre-launch) | P1: Forma 3D Site loads too slowly (71 MB BIM→GLB export + 4342 Overpass footprints + full massing rebuild on the synchronous activation path). P2: provider-agnostic Context Engine for engineering-grade context. | Perf is P1 pre-launch. Strategy depends on a Context-Engine contract (MISSING-CONTRACTS-AUDIT). Related SPEC-FORMA-SITE-VIEW, ADR-0065, L-272. |
| **L-356** Cesium 3D-tiles robustness/speed | UNASSIGNED | TBD | P2 perf-hardening of the sound Cesium globe view; shares the L-355 BIM→GLB-export root cause + double-massing-render. | Shared workstream with L-355. Cluster L-353/355/356/357. |
| **L-357** Canvas GIS-awareness maturity (PARENT) | UNASSIGNED | TBD | Architecture review: should Three.js Canvas become GIS-aware; hybrid vs Cesium-only; maturity 0-5; coordinate authority; roadmap to Cityweft quality. | PARENT decision gating L-353/L-355/L-356. Coverage gap → C-CONTEXT-ENGINE + coordinate authority. P2 single-THREE-owner constraint. |
| **L-358** GLB-export cache invalidation | UNASSIGNED (agent in progress) | pre-launch-eligible | P2 fast win: cache the ~70MB GLB blob by (signature,variant) so globe↔Forma round-trips skip re-export. FIX IN PROGRESS. Follow-on = worker + diff export. | Root cause of L-355 P1 + L-356. Root-caused to export-perf, not GIS. |
| **L-359** author C-CONTEXT-ENGINE contract | UNASSIGNED | **NONE — post-launch/backlog, NO Sept pressure** | P3 governance gap: one contract for coordinate authority + fidelity tiers + provider model (ADR-0065) + provenance bar (L-354). Documentation only. | Consolidates L-355 + L-357 gap notes. Gates L-353/L-355/L-356. NOT a launch risk. |
| **L-360** checksum bricks projects (P0 regression) | UNASSIGNED (fix in progress) | ASAP | L-334 checksum false-positives on lossy-codec round-trip + hard-refuse bricks projects. Fix: round-trip-stable + load-best-effort+warn. | Regression from L-334. Persistence queue. |
| **L-361** WebGPU batch-resi crash → browser GPU lockout (P0) | UNASSIGNED | ASAP | Batch resi-gen on WebGPU crashes GPU until browser disables it; no project opens. Reopens L-139. Root = TSL invalid-code + CW PSO storm. | Render/renderer-three. Ties L-139/L-312. Workaround: WebGL. |
| **L-405** unified view-mode switcher (2D parcel-select first-class + enhanced-Forma/LOD200 slot) | UNASSIGNED | TBD (post-launch-acceptable; P2) | Founder: clarify the segmented view-mode bar (`mountResultToggleBar`, `GISAreaLayout.ts:789` — `◧ 3D+plan` / `◉ 3D globe` / `◉ 3D Site` Forma) into the intended set + close two gaps: (a) make the **2D parcel-SELECT map** (`SiteBoundaryMap2D.ts`, today onboarding-only) a persistent first-class switcher segment; (b) wire an **enhanced-Forma / LOD200 "super-enhanced context" segment NOW**, placeholder-backed by the current Forma/Context path, enhanced-data swapping behind the same view id later. (b) IS L-374g's `▤ Context` peer segment (CONTEXT-VIEW-DESIGN.md). Correct-fix = register each segment in a declared view-mode registry (C06 §1 `viewRegistry` intent) so per-view camera/L-378-pose-guard + L-270 overlay + persistence apply — NOT a one-off toggle. Cross-cutting: C06 launcher UI + view-state/camera registry (`MultiViewCameraManager`/`ViewCameraStateStore`) + parcel-select lifecycle (C57/C19) + Cesium/Forma surfaces (C04/C12). LOGGING ONLY. | **Depends on / do-together-with L-374g** (enhanced-Forma segment — land ONE switcher, not two). **Ties the compliance-authoring track** (the 2D parcel-select map is that flow's entry — L-380/C57) + **L-384** (shared site-authoring edit/undo/ESC/BACK affordances) + CONTEXT-VIEW-DESIGN two-views work. Contracts: C06 §7/§1, C04, C12, C19 §1.4, C57 §5/§5.4, C55. Coverage gap → view-mode-switcher registry / SPEC-CONTEXT-VIEW / SPEC-PARCEL-SELECTION (logged to MISSING-CONTRACTS-AUDIT). Queue: view-system / UI-shell + geospatial. |
| **L-414** SPIKE — interrogate & manipulate BIM/IFC DATA inside the Cesium geospatial view | UNASSIGNED | **TBD — post-launch / backlog, QUEUED (NOT priority, founder-explicit)** | **SPIKE (research-only, docs only this pass).** Founder: *"if in the future we want to manipulate / interrogate the DATA in the Cesium views … how IFC data in Cesium could adapt to a super-performant and flexible process — QUEUED, not priority."* Today the Cesium / Forma 3D Site is a **passive context surface** — BIM massing + envelope are a whole-scene glTF `Cesium.Model` (`CesiumViewport.ts` `realModelOnGlobe`/`realModelOnForma`) with no per-element feature IDs; `scene.pick` (`:1707`) resolves only the whole model. Candidate directions (not yet researched): per-feature 3D-Tiles metadata (`EXT_mesh_features`/`EXT_structural_metadata`), ThatOpen fragment-ID → Cesium feature-ID picking, streaming-vs-in-memory, a selection bridge into C27 Inspect (P7) / C28 Data-panel. Spans layers (BIM data C03 + IFC/ThatOpen C25/C26 + Cesium render/bridge C12/C55 + perf C10). **COVERAGE GAP: no contract governs interactive BIM/IFC interrogation in the geospatial view** — a forward tension with C55 §1.2 (context-only), NOT a present violation. Spike stub: `docs/03-execution/spikes/spike-bim-ifc-data-interrogation-in-cesium.md`. | **Relates to C55 §1.2** (geodata layers — context-only; §4.1 cross-refs this spike), **C27** (Inspect) / **C28** (Data panel), **C25/C26** (IFC / ThatOpen), C12/C03/C10/C04. Adjacent to the context-engine cluster **L-353/L-355/L-356/L-357** + the C-CONTEXT-ENGINE gap **L-359** + L-374/L-412 (context/pane view work). **FOUNDER DECISION — RESOLVED 2026-07-18 (direction chosen, deferred): the geospatial view SHOULD eventually become a first-class interactive BIM/IFC surface ("definitely nice to have"), NOT stay permanently context-only — but explicitly QUEUED/not-priority (post-September). C55 stays context-only until a governing contract lands; when picked up the FIRST deliverable is that contract (C55 §1.2 amendment + C27/C28 extension, or a new contract) before any code.** Gap logged to `MISSING-CONTRACTS-AUDIT-2026-06-01.md`. Queue: geospatial / context-engine (primary) + data-platform (C03/C27/C28) + interop/IFC (C25/C26 + ThatOpen). |


| **L-442** Deploy boot-timeout — precompile the server (P0 mitigated, durable fix open) | UNASSIGNED | TBD | Cold boot transpiles ~100 workspace TS packages under tsx before binding 0.0.0.0:5000, exceeding Fly's 60s health ceiling; every deploy rolled back. Mitigated by VM 512MB/1cpu → 1024MB/2cpu + honest 60s grace. DURABLE FIX: precompile so the runtime is plain node. Boot cost grows with every package, so this recurs until fixed. | Infra/deploy. fly.toml, Dockerfile, write-prod-shim.mjs, deploy-fly.yml. Depends on nothing; BLOCKS every other deliverable reaching production. |
| **L-443** Planning Rules Engine — extend C58's rule model + calificación layer (P1, CONFLICT) | UNASSIGNED | TBD | The founder's "compiler" model IS C58 (§1.5/§2.2/§2.4) — no new architecture needed. But §2.2 admits only a setback triple and CANNOT express alignment-governed zones (Madrid publishes buildable depth as a POLYLINE, verified live). Needs a human decision: extend the rule family, or publish a scope restriction. Practical blocker is the calificación (zone-code) layer, not the schema. Portugal = zero coverage, unstarted jurisdiction. | Compliance/geospatial. C58 §2.2/§2.4 (KG-1), C57, ADR-0269, C19 §1.6. Depends on L-441 (classification landed) + unbuilt calificación layer. Related L-438/L-439. Three-city proof: Madrid, Barcelona, Córdoba. |
| **L-511** 3D context-data country study — buildings/LOD/height, roads, pedestrian, water, parks/trees (P2; **ES ship-first**) | UNASSIGNED | TBD (Spain first, post-launch quality) | Upgrade context layers OSM→national-authoritative behind the SAME tiered per-country resolver as Barcelona zoning. **Live-probed 2026-07-21:** 3DBAG 200 (LOD2.2, EPSG:7415, CC BY 4.0), Catastro Buildings WFS 200 (`bu:Building`+`numberOfFloorsAboveGround`), CNIG PNOA 200; PDOK BGT HTTP 000 (flagged, re-verify). NL + ES spike gates evidence-passed. Study: `docs/04-reference/geospatial/CONTEXT-DATA-COUNTRY-STUDY.md`. | **Depends on** the Barcelona zoning resolver pattern (proven) + shared nDSM module (unbuilt, serves ES/FR/PT). Two-phase gated: no impl before each spike Gate passes + go-ahead. Contracts C12/C19/C21/C23/C55. Ties L-443 (planning-rules resolver), L-405 (view-modes), context-engine cluster. Queue: geospatial / context-ingestion. |
| **L-512** Spain context deep-dive — hybrid height + concrete ES source map (child of L-511; P2, ES ship-first) | UNASSIGNED | TBD | Reviewed founder deep-dive + re-probed live. Height is a HYBRID: Catastro `ALTURAS` floor-count + LiDAR nDSM 90th-pctile (cycle-tagged, <10cm→40cm by PNOA coverage) + confidence — NOT one "REAL". No LOD2 anywhere in ES (ICGC Catalonia is LOD1) → roofs = our RANSAC "reconstructed". Real ES sources (live 2026-07-21): roads BTN25+CartoCiudad `portales`, water BTN25+IGR Hidrografía, parks BTN25 zonas verdes, trees Open Data BCN (slug TBC). Docs: `docs/04-reference/jurisdictions/es/SPAIN-HEIGHT-MEASUREMENT.md` + `spain/topics/*`. | **Child of L-511.** Its nDSM pipeline IS L-511c (shared ES/FR/PT). **Blocks:** C23 provenance must extend to graded/multi-field/cycle-tagged BEFORE the badge ships (gap logged in MISSING-CONTRACTS-AUDIT). Contracts C12/C19/C21/C23/C55. Queue: geospatial / context-ingestion + provenance (C23). |
| **L-514** Portugal context deep-dive — source map + single-source LiDAR height (child of L-511; P2, build-order #7) | UNASSIGNED | TBD | Reviewed founder deep-dive — **NOT live-probed this session** (all sources UNVERIFIED, spike leads to re-verify, unlike the live Spain pass). **Reframing:** PT is NOT uniformly the weakest — new 2023–25 **Carta Cadastral** (DL 72/2023, SNIC, DGT; NIC + geometry, but only **~134 munis** = 127 CGPR rural + 7 SiNErGIC, **NOT city cores — check per-municipality**; BUPi is NOT a parcel source) + **Lisbon CML 3D model** (LOD2/3-ish, incl. modelled sidewalks) richer than any single Spain source. Weakness = patchy parcel coverage + no national LOD outside Lisbon. Height = **DGT national LiDAR** nDSM (10 pts/m², DTM 50cm/DSM 2m) — **KEY DIFFERENCE vs ES: NO floor-count cross-check → stands ALONE, lower confidence; no published RMSE-Z** (don't quote PNOA parity). Water SNIRH/DGT, parks COS/COSc (coarse), trees Lisbon "Arvoredo" per-tree register. Docs: `docs/04-reference/jurisdictions/pt/PORTUGAL-CONTEXT-DEEP-DIVE.md` + `portugal/README.md` + `portugal/topics/*`. | **Child of L-511.** Its nDSM pipeline IS the SAME shared module as L-511c / L-512b (ES+FR+PT, build once). **Curation/coverage-gated:** per-municipality parcel coverage + **Lisbon CML licence verification** (BLOCKER, UNVERIFIED at geodados-cml.hub.arcgis.com). Governed by the SAME graded-provenance gap as L-512 (MISSING-CONTRACTS-AUDIT) — **more acute** (single-source height). UNVERIFIED: Lisbon licence, IP roads, OGC-API status, DGT RMSE, DGT class codes. Contracts C12/C19/C21/C23/C55. Queue: geospatial / context-ingestion. |
| **L-522** Terrain / DTM context deep-dive — byproduct-of-height reframing + per-country DTM adapters (child of L-511; P2) | UNASSIGNED | TBD | Reviewed founder deep-dive — **NOT live-probed this session** (all sources UNVERIFIED, spike leads to re-verify, like the L-514 pass). **Reframing: terrain is NOT a new source** — the DTM is a byproduct of the SAME classified LiDAR pull already done for building height (class-2 ground -> DTM); its `DTM` half IS the draping surface for roads/water/parks/site-grading. **ES needs NO new sourcing** (DTM = the L-511c/L-512b height-pipeline byproduct); add a DTM grid-resolution badge alongside the coverage-cycle badge. Per-country (UNVERIFIED): **FR** IGN LiDAR HD (MNT 50cm, ~10cm vert; RGE ALTI 5m fallback) — best in Europe; **UK** EA LIDAR Composite DTM 1m (~99% England, +/-15cm) + Scotland own + **Wales/NI check individually**; **DE** DGM1 1m **per-Land (16 adapters, no federal endpoint)**; **NL** AHN sub-decimetre = reference adapter; **global fallback FABDEM** (bare-earth GLO-30+ML, ~30m) NOT raw SRTM/GLO-30 (surface models — never drape on trees-baked-in). Adapter interface: `country/region -> terrain adapter` + `grid_resolution` + `vertical_accuracy_rmse`. Docs: `docs/04-reference/geospatial/CONTEXT-DATA-TERRAIN.md`. | **Child of L-511.** Shares the SAME nDSM/DTM pipeline as L-511c / L-512b (ES DTM = height byproduct, no new fetch). Governed by the SAME graded-provenance gap as L-512 (MISSING-CONTRACTS-AUDIT) — grid+RMSE tagging richer than C23's binary badge. Open decisions: **DE 16-adapter cost**, **UK Wales/NI coverage**, **FABDEM non-commercial licence**. UNVERIFIED: all national grids/RMSE/coverage. Contracts C12/C19/C55 (+ C23 provenance). Queue: geospatial / context-ingestion. |
| **L-515** Barcelona envelope depth applied to the WRONG parcel edge (P1) | UNASSIGNED | TBD | Founder: profunditat edificable band runs off a SIDE edge, not measured back from the street frontage. Depth SOLVE is correct (`blockDerivedDepth.ts` per-edge inset, refuses w/o `front`); bug is upstream **frontage classification** marking a side edge `front`. Probe §L-515-FRONTAGE-DIAG shipped v244 (logs which parcel/block edges are `front` + bearings). Fix gated on reading it — NOT blind (L-508 lesson). | Contracts C58/ADR-0270/ADR-0271/C19. Relates **L-502** (manzana-perimeter frontage — likely interaction), **L-507** (depth shape). Correct-fix = road-adjacency frontage, not longest-edge. Queue: zoning / envelope geometry. |
...
## L-645 — the 3D-Site SCOPE slab (cityweft-style; RE-OPENED 2026-09-07) (P1, founder-prioritised) · OWNER: lane SCOPE-SLAB · TARGET: Phase 2 after the extent lane (L-13058) lands
- **What:** founder 2026-09-07: *"a slide of the scope on 3D Site view — like cityweft does — circular or rectangular — crop everything — absolutely everything — but within the scope should be sound — really detailed and completed."* Built once as §CTX-EARTH-SLAB (392601e6) and RETIRED (1b366ed2) — root cause now CONFIRMED against Cesium 1.143.0 source: `clippingPolygons` exists on Globe / Cesium3DTileset / Model only, never on an entity or a `Primitive`, which is every visible layer. Audit: `AUDIT-3D-SITE-SCOPE-CROP.md` (25 layers, 14 findings, 5 founder questions).
- **Approach (ADR-0382 · C12 §13 · SPEC-3D-SITE-PRODUCTION-CONTEXT §7):** ONE persisted value `SiteModel.scope` (L0 `SiteScopeSchema`, written only by `site.setScope`) that every layer's radius AND read derive from; a circle IS its sagitta-bounded n-gon; the globe cut with `globe.clippingPolygons` (`inverse:true`); every entity / primitive layer cut GEOMETRICALLY at build (`scopeClip.ts` on `intersectPolygons2D`), straddlers cut never dropped; a flat-shaded neutral-grey slab side (a lit material is what read as the "red ring"); the outside is the existing pale backdrop; caps inside the scope print the numbers (`capVerdict`) and the slider carries the "complete" mark. **Phase 1 LANDED** (`d602e1bf`: schema + command + `siteScope.ts` + `scopeClip.ts`, 74 tests, all gates RC=0). **Phase 2 PLANNED:** the render-path wiring + the per-pane slider — work list in SPEC §7.6. CONTEXT-EXTENT-2X landed (`854cbdd7`/`11426e2a`) and its runtime `SiteContextScope` is now a type alias of the schema with ONE outer-radius body and ONE exported range (`SITE_SCOPE_RANGE`); `setContextScope` is the value's single entry into the load. Founder rulings open: default extent (complete ≈ 900 m vs 1781 m), rectangle vs circle default, scope on the photoreal globe, BIM model never cropped, undo.

## L-646 — Building HEIGHTS honesty + per-city standard (P1) · OWNER: UNASSIGNED · TARGET: TBD
- **What:** "not many building heights are correct." Heights bottom out in a fabricated 9 m default (the "9 m carpet"); no measured fallback. Full write-up: audit **L-646** + `docs/04-reference/standards/BUILDING-HEIGHT-REPLICATION-STANDARD.md` (authored this pass).
...
## L-663 — viewport crash-recovery is an UNBOUNDED pipeline-rebuild loop (P1) · OWNER: UNASSIGNED · TARGET: TBD (instrument first)
- **What:** founder 2026-08-01, prod, WebGPU, 48 meshes — *"Just changing dim of kitchen arms — the project got frozen."* `ViewportCrashGuard` fires `RPM.onProjectSwitch()` as "soft recovery" **with no project switch in the session**, producing a repeating `onProjectSwitch → SHADOW_REBUILD_SCHEDULED → Rebuilding pipeline → SHADOW_REBUILD_COMPLETE` cycle. Full write-up: audit **L-663** + `V1-LAUNCH-IMPLEMENTATION-PLAN.md` **L-663** + **C04 §RECOVERY** (Known Violations).
- **Root (code-confirmed, 3 defects / 1 symptom):** ① recovery resets every bound and adds none — `onProjectSwitch()` zeroes `_retryCount` (`RenderPipelineManager.ts:1851`), the counter whose exhaustion creates `phase='error'`; `ViewportCrashGuard.onRetry` also clears `_hasCrashed` + `_consecutiveFailures` (`:236-239`); **no recovery-attempt counter exists**, so the cycle never escalates to a hard reload. ② the guard destroys its own evidence — `initScene.ts:2756` calls `handlePipelineError()` with **no argument** and `PipelineStatus` has **no `lastError`**, so the overlay and Sentry both get a synthetic string. ③ `onProjectSwitch` is a **C13** lifecycle primitive being reused for error recovery. ⚠ **The TRIGGERING throw is NOT established** — the founder's log shows the recovery, never the cause.
- **Approach (strict order, per ADR-0089 + ADR-0257):** (1) **instrument first** — thread the real `Error` (`lastError` on `PipelineStatus`) and log the suppressed `usedTimes` stack; ship alone and re-run the repro. (2) mint a bounded `recoverFromRenderFault()` (≥1 OTel span, **P8**) that never resets `_retryCount` and escalates rebuild → `_downgradeToLightweightPipeline()` → hard reload; `onProjectSwitch()` reverts to pure C13. (3) de-duplicate the double `bim-furniture-updated` emit (`UpdateFurnitureParametersCommand.ts:222` on top of `FurnitureStore.ts:34`) that rebuilds a *selected, outline-composited* furniture root **twice per Apply**. (4) apply **ADR-0257**'s element-scoped rule to the furniture path (today a dimension change triggers a whole-scene `onGeometryAdded` traversal + shadow freeze/thaw, `initScene.ts:3350-3365`). (5) contract close-out in a separate pass. **Fast-but-wrong REJECTED:** a bare counter wrapped around the existing `onProjectSwitch()` call.
- **Pinned by:** `packages/renderer-three/__tests__/RenderPipelineManager.recoveryLoopUnbounded.test.ts` (4/4 green) — a **characterisation** test asserting today's *defective* shape, so the fix must rewrite it.
- **Queue:** rendering / viewport crash-recovery (routed by architecture, **not** by the furniture panel where the symptom appeared). **Dependencies:** none blocking; shares the `RenderPipelineManager` shadow/rebuild machinery with **L-231 / L-316 / ADR-0111** — coordinate with any in-flight shadow work, and with the in-flight §CITY-COMPLETION fan-out only insofar as both touch `docs/04-reference/V1-LAUNCH-*.md` (append-only, disjoint rows).
- **Do NOT conflate:** the same log's `[captureThumbnail] Read a blank/transparent frame` is the pre-existing non-fatal `§THUMB-BLANK-REUSE` WebGPU swapchain read race (`initPersistence.ts:209-222`), and `siteCapture.status='degraded' site=NULL` is the independent **L-489/L-545** georeference family (other owners). The serializer line is also the evidence the auto-save **did** persist a version — which is why this is P1, not P0.
...
## L-668 — residential SETUP panel fires before parcel + envelope (P2 UX/flow gap) · OWNER: UNASSIGNED · TARGET: TBD

| Item | Owner | Target date | Description | Dependencies |
|---|---|---|---|---|
| **L-668** residential setup panel fires at the wrong step (P2) | UNASSIGNED | TBD | **Second report of the same defect — L-435 (2026-07-20) logged it and is still OPEN.** `renderGenerateConfirmStep` branched straight into `renderResidentialProgramStep`, so the full-screen setup panel REPLACED the compact confirm card apartment/house get, firing before the user had seen the parcel + envelope. **IMPLEMENTED — PENDING VERIFICATION** (`8094c3fb`, §RESI-SETUP-AFTER-GENERATE): residential now renders the same generic non-blocking card, and the setup panel opens only from the Generate button; its Back returns to the card instead of discarding the C19 boundary. | **Ordering is un-contracted (L-670)** — the reason this needed a founder to catch it twice. Touches ADR-0069 (wizard pattern rejected), ADR-0092, C06 §7, C50 §5. **L-435 stays OPEN** for its clause (b): offer the *nearest feasible band*, not just an explanation (reuses L-401 `capStoreysToEnvelope` `binding`/`explanation`). **Not done and will re-report:** `renderOfficeProgramStep` copied the same wrong-side shape (ADR-0092 §OFFICE-PREVIEW-STEP). Queue: onboarding / ux. |
| **L-669** residential refuses feasible plots + the refusal contradicts itself (P1) | UNASSIGNED | TBD | Founder: *"why is a plot of 674 m² too small? … you could even build a resi building in a plot of 150 m²."* Modal read "~674 m² is too small … needs ≥400 m² of plate". **MEASURED: no plot-area gate exists in the engine** — 16 × 45 m (720 m²) REJECTED while 16.5 × 16.5 m (272 m²) BUILT; the binding quantity is plate WIDTH, and the 400 was a display constant read by no engine. **IMPLEMENTED — PENDING VERIFICATION** (`8094c3fb`): §RESI-NARROW-PLATE-SIDE-CORE adds the MISSING ARRANGEMENT (centred plan tried first, side-core single-loaded fallback only when it places zero), dropping the derived width floor 19.6 m → 11.1 m; a ribbon guard (`MAX_RECT_ASPECT` 3.5:1) stops the fallback buying a build with junk; §RESI-REFUSAL-TRUE deletes `RESIDENTIAL_MIN_PLATE_M2` and makes the UI quote only what the engine measured. 674 m² narrow plate and a 150 m² plot now build; 11 m wide still refuses at any area. | **BLOCKER for close-out = a contract decision (L-671)**: nothing requires a refusal to name the constraint the engine evaluated. **ADR-0092 §OFFICE-PLATE-AUTOFIT must be reconciled** — it asserts "the residential building never hard-rejects" and the OFFICE clamp-up posture was justified by that (false) premise. **Separate defect, not fixed:** `platePartition` §RESI-T3-FIT scales its keep-band by `MAX_APARTMENT_DEPTH_M / depth` with no upper bound, authorising cell widths far past `engineMaxCellWidth` at shallow depths. Contracts C50 §1.7, C53; ADR-0075 (preview↔execution parity). Relates L-435, §RESI-SMALL-PLATE-CORE-SCALE. Queue: generative / residential-engine. |
| **L-670** typology CREATION-FLOW ORDERING un-contracted (P2 coverage gap) | UNASSIGNED | TBD | No contract or spec owns parcel → envelope → GENERATE → setup → preview → build. C60 stops at parcel arrival (its §8: flow "not yet reachable"); C50 §5 governs dispatch not sequence; C06 governs z-index not order; C59 governs panes. The ordering lives only in `onboarding-workflow-design-2026-06-03.md` §3.3, status **"PROPOSAL for founder review. No code yet."** ⚠ L-593/C60 is NOT this gap — it closed *arrival at a parcel* only. **LOGGED, not closed.** | **BLOCKED on contract-owner decision.** Proposed: a normative C50 §5 invariant (setup surface must not precede the generate opt-in; site visible at opt-in) + a step-order table of shared seams. Not a new contract. Recorded in MISSING-CONTRACTS-AUDIT. Queue: contract owner + onboarding/ux. |
| **L-671** generator refusals may quote thresholds the engine never computed (P1 coverage gap) | UNASSIGNED | TBD | C50 §1.7 mandates soft-fail but is silent on what `reason` must contain and what the UI may add. Paid instance L-669: a UI-only constant contradicted by no engine and no test shipped a self-falsifying sentence for weeks. Same family as §CONTEXT-DATA-HONESTY (L-422/457/467/469) one layer up — refusal and a fabricated reason for refusal are the same value to the user. **LOGGED, not closed.** | **BLOCKED on contract-owner decision.** Proposed: extend C50 §1.7 (reason MUST carry the measured value + threshold, both engine-sourced; UI MUST NOT introduce a numeric feasibility threshold absent from the reason) + a `tools/ga-gate/` grep gate on `MIN_*`/`MAX_*` thresholds declared under `apps/editor/src/ui/**` and interpolated into copy (~½ day, mirrors `check:commandmanager`). Recorded in MISSING-CONTRACTS-AUDIT. Queue: contract owner + generative. |
