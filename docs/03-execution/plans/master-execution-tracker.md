...
| **68.13** | **~51 s main-thread stall during house finish** (`§BN-07 DEFERRED-RESUME-FLUSH delayed 51086ms`; `§E1-CRDT-BLACKOUT 6251ms`) | perf — batch/PBR/PSO during furnish+light+floor post-gen chain | 🔴 QUEUED. The post-gen finish chain (furnish→light→floor→ceiling per storey) blocks the main thread tens of seconds on large houses. Cross-ref the perf trio (OI-053). Needs a profile: PBR upgrade / PSO compile / scene-wide traverse during the batch critical window. |
