...
| **A.7.c.5** | **site.linkClimate + site.linkBuilding + site.replace + site.delete** | Cross-element refs (C20/C21) + whole-site replace per §1.4 + cascade-from-project-delete. | ⚪ PLANNED (Sprint 3) |
...
| **A.7.e** | **Legacy `Project.location` migration** | Project.location becomes a read-only getter delegating to Site.location (per [C19 §8.2](../../02-decisions/contracts/C19-SITE-MODEL-AND-PARCEL.md)). | ⚪ PLANNED (Sprint 3) |
...
| **A.10.b** | **L2 EPW parser** | NEW `packages/climate-host/` (L2 pure). Parses EPW TMY3 text files into validated EPWRecord[] + computes monthlyNormals + windRose + designTemps + degreeDays. Returns ClimateIngestionError on parse failure. | ⚪ PLANNED (Sprint 3) |
| **A.10.c** | **L2 SolarPathReader** | Extract pure NOAA solar-position algorithm from `core-app-model/src/rendering/RealSunService.ts` into `@pryzm/climate-host/src/solarPath.ts`. Pure + deterministic per [C21 §1.3](../../02-decisions/contracts/C21-CLIMATE-INGESTION.md). | ⚪ PLANNED (Sprint 3) |
| **A.10.d** | **L3 ClimateStore** | `packages/stores/src/ClimateStore.ts` — siteRef → ClimateDataset resolver + cache (keyed by ClimateCacheKey per §1.4). EPW > NOAA > fallback-defaults priority per §1.2. | ⚪ PLANNED (Sprint 4) |
| **A.10.e** | **L3 climate.* commands** | climate.ingestEPW · climate.refreshNOAA · climate.resolveSite · climate.invalidateCache · climate.solarSample · climate.windRose. Per [C21 §4](../../02-decisions/contracts/C21-CLIMATE-INGESTION.md). | ⚪ PLANNED (Sprint 4) |
