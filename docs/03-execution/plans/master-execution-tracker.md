...
| **A.21.D47** | 🚪 **DOOR MINIMUMS + EVERY-ROOM-ACCESS** (founder, recurring: "every door needs to have a [minimum] door … and the rules were defined earlier") — two architectural guarantees: (1) every habitable/service room reachable through ≥1 door (no sealed room); (2) every emitted door clears a MINIMUM clear width per room type. | INVESTIGATED: door WIDTH flows `buildWallsAndDoors.addDoor` → `OpeningSpec.widthM` → semanticGraph `attrs.widthM` → emitGeometry `doors[].width` → executePlan `wall.createOpening` (single source of truth = `addDoor`). Live widths confirmed = the L3-γ-3 per-EdgeType set **SOCIAL_FLOW 1.10 / CEREMONIAL 1.00 / BUFFER·SERVICE 0.90 / INTIMATE 0.80** (default 0.90) — the founder's "0.9/1.0/1.1 m" log. DEFECT: `width = min(preferred, len−2·clear)` had only a 0.6 m hard floor → a short host wall could shrink a door toward 0.6 m with NO per-room-type minimum (sub-minimum bathroom/bedroom doors). FIX (deterministic, reuses the rules-DB): NEW **§DOOR-MINIMUMS** standalone section in `programRules.ts` (kept clear of the parallel corridor-rules edits) — `MIN_DOOR_WIDTH_BY_TYPE` (Part M clear-opening floors: habitable+circulation **0.80 m**, entrance/hall **0.90 m**, wet rooms bathroom/ensuite/wc **0.70 m**) + `minDoorWidthFor` + `minDoorWidthBetween` (a door serving two rooms takes the MAX of the two floors). `addDoor` now CLAMPS the width UP to the pair floor (`width = max(min(preferred, usable), minW)`) and REFUSES a wall too short for the floor (`usable < minW → return false`) so reconciliation picks a LONGER wall / the room is flagged sealed rather than emitting a sub-minimum door — never shrinks the door below its floor. Caller `opts.doorWidthM` override still bypasses the floor (test back-compat). EVERY-ROOM-ACCESS already guaranteed by §SEALED-ROOMS + §CIRCULATION-REROUTE (passes 1→2c-ii); confirmed no doorless room ships on a clean shell (enumerate gate ranks on `connected`/`compromises`/`unroutedToCirculationRoomIds`). A.26.4 per-room door TYPE (`dt-*`) resolution untouched (this is WIDTH + presence only). Does NOT regress D41 central-blob, the D42 stair fix, A.25.3, per-room opening TYPES, or §A.21.D29 entrance-door (separate `entranceDoor.ts`, 1.0 m). TESTS: `__tests__/doorMinimums.test.ts` (12) — per-type floors exhaustive; every emitted door ≥ its floor (2-bed + 3-bed full pipeline); a short wall yields NO sub-minimum door (re-route / sealed-flag); override bypass; determinism. FULL ai-host suite **2002/2002** green (was 1990 + 12). | ✅ DONE |
