...
|---|---|---|---|---|---|---|---|
| **🎯 1 — DRIVING NOW** | **IP-A5** | Sprint 5 · 2026-09-08 | **Brand cutover + C19/C21 canonical** | "I navigate to pryzm.so / pryzm.app, see the new landing + pricing pages, and AI-generated apartment layouts show provenance details in the inspect panel." | 🟡 A.17 (DNS cutover — user has `pryzm.so` in Cloudflare ✅; `pryzm.app` acquisition still pending), ⚪ A.M.3 (manifesto page), ⚪ A.M.4 (trust page), ✅ A.18 (pricing-page generator — L2 + L5 Astro page DONE), 🟢 A.31 (provenance backend ✅; **A.31.e L5 inspect-tree Provenance tab — DRIVING NEXT**), ⚪ C19 + C21 ratifications | (1) pryzm.so + pryzm.app redirect work — 🟡 user-assisted DNS in flight; (2) Pricing page reads from entitlement registry ✅; (3) Right-click element in inspect tree → "Show AI provenance" — ⚪ L5 tab next | 🟢 ~ 50 % — engineering driving A.31.e + 2 marketing pages; user driving DNS |
| **🎯 2** | **IP-A3** | Sprint 3 · 2026-08-11 | **RAC chatbot end-to-end + Climate substrate parses** | "I sign up, the chatbot asks who I am and what project type, I type 'apartment', it routes me to apartment generation in < 60 seconds." | 🟢 A.5 (RAC L3 + parsers + summarizeCapturedState DONE; L5 React A.5.b PENDING), 🟢 A.6 (TypologyPicker L3 model DONE; L5 React A.6.b PENDING), 🟢 A.10 (climate EPW + NOAA L0+L2+L3+commands DONE; A.11 UI PENDING), ⚪ A.15 part 2 (3 more plugins), ⚪ A.M.1/2 (landing-page rebuild), ⚪ A.U.5 (tool-registry refresh) | (1) RAC chatbot live in editor — needs A.5.b React; (2) TypologyPicker shows 10 categories — needs A.6.b React; (3) End-to-end signup → … → generate < 5 min — gated on the 2 above; (4) EPW parses 10 reference files ✅ | 🟢 ~ 50 % — substrate fully ready; 2 React components remain |
...
| **A.31.e** | **L5 Inspect-tree Provenance tab (IP-A5 iteration 5.2)** | New self-contained DOM panel `apps/editor/src/ui/inspect/ProvenanceTab.ts`. Renders the C23 provenance graph for a selected element. Reads `ProvenanceStore.listArtefactsForProject(projectId)` + filters by `producedElementIds.includes(elementId)`. Per-artefact card shows model · workflow version · reproducibility (deterministic with seed, or non-deterministic) · cost (USD formatted by magnitude) · tokens · duration · cache status · timestamp · prompt SHA · produced-element count · redacted prompt preview in `<details>`. Approval-status badge uses semantic CSS classes (`pv-badge--success` / `--warning` / `--error` / `--info` / `--muted`). Subscribes to the store so live appends re-render automatically. `role="region"` + `aria-label="AI provenance for selected element"` for screen readers. Lifecycle: `build()` idempotent · `setSelectedElement(id)` swap · `dispose()` releases store subscription. 31 tests (pure-helpers 13 · happy-dom integration 18). Demo runbook: [docs/05-guides/demos/IP-A5-iteration-5-2-provenance-tab.md](../../05-guides/demos/IP-A5-iteration-5-2-provenance-tab.md). NOT yet wired into ModelTree right-click menu — that's iteration 5.2.b. | ✅ DONE (Sprint 2) |
