...
| **A.21.D52** | Stair STILL centred in REAL generated houses (founder flagged TWICE on a build that HAS the D42/D45 fix — windows correctly off-corner) | **ROOT CAUSE** — the perimeter-worst-aspect scorer (`stairPosition.ts`/`stairCore.ts`/`houseOrchestrator.ts`) was sound: a repro sweep proved it picks a perimeter wall (flush `left`, back corner) for **729/729** clean rectangular plates, NEVER central. The bug was the **A.21.D34(a) shell-containment cull**: it tested perimeter candidates against the shell POLYGON with a 0.001 mm boundary tolerance. The D42 unit tests use a MATHEMATICALLY PERFECT rectangle (4 exact corners) → pass. But a REAL drawn boundary is drawn edge-by-edge + mitred by WallJoinResolver, so the shell polygon **wobbles by a few cm**. On a jittery shell every flush perimeter candidate (`x=0` etc.) was culled the moment the matching wall dipped even 1 mm inward → the candidate set collapsed to `central` ONLY → the stair marooned in the dead centre (repro: `minEdgeDist≈4.2 m`, ALL 30 jittery plates centred). The unit tests never caught it because they never fed a jittery polygon. **FIX (ai-host houseLayout only, deterministic):** (1) `pointInPoly`/`rectInsidePoly` now take a tolerance and treat a sampled point within a realistic draw/miter jitter band (`SHELL_JITTER_MM = 150`) of the boundary as contained — so a flush candidate hugging a slightly-wobbly wall is no longer culled, while a genuine skew/notch (metres) is still rejected; (2) when the exact flush position is still culled (corner pulled inward > the band, or a mild concavity), a small deterministic INWARD-NUDGE ladder (≤ one wall-landing) retries near-wall positions before giving up — so central fallback is now RARE (only genuinely-infeasible plates), not the default. Worst-aspect/north preference + min footprint + entrance clearance + fully-inside-shell + the D42 variant-0=single-best + §STAIR-CARVE-NO-DROP all preserved. **REPRO TEST** (`stairPosition.test.ts` §A.21.D52): drives `generateHouseLayout` (the modal path) with an 8-vertex ±30 mm jittery boundary + a size sweep + a concave-L cull check — FAILED before (`centred=30`, edge-dist > 0.95 m), PASSES after (`centred=0`, stair flush to a wall ≤ 0.95 m). GATE: ai-host **2009/2009** (+3), changed-file typecheck clean. | ✅ MERGED |
