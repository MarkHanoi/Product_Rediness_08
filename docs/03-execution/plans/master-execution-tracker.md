...
| **A.21.D24** | 🪜 **House stair inherits the layout's principal-axis ROTATION on a skewed plot** (founder 2026-06-06: on a non-90° plot the staircase + its plan symbol stayed axis-aligned to the world while the house layout sat at the plot angle) | On a SKEWED plot the D-TGL engine rotates the whole layout to its principal (dominant-edge) axis, lays out axis-aligned in that frame, then rotates walls/rooms back to world (`runDeterministicLayout` §PRINCIPAL-AXIS). The STAIR was left behind: `reserveStairCore` sized the core from the WORLD-axis bbox of the skewed footprint + flight directions were the WORLD axes (`{0,0,1}`/`{1,0,0}`) → the staircase + plan symbol (walking line / direction arrow, both derived from `startPosition` + flight `direction`/`startOverride`) stayed orthogonal to the world, crossing partitions + not fitting the rotated plate. ✅ **SHIPPED 2026-06-06** (house orchestrator + house executor; pure rotation math). **(1)** `houseOrchestrator` computes the footprint's `principalAxisAngle` + centroid pivot (SAME angle + ~0.6° `PRINCIPAL_AXIS_MIN_RAD` the engine uses), reserves the core in the rotated LAYOUT frame (`reserveStairCoreShaped` on the `-angle`-rotated footprint → tight rect aligned to the plate), and `resolveFlightPlans` authors flight directions axis-aligned then rotates them to world by `+angle`. `StairCore` carries new `principalAxisRad` + `pivot` (additive). **(2)** The `keepOutRectsWorld` is built by rotating the layout-frame core-rect corners BACK to world (`+angle`) so it stays a genuine world rect — the engine maps it into its own principal-axis frame internally exactly as before (round-trip exact). **(3)** `HouseLayoutExecutor._createStair` builds the stair geometry (start + U-shape `startOverride`) in the LAYOUT frame, then rotates the RIGID body back to world (`_rotateXZ` about pivot) + uses the engine's already-world-rotated flight directions; plan symbol follows for free (no `geometry-stair` change). **No regression:** axis-aligned plot → `principalAxisRad === 0` → identity → byte-identical `rectMm`/flights/`startPosition`/void. **Apartment + manual stair tool untouched.** Determinism preserved (no `Math.random`). **+5 ai-host tests** ("stair principal-axis rotation (A.21.D24)" in `houseLayout.test.ts`: angle 0 on a rectangle, ~20° carried+applied on a skewed shell, flight 1 == world-axis run rotated by `+angle`, off-axis unit dirs, deterministic). Full ai-host suite **1727/1727**. Editor typecheck for the changed file: worktree has no node_modules → tsc resolves `@pryzm/ai-host` from the MAIN checkout (lacks the new fields) so it reports only the 2 expected `??`-guarded new-field accesses; every other added line typechecks against the old type → clean once node_modules points at the worktree (CI). SPEC-CASA §7.4 added. **NEEDS IN-BROWSER VERIFY:** generate a skewed 2-storey house → the staircase sits squarely within the rotated floor plate (runs along the rotated walls, not crossing partitions) + the plan-view stair symbol (walking line + arrow) is rotated to match. | ✅ DONE (ai-host + editor; in-browser verify pending) |
