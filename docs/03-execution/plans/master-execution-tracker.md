...
| **A.21.D18** | 🪜 **Stair shapes I/L/U + matching slab void + housing roof** (founder 2026-06-06: "I (straight) we can do now, L-shape, and U — architecturally + orchestration sound into the pipeline; consider the hole in the slab during stair creation; and the roof for the housing use case") | `CreateStairCommand` ALREADY supports `shape:'I'\|'L'\|'U'` (flights[]+landings[]+`risersBeforeLanding`+L-turn dir+U-side) + `autoCreateOpening` punches the slab void; `geometry-stair` renders all three. GAP: `houseLayout/stairCore.ts` reserves a fixed 1.0×3.0 m I-run rect and `HouseLayoutExecutor` emits only straight 'I'. **Implement:** (1) stairCore CHOOSES I/L/U from core area/aspect (long-thin→I; squarer→L; generous square→U), reserves the shape's true footprint, `HouseLayoutResult.stairs[]` carries `{shape,flights,landingDepth,footprint}`; (2) executor emits matching `CreateStairInput` (risers split across flights for L/U); (3) `autoCreateOpening` void matches the L/U bbox (review the void-footprint calc); (4) **housing roof** — real pitched roof sized to the shell (gable default, hip option) + eave overhang + sensible pitch on the top storey. SPEC-CASA §7 + roof note. Pure ai-host (stairCore/types/orchestrator) + editor (HouseLayoutExecutor). | 🔵 QUEUED (founder 2026-06-06) |
| **A.21.D19** | 🎨 **Furnishing styles — Nordic · Mediterranean · Minimalist · Classic (per-style materials)** (founder 2026-06-06: "different materials depending on the style — massive review, analyse, document, implement") | EXTENDS A.21.D4 (`furnishLayout/styleFinish.ts` maps modern/classic/minimal/warm → per-category colour+material today). **Implement:** richer architecturally-grounded styles — **Nordic** (pale ash/birch, white+soft grey, linen, matte), **Mediterranean** (terracotta/lime-plaster, olive/ochre, ceramic tile, rattan), **Minimalist** (mono white/grey/black, lacquer, glass, hidden hardware), **Classic** (walnut/mahogany, brass, deep upholstery, marble) — each driving DISTINCT material+colour per furniture category AND floor finish (`floors/floorFinish.ts`) AND wall finish where applicable. Update typology `briefSchema` style options (`typology-pack-*/manifest.ts`, back-compat aliases). DESIGN OF RECORD = new SPEC-FURNISHING-STYLES (palette table style×category). Pure ai-host + command-registry finishes + manifests; +tests. | 🔵 QUEUED (founder 2026-06-06) |
| **A.21.D20** | 🍳 **Kitchen + wardrobe I/L/U layouts (UI-selectable) + real appliances** (founder 2026-06-06: "Kitchen and wardrobe Straight (I), L-shape and U-shape accessible via UI — adding proper oven, hob, fridge, dishwasher, washing machine, etc.") | TWO parts. **(a) Layout shapes:** kitchen + wardrobe get I/L/U run layouts (mirror the stair-shape doctrine) — `furnishLayout/archetypes.ts` lays units along 1 wall (I) / 2 walls (L) / 3 walls (U) by room aspect + a UI selector. **(b) Appliances as first-class furniture:** extend `FurnitureType` (L0 `packages/schemas`) + geometry builders for **oven, hob/cooktop, fridge, dishwasher, washing machine, sink, extractor** + place them in the kitchen run (work-triangle sink↔hob↔fridge) + utility. Crosses L0 schemas → geometry builders → archetypes → `buildFurnishCommands` → UI picker. DESIGN OF RECORD = new SPEC-KITCHEN-WARDROBE-APPLIANCES. Biggest of the three (new element types). Pairs with [[ai-creation-default-element-types-queue]]. | 🔵 QUEUED (founder 2026-06-06) |
