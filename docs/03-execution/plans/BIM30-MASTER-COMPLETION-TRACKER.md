...
| **PR-05** | `clearGraphAuthoritative` has 1 definition, 0 production callers — every generated level keeps its rooms suppressed by default | C72 §4.1, §4.3 | `check-source-verified-invariants` SV5 — `clearGraphAuthoritative` has >=1 production CALL SITE. ⚠ the suppression-reversibility gate is deliberately NOT named here: its S1 arm counts callers OUTSIDE the defining file and the only release is same-file, so it is blind to this row by design (§PR-05-ONE-RELEASE-AUTHORITY) and naming it would let a gate that cannot see the subject decide the row | **CLOSED** *(moved OPEN → CLOSED)* | **GENERATED** — run `npx tsx tools/bim30-status/bim30-status.ts --row PR-05`; this row is no longer CARRIED | GATED 2026-08-17 — deciding instrument is now a runnable gate |
