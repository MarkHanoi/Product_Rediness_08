...
| **GR-09** | `CreateWallCommand` writes no graph edges at all | C71 §5.5 | H5 `graphruntime.cert` | **CLOSED** | H5 **16/16**; `sitsOn` REACHED, `contains` REACHED, never-created negative control reads `[]` · 2026-08-15T07:52Z | RE-MEASURED 2026-08-15 |
...
| **GR-11** | all three C71 gates are unbuilt at HEAD | C71 §6 | the three gates themselves | **CLOSED** | write-coverage `[1]` 2/2 · delete-integrity `[1]` 10/10 · persistence `[1]` 3/3 — all three exist and ran · 2026-08-15 | RE-MEASURED 2026-08-15 |
...
| **GR-15** | `measuredAt`'s writer is broken — a positional 4-arg call against a 1-object signature, invisible because the global is `any` | C71 §5 | source verification of `PhysicsEngine.ts:380` | **CLOSED** | **NOT DETERMINED** at this pass | NOT RE-MEASURED — last known CLOSED (source-verified 2026-08-13); one of the three weaker source-verified closures |
| **GR-16** | no `graph.*` bus verbs; the UBG has no runtime home | C70 D-INV-1/3 | `graphQueryBusHandlers.ts` + its spec suite | **CLOSED** | **NOT DETERMINED** at this pass | NOT RE-MEASURED — last known CLOSED 2026-08-14 (`393f81bd`) |
| **GR-17** | the UBG's own header claims snapshot persistence it does not have | C71 §4 | `ubgSnapshotIsDerivedNotAuthored.test.ts` | **CLOSED** | **NOT DETERMINED** at this pass | NOT RE-MEASURED — last known CLOSED 2026-08-14 (`805c333e`, 4/4) |
| **GR-18** | no runtime probe has ever been executed against a live graph — *a writer that exists and is never reached still reads ✅* | C71 §5.8 | H5 `graphruntime.cert` | **CLOSED** | H5 **16/16**, results regenerated 2026-08-15T07:52Z; both verdict states demonstrated every run | RE-MEASURED 2026-08-15 |
...
| **PR-05** | `clearGraphAuthoritative` has 1 definition, 0 production callers — every generated level keeps its rooms suppressed by default | C72 §4.1, §4.3 | caller census + `gr2UpdateSurrenderSeam` / `observerGraphAuthoritative` on the real store emit | **CLOSED** *(moved OPEN → CLOSED)* | **1 production caller** (`RoomTopologyObserver.ts:351`) in the live add/remove/update subscriber; seam suites **8/8** on the real store emit. `check-suppression-is-reversible` stays 41/41 because S1 counts callers OUTSIDE the defining file and the release is same-file (§PR-05-ONE-RELEASE-AUTHORITY) — **blind by design, not contradicting** · 2026-08-15 | RE-MEASURED 2026-08-15 |
| **PR-06** | `initPersistence.ts` pauses the topology observer and sync-state engine with no `finally` — a throw leaves them off for the session | C72 §4.2 | source verification | **CLOSED** | **NOT DETERMINED** at this pass | NOT RE-MEASURED — last known CLOSED 2026-08-14 (`finally` at `:374`) |
| **PR-07** | `RECONCILABLE_TYPES` names 13 types, is exported, has zero consumers; the real reconcile rebuilds walls and slabs only | C72 §5.1–5.2 | source verification | **CLOSED** | **NOT DETERMINED** at this pass | NOT RE-MEASURED — last known CLOSED 2026-08-14 (`10bdc639`) |
| **PR-08** | `check-prevstate-contract` and `check-suppression-is-reversible` are specified and NOT BUILT | C72 §6 | both gates | **CLOSED** | prevstate-contract `[0]` · suppression-is-reversible `[1]` **41/41** — both exist and ran · 2026-08-15 | RE-MEASURED 2026-08-15 |
...
| **PR-10** | roof → walls-beneath propagation is MEASURED-ABSENT in both directions | C72 §1.1 | the roof-clash detector + its suites | **CLOSED** | **NOT DETERMINED** at this pass | NOT RE-MEASURED — last known CLOSED 2026-08-14 (both halves executed green) |
...
| **PR-12** | schedules have no geometry subscription — open panels show stale areas | C72 §1.1 | the staleness suites | **CLOSED** | **NOT DETERMINED** at this pass | NOT RE-MEASURED — last known CLOSED 2026-08-14 (`b10828ff` measured 24.00 m² against 40.00 m²) |
| **PR-13** | three stores duplicated (Column / Door / Window) — any cascade wired to one is blind to the other | C72 §0.3 | repo-wide store-file census | **CLOSED** *(moved OPEN → CLOSED; see §0.2 — it was already true at the last stamp)* | **1 `ColumnStore` file repo-wide**; the `core-app-model` fork is gone; Door/Window likewise single-authority · 2026-08-15 | RE-MEASURED 2026-08-15 |
...
| **GE-07** | three C73 gates specified and NOT BUILT | C73 §5 | the three gates | **CLOSED** | all three exist and run: epsilon-policy `[3]` · predicate-canonical `[3]` · deterministic-regeneration `[3]`. **The row is about existence; their readings are GE-01/02/03 and §6.1's business** · 2026-08-15 | RE-MEASURED 2026-08-15 |
...
| **CO-11** | all three C74 gates are UNBUILT | C74 §6 | the three gates | **CLOSED** | constraint-honesty `[1]` 2/2 · solver-is-real `[0]` · no-hidden-mock `[1]` 9/9 · 2026-08-15 | RE-MEASURED 2026-08-15 |
...
| **PV-07** | all three C75 gates are UNBUILT; per-kind coverage is therefore UNPROVEN, not zero | C75 §6 | the three gates | **CLOSED** | **NOT DETERMINED** by the row lane; M1's HEAD sweep reads provenance-not-invented `[1]` 2/2 · provenance-coverage PASSED · derived-not-authored `[0]` (5 arms) | NOT RE-MEASURED — last known CLOSED 2026-08-14 |
