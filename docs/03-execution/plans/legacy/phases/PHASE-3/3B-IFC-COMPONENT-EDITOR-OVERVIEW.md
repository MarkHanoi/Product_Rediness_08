...
2. **`packages/ui/` design tokens + primitives lit AND legacy `src/styles/` migrated** (S56, S58, S64) — the design system is not just a Tailwind config; per `[strategic ADR-0226]` it is a token layer + primitive component library that the editor consumes via dependency injection. Migration runs across three sprints.
...
4. **PDF-to-BIM door/window symbol matching** (S58) — per ADR-0229 the second pipeline stage. Symbol matching is harder than wall extraction because building codes vary and symbol vocabularies differ regionally; the confidence model in S60 absorbs this variance.

**Cut-list discipline**: S59 is the gate where DXF/SVG export per `[strategic ADR-0218]` T2.1 is decided ship-or-defer based on Phase 2 velocity. Founder + agent decision; recorded in `apps/bench/reports/M30-3B.md`.
...
| Audit-log schema lit per `[strategic ADR-0221]` + ADR-0228 Part G; SOC2 evidence pipeline begins | S57 |
| Legacy 11-wave Visibility-Gate engine **deleted** per SPEC-27 §4.3 + SPEC-30 §6.2 | S58 |
| PDF-to-BIM door/window symbol matching lit per `[strategic ADR-0229]` | S58 |
| PDF-to-BIM confidence model + review queue UI per ADR-0229 Part A | S60 |
...
| Item | Sprint |
|---|---|
| `packages/ui/` design tokens + primitives lit per `[strategic ADR-0226]` | S56 |
...
| DXF/SVG export ship-or-defer decision per `[strategic ADR-0218]` T2.1 | S59 |
| Idle-CPU bench audit per ADR-0223 Part F | S60 |
| Reserved-VM capacity review per ADR-0222 Part D | S60 |
...
The IFC plugin is the highest-stakes plugin in Phase 3B for a reason: IFC interchange is **the** interoperability test for any BIM platform. Per `[strategic ADR-0208]` the scope is IFC 4.0 import/export + IFC 4.3 read-only; advanced IFC 4.3 features are explicitly post-GA.
...
Per `[strategic ADR-0226]` the design system is a token layer (`packages/ui/tokens/`) + primitives (`packages/ui/primitives/{Button,Input,Dialog,Tooltip,Menu,...}`) that the editor consumes via dependency injection. The post-migration end-state per ADR-0226 Part C is: editor production bundle has **zero `react` symbols** (the editor is vanilla TS by Phase 2's contract; `packages/ui/` provides a vanilla-TS primitive layer suitable for both editor consumption and any third-party React-based plugin shell at S64).
...
**Why custom elements, not React**: per ADR-0226 Part C the editor's first-paint bundle has zero `react` symbols. `packages/ui/` primitives are framework-agnostic web components; React-based plugin shells (S64) consume them via React wrappers shipped in `packages/ui-react/`.
...
Audit-log schema per `[strategic ADR-0221]` + ADR-0228 Part G is the foundation of the SOC2 evidence pipeline that S65 (workspace admin) and S68 (SOC2 quarterly automation) depend on.
...
```sql
-- audit_log table per ADR-0228 Part G; ADR-0221 enterprise security
CREATE TABLE audit_log (
  ...
  permission_used TEXT,                  -- the [strategic ADR-0211] permission tuple matched
...
Per ADR-0228 Part G every gateway route emits an audit-log row whether or not the action succeeds. SOC2 evidence is *pulled* from this table by the quarterly automation in S68.
...
PDF-to-BIM door/window symbol matching per `[strategic ADR-0229]` is the second pipeline stage; the confidence model in S60 absorbs symbol-matching variance.
...
Per `[strategic ADR-0218]` T2.1 the DXF/SVG export ship-or-defer decision happens this sprint based on Phase 2 velocity, beta cohort feedback, and current cut-list carry. If shipped, both formats land before M30. If deferred, both move to v2 backlog with clear rationale.
...
1. **PDF-to-BIM confidence model + review queue UI** per ADR-0229 Part A — the review queue surfaces all extracted elements with confidence < 0.85 for human review. This is the gate that lets the public preview at S70 happen.
2. **Idle-CPU bench audit** per ADR-0223 Part F — single-frame-owner audit; verify that `pnpm bench single-frame-owner-audit` is green and no plugin is requesting frames during idle without justification.
3. **Reserved-VM capacity review** per ADR-0222 Part D — possible second VM for >2k concurrent users per SPEC-15 §2.2.1. Decision deferred to S67 if cut per ADR-0218 T2.6, else provisioned this sprint.
...
export interface ConfidencedElement {
  ...
  factors: {                    // contributing factors per ADR-0229 Part A
    geometricFit: number;
...
- **D5**: reserved-VM capacity decision per ADR-0222 Part D.
...
|---|---|---|
| **S55** | OBC removed from editor bundle per SPEC-12 §5; `src/import/ifc/` migrated to `plugins/ifc-import/`; OBC library-mount entry deleted per `[ADR 0023-...]`. Bundle size budget verified per SPEC-12 §7. PDF-to-BIM wall extraction lit per `[strategic ADR-0229]`. Print-canvas backend lit per SPEC-29 §4.4. Backup verification bench live per SPEC-24 §3.4. | SPEC-12, `[ADR 0023-...]`, `[strategic ADR-0229]` |
| **S56** | `packages/ui/` design tokens + primitives lit per `[strategic ADR-0226]`; half of `src/styles/` migrated. | `[strategic ADR-0226]` |
| **S57** | Audit-log schema lit per `[strategic ADR-0221]` + ADR-0228 Part G; SOC2 evidence pipeline begins. | `[strategic ADR-0221]`, ADR-0228 |
| **S58** | Legacy 11-wave Visibility-Gate engine **deleted** per SPEC-27 §4.3 + SPEC-30 §6.2. PDF-to-BIM door/window symbol matching lit per `[strategic ADR-0229]`. | SPEC-27, SPEC-30, `[strategic ADR-0229]` |
| **S59** | DXF / SVG export per `[strategic ADR-0218]` T2.1 — decide v1 ship-or-defer based on Phase 2 velocity. | `[strategic ADR-0218]` T2.1 |
| **S60** | PDF-to-BIM confidence model + review queue UI lit per `[strategic ADR-0229]` Part A. Idle-CPU bench audit per ADR-0223 Part F. Reserved VM capacity review per ADR-0222 Part D — possible second VM for >2k concurrent users per SPEC-15 §2.2.1. | ADR-0222, ADR-0223, `[strategic ADR-0229]` |
