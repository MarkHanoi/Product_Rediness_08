...
> - **Phase B** adds **B.6, B.7, B.8, B.9, B.10** — see Chunk 24 §24.5 (audit removed 2026-08-09 — recoverable from git history) (per-folder coverage closure: `commands` widening, `core` legacy injection, `services` constructor wiring, `ai` host adapter, `ai-host` plugin façade).
> - **Phase C** adds **C.14** — see Chunk 24 §24.5 (audit removed 2026-08-09 — recoverable from git history) (`src/persistence/` move into `packages/persistence-client/`).
