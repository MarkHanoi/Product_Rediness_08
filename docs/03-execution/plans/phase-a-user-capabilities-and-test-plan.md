...
> **Reconciled 2026-06-03** to ADR-0255/C51 (apex/app split; `pryzm.so` apex marketing + `app.pryzm.so` editor; `pryzm.app` retired; pricing renders from `@pryzm/entitlements` in `apps/editor`, NOT Astro).
...
| 6.6 | Undo / redo (ring-buffer ADR-0251) | Cmd-Z / Cmd-Shift-Z | (existing) | ✅ DONE | Three-store undo unified; verified live for wall + curtain-wall + door |
...
| Believer | Test |
|---|---|
| Pricing copy is generated, never hand-edited | grep `apps/editor/src/ui/platform/PricingPage.ts` — only `@pryzm/entitlements` consumed; no string literals (Astro `docs-site` pricing is deleted per ADR-0255 §7 / C51 §6.3) |
