...
| `12` | 9.38 % | ✅ packed | — | — | — | — | — | done. ⚠ **The "Ciutat Vella predicate" line is WITHDRAWN** — Art. 315.2 states no geographic test to encode (§CLAU-12-PREDICATE, L-674); see Open risks 1 |
...
| bare `20a` | 1.55 % | ✅ **cited `regime-undetermined` refusal** (§BARE-20A-EXHAUSTED, L-673) | a subzone-granular layer carrying the `20a/N` suffix | 2 | 0.5 | +1.1 | +0.22 | **DATA AVAIL.** — ⚠ **the LEGAL search is now exhausted**: PGM Arts. 314.5/338.2 enumerate ten **suffixed** qualificacions and no bare one, and Arts. 340/342/343 key every parameter to the suffix, so what is missing is a **SELECTOR, not a rule**. Refusing is confirmed correct; the ROI is unchanged because a layer would still return the 1.55 % |
...
1. ~~⚠ **Clau `12` is mapped unconditionally with NO geographic predicate**~~ ✅ **WITHDRAWN AS STATED — §CLAU-12-PREDICATE (L-674), 2026-08-01.** PGM **Art. 315.2 states no geographic test to encode**: it says which nuclei the two subzones were drawn for, **delimits neither** (no boundary, no district, no coordinate) and **hedges** the `12b` half (*"referida **preferentment** a aquell"*). The operative predicate is the ***plànol d'ordenació*, parcel by parcel**, which the MUC serves and PRYZM already reads — a polygon test coded in PRYZM would be a **second, weaker classifier that could only ever ACT where it DISAGREED with the plànol**. ⇒ `NOT-THE-RULE-KIND`. What survives is narrower and differently bucketed: *"is the MUC faithful to the plànol in Ciutat Vella?"* — an **evidence question about a data source**, and **no predicate reduces it**. Mitigation shipped + asserted: `12` and `12b` never share a code path, so the feared outcome can arise only from a wrong clau in the SOURCE. See `08019-barcelona/claus/12/CLAU.md`.
2. ~~⚠ **Street width uses the MEDIAN of ray samples**~~ ✅ **STALE — FIXED AND PINNED, AND THIS ROW WAS WRONG.** `governingPunctualWidth` has returned the ordinance's **MINIMUM** since **`0edd302b` (2026-07-22)** and has been test-pinned since **`8e590846` (2026-07-31)**; both are ancestors of `main` (`git merge-base --is-ancestor`). ⚠ **This row said "Fix in flight" for ten days after it landed and caused at least one agent to be briefed on a live defect that did not exist. Grep the §-tag and read the git history before filing a fix.** Residual Art. 238 clauses are tracked as CLOSURE-REGISTER row 12 — **12B/12C/12D are DATA gaps that err SAFE**, and **12A** (*tram* chaining) is the one engineering lever, newly named and deliberately unshipped pending a live blast-radius measurement.
