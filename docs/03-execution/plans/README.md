...
| [UCE-REACHABILITY-AUDIT.md](./UCE-REACHABILITY-AUDIT.md) | **STATUS-BEARING programme tracker** — *is the Universal Component Editor/Creator finished?* Every capability in the UCE master spec + ADR-0376, classified **EXISTS-AND-WIRED / EXISTS-BUT-UNWIRED / PARTIAL / ABSENT**, each WIRED row naming the caller chain a USER traverses. ⭐ Answers the founder's question: the **model** layer works end to end through `composeRuntime`; the **presentation** layer draws nothing and an authored definition dies on reload. Corrects ADR-0376's overtaken *"family-runtime is not reachable"* parenthetical and C113 §12.4's stale ratchet row |
