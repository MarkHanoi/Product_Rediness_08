...
| **Plumbing** | ✅ | ✅ | ✅ (+ MOVE_PLUMBING `2026-06-20`) | 🟡 | 🟡 | ⬜ | ⬜ | UPDATE_PLUMBING_PARAMETERS factory `497f9d54`. **MovePlumbingCommand AUTHORED 2026-06-20** (new MOVE_PLUMBING CommandType + factory; clones position Vector3 + translates bath start/end by the delta; fully undoable; mirrors MoveLighting). The command now exists + replays. **Remaining:** the gizmo commit branch in registerTransformDragHandler + a `plumbing.move` bus handler + `rotate`/`move` capability — browser-gated on confirming the selected mesh's normalised elementType (mesh tags `PlumbingFixture`; capability key is `plumbing`). |
