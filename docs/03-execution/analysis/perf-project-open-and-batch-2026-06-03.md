...
+ [ADR-0257](../../02-decisions/adrs/ADR-0257-realtime-geometry-and-view-interactivity.md)): this
...
door's `CreateWallOpeningCommand` triggering a whole-level wall rebuild** (the ADR-0257 hole-rebuild
...
mutates its host wall's openings, which (per the ADR-0257 analysis) triggers
...
([ADR-0257](../../02-decisions/adrs/ADR-0257-realtime-geometry-and-view-interactivity.md);
...
same O(walls-per-level) hole-rebuild cost ADR-0257 identifies, now incurred during the batch. This is
...
**S1 — Incremental opening creation in the batch (adopt ADR-0257 P1 on the create path).**
Make `CreateWallOpeningCommand` (and the `wall.createOpening` plugin) take the **openings-only,
single-wall** rebuild branch ADR-0257 P1 defines — rebuild only the host wall, skip
`resolveLevel` / level `refreshV2Cache` / `computeJunctionInfills`. This collapses the §4.3
O(walls-per-level)×doors cost to O(doors). **This is the same fix ADR-0257 P1 already proposes for
door-move; the apartment batch is a second, high-value caller.**
- *Effort:* M (shared with ADR-0257 P1).  *Impact:* High (the batch CPU half).  *Risk:* Medium
  (correctness of the delta classification — covered by ADR-0257's tests).
...
(batch quick wins) → S1 (shared with ADR-0257 P1) → S2 → S3.
...
- `docs/03-execution/specs/SPEC-PROJECT-OPEN-CREATE-PIPELINE.md` — O1–O10 stages + OI-053 items
  (this doc closes (c) + (d)).
- `docs/02-decisions/adrs/ADR-0257-realtime-geometry-and-view-interactivity.md` — the openings-only
