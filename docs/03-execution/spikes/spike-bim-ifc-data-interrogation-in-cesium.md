...
- **Decision the spike must surface to the founder** (do NOT silently resolve): does the Cesium/geospatial view stay **context-only** (C55 as written), or does it become a **first-class interactive BIM/IFC surface**? The correct end-state of the latter needs a governing contract before any code.
