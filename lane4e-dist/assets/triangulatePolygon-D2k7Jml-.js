import { p as polygonSignedAreaOrdinates } from './SteelProfileLibrary-NgbfwhrM.js';

function asMaterialKey(s) {
...
function triangulateRingOrdinates(vertexCount, xAt, yAt) {
  if (vertexCount < 3) return [];
  const flat = new Array(vertexCount * 2);
...
function triangulationAreaDeviation(vertexCount, xAt, yAt, triangles) {
...
function zOrder(x, y, minX, minY, invSize) {
  ...
  xi = (xi | xi << 8) & 16711935;
  xi = (xi | xi << 4) & 252645135;
  xi = (xi | xi << 2) & 858993459;
  xi = (xi | xi << 1) & 1431655765;
  yi = (yi | yi << 8) & 16711935;
  yi = (yi | yi << 4) & 252645135;
  yi = (yi | yi << 2) & 858993459;
  yi = (yi | yi << 1) & 1431655765;
  return xi | yi << 1;
}
...
function splitPolygon(a, b) {
  const a2 = { i: a.i, x: a.x, y: a.y, prev: null, next: null, z: 0, prevZ: null, nextZ: null, steiner: false };
  const b2 = { i: b.i, x: b.x, y: b.y, prev: null, next: null, z: 0, prevZ: null, nextZ: null, steiner: false };
...
function insertNode(i, x, y, last) {
  const p = { i, x, y, prev: null, next: null, z: 0, prevZ: null, nextZ: null, steiner: false };
  if (!last) {
    p.prev = p;
    p.next = p;
  } else {
...
export { asMaterialKey as a, triangulationAreaDeviation as b, earcut as e, triangulateRingOrdinates as t };
