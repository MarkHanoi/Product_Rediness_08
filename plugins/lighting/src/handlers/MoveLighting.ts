...
/**
 * §FIX-DEAD-MOVE-VERB-REFUSE (W3-4, Class A "dead verb") — why `lighting.move` now REFUSES.
 *
 * This handler produced a correct Immer patch against `ctx.stores.lighting`, reported
 ...
 * Neither names `lighting.move`. The `plugins/cross` cascade rules that synthesise it are
 ...
 *
 * THE UNDO HAZARD THIS ALSO CLOSES. `affectedStores` is `['lighting']`, and CommandBus
  ...
  implements CommandHandler<MoveLightingPayload, Stores>
{
  ...
  validatePayload(ctx: HandlerContext<Stores>, cmd: MoveLightingPayload): ValidationResult {
