 ...
 *   1. Click to pick point A (lower / start point)
 *   2. Click to pick point B (upper / end point)
 *   → Fires CreateAnnotationCommand for a 'slope-dim' annotation
 *
 * Geometry stored in modelPoints:
 *   [0] = pointA  [1] = pointB
 *
 * Parameters stored:
 *   { slopeRatio: number, slopePercent: number, unit: 'ratio'|'percent'|'degrees' }
 *
 * Rendered as: inclined line from A to B with rise/run indicator and slope value label.
 *   slopeRatio = vertical rise / horizontal run (e.g. 0.25 = 1:4)
 *
 * Contract compliance:
...
export class SlopeDimensionTool {
    public isActive = false;
    private _state = SlopeDimToolState.IDLE;
    ...
    activate(): void {
        ...
        this.isActive = true;
        this._state = SlopeDimToolState.PICK_A;
        this._attachListeners();
        if (this._domElement) this._domElement.style.cursor = 'crosshair';
        console.log('[SlopeDimTool] activated — click the start point');
    }

    deactivate(): void {
        if (!this.isActive) return;
        this.isActive = false;
        this._state = SlopeDimToolState.IDLE;
    ...
    private _onMouseMove = (e: MouseEvent): void => {
        if (!this.isActive) return;
        this._updateMouse(e);
        if (this._state === SlopeDimToolState.PICK_B && this._pointA) {
    ...
    private _onMouseDown = (e: MouseEvent): void => {
        ...
        if (this._state === SlopeDimToolState.PICK_A) {
            this._pointA = hit.point.clone();
            this._refA   = this._makeRefFromHit(hit);
            this._state  = SlopeDimToolState.PICK_B;
            console.log('[SlopeDimTool] Point A picked at', this._pointA.toArray());

        } else if (this._state === SlopeDimToolState.PICK_B) {
            if (!this._pointA || !this._refA) return;
            const pointB = hit.point.clone();
            const refB   = this._makeRefFromHit(hit);
            this._createSlopeDim(this._refA, refB, this._pointA, pointB);
            this._reset();
            this._state = SlopeDimToolState.PICK_A;
        }
    };
    ...
    private _createSlopeDim(
        ...
        if (!this._activeViewId) {
            console.warn('[SlopeDimTool] No active view — annotation not created');
            return;
        }
        ...
        const slopeRatio   = run > 0.001 ? rise / run : 0;
