  ...
  execute(
    ...
    return withHandlerSpan(
      ...
      () => {
        ...
        // :186-209. `execute()` used to end in the same `{forward:[],inverse:[]}`
        // whether N walls were retyped, the command refused EVERYTHING, the
        // bridge threw, or there was no command sink at all — so failure and
        // emptiness were one value at the bus boundary. Set inside the try,
        // thrown after it, always AFTER the CustomEvent (L-996 stays closed).
