...
/**
 * §FIX-DEAD-MOVE-VERB-REFUSE (W3-4, Class A "dead verb") — why `wall.transform` now REFUSES.
 *
 ...
 * Neither names `wall.transform`. The `plugins/cross` cascade rules that synthesise it are
 ...
 */
const WALL_TRANSFORM_UNREACHABLE =
  ...
  implements CommandHandler<TransformWallPayload, WallHandlerStores>
{
   ...
   */
  canExecute(ctx: HandlerContext<WallHandlerStores>,
    cmd: TransformWallPayload,): ValidationResult {
