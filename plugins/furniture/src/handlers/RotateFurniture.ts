...
/**
 * §FIX-DEAD-MOVE-VERB-REFUSE (W3-4, Class A "dead verb") — why `furniture.rotate` now REFUSES.
 *
 ...
 * Neither names `furniture.rotate`. The `plugins/cross` cascade rules that synthesise it are
  ...
  implements CommandHandler<RotateFurniturePayload, Stores>
{
  ...
  validatePayload(ctx: HandlerContext<Stores>, cmd: RotateFurniturePayload): ValidationResult {
