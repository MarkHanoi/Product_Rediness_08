// SetViewCrop handler — Phase F-1.1 (fully promoted from bridge-observability stub).
//
// Performs an authoritative Immer-patch mutation on the ViewRegistry.
// The legacy SetViewCropCommand / commandManager.execute() dual-write in
// ViewPropertiesPanel.ts has been removed as of Phase F-1.1; this handler
// is now the sole state-mutation path for view.setCrop.
//
// Contract compliance:
//   §01 §2     — Command-first mutation via bus handler; no direct store write from UI
//   §01 §2.7   — No builders; no Three.js scene access
//   §03 §1.1   — ViewCropSettings fields are serialisable primitives
...
export interface SetViewCropPayload {
...
export const SetViewCropHandler: CommandHandler<SetViewCropPayload, Stores> = {
  type: 'view.setCrop',
  affectedStores: ['view'],

  canExecute(ctx: HandlerContext<Stores>, cmd: SetViewCropPayload): ValidationResult {
  ...
  execute(ctx: HandlerContext<Stores>, cmd: SetViewCropPayload): HandlerResult {
    return withHandlerSpan('view.setCrop.handler', { 'pryzm.command.type': 'view.setCrop' }, () => {
      ...
      if (cmd.crop !== null) {
        updated['crop'] = cmd.crop;
      } else {
        delete updated['crop'];
      }

      console.log(`[CommandBus] view.setCrop — viewId=${cmd.viewId} crop=${cmd.crop === null ? 'cleared' : 'set'}`);
