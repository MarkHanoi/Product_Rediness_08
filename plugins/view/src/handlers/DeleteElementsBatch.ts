      ...
      () => {
        ...
        if (cm) {
          try {
            ...
            //
            // ⚠ This verb DELETES. "I could not tell you whether anything was
            // removed" reaching a caller as the same empty pair a clean delete
            // returns is the worst instance in this family, not the mildest.
            if (!report.success) {
              refusal = report.info[0] ?? 'the delete was refused, and no reason was given';
            }
          } catch (e) {
        ...
        if (refusal !== null) {
          ...
          // dispatch site.
          throw new Error(`element.deleteBatch: ${refusal}`);
        }
