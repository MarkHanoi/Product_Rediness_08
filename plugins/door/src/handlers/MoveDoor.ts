...
/**
 * §FIX-DEAD-MOVE-VERB-REFUSE (W3-4, Class A "dead verb") — why `door.move` now REFUSES.
 *
 * This handler produced a correct Immer patch against `ctx.stores.door`, reported
 ...
 * Neither names `door.move`. The `plugins/cross` cascade rules that synthesise it are
 ...
 *
 * THE UNDO HAZARD THIS ALSO CLOSES. `affectedStores` is `['door']`, and CommandBus
 ...
 */
const DOOR_MOVE_UNREACHABLE =
  "door.move writes the detached plugin door store that nothing renders, exports or persists, and no production surface dispatches it. Moving a door along its host wall commits through door.setOffset (payload keys: doorId, newOffset, prevOffset) → SetDoorOffsetCommand → the geometry wallStore opening, which is what MOVE_COMMAND_BY_TYPE and the 3-D gizmo already dispatch.";
...
export class MoveDoorHandler implements CommandHandler<MoveDoorPayload, DoorHandlerStores> {
  ...
  validatePayload(ctx: HandlerContext<DoorHandlerStores>, cmd: MoveDoorPayload): ValidationResult {
