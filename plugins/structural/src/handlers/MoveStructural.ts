...
/**
 * §FIX-DEAD-MOVE-VERB-REFUSE (W3-4, Class A "dead verb") — why `structural.move` now REFUSES.
 *
 * This handler produced a correct Immer patch against `ctx.stores.structural`, reported
 ...
 * Neither names `structural.move`. The `plugins/cross` cascade rules that synthesise it are
 ...
 *
 * THE UNDO HAZARD THIS ALSO CLOSES. `affectedStores` is `['structural']`, and CommandBus
  ...
  implements CommandHandler<MoveStructuralPayload, Stores>
{
  ...
  validatePayload(ctx: HandlerContext<Stores>, cmd: MoveStructuralPayload): ValidationResult {
