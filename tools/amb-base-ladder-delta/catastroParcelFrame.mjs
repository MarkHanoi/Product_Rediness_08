...
// ═══════════════════════════════════════════════════════════════════════════════════════════════
// §COPIED-FROM tools/cold-start-probe/catastroParcelFrame.mjs — VERBATIM except for this block.
// ═══════════════════════════════════════════════════════════════════════════════════════════════
// The brief is explicit: "COPY the logic into your directory; do not import from another agent's
// tree." So this is a copy, and the ONLY divergence from the original is the read-through cache
// below. Every parsing, ATOM-matching, CRS and sampling behaviour is byte-identical to the file
// that produced the baseline artefact — which is the point, because a re-run that changed the
// DENOMINATOR would make the ladder delta unmeasurable.
//
// ⭐ THE READ-THROUGH IS A READ, NEVER A WRITE. `SEED_CACHE` is the sibling probe's `.cache/`. It
//   holds `atom_08.xml` and two already-inflated municipality GMLs — Barcelona's alone is 176 MB.
//   Re-downloading them would cost hundreds of megabytes to obtain BYTE-IDENTICAL files, and the
//   Catastro publisher would carry that for nothing.
// ⛔ NOTHING IS EVER WRITTEN TO `SEED_CACHE`. New downloads land in MY `.cache/` only. The sibling
//   agent's tree is read-only to this run, cache included.
// ⚠ THE MAIN CHECKOUT, NOT `join(HERE,'..')`. This agent runs in a WORKTREE. The worktree has its
//   own `tools/cold-start-probe/`, whose `.cache/` is gitignored and therefore EMPTY — a relative
//   hop lands there, finds nothing, and silently re-downloads everything. Measured: it re-fetched
//   Barcelona's 176 MB GML. Harmless to correctness (an independent re-fetch is if anything
//   stronger evidence) but it is the same class of mistake as reading a stale worktree table, so
//   the absolute path is used and the reason is recorded.
const SEED_CACHE = 'c:/Users/LENOVO/OneDrive/Desktop/PRYZM/Product_Rediness_08/tools/cold-start-probe/.cache';

/**
 * Resolve a cache entry for READING: prefer my own copy, fall back to the seed cache.
 * Writes always target `join(CACHE, name)` — never the value this returns.
 */
function cacheReadPath(name) {
    const mine = join(CACHE, name);
    if (existsSync(mine)) return mine;
    const seeded = join(SEED_CACHE, name);
    if (existsSync(seeded)) return seeded;
    return mine; // does not exist in either; the caller's existsSync will fail and it will download
}
...
export async function resolveMunicipalityZipUrl(ine, name) {
    ...
    const atomPath = cacheReadPath(`atom_${prov}.xml`);   // §READ-THROUGH — write target is CACHE below
    let xml;
...
export async function fetchMunicipalityGml(ine, name) {
    if (!existsSync(CACHE)) mkdirSync(CACHE, { recursive: true });
    // §READ-THROUGH — an already-inflated GML in the seed cache is used in place. Barcelona's is
    // 176 MB and re-downloading it would obtain a byte-identical file at the publisher's expense.
    const gmlRead = cacheReadPath(`CP_${ine}.gml`);
    if (existsSync(gmlRead) && statSync(gmlRead).size > 0) {
        return { ok: true, gmlPath: gmlRead, bytes: statSync(gmlRead).size, cached: true };
    }
