...
function main(): void {
  ...
  if (ran && r.status !== 0) findings.push(`E4 the cited suite exited ${r.status} — PV-06's evidence no longer holds`);
  if (/No test files found/.test(out)) findings.push('E4 vitest matched NO test files — the run measured nothing');

  lines.push(
    `E4  vitest exited ${r.status === null ? 'NULL (spawn failure or timeout)' : r.status} · ${passed} test(s) passed`,
    '',
