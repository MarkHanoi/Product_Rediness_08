...
const BASELINE_C_UNROUTED_PRODUCERS = 14; // 17 -> 16 -> 14, L-1038 S16 (2026-08-19): `ceiling`, `stair`, `handrail` routed. Stair and handrail had NO COLOUR SLOT IN THE KEY AT ALL - stair's bridge picked by SLOT and handrail's `colorOfHandrailMaterialKey(_key)` ignored its own ARGUMENT - so both gained one, with a legacy-shape fallback so cached keys do not regress. Each landed with a test that asserts the MASTER's hex and was verified to FAIL without the fix. Fourteen to go, one at a time - a shared harness would repeat the C100 §9.3 retraction.
