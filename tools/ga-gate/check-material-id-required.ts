...
const BASELINE_C_UNROUTED_PRODUCERS = 9; // 17 -> 13 -> 11 -> 10 -> 9 over L-1038 S16/S17 (2026-08-19). S16 closed `ceiling`, `stair`, `handrail`, `roof`; S17 closed `door` and `window`; then `furniture`, then `lighting`. ⛔ FURNITURE hashed the materialId (djb2) into an EIGHT-ENTRY PALETTE against a 205-row master, so `wood-oak` (#c8a96e) and `wood-walnut` (#5a3a28) COLLIDED onto one grey-teal and ~197 of 205 rows were indistinguishable; it survived because a stable, tasteful wrong colour reads as a design decision (C100 §5's exact thesis). ⛔ LIGHTING is a DIFFERENT shape and must not be filed with it: its key HAD a colour slot, filled correctly with a different QUANTITY — `rgbToHex(l.color)`, the light the fixture EMITS — which the bridge applied to the body's `color` AND `emissive`, so a blackened-steel downlight rendered warm white and its `materialId` (slot 2, unread since the key was written) changed nothing. Fixed by APPENDING slot 7, because that key's own header records its slot indices as a contract. Each family landed with its OWN test asserting the MASTER's hex, verified to FAIL without its fix. Nine to go, one at a time - a shared harness would repeat the C100 §9.3 retraction.
