...
const BASELINE_C_UNROUTED_PRODUCERS = 11; // 17 -> 13 -> 11 over L-1038 S16/S17 (2026-08-19). S16 closed `ceiling`, `stair`, `handrail`, `roof`; S17 closed `door` and `window`. Stair and handrail had NO COLOUR SLOT AT ALL (stair's bridge picked by SLOT; handrail's `colorOfHandrailMaterialKey(_key)` ignored its own ARGUMENT), so both gained one with a legacy-shape fallback. Roof resolves the SHINGLE slot only - deck/trim/interior stay canonical on purpose (§9.6.b), and the test pins that they did NOT move. Each landed with a test asserting the MASTER's hex, verified to FAIL without its fix. Eleven to go, one at a time - a shared harness would repeat the C100 §9.3 retraction.
