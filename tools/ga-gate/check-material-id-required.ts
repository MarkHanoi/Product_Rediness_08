...
const BASELINE_C_UNROUTED_PRODUCERS = 10; // 17 -> 13 -> 11 -> 10 over L-1038 S16/S17 (2026-08-19). S16 closed `ceiling`, `stair`, `handrail`, `roof`; S17 closed `door` and `window`; then `furniture`. ⛔ FURNITURE WAS THE WORST OF THE SEVENTEEN and its shape is the stair/handrail one a THIRD time — no colour slot in the key at all — but with a far more dangerous bridge: `colorOfFurnitureMaterialKey` hashed the materialId (djb2) into an EIGHT-ENTRY PALETTE while the master holds 205 materials, so `wood-oak` (#c8a96e) and `wood-walnut` (#5a3a28) COLLIDED onto one grey-teal #7d8c8c and ~197 of 205 master rows were indistinguishable. It survived because a hash is stable and the palette is tasteful: the wrong colour returned identically every session and read as a design decision, which is C100 §5's exact thesis that a wrong-but-believable colour is worse than a missing one. Legacy 5-part keys still get the hash, by content-discrimination on the `lod=` token, so a cached descriptor does not repaint. Each family landed with its OWN test asserting the MASTER's hex, verified to FAIL without its fix. Ten to go, one at a time - a shared harness would repeat the C100 §9.3 retraction.
