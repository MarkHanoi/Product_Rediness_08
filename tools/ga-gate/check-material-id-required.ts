...
const BASELINE_C_UNROUTED_PRODUCERS = 8; // 17 -> 13 -> 11 -> 10 -> 9 -> 8 over L-1038 S16/S17 (2026-08-19). S16 closed `ceiling`, `stair`, `handrail`, `roof`; S17 `door` and `window`; then `furniture`, `lighting`, `plumbing`. ⭐ THE THREE SHAPES ARE DIFFERENT AND FILING THEM TOGETHER PRODUCES THE WRONG FIX: furniture had NO colour slot and a bridge that HASHED the id (djb2) into an 8-entry palette against a 205-row master, collapsing `wood-oak` and `wood-walnut` onto one grey-teal; lighting HAD a colour slot filled correctly with a DIFFERENT QUANTITY (the light it EMITS), so a blackened-steel downlight rendered warm white; plumbing had the RIGHT slot in the RIGHT place filled from the wrong SOURCE (the service tag), so copper and cast iron on one service painted identically. Furniture inserted a slot, lighting APPENDED one (its indices are a stated contract), plumbing changed only the VALUE. In every case the family default SURVIVES as the answer for an element naming no material - plumbing's blue-for-cold engineering convention especially, which is a DEFAULT and not a placeholder to delete. Each landed with its OWN test asserting the MASTER's hex, verified to FAIL without its fix. Eight to go, one at a time - a shared harness would repeat the C100 §9.3 retraction.
