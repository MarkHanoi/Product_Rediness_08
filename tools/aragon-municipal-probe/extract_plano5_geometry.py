...
What it does:
  ...
  * records the paint operator (S/s/f/f*/B/b/n) and the current RGB stroke+fill
  * reports: subpath count, vertex count, drawing bbox, straight-run histogram,
    and the distinct colour table (a calificacion plan encodes ZONES AS COLOURS)
...
def analyse(subpaths) -> dict:
    ...
    for pts, paint, srgb, frgb in subpaths:
        ...
        for i in range(len(pts) - 1):
            ...
            if d > 0:
                runs.append(d)
        key = ("fill:%.2f,%.2f,%.2f" % frgb) if paint in ("f", "F", "f*", "B", "b", "B*", "b*") \
            else ("stroke:%.2f,%.2f,%.2f" % srgb)
    ...
    return {
        ...
        "distinct_paint_colours": len(colours),
        "top_colours": sorted(colours.items(), key=lambda kv: -kv[1])[:15],
    }
...
def main() -> int:
    ...
    results = []
    for fn in files:
        raw = open(os.path.join(CACHE, fn), "rb").read()
        ...
        print(f"{fn}: {a.get('subpath_count'):>7,} subpaths  "
              ...
              f"colours {a.get('distinct_paint_colours')}")
