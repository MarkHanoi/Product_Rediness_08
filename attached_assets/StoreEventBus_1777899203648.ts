...
export class StoreEventBus {
        ...
        try {
            ...
            if (this._batchDepth === 0) {
                if (success) {
                    ...
                    if (discarded > 0) {
                        console.warn(
                            `[StoreEventBus] batch() threw — discarded ${discarded} buffered event(s). ` +
     ...
     *
     * PERF-FIX-3 DIAGNOSTIC: Set `StoreEventBus._debugEmitCallers = true` in the
    ...
    emit(event: StoreChangeEvent): void {
        ...
        if (this._batchDepth > 0) {
            if (StoreEventBus._debugEmitCallers && (import.meta as any).env?.DEV) {
                const stack = new Error().stack?.split('\n').slice(2, 5).join(' | ') ?? '(no stack)';
                console.debug(
                    `[StoreEventBus] emit #${this._buffer.length + 1} ` +
    ...
    beginBatch(): void {
        this._batchDepth++;
        console.log(
            `[StoreEventBus] beginBatch() — depth now ${this._batchDepth}. ` +
    ...
    endBatch(): void {
        if (this._batchDepth === 0) {
            console.warn('[StoreEventBus] endBatch() called with depth already 0 — no-op.');
            return;
        }
        this._batchDepth--;
        if (this._batchDepth === 0) {
            ...
            this._flush();
            console.log(
                `[StoreEventBus] endBatch() — flushed ${count} buffered event(s) ` +
                'in emission order (no coalescing — all events delivered).'
            );
        } else {
            console.log(`[StoreEventBus] endBatch() — depth now ${this._batchDepth}, not flushing yet.`);
        }
    }
...
<<<<<<< HEAD
    ...
    discardBatch(): void {
        ...
        this._batchDepth = 0;
        console.warn(
            `[StoreEventBus] discardBatch() — depth reset 0, ${count} buffered event(s) ` +
...
=======
     ...
     *
     * Layer constraint: StoreEventBus has no knowledge of FrameScheduler (C01 §2
    ...
    endBatchYielded(
        ...
        if (this._batchDepth === 0) {
            console.warn(
                '[StoreEventBus] endBatchYielded() called with depth already 0 — ' +
        ...
        if (this._batchDepth > 0) {
            ...
            console.log(
                `[StoreEventBus] endBatchYielded() — depth now ${this._batchDepth}, ` +
                'not flushing yet.'
            );
            onComplete();
            return;
        ...
        if (total === 0) {
            console.log(
                '[StoreEventBus] endBatchYielded() — buffer empty, calling onComplete() immediately.'
            );
            onComplete();
            return;
        }
        ...
        const drainNextChunk = () => {
            ...
            if (offset < total) {
                console.log(
                    `[StoreEventBus] endBatchYielded chunk ${chunkIndex}/${totalChunks}: ` +
                    `${offset}/${total} events (${frameMs}ms this chunk).`
                );
                scheduler(drainNextChunk);
            } else {
                console.log(
                    `[StoreEventBus] endBatchYielded() — all ${total} event(s) delivered ` +
        ...
        console.log(
            `[StoreEventBus] endBatchYielded() — ${total} event(s) will be delivered ` +
            `in ${totalChunks} chunk(s) of ${chunkSize} via frame scheduler.`
        );
        scheduler(drainNextChunk);
>>>>>>> 53d09a1e0afdf41b35c5aa0a46603df41d338c6d
    }
    ...
    private _dispatch(event: StoreChangeEvent): void {
        this._listeners.forEach(listener => {
            try {
                ...
                console.error('[StoreEventBus] Listener error:', err);
            }
        });
    }
}
